...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/q8idevai
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/q8idevai/$(dirname "$path")"
      cp -a "$path" "$out/lib/q8idevai/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/q8idevai/

    # Web UI Assets
    cp -r packages/server/dist/server/web-ui $out/lib/q8idevai/packages/server/dist/server/

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    # Keep Q8iDevAI's runtime mode separate from NODE_ENV, which belongs to spawned agents.
    makeWrapper ${nodejs}/bin/node $out/bin/q8idevai-server \
      --add-flags "$out/lib/q8idevai/packages/server/dist/scripts/supervisor-entrypoint.js" \
      --set Q8IDEVAI_NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/q8idevai \
      --add-flags "$out/lib/q8idevai/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/q8idevai/node_modules"
