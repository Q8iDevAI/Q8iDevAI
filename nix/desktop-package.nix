{
  ...
  # (`q8idevai-desktop-<v>-npm-deps`) and refetches the entire registry. Override
  # the upstream hash via `q8idevai.override { npmDepsHash = "..."; }`.
  q8idevai,
}:
buildNpmPackage {
  pname = "q8idevai-desktop";
  ...
  buildPhase = ''
    ...
    npm run build --workspace=@q8idevai/expo-two-way-audio

    # Expo web export for the Electron renderer
    ( cd packages/app && Q8IDEVAI_WEB_PLATFORM=electron npx expo export --platform web )

    # Desktop main process
    npm run build:main --workspace=@q8idevai/desktop
  ...
  installPhase = ''
    ...
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      mkdir -p $out/share/q8idevai-desktop
      ...
      Q8IDEVAI_TRACE_DESKTOP=1 node scripts/trace-daemon.mjs > desktop-files.txt

      while IFS= read -r path; do
        [ -z "$path" ] && continue
        mkdir -p "$out/share/q8idevai-desktop/$(dirname "$path")"
        cp -a "$path" "$out/share/q8idevai-desktop/$path"
      done < desktop-files.txt

      # Keep the same unpackaged monorepo layout expected by main.js.
      cp package.json $out/share/q8idevai-desktop/
      mkdir -p $out/share/q8idevai-desktop/packages/app
      cp -a packages/app/dist $out/share/q8idevai-desktop/packages/app/
      ...
      for runtime_path in \
        ...
        if [ ! -e "$out/share/q8idevai-desktop/$runtime_path" ]; then
          echo "desktop runtime trace omitted $runtime_path" >&2
          exit 1
        fi
      done

      if [ -e $out/share/q8idevai-desktop/node_modules/electron ]; then
      ...
      install -Dm644 packages/desktop/assets/icon.png \
        $out/share/icons/hicolor/512x512/apps/q8idevai-desktop.png

      # Electron derives Wayland's toplevel app_id from the package name in the
      # app root it launches. Point it at a one-file app named "q8idevai-desktop"
      # so shells can match the window to the desktop entry and hicolor icon.
      mkdir -p $out/share/q8idevai-desktop/electron-app
      printf '%s\n' "{ \"name\": \"q8idevai-desktop\", \"version\": \"$version\", \"main\": \"index.js\" }" \
        > $out/share/q8idevai-desktop/electron-app/package.json
      printf '%s\n' 'require("../packages/desktop/dist/main.js");' \
        > $out/share/q8idevai-desktop/electron-app/index.js

      # Chromium's setuid sandbox cannot live in the immutable Nix store.
      makeWrapper ${electron}/bin/electron $out/bin/q8idevai-desktop \
        --add-flags "$out/share/q8idevai-desktop/electron-app" \
        --add-flags "--no-sandbox" \
        --add-flags "--class=q8idevai-desktop" \
        --set EXPO_DEV_URL "q8idevai://app/" \
        --set CHROME_DESKTOP "q8idevai-desktop.desktop"

      copyDesktopItems
    ''}

    ${lib.optionalString stdenv.hostPlatform.isDarwin ''
      app="$(find packages/desktop/release -maxdepth 3 -type d -name Q8iDevAI.app -print -quit)"
      if [ -z "$app" ]; then
        echo "electron-builder did not produce Q8iDevAI.app" >&2
        exit 1
      fi
      mkdir -p "$out/Applications"
      cp -R "$app" "$out/Applications/Q8iDevAI.app"
      ln -s ../Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI "$out/bin/q8idevai-desktop"
    ''}
  ...
  desktopItems = lib.optionals stdenv.hostPlatform.isLinux [
    (makeDesktopItem {
      name = "q8idevai-desktop";
      desktopName = "Q8iDevAI";
      genericName = "AI Coding Agents";
      comment = "Self-hosted daemon for AI coding agents";
      exec = "q8idevai-desktop";
      icon = "q8idevai-desktop";
      categories = ["Development"];
      startupWMClass = "q8idevai-desktop";
    })
    ...
    # package.json `name` ("q8idevai-desktop"), 38 uses the runtime app name that
    # main.ts sets ("Q8iDevAI"). Ship a NoDisplay entry for the second spelling so
    ...
    (makeDesktopItem {
      ...
      icon = "q8idevai-desktop";
      categories = [ "Development" ];
      startupWMClass = "Q8iDevAI";
      noDisplay = true;
    })
  ];

  meta = {
    description = "Q8iDevAI desktop app (Electron wrapper)";
    homepage = "https://github.com/Q8iDevAI/Q8iDevAI";
    license = lib.licenses.agpl3Plus;
    mainProgram = "q8idevai-desktop";
