...
do not use `createQ8iDevAIDaemon` as a product launch path.
...
import { createQ8iDevAIDaemon } from "./bootstrap.js";
import { DaemonClient } from "./test-utils/daemon-client.js";

const logger = pino({ level: "warn" });
const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-test-"));
...
const daemon = await createQ8iDevAIDaemon(
  {
...
For simpler cases, `createTestQ8iDevAIDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. In test harnesses, use `createQ8iDevAIDaemon` directly when you need it (see quick start above).
