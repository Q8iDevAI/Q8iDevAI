...
Q8iDevAI executes `index.server.ts` in a subprocess and `index.client.tsx` in every connected app.

> **Trust every plugin you add.** `q8idevai plugin add` and `q8idevai plugin install` mean “I trust this codebase.” Plugins are unsandboxed: server code and Git preparation commands run with the daemon user's access on the daemon host, and client contributions run inside Q8iDevAI. The repository's dependencies and future updates are part of that trust decision. With `--host`, preparation runs on that remote daemon host.
...
npm install
npm run typecheck
q8idevai plugin install /absolute/path/to/my-plugin
q8idevai plugin install /absolute/path/to/my-plugin --id another-runtime-id
q8idevai plugin ls
```
...
runtime-safe: run `q8idevai reload` after editing `config.json`. Enabling starts every configured,
...
The directory contains a manifest declaring identity and Q8iDevAI requirements, one optional entry per runtime, runtime-owned
...
dependencies for local typechecking and tests. Q8iDevAI compiles TypeScript and TSX and supplies the
...
Declare the supported Q8iDevAI range and keep it current when adopting newer APIs. See the
...
backend code can access the daemon machine, while client contributions run inside the Q8iDevAI app.

Source changes are explicit. Run `q8idevai plugin reload <id>` to stop and fully tear down the old
plugin before compiling and starting from disk. A failed reload stays failed; Q8iDevAI does not restore
...
Removing a Git source deletes Q8iDevAI's managed checkout.
...
shell command strings are rejected. Q8iDevAI starts the executable directly, without a shell, from the
plugin directory in the staged checkout. It never detects lockfiles or chooses a package manager.

On install and every update, Q8iDevAI resolves the exact Git revision and manifest, runs the declared
commands, then validates, compiles, and activates the candidate. It logs each argv command and its
output in the daemon log. If a command fails, the error includes its output, Q8iDevAI discards the
...
Server contributions can write to stdout and stderr with normal Node logging. Q8iDevAI adds `[q8idevai]`
...
tail from the host plugin settings or with `q8idevai plugin logs <id>`. Git preparation commands are
recorded in `$Q8IDEVAI_HOME/daemon.log` before a plugin exists, rather than the plugin log tail. Reload, disable, and process
failure retain the tail; removing the plugin clears it. Daemon restarts do not retain the tail, but
structured copies remain in `$Q8IDEVAI_HOME/daemon.log`. Plugin output can contain secrets, so do not
...
Shared files import contract helpers and types from `@q8idevai/plugin`. Server handler files import
`PluginHandlerContext` from `@q8idevai/plugin/server`. Client files import Q8iDevAI UI from
`@q8idevai/plugin/client/react-native`. Its `Icon` resolves a Lucide name using the client's installed icon
...
| ---------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------- |
| `@q8idevai/plugin`                                   | Shared data, schemas, RPC/settings definitions, runtime-neutral helpers    | Shared code only       |
| `@q8idevai/plugin/server`                            | Server contribution/handler contexts and lifecycle contracts               | Shared and server code |
| `@q8idevai/plugin/server/provider`, `/server/acp`    | Server provider contracts and adapters                                     | Shared and server code |
| `@q8idevai/plugin/client`                            | Client contribution contexts, hooks, navigation, and UI contribution types | Shared and client code |
| `@q8idevai/plugin/client/react-native`, `/client/ui` | Host-provided UI components                                                | Shared and client code |
| `@q8idevai/plugin/client/host`                       | App-owned rendering integration; not a plugin-author entry                 | Shared and client code |
...
The contribution function must return cleanup. Server cleanup may be async; Q8iDevAI waits for it when
the plugin is reloaded, disabled, removed, disconnected, or shut down. Cleanup is for resources
created by plugin code. Q8iDevAI removes registered contributions, unmounts surfaces, clears query
...
Q8iDevAI owns the route, screen header, Lucide icon validation, close action, theme DTO, layout facts,
...
Q8iDevAI gives each plugin installation its own query client.

`useQ8iDevAI()` and the handler's `{ q8idevai }` context expose the same `Q8iDevAIApi`: projects,
workspaces, agents, terminals, providers, and daemon config. They do not expose connection lifecycle. A surface borrows the
selected host's existing connection; switching the screen's host changes both `useQ8iDevAI()` and
...
Use plugin RPC for plugin-specific backend behavior that is not a normal Q8iDevAI operation.
...
When the same plugin contribution exists on multiple hosts, Q8iDevAI shows it once in the sidebar and
...
Command Center callbacks use the selected host's existing `Q8iDevAIApi` for normal Q8iDevAI operations.
...
```ts
import type { PluginServerContext } from "@q8idevai/plugin/server";
import type { ProviderRegistration } from "@q8idevai/plugin/server/provider";
...
Provider settings are toggle/select data that Q8iDevAI renders in the composer. Keep private options in
...
`@q8idevai/plugin/server/acp`. Its transformer hooks cover narrow vendor differences; do not translate the
...
Request observation with `client.q8idevai.agents.list({ subscribe: {} })` and consume the returned
...
Q8iDevAI memoizes by source-item reference and phase and derives replacement IDs from source identity, so
...
replaces it, and an empty array removes it. Output `data` must be JSON-compatible. Q8iDevAI adds the
...
the same `q8idevai`, `rpc`, `openSurface`, workspace, agent, and `openPanel` capabilities as the matching
...
Q8iDevAI owns the autocomplete row, input clearing, and error toast. It never sends the command text to
...
Register a declarative attachment source backed by a plugin RPC. Q8iDevAI owns the attachment menu,
...
`addTheme` takes a small light or dark palette and a display name. Q8iDevAI expands it through the
