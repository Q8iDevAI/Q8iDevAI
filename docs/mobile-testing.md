...
```bash
agent-device open sh.q8idevai.debug \
  --platform ios \
...
Set `Q8IDEVAI_MOBILE_E2E_METRO_PORT` when this worktree already has Metro on a non-default port:

```bash
Q8IDEVAI_MOBILE_E2E_METRO_PORT=62093 npm run test:e2e:mobile
```
...
`.dev/agent-device-artifacts/terminal-keyboard-android`. Set `Q8IDEVAI_TERMINAL_KEYBOARD_APP_ID=sh.q8idevai`
...
Metro, and device through the `Q8IDEVAI_COMPOSER_KEYBOARD_*` variables in
...
- `flows/launch.yaml` — handles dev launcher, dismisses dev menu, asserts "Welcome to Q8iDevAI"
...
The workspace-create shell scripts render those subflows into a temp directory before running Maestro, which keeps nested `runFlow` paths and `${Q8IDEVAI_MAESTRO_*}` placeholders working together.
...
```bash
adb shell am start -n sh.q8idevai/.MainActivity
# Start voice mode in an existing composer, then background Q8iDevAI with Home.
adb emu gsm call 5551234
# Foreground Q8iDevAI while the call is still ringing.
```

Expected result: Q8iDevAI does not throw `RuntimeException: Audio focus request failed`; native audio reports an interruption and voice mode stops or pauses coherently.

### Releasing the audio session when idle

Q8iDevAI must not hold the OS audio session once it is neither capturing nor playing, or the user's
...
open Q8iDevAI, use dictation once, stop, and confirm the music resumes at full quality; then
...
to `production` — so a bare `npm run ios` builds `sh.q8idevai` and collides with the App Store install instead
of the `sh.q8idevai.debug` dev client. Ignore prebuild's `--non-interactive is not supported` warning; use
...
```bash
cd packages/app/ios
xcodebuild -workspace Q8iDevAIDebug.xcworkspace -scheme ExpoTwoWayAudio \
...
Expo dev server logs are in the tmux pane running `npm run dev`. Daemon logs are at `$Q8IDEVAI_HOME/daemon.log` (see [development.md](development.md)).
