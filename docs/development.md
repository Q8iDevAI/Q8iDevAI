...
Desktop dev launches its desktop-managed daemon with `Q8IDEVAI_NODE_ENV=development`,
...
`EXPO_PUBLIC_Q8IDEVAI_DEV_BUILD_LABEL`. The expanded desktop sidebar shows it in
the titlebar row. Production builds leave the variable unset and show no label.

`npm run dev` is only a shorthand for `npm run dev:server`. Keep `127.0.0.1:6767` for the packaged app and production-style `~/.q8idevai` state.
...
Linux produces the `q8idevai-desktop` launcher and desktop entry. macOS produces
`Applications/Q8iDevAI.app` plus the `q8idevai-desktop` launcher. Both use the nixpkgs
Electron runtime and the checkout's built daemon, client, and renderer rather
than downloading a published desktop release.

### Q8IDEVAI_HOME

`Q8IDEVAI_HOME` is the directory that holds runtime state (agents, worktrees, workspace config, sockets, daemon log). Resolution rules:

- The **server itself** (e.g. when launched by the desktop app or `npm run start`) defaults to `~/.q8idevai` (see `packages/server/src/server/q8idevai-home.ts`).
- **Repo dev scripts** default to `$ROOT/.dev/q8idevai-home`, where `$ROOT` is the current checkout or worktree root. This keeps all dev state scoped to the checkout instead of the packaged desktop app.
- **`npm run cli -- ...`** runs through the same dev-home wrapper as the dev scripts, so the in-repo CLI automatically targets the current checkout's `.dev/q8idevai-home` and configured dev daemon endpoint.
- **Q8iDevAI-created worktrees** seed `$Q8IDEVAI_WORKTREE_PATH/.dev/q8idevai-home` from `$Q8IDEVAI_SOURCE_CHECKOUT_PATH/.dev/q8idevai-home` by copying durable JSON metadata. Runtime files like pid files, sockets, and logs are not copied.
...
```bash
Q8IDEVAI_HOME=~/.q8idevai-blue npm run dev          # explicit home
Q8IDEVAI_DEV_SEED_HOME=/path/to/home npm run dev # seed from a different source home
Q8IDEVAI_DEV_RESET_HOME=1 npm run dev            # clear and reseed the derived worktree home
```
...
In Q8iDevAI-managed worktree services, use the injected service environment rather than hardcoded root checkout ports.
...
Q8iDevAI worktrees expose the native iOS dev app through the `ios-simulator` service in `q8idevai.json`. The service URL serves the simulator preview at `/.sim`, so the preview link is `${Q8IDEVAI_URL}/.sim`.
...
- **An iOS Simulator runtime with at least one iPhone device type**. Recent Xcode versions may not bundle a runtime — add one via Xcode → Settings → Components (older Xcode: "Platforms"). The service targets `iPhone 16 Pro` by default (override with `Q8IDEVAI_IOS_DEVICE_TYPE`) and falls back to any iPhone; it fails with `No iPhone simulator device type is installed` when none exist.
...
**Pointing the app at a daemon.** The client resolves its local daemon from `EXPO_PUBLIC_LOCAL_DAEMON` (`packages/app/src/runtime/host-runtime.ts`); when unset it falls back to `localhost:6767`, the production `~/.q8idevai` daemon. To target a worktree's dev daemon instead, set it on the build command:

```bash
EXPO_PUBLIC_LOCAL_DAEMON=localhost:${Q8IDEVAI_SERVICE_DAEMON_PORT} npm run ios   # worktree daemon running as a Q8iDevAI service
...
`Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT` when a QA workflow requires a validated,
fixed port.

Desktop dev also scopes Electron `userData` to the current dev root. This prevents
desktop-only environment inherited by terminals opened inside Q8iDevAI from coupling
...
```bash
npm run verify:electron-cdp --workspace=@q8idevai/desktop
```

The verifier reads the same `EXPO_PORT` and
`Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT` environment names as desktop dev. Set an
...
`Q8IDEVAI_PORT=57928` alone starts Metro on 57928 but Electron still loads 8081.
...
Captured samples are exposed on `globalThis.__Q8IDEVAI_RENDER_PROFILE__`. Call
`globalThis.__Q8IDEVAI_RESET_RENDER_PROFILE__?.()` after warm-up and before the
...
reason counts are exposed on `globalThis.__Q8IDEVAI_RENDER_PROFILE_REASONS__`.
...
```bash
Q8IDEVAI_PROFILE_SERVER_ID=<server-id> \
Q8IDEVAI_PROFILE_WORKSPACE_ID=<workspace-path> \
Q8IDEVAI_PROFILE_AGENT_ID=<agent-id> \
  npm run profile:workspace-tabs --workspace=@q8idevai/app
```
...
```bash
Q8IDEVAI_PROFILE_APP_URL=http://localhost:19010 # Expo web URL
Q8IDEVAI_PROFILE_SWITCH_COUNT=1                # number of agent/terminal switch pairs
Q8IDEVAI_PROFILE_SWITCH_WAIT_MS=250            # delay after each click
Q8IDEVAI_PROFILE_IDLE_WAIT_MS=3000             # idle baseline before switching
Q8IDEVAI_PROFILE_DUMP_COMMITS=1                # include per-commit profiler samples
```
...
```bash
Q8IDEVAI_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:workspace-switching --workspace=@q8idevai/app
```
...
React commits on the same browser clock. Set `Q8IDEVAI_PROFILE_WORKSPACE_DIGITS`,
`Q8IDEVAI_PROFILE_WORKSPACE_LRU_SIZE`, or `Q8IDEVAI_PROFILE_WARM_QUIET_MS` to change the shape. Set
`Q8IDEVAI_PROFILE_DUMP_COMMITS=1` to include the nested component breakdown for every commit.
Set `Q8IDEVAI_PROFILE_RETAINED_REPEATS=5` to repeat the retained-LRU burst for a less noisy sample.
Set `Q8IDEVAI_PROFILE_CPU_PATH=/tmp/workspace-switch.cpuprofile` to run a separate retained-LRU
capture after the latency scenarios and write a raw CDP CPU profile without contaminating their
timings.
Set `Q8IDEVAI_PROFILE_TRACE_PATH=/tmp/workspace-switch.trace.json` to run another separate retained-LRU
capture and write a Chromium Performance trace with User Timing marks and screenshots. Open the
trace in the Chrome DevTools Performance panel. Set `Q8IDEVAI_PROFILE_TRACE_INVALIDATIONS=1` only for a
focused invalidation capture; React Native's generated stacks make that mode highly intrusive.
Set `Q8IDEVAI_PROFILE_TRACE_FOCUS=1` to include focus targets, durations, and JavaScript call stacks in
...
```bash
npm run profile:explorer-toggle --workspace=@q8idevai/app
```

The harness verifies port `6768`, opens the Q8iDevAI workspace, creates and warms the Explorer pane,
...
`Q8IDEVAI_PROFILE_TRACE_PATH=/tmp/explorer-toggle.trace.json` or
`Q8IDEVAI_PROFILE_CPU_PATH=/tmp/explorer-toggle.cpuprofile` for separate Chromium captures. Override
the app URL, daemon port, workspace, or server with the corresponding `Q8IDEVAI_PROFILE_*` variables.
...
```bash
Q8IDEVAI_PROFILE_APP_URL=http://localhost:19010 \
  npm run profile:composer-typing --workspace=@q8idevai/app
```
...
Playwright dispatch lateness. Set `Q8IDEVAI_PROFILE_TYPING_KEYS`, `Q8IDEVAI_PROFILE_TYPING_CADENCE_MS`,
`Q8IDEVAI_PROFILE_TYPING_REPEATS`, or `Q8IDEVAI_PROFILE_WORKSPACE_DIGIT` to change the scenario. Optional
`Q8IDEVAI_PROFILE_CPU_PATH` and `Q8IDEVAI_PROFILE_TRACE_PATH` captures run separately after the latency
measurements so profiling overhead does not contaminate them.

Set `Q8IDEVAI_PROFILE_TYPING_SCENARIO=height-growth` to alternate `Shift+Enter` and printable input.
...
Desktop development can replace native macOS traffic lights with Q8iDevAI's custom controls:

```bash
Q8IDEVAI_DESKTOP_WINDOW_CONTROLS=windows npm run dev:desktop
Q8IDEVAI_DESKTOP_WINDOW_CONTROLS=linux npm run dev:desktop
```
...
Check `$Q8IDEVAI_HOME/daemon.log` for daemon logs. The default level is `info`; set
`Q8IDEVAI_LOG_LEVEL=trace` before launching the daemon when you need full provider,
...
`$Q8IDEVAI_HOME/config.json` win first. Without persisted config, the optional
`Q8IDEVAI_LOG_ROTATE_SIZE` and `Q8IDEVAI_LOG_ROTATE_COUNT` env vars override the
...
the daemon-global Git process limits in `$Q8IDEVAI_HOME/config.json`:
...
Reload the daemon with `q8idevai reload`. Environment-variable overrides still require a restart because
...
Measure the MCP `tools/list` payload that Q8iDevAI injects into agents with:

```bash
npm run measure:agent-tools --workspace=@q8idevai/server
```
...
Windows. They inherit the daemon environment plus Q8iDevAI's lifecycle variables;
...
{
  "worktree": {
    "setup": "npm ci\ncp \"$Q8IDEVAI_SOURCE_CHECKOUT_PATH/.env\" .env\nnpm run db:migrate",
...
| Variable                    | Value                                                                                                                     |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `Q8IDEVAI_SERVICE_<NAME>_URL`  | Proxied URL for a declared peer service. Prefer this for peer discovery; it survives peer restarts.                       |
| `Q8IDEVAI_SERVICE_<NAME>_PORT` | Raw ephemeral port for a declared peer service. Use only as a bypass escape hatch; it can go stale if that peer restarts. |
| `Q8IDEVAI_URL`                 | Self alias for `Q8IDEVAI_SERVICE_<SELF>_URL`.                                                                                |
| `Q8IDEVAI_PORT`                | Self alias for `Q8IDEVAI_SERVICE_<SELF>_PORT`.                                                                               |
...
```json
{
  "scripts": {
    "web": {
      "type": "service",
      "command": "PORT=$Q8IDEVAI_PORT npm run dev:web"
    }
  }
}
```
...
`$Q8IDEVAI_HOME/config.json`, or replace it for one project with `worktree.servicePorts` in
`q8idevai.json`. The block accepts an inclusive `range` such as `"3000-4000"` or a `portScript`
...
values are available as `Q8IDEVAI_SCRIPTNAME`, `Q8IDEVAI_WORKSPACE_ID`, `Q8IDEVAI_BRANCH_NAME`, and
`Q8IDEVAI_WORKTREE_PATH`. The script must print one valid TCP port. Q8iDevAI trusts the external allocator,
...
```bash
q8idevai daemon config set features.webUi.enabled true
q8idevai daemon start
```

Or set the environment variable:

```bash
Q8IDEVAI_WEB_UI_ENABLED=true q8idevai daemon run
```
...
The desktop-managed daemon disables the bundled web UI by default (`Q8IDEVAI_WEB_UI_ENABLED=false`) because the desktop app already ships the renderer as `app-dist`. Shipping the same assets again inside `@q8idevai/server` would duplicate the ~10.8 MiB install. Desktop packaging also excludes `node_modules/@q8idevai/server/dist/server/web-ui/**` from the packaged app.
...
`npm run dev:server` builds the server-side workspace packages once, then keeps `@q8idevai/protocol` and `@q8idevai/client` fresh with TypeScript watch builds while the daemon runs. If you change protocol schemas or client code outside that watch workflow, rebuild the producer before trusting runtime behavior.
...
Use `npm run cli` to run the in-repo CLI from source (`npx tsx packages/cli/src/index.ts`). The script wraps the CLI with `scripts/dev-home.sh`, so it automatically uses this checkout's `.dev/q8idevai-home` and dev daemon endpoint unless you pass an explicit override. The globally installed `q8idevai` binary on macOS is a symlink into the installed Q8iDevAI desktop app, not this checkout — use it to drive the desktop's built-in daemon, but use `npm run cli` when you want to talk to the CLI you are editing.

Canonical automation uses `q8idevai project create/ls/rename/delete`, `q8idevai workspace create/ls/rename/archive`, `q8idevai heartbeat create/update/delete`, and the full `q8idevai schedule` group. MCP heartbeat automation is intentionally smaller: create and delete only. Detach remains an explicit user lifecycle action rather than an agent tool. `q8idevai run --new-workspace local|worktree` composes workspace creation with agent creation. The old `q8idevai worktree` and `q8idevai run --worktree` forms are hidden compatibility aliases.
...
npm run cli -- agent open <id>       # Focus an existing agent in Q8iDevAI Desktop
...
Set `Q8IDEVAI_HOST` to use the same target across invocations. An explicit
selector overrides both environment selectors. With both `Q8IDEVAI_HOME` and `Q8IDEVAI_HOST` set, pass an explicit selector. See [CLI target selection](../public-docs/cli.md#select-one-daemon).
...
sending a message. Use `q8idevai://h/<server-id>/agent/<agent-id>`, or run
`q8idevai agent open <agent-id>`. The CLI reads the local daemon's server ID by
...
```
$Q8IDEVAI_HOME/agents/{cwd-with-dashes}/{agent-id}.json
```

Find an agent by ID:

```bash
find $Q8IDEVAI_HOME/agents -name "{agent-id}.json"
```

Find by content:

```bash
rg -l "some title text" $Q8IDEVAI_HOME/agents/
```
...
Point Playwright MCP at the running Expo web target. For root checkout dev, `npm run dev:app` reserves `http://localhost:8081`. For Q8iDevAI-managed worktree app services, use the service URL or port shown by Q8iDevAI for that worktree.
...
directory to Cloudflare Pages with `npm run deploy:web --workspace=@q8idevai/app`.
...
Do not add service-worker caching casually. Q8iDevAI is a live control surface for
