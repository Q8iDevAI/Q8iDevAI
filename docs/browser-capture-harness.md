...
```bash
npm run capture-harness --workspace=@q8idevai/desktop
```
...
Q8IDEVAI_CAPTURE_HARNESS_GROUP=automation npm run capture-harness --workspace=@q8idevai/desktop
```

Run the shared browser profile fixture with:

```bash
Q8IDEVAI_CAPTURE_HARNESS_GROUP=browser-profile npm run capture-harness --workspace=@q8idevai/desktop
```
...
The existing `npm run test:e2e:browser-tabs --workspace=@q8idevai/desktop` journey
...
[the full-page repetition bug](https://github.com/Q8iDevAI/Q8iDevAI/issues/3196).
