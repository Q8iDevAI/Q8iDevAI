...
- the `q8idevai://app/` renderer mounts into `#root`;
...
```bash
Q8IDEVAI_DESKTOP_SMOKE=1 \
Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR=/tmp/q8idevai-desktop-smoke \
...
`packages/highlight/src/__tests__/dependency-closure.test.ts` replicates the packer's traversal statically and runs with the normal unit tests. It is scoped to `@q8idevai/highlight` on purpose: that tree is small and pure, so the check is exact. Running the same walk over `@q8idevai/server` produces dozens of false positives from optional dependencies loaded behind `try`/`catch`.
...
```bash
npm run test:e2e:browser-tabs --workspace=@q8idevai/desktop
```
...
- Server: `packages/server/src/test-utils/vitest-setup.ts` loads `.env.test`, sets `Q8IDEVAI_SUPERVISED=0`, and disables Git/SSH prompts. Add new global env shims here, not in individual tests.
...
- App Playwright shares one warmed Metro server per run and gives every Playwright worker its own isolated daemon and `Q8IDEVAI_HOME`. Spec files run concurrently without exposing one file's projects, agents, terminals, history, or provider configuration to another worker; tests within a file remain together so file-level setup is not repeated.
