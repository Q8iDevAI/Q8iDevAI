...
Q8iDevAI is a client-server system for monitoring and controlling local AI coding agents. The daemon runs on your machine, manages agent processes, and streams their output in real time over WebSocket. Clients (mobile app, CLI, desktop app) connect to the daemon to observe and interact with agents.

Your code never leaves your machine. Q8iDevAI is local-first.
...
The heart of Q8iDevAI. A Node.js process that:
...
| `server/agent/agent-storage.ts` | File-backed JSON persistence at `$Q8IDEVAI_HOME/agents/`                          |
| `server/agent/tools/`           | Transport-neutral catalog for workspaces, agents, permissions, and automation  |
| `server/agent/mcp-server.ts`    | Thin MCP adapter that registers the Q8iDevAI tool catalog with the MCP SDK        |
...
and clients. Server, app, CLI, and `@q8idevai/client` all depend on this package;
it does not depend on the server.

### `packages/client` — Daemon client library and SDK facade

Owns the low-level daemon WebSocket driver plus the higher-level `Q8iDevAIClient`
facade. App and CLI may import the low-level driver from
`@q8idevai/client/internal/daemon-client` during migration, while new SDK-shaped
code imports from `@q8idevai/client`.

`Q8iDevAIApi` is the capability-only boundary over workspaces, agents, terminals, providers, and config.
`Q8iDevAIClient` adds connection lifecycle. App plugin surfaces borrow an API over their selected
...
Commander.js CLI with Docker-style commands. Common agent operations are also exposed at the top level (e.g. `q8idevai ls`, `q8idevai run`).

- `q8idevai agent ls/run/import/attach/logs/stop/delete/send/inspect/wait/archive/reload/update/mode`
- `q8idevai daemon start/run/stop/restart/status/reload/config/pair/set-password`
- `q8idevai terminal ls/create/capture/send-keys/kill`
- `q8idevai script ls/start/stop`
- `q8idevai schedule create/ls/inspect/update/pause/resume/run-once/logs/delete`
- `q8idevai heartbeat create/update/delete`
- `q8idevai project create/ls/rename/delete`
- `q8idevai workspace create/ls/rename/archive`
- `q8idevai permit allow/deny/ls`
- `q8idevai provider ls/models`
- hidden legacy `q8idevai worktree create/ls/archive` compatibility alias
- `q8idevai speech …`
...
- Self-hosted relays opt into TLS with `daemon.relay.useTls` or `Q8IDEVAI_RELAY_USE_TLS=true`; the public (client-facing) TLS setting can be overridden independently via `daemon.relay.publicUseTls` or `Q8IDEVAI_RELAY_PUBLIC_USE_TLS`

The production relay server lives in [getq8idevai/q8idevai-relay](https://github.com/Q8iDevAI/Q8iDevAI-relay). It is a distributed Elixir service. The Cloudflare relay implementation in this monorepo is retained as legacy code and is not deployed.
...
>
> **Browser keyboard boundary.** Guest pages receive renderer-published shortcuts first. `Cmd/Ctrl+L` and `Cmd/Ctrl+R` are explicit guest-shell reservations; ordinary Q8iDevAI shortcuts run only after the page declines them. The sandboxed guest preload runs in every frame so focused iframes use the same boundary, while Node integration remains disabled. Human guest input disables Electron's menu fallback for plain keys. Agent-generated keys use guest `sendInputEvent` with `skipIfUnhandled`, so an unhandled Enter stops at the guest instead of reaching the host composer. Main selects the preload; it exposes no APIs to guest pages.
...
Human key -> guest WebContents
  ...
  `-- page keydown
        ...
        `-- published shortcut -> guest preload -> IPC(browserId) -> Q8iDevAI resolver
...
TanStack Router + Cloudflare Workers. Serves q8idevai.sh.
...
- Agent state persists to `$Q8IDEVAI_HOME/agents/{cwd-with-dashes}/{agent-id}.json`. Timeline rows are runtime memory; provider history is the durable transcript authority and resumed agents rebuild from it. That storage path is derived from `cwd`, not from workspace id.
...
- Handle their own authentication (Q8iDevAI does not manage API keys)
...
Providers that can accept native tool definitions should set `supportsNativeQ8iDevAITools` and read `launchContext.q8idevaiTools`. The daemon then passes the shared Q8iDevAI tool catalog directly and removes the internal Q8iDevAI MCP server from that provider launch config. Providers that only support MCP continue to receive the same tools through the MCP fallback at `/mcp/agents`.
...
`$Q8IDEVAI_HOME` defaults to `~/.q8idevai`. The most important files:
...
1. **Local daemon** (default): `q8idevai daemon start` on `127.0.0.1:6767`
