...
Q8iDevAI client UI translations live in `packages/app/src/i18n`.
...
The persisted app language setting is `"system" | "ar" | "en"`. `"system"` follows the device or browser locale when it maps to a supported locale; unsupported system locales fall back to English. Arabic maps from system locales `ar` and Arabic regional locales.
...
- Batch 4Z added full Arabic (`ar`) resource coverage, language selector labels, i18next registration, and system-locale mapping. Arabic does not enable RTL layout direction in this batch.
