...
Q8iDevAI has one linear release track even though npm dist-tags are independent
...
- A stable release moves both `latest` and `beta` to that stable version. This
  keeps users who install `@q8idevai/cli@beta` on the newest Q8iDevAI release after
...
approval; Q8iDevAI remains on major version zero until that deliberate decision.
...
```bash
Q8IDEVAI_VERSION=$(node -p "require('./package.json').version")
for package in highlight relay protocol client plugin server cli; do
  npm dist-tag add "@q8idevai/$package@$Q8IDEVAI_VERSION" beta
done
```

Verify both npm tags now resolve to `Q8IDEVAI_VERSION` before considering the
stable release complete.

The Docker workflow builds images from the checked-out source tree on pull requests and on `main` as non-publishing checks. Stable `vX.Y.Z` tag pushes publish `ghcr.io/getq8idevai/q8idevai:X.Y.Z` and `ghcr.io/getq8idevai/q8idevai:latest`; beta `vX.Y.Z-beta.N` tag pushes publish only `ghcr.io/getq8idevai/q8idevai:X.Y.Z-beta.N` and never move `latest`.

The production relay is the Elixir service in [getq8idevai/q8idevai-relay](https://github.com/Q8iDevAI/Q8iDevAI-relay), with its own deployment process. Q8iDevAI releases and pushes to this repository do not deploy it. The Cloudflare relay code and workflow in this repository are legacy and are not used in production.
...
- Betas publish npm packages with `--tag beta`, so `npm install @q8idevai/cli@beta` opts in while plain `npm install @q8idevai/cli` stays on `latest`
- Betas publish desktop assets and APKs for testing. They also build iOS, upload it to TestFlight, add it to the `Q8iDevAI Beta` external group, and submit it for Beta App Review. They do not submit mobile builds to the production stores.
...
Beta tags run `Release iOS Beta`. The workflow uploads the build to TestFlight, distributes it to the persistent `Q8iDevAI Beta` external group, and submits it for Beta App Review. Testers and the group are managed once in App Store Connect; releases require no dashboard action.
...
```jsonc
// mcp__q8idevai__create_heartbeat arguments
{
...
gh workflow run docker.yml \
  --ref main \
  -f q8idevai_version=X.Y.Z-beta.N \
  -f publish=true
```

This replaces `ghcr.io/getq8idevai/q8idevai:X.Y.Z-beta.N` in place without touching
desktop, APK, or EAS release builders. The Docker exception is safe because the
dispatch runs from `--ref main` and uses the explicit `q8idevai_version`; it does
...
```bash
RELEASE_LOOKUP=$(node scripts/github-release.mjs --repo getq8idevai/q8idevai --tag vX.Y.Z)
...
RELEASE_LOOKUP=$(node scripts/github-release.mjs --repo getq8idevai/q8idevai --tag vX.Y.Z-beta.N)
...
- `version:all:*` bumps root + syncs workspace versions and `@q8idevai/*` dependency versions
...
The changelog is shown on the Q8iDevAI homepage. Each bullet is a compact factual record of
...
- **Keep the scope exact.** A conditional bug is not a general reliability problem. Do not
  broaden one failure into claims that Q8iDevAI is now faster, smoother, responsive, or reliable.
...
| Avoid                                                        | Write                                                 |
| ------------------------------------------------------------ | ----------------------------------------------------- |
| Q8iDevAI stays responsive with many idle Git workspaces         | Removed periodic Git polling for idle workspaces      |
...
Format: append `([#123](https://github.com/Q8iDevAI/Q8iDevAI/pull/123) by [@user](https://github.com/user))` at the end of each bullet. For changes spanning multiple PRs or contributors:

```markdown
- Voice mode now works on tablets with proper microphone permissions. ([#210](https://github.com/Q8iDevAI/Q8iDevAI/pull/210), [#215](https://github.com/Q8iDevAI/Q8iDevAI/pull/215) by [@alice](https://github.com/alice), [@bob](https://github.com/bob))
```

Rules:

- **Always link the PR number** as `[#N](https://github.com/Q8iDevAI/Q8iDevAI/pull/N)`.
