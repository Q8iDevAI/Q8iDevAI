...
catalog probe. `Q8IDEVAI_PROVIDER_REFRESH_TIMEOUT_MS` sets it when the config field is absent.
...
Q8iDevAI passes those variables through to the Codex app-server process **and** maps them into Codex's thread config under `model_provider` / `model_providers`, because Codex reads provider routing from config rather than from `OPENAI_BASE_URL` alone.
...
### What Q8iDevAI wires up

Under the hood, for each custom Codex provider Q8iDevAI injects this into Codex's config:
...
- `base_url` — taken from `OPENAI_BASE_URL`. If it does not already end in `/v1`, Q8iDevAI appends `/v1`. Trailing slashes are stripped.
- `wire_api` — always `"responses"` (OpenAI Responses API protocol).
- `env_key` — set to `"OPENAI_API_KEY"` when that env var is present and non-empty, so Codex reads the key from the same env var Q8iDevAI passes through.
...
- Set `models` explicitly. Custom endpoints expose their own model IDs (`anthropic/claude-opus-4-7`, `qwen/qwen3-coder`, `local/llama`, etc.), and Q8iDevAI does not discover them automatically for Codex.
...
Each profile appears as a separate provider in the Q8iDevAI app. You can select which one to use when launching an agent.
...
The `command` array completely replaces the default command for that provider. The binary must exist on the system — Q8iDevAI checks for its availability and will mark the provider as unavailable if not found.
...
Custom OMP profiles should extend `omp`. They inherit the OMP adapter's `rpc-ui` approvals, native Q8iDevAI host tools, provider-managed subagents, and import behavior:
...
`params.sessionDir` is used only for importing sessions that were started outside Q8iDevAI. If `command` or XDG env vars move OMP's state directory, set `params.sessionDir` to the resulting OMP JSONL session directory; launching and resuming still go through the configured command. OMP waits 20 seconds for its initial `ready` frame and 60 seconds for later control-plane RPCs by default. `params.rpcTimeoutMs` overrides both deadlines.
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) is an open standard for communication between editors and AI coding agents — think LSP but for AI agents. Any agent that supports ACP can be added to Q8iDevAI as a custom provider.

ACP agents communicate over JSON-RPC 2.0 on stdio. Q8iDevAI spawns the agent process and talks to it through stdin/stdout.

Q8iDevAI also ships an in-app ACP provider catalog for common agents, including CodeWhale, Cursor, DeepAgents, DimCode, Gemini CLI, Hermes, Qwen Code, and Kimi Code. Catalog entries create the same `extends: "acp"` provider config shown below.
...
Q8iDevAI tools such as subagent creation come from the shared internal tool catalog. ACP providers receive those tools through the MCP fallback by default because ACP exposes `mcpServers`, not Q8iDevAI's native tool catalog. Some ACP adapters cannot create sessions when `mcpServers` is non-empty. Disable injected MCP for those providers with `params.supportsMcpServers: false`:
...
while terminal operations run through Q8iDevAI on the host. To customize which
operations Q8iDevAI handles, configure client capabilities in provider params:
...
container. When delegating filesystem operations to Q8iDevAI (`fs.readTextFile: true`
or `fs.writeTextFile: true`), ensure the agent and Q8iDevAI share equivalent
absolute workspace paths.

### Generic ACP diagnostics

Q8iDevAI diagnostics for `extends: "acp"` providers report the configured command, resolved launcher binary, version output, ACP `initialize`, ACP `session/new`, model count, modes, and final status.
...
### How ACP providers work in Q8iDevAI

When you launch an agent with an ACP provider:

1. Q8iDevAI spawns the process using the configured `command`
2. Sends an `initialize` JSON-RPC request over stdin
3. The agent responds with its capabilities, available modes, and models
4. Q8iDevAI creates a session and sends prompts through the ACP protocol
5. The agent streams responses, tool calls, and permission requests back over stdout

Every ACP provider exposes an **Auto Accept** toggle. Enable it per session to let Q8iDevAI approve
ACP permission requests without surfacing each prompt. If the provider sends no allow option,
Q8iDevAI leaves the request for you to answer. Unattended agents enable Auto Accept unless you
