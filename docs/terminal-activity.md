...
Q8iDevAI surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
The plugin translates both event contracts into the existing Q8iDevAI hook events. OpenCode 2 disposes its event subscription when the plugin unloads.
...
removes Q8iDevAI's marker-matched hooks on disable. `q8idevai hooks` keeps working regardless — the gate
...
When enabled, Q8iDevAI installs provider hooks globally:

- Claude hooks are written to `~/.claude/settings.json` (or `CLAUDE_CONFIG_DIR/settings.json` when that override is set).
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each Q8iDevAI hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/q8idevai-terminal-activity.js` (or `~/.config/opencode/plugins/q8idevai-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).

Installation is marker-based/idempotent for config hooks and exact-file/idempotent for the OpenCode plugin. Q8iDevAI preserves user hooks, removes only its own marker-matched command hooks, and leaves hooks installed across daemon shutdown. Outside a Q8iDevAI terminal they are inert because the command or plugin is gated on `Q8IDEVAI_TERMINAL_ID`.
...
```sh
[ -n "$Q8IDEVAI_TERMINAL_ID" ] && "${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks claude <event>
```

Codex also receives the Windows equivalent:

```bat
if defined Q8IDEVAI_TERMINAL_ID (if defined Q8IDEVAI_HOOK_CLI ("%Q8IDEVAI_HOOK_CLI%" hooks codex <event>) else (q8idevai hooks codex <event>))
```

The daemon resolves the current CLI through `Q8IDEVAI_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `Q8IDEVAI_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `q8idevai` if the hook env is missing and no-ops outside Q8iDevAI terminals because the `Q8IDEVAI_TERMINAL_ID` gate remains first. Q8iDevAI also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `q8idevai hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
