...
Q8iDevAI uses **file-based JSON persistence** instead of a traditional database. All data is validated at runtime with Zod schemas. Most stores write atomically (write to temp file, then rename); a few still use plain `writeFile` — see each section. There is no schema-versioning/migration framework — schemas rely on optional fields with defaults for forward compatibility, with a small amount of inline normalization in `persisted-config.ts` for legacy provider/speech entries.

All server-side stores live under `$Q8IDEVAI_HOME` (defaults to `~/.q8idevai`).
...
├── q8idevai.pid                            # Daemon PID lock file
...
├── runtime/
│   └── managed-processes/
│       └── {recordId}.json              # Helper processes owned by Q8iDevAI; reconciled on daemon bootstrap
├── plugins/
...
**Path:** `$Q8IDEVAI_HOME/agents/{project-dir}/{agentId}.json`
...
| `labels`             | `Record<string, string>`                 | Key-value labels (default `{}`). Q8iDevAI uses `q8idevai.parent-agent-id` for parentage and client-scoped `q8idevai.open-agent-tab.*` labels while managed subagent tabs are open — see [agent-lifecycle.md](./agent-lifecycle.md)                                                                                                                                                           |
...
**Path:** `$Q8IDEVAI_HOME/config.json`
...
`q8idevai reload` reads and validates this file once inside the daemon. That snapshot drives resolution,
...
```
{
  ...
  worktrees?: {
    root?: string            // optional root for new worktrees; defaults to $Q8IDEVAI_HOME/worktrees
...
Q8iDevAI writes it atomically. An update creates and validates a new version directory before changing
...
Each entry may include a Q8iDevAI-tool policy:
...
Absent `q8idevaiTools`, or absent fields within it, means Q8iDevAI tools are enabled and all tools are
allowed. `enabled: false` disables the provider's Q8iDevAI catalog; `disabledTools` lists exact tool
...
Q8iDevAI tools; otherwise the provider policy applies. Provider and global policy are resolved when a
...
`agents.metadataGeneration.providers` controls the preferred structured-generation fallback order for daemon-side metadata tasks such as commit messages, PR text, branch names, and generated agent titles. Entries are tried first in the configured order, then Q8iDevAI falls through to dynamically discovered defaults and finally the current selection when available.
...
| ------------------------------------ | ------------------------ |
| `Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND` | `maxProcessesPerSecond`  |
| `Q8IDEVAI_GIT_MAX_PROCESS_CONCURRENCY`  | `maxProcessConcurrency`  |
| `Q8IDEVAI_GIT_CONCURRENCY`              | Legacy concurrency alias |

`Q8IDEVAI_GIT_MAX_PROCESS_CONCURRENCY` wins when it and the legacy alias are both set. Run `q8idevai reload`
...
| Env var                        | Applies to                      |
| ------------------------------ | ------------------------------- |
| `Q8IDEVAI_VOICE_STT_PROVIDER`     | Voice mode STT provider         |
| `Q8IDEVAI_DICTATION_STT_PROVIDER` | Composer dictation STT provider |
| `Q8IDEVAI_VOICE_TTS_PROVIDER`     | Voice mode TTS provider         |
...
**Path:** `$Q8IDEVAI_HOME/schedules/{id}.json`
...
**Path:** `$Q8IDEVAI_HOME/projects/projects.json`
...
**Path:** `$Q8IDEVAI_HOME/projects/workspaces.json`
...
| `baseBranch`                   | `string \| null`                                             | Normalized branch the Q8iDevAI worktree was created from; null for directories, local checkouts, and checkout-branch worktrees                                                                   |
| `isQ8iDevAIOwnedWorktree`         | `boolean`                                                    | Whether Q8iDevAI owns and may remove/recreate the backing `worktreeRoot`                                                                                                                         |
...
**Path:** `$Q8IDEVAI_HOME/projects/workspace-labels.json`
...
**Path:** `$Q8IDEVAI_HOME/push-tokens.json`
...
These small files are not validated as full Zod schemas but are persisted under `$Q8IDEVAI_HOME` for daemon identity and runtime coordination.

| Path                  | Format                                                         | Notes                                                                             |
| --------------------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `server-id`           | Plain text, e.g. `srv_<base64url>`                             | Stable per-`$Q8IDEVAI_HOME` daemon ID. Overridable via `Q8IDEVAI_SERVER_ID` env.        |
...
| `q8idevai.pid`           | JSON `{ pid, startedAt, ... }`                                 | PID lock; prevents two daemons sharing one `$Q8IDEVAI_HOME`.                         |
...
- **Workspace-owned** (independent per workspace): keyed by `workspaceId`, with `cwd` used only as a fallback when no `workspaceId` is present. Review draft comments (`@q8idevai:review-draft-store`), diff-mode overrides (in-memory), workspace composer attachments, and file-explorer nav/expand state. The `workspaceId` part of these keys is **opaque** — never parse it back into a path.
...
**AsyncStorage key:** `q8idevai-drafts` (version 2)
...
**IndexedDB database:** `q8idevai-attachment-bytes`, object store: `attachments`
