...
Q8iDevAI lets your coding agents coordinate other agents, split work across providers and machines, and keep tasks moving automatically.
...
- **Work across machines:** use the [CLI](/docs/cli#connecting-to-a-remote-daemon) to launch and manage agents on another reachable Q8iDevAI host.
...
Use built-in Q8iDevAI tools or the CLI. Both let an agent launch and coordinate workers.

### Built-in Q8iDevAI tools (MCP)

Enable Q8iDevAI tools so agents running inside Q8iDevAI can manage agents and workspaces on their host directly.

1. Open **Settings → your host → Agents**.
2. Turn on **Enable Q8iDevAI tools**. Tool injection is off by default.
3. Start a new agent, or reload an existing agent so it receives the tools.
4. Ask:

> Use Q8iDevAI to launch a second agent to review this branch. Have it report potential bugs without changing files, then summarize its findings.
...
Agents with shell access can also use the Q8iDevAI CLI. This route does not require enabling tool injection. With Q8iDevAI installed, a running host, and Codex configured:

```bash
q8idevai run --provider codex --background \
  "Review this branch without changing files"
q8idevai ls -a
```

The first command starts a worker and returns immediately; the second lists agents from active workspaces, including archived agents. When a Q8iDevAI agent runs the command, the worker becomes its subagent in the same workspace. From your own terminal, it starts in a new local workspace.
