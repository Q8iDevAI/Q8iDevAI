...
description: Create and manage Q8iDevAI schedules with q8idevai schedule.
nav: CLI
order: 27
category: Schedules
---
...
`q8idevai schedule` creates and manages new-agent [schedules](/docs/schedules) from your terminal, useful for headless boxes and scripts. Every run starts a fresh agent.
...
Inside a running Q8iDevAI agent, create a heartbeat for that same conversation:

```bash
q8idevai heartbeat create \
  --cron "*/20 * * * *" \
  --name heartbeat \
...
```bash
q8idevai heartbeat update <id> --cron "*/10 * * * *"
q8idevai heartbeat delete <id>
```

Updating a heartbeat changes only its cron cadence and optional time zone. Its target and prompt stay fixed. Heartbeat commands require `Q8IDEVAI_AGENT_ID`, which Q8iDevAI sets inside agent sessions.
...
q8idevai schedule inspect <id>
q8idevai schedule logs <id>
q8idevai schedule pause <id>
q8idevai schedule resume <id>
q8idevai schedule run-once <id>
q8idevai schedule update <id> --every 10m --max-runs 6
q8idevai schedule delete <id>
```
...
Start the command with `q8idevai --host <target> schedule create ...` when targeting a remote daemon. Pass `--cwd`; your local working directory may not exist on the remote machine.
