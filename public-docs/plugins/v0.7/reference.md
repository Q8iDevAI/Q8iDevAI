...
> **For Q8iDevAI v0.7.x.** Return to the [v0.7 quickstart](/docs/plugins/v0.7).
...
The required root manifest is `q8idevai-plugin.json`. It contains the default plugin ID:
...
Q8iDevAI builds separate client and server bundles from `index.ts`. It rejects imports from `*.server` files into client modules and imports from `*.client` files into server modules. Keep shared modules free of Node and React Native runtime code.
...
| Module                          | Use it for                            |
| ------------------------------- | ------------------------------------- |
| `@q8idevai/plugin`              | Contribution contracts and data hooks |
| `@q8idevai/plugin/react-native` | Q8iDevAI UI components and UI hooks      |
| `@q8idevai/plugin/server`       | Shared RPC and attachment contracts   |
...
Client components are React Native components rendered by Q8iDevAI. Web clients render them through React Native Web. Browser globals such as `localStorage` and `location` exist only when `layout.platform === "web"`; iOS and Android have no equivalent. Gate any use on that field.

There is no plugin storage API. Browser storage does not persist settings across Q8iDevAI clients. There is also no general host navigation API: plugin code cannot open native Q8iDevAI routes. Command Center callbacks can only open surfaces and panels registered by the same plugin.

### Server runtime

Q8iDevAI provides `@q8idevai/plugin`, `@q8idevai/plugin/server`, and `zod` to server code. Backend contributions run in a daemon subprocess with Node access to the host machine. Keep filesystem, process, credential, and other machine-local work in `*.server.ts` files.
...
```tsx
import type { PluginSurfaceProps } from "@q8idevai/plugin";
...
Import Q8iDevAI-owned UI from `@q8idevai/plugin/react-native` in `*.client.tsx` files. This example
opens a controlled modal, renders a host icon, and confirms the action with a toast:

```tsx
import type { PluginSurfaceProps } from "@q8idevai/plugin";
import { Icon, Modal, useToast } from "@q8idevai/plugin/react-native";
...
```tsx
import type { PluginContext, PluginTimelineItemProps } from "@q8idevai/plugin";
...
```ts
import type { PluginHandlerContext } from "@q8idevai/plugin";
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@q8idevai/plugin";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
```tsx
import { defineRpc } from "@q8idevai/plugin/server";
import { z } from "zod";
...
Register a command that runs entirely in the Q8iDevAI client when the user submits it from the message
composer:
...
`args` is `"src"`; Q8iDevAI trims only the remainder's leading and trailing whitespace. Q8iDevAI owns the
...
export function contributeClient(client: PluginClientContext) {
  ...
  const unsubscribe = client.q8idevai.agents.subscribe((update) => {
...
`q8idevai`, typed `rpc`, `openSurface`, explicit-context `openPanel`, and `addComposerPill`.
`addComposerPill` returns an idempotent removal function. Q8iDevAI also removes every outstanding pill
when the client entrypoint, plugin installation, or host connection is torn down.

Q8iDevAI owns the pressable, shared pill chrome, pending state, error reporting, and track-bar
...
```tsx
import { useQ8iDevAI } from "@q8idevai/plugin";
...
The returned API covers projects, workspaces, agents, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because Q8iDevAI owns the connection.
...
```tsx
import { useRpc } from "@q8idevai/plugin";
...
Backend handlers receive the same `Q8iDevAIApi` as `{ q8idevai }`. Their connection belongs to the subprocess and closes when the plugin stops. Backend code can use Node APIs and dependencies installed in the plugin directory.
...
q8idevai plugin logs my-plugin --host <url>
```
...
```ts
import { defineAttachmentSource, defineRpc } from "@q8idevai/plugin/server";
import { z } from "zod";
