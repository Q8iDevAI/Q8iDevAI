...
> **For Q8iDevAI v0.8 beta.** This migration is not required for Q8iDevAI v0.7.
...
7. Keep `q8idevai-plugin.json`, `package.json`, and `tsconfig.json` at the root.
8. Delete the old root entry. Q8iDevAI does not load it.
...
| `import { defineRpc, defineAttachmentSource } from "@q8idevai/plugin/server"` in shared files | `import { defineRpc, defineAttachmentSource } from "@q8idevai/plugin"`                                       |
| `ZodOutput<typeof contract.input>` handler parameter types                                    | `RpcInput<typeof contract>` from `@q8idevai/plugin`; `RpcOutput` for return types                            |

Import `PluginClientContext` from `@q8idevai/plugin/client` and `PluginServerContext` from
`@q8idevai/plugin/server`. Remove imports of the old context type. Client registrations return idempotent removal functions, except header buttons and composer pills, which return `{ update, remove }` handles. Preserve any remover the plugin calls before teardown; Q8iDevAI removes outstanding
registrations after the entry cleanup runs.

### Composer pills

Update the plugin project's `@q8idevai/plugin` dependency, then run `npm run typecheck`.
...
Move hooks (`useQ8iDevAI`, `useRpc`, `useSettings`, `useAgent`, `useWorkspace`) and client contribution
types from `@q8idevai/plugin` to `@q8idevai/plugin/client`. Move `Icon` to
`@q8idevai/plugin/client/react-native`. Import server contexts and lifecycle contracts from
`@q8idevai/plugin/server`. Shared helpers (`defineRpc`, `defineSettings`, `defineAttachmentSource`),
...
| Old entry                       | 0.8 entry                              |
| ------------------------------- | -------------------------------------- |
| `@q8idevai/plugin/react-native` | `@q8idevai/plugin/client/react-native` |
| `@q8idevai/plugin/ui`           | `@q8idevai/plugin/client/ui`           |
| `@q8idevai/plugin/provider`     | `@q8idevai/plugin/server/provider`     |
| `@q8idevai/plugin/acp`          | `@q8idevai/plugin/server/acp`          |

The old entries and the pre-0.8 `@q8idevai/plugin` scope are removed. `/client/host` is private to
Q8iDevAI's app integration and is never a plugin-author import.
...
| -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `This plugin has no requirements.q8idevai`                                                                                    | Complete the migration and declare the range in step 7.                                                                   |
| `This plugin was made for an older version of Q8iDevAI`                                                                       | The directory still has only the old root entry. Create a runtime entry, move registrations, then delete the old file.    |
...
## 7. Declare the Q8iDevAI requirement

After migrating the entries and imports, add the minimum runtime version to `q8idevai-plugin.json`:
...
Keep your existing ID and build commands. Missing `requirements.q8idevai` means `<0.8.0`, so Q8iDevAI 0.8
rejects the plugin even if its files have been moved. Adding the field alone does not migrate the
code. Update the local `@q8idevai/plugin` development dependency to the version you target and
...
```bash
npm run typecheck
q8idevai plugin reload <plugin-id>
q8idevai plugin ls
```
