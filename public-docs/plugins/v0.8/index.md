...
> **For Q8iDevAI v0.8 beta.** Use the [v0.7 docs](/docs/plugins/v0.7)
> if you run the stable release.

> **Experimental:** The plugin API is still evolving, so expect breaking changes and updates to
> your plugins as Q8iDevAI evolves. See the [plugin roadmap](https://github.com/Q8iDevAI/Q8iDevAI/labels/plugins)
> for planned contribution surfaces.

A plugin is a TypeScript project installed into one Q8iDevAI daemon. It can add
...
contributions run on every Q8iDevAI client connected to that daemon, including mobile.
...
development dependencies for typechecking and tests only; Q8iDevAI supplies the plugin SDK, React,
...
```text
workspace-plugin/
  q8idevai-plugin.json      # plugin ID and supported Q8iDevAI versions
  index.client.tsx       # runs in the Q8iDevAI app
...
user's access on the daemon machine, and client code runs inside the Q8iDevAI app. Installing a plugin
...
daemon's `config.json`; after editing the file, apply it with `q8idevai reload --json`. An automated
...
`q8idevai plugin ls` should report the plugin as `running`. Open Q8iDevAI, choose **Greeting** in the
...
the client is viewing the host you installed into. `q8idevai plugin logs workspace-plugin` shows the
...
```tsx
import { type PluginWorkspacePanelProps, useWorkspace } from "@q8idevai/plugin/client";
...
`theme.colors`, and `layout.compact` drives spacing, so the panel works in every Q8iDevAI theme and on
...
A failed reload stays failed and reports its error in `q8idevai plugin ls`; fix the source and reload
again.
...
Q8iDevAI compiles TypeScript itself, so most plugins need no build step. A repository that must
install a dependency Q8iDevAI does not provide, or generate files, declares
...
The tail includes `[q8idevai]` loading, ready, stopping, and stopped entries, plus compilation and load
...
- [TypeScript SDK](/docs/sdk): the workspace, agent, provider, and config API available as `q8idevai`
