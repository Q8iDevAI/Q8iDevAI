...
description: Add a coding agent to Q8iDevAI directly or adapt an ACP agent.
...
> **For Q8iDevAI v0.8 beta.** Start with the
> [plugin quickstart](/docs/plugins/v0.8) if you have not built a Q8iDevAI plugin before.

A provider plugin connects a coding agent to Q8iDevAI without adding it to Q8iDevAI core. Publish the
plugin in a Git repository and users can install and update it with `q8idevai plugin add` and
`q8idevai plugin update`.
...
- [`provider-direct`](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/provider-direct): sessions, settings, prompts, persistence, child sessions, and a provider-owned timeline renderer;
- [`provider-acp-transformer`](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/provider-acp-transformer): an ACP command template with a Zod-validated vendor edit transformer. Replace `example-acp --stdio` with an installed ACP agent before loading it;
- [`inline-thinking`](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/inline-thinking): a renderer-only plugin that does not implement a provider.
...
} from "@q8idevai/plugin/server/provider";
...
Q8iDevAI requests the catalog before creating a session. Return the choices needed by the agent form:
...
the user timeline item so Q8iDevAI replaces its optimistic message. Every started turn needs one
...
Q8iDevAI renders these controls in the composer. A user change arrives as `session.configure`; apply it
...
for provider-specific configuration that Q8iDevAI does not render. A provider-specific token budget
...
Return an opaque persistence value in `session.opened` or `session.persistence`; Q8iDevAI stores it
...
Q8iDevAI refreshes an agent by closing its current provider session and opening it again with current
...
Use `restoration: "core"` when Q8iDevAI can reopen a session from persistence. A provider-owned child
...
Advertise only capabilities you implement. If steering is unsupported, omit `prompt.steer`; Q8iDevAI
...
a running tool, or a todo list. Q8iDevAI derives the live delta and keeps the normal timeline.
...
```ts
import type { AcpTransformer } from "@q8idevai/plugin/server/acp";
import { z } from "zod";
...
q8idevai plugin update my-provider-plugin
```

Keep vendor compatibility and releases in that repository. Q8iDevAI core should only change when the
