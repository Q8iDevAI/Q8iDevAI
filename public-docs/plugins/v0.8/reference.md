...
> **For Q8iDevAI v0.8 beta.** Return to the [v0.8 quickstart](/docs/plugins/v0.8).

Migrating an existing plugin? Follow the standalone [runtime-entry migration guide](/docs/plugins/v0.8/migration).

Local plugins are directory sources installed into one Q8iDevAI daemon. A plugin can contribute:

- React Native surfaces and sidebar items to Q8iDevAI clients;
...
- normal Q8iDevAI operations through the TypeScript SDK;
- searchable external resources in the message composer.

Plugin code is trusted and unsandboxed. Client surfaces run in the Q8iDevAI app. Backend contributions run in a subprocess with access to the daemon machine, including its files, processes, credentials, and network.

## Project files

`q8idevai plugin init /absolute/path/to/my-plugin` creates:

```text
my-plugin/
  q8idevai-plugin.json
  index.client.tsx
  index.server.ts
  client/greeting.tsx
  server/greeting.ts
...
The required root manifest is `q8idevai-plugin.json`. It contains the default plugin ID and supported Q8iDevAI versions:

```json
{ "id": "my-plugin", "requirements": { "q8idevai": ">=0.8.0" } }
```

### Requirements

`requirements` is an optional object. Its currently supported key, `q8idevai`, accepts an npm semver
range. An omitted `requirements.q8idevai` means `<0.8.0`: the plugin predates the first breaking
plugin release. Q8iDevAI 0.8 and later reject it with a link to the [migration guide](migration).
...
Prerelease Q8iDevAI versions also satisfy a range their stable core (`major.minor.patch`) satisfies, so `0.8.0-beta.1` satisfies `>=0.8.0` but not `<0.8.0`.

`q8idevai plugin init` writes `>=` followed by the current CLI version and pins the matching SDK
...
For example: `Plugin "review" requires Q8iDevAI >=0.8.0. Your daemon is 0.7.2.` Use a compatible plugin
...
| ------------------ | --------------------- | --------------------- | ------------------------------------------------------------------------------- |
| `index.client.tsx` | Q8iDevAI app, per client | `PluginClientContext` | When the plugin has any UI, callback, theme, or attachment source               |
...
The generated `package.json` installs `@q8idevai/plugin` and the other host modules as development
dependencies for local typechecking and tests. Q8iDevAI supplies their runtime instances. Consumers do
...
Q8iDevAI builds each bundle from its matching entry. An import from `client/` into the daemon bundle,
...
The SDK root (`@q8idevai/plugin`) contains shared data, schemas, and runtime-neutral helpers only.
...
Q8iDevAI provides these modules to client code:

| Module                                 | Use it for                                                                                        |
| -------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `@q8idevai/plugin`                     | Shared data, `defineRpc`, `defineSettings`, `defineAttachmentSource`, `RpcInput`, and `RpcOutput` |
| `@q8idevai/plugin/client/ui`           | Named, composable settings components                                                             |
| `@q8idevai/plugin/client/react-native` | Q8iDevAI UI components and UI hooks                                                                  |
| `@q8idevai/plugin/client`              | Client contribution contexts, `useQ8iDevAI`, `useRpc`, `useSettings`, and data hooks                 |
...
Do not import `lucide-react-native`, `react-native-svg`, or DOM libraries. Set contribution `icon` fields to a [Lucide icon name](https://lucide.dev/icons/); Q8iDevAI validates the name and renders the icon.
...
Q8iDevAI provides `@q8idevai/plugin`, `@q8idevai/plugin/server`,
`@q8idevai/plugin/server/provider`, `@q8idevai/plugin/server/acp`, and `zod` to server code. Backend
...
`@q8idevai/plugin/server/provider`. Its connection accepts inputs with `send()` and emits complete state
...
Provider settings are toggle/select descriptors that Q8iDevAI renders in the composer. Keep
provider-private JSON under `providerOptions`. Host tools arrive as MCP servers in the complete
session config.

Q8iDevAI refreshes an agent by closing its current provider session and opening it with current
configuration and persistence. Providers re-read external state during `session.open`.

Use `runAcpProvider()` from `@q8idevai/plugin/server/acp` to adapt a command-backed ACP. Add transformer
...
allowed. Q8iDevAI reads and sanitizes the file when the plugin starts; the string is never an inline
SVG or URL.
...
which return `{ update, remove }` handles. The entry cleanup runs before Q8iDevAI removes remaining registrations.
...
Cleanup can be async. Release timers, watchers, sockets, and other resources created by the plugin. Q8iDevAI also removes registrations, unmounts surfaces, rejects pending RPCs, closes the plugin's daemon session, and stops its subprocess on reload, disable, removal, disconnect, or daemon shutdown.
...
| ------------------------------- | ---------------------------------- | ------------------------------------------------------------ |
| `server.on(name, callback)`     | `(event, { q8idevai, signal })`       | `void` or `Promise<void>`                                    |
| `server.before(name, callback)` | `({ request }, { q8idevai, signal })` | Modified request, or `undefined` to keep it; async supported |
...
Copy [server/inspect.ts](https://github.com/Q8iDevAI/Q8iDevAI/blob/main/plugin-examples/lifecycle-actions/server/inspect.ts)
into your plugin. The helper imports types from `@q8idevai/protocol/agent-types`; add
`@q8idevai/protocol` at the same version as your plugin SDK to your development dependencies
...
export default function contribute(server: PluginServerContext) {
  server.on("agent.turn_ended", async (event, context) => {
    ...
    if (/out of credits/i.test(text)) {
      await context.q8idevai.agents.ref(event.agent.id).send("Try again.");
    }
  });
...
Using `shellCommand` from the same [helper file](https://github.com/Q8iDevAI/Q8iDevAI/blob/main/plugin-examples/lifecycle-actions/server/inspect.ts):
...
export default function contribute(server: PluginServerContext) {
  server.on("agent.permission_requested", async (event, context) => {
    ...
    const agent = context.q8idevai.agents.ref(event.agent.id);
...
**Shared payload shapes** (`@q8idevai/plugin/server`):
...
Creation request
  ...
  → derive launch configuration with Q8iDevAI runtime tools and daemon prompt
  → agent.session_open hooks (same ordering; env only)
  → set Q8IDEVAI_AGENT_ID and Q8IDEVAI_AGENT_CWD
...
| Contract                           | Behavior                                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `context.q8idevai`                    | Existing SDK connected to this daemon                                                                     |
...
| Plugin                                                                                                 | Includes                                                                     |
| ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| [lifecycle-logger](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/lifecycle-logger)       | All eleven hooks; JSON logs with environment values redacted                 |
| [lifecycle-actions](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/lifecycle-actions)     | Follow-ups, permissions, environment, provider switching, worktree selection |
| [agent-configuration](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/agent-configuration) | MCP injection and Codex sandbox/approval options                             |

Read logger output with `q8idevai plugin logs lifecycle-logger` or the host's `daemon.log`.
...
```tsx
import type { PluginSurfaceProps } from "@q8idevai/plugin/client";
...
| Field        | Meaning                                                                                                                      |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `theme`      | Typed `PluginTheme` color tokens for the active Q8iDevAI theme.                                                                 |
...
Q8iDevAI owns the route, header, close action, host picker, error boundary, and query client. The plugin owns the surface body.

## Host UI

Import Q8iDevAI-owned UI from `@q8idevai/plugin/client/react-native` in client code. This example
opens a controlled modal, renders a host icon, and confirms the action with a toast:

```tsx
import type { PluginSurfaceProps } from "@q8idevai/plugin/client";
import { Icon, Modal, useToast } from "@q8idevai/plugin/client/react-native";
...
Modal children keep the plugin runtime context. `useQ8iDevAI`, `useRpc`, `useWorkspace`, and
`useAgent` work inside them.

### Scrolling

Import `ScrollView` and `FlatList` from `@q8idevai/plugin/client/react-native` when content can appear in a
Q8iDevAI modal. They accept React Native props and refs and integrate with the sheet's gestures. Outside
...
```tsx
import { FlatList, Modal } from "@q8idevai/plugin/client/react-native";
...
```tsx
import { copyText, useToast } from "@q8idevai/plugin/client/react-native";
...
for long-press selection and OS Copy. Import `TextInput` from `@q8idevai/plugin/client/react-native` for modal forms. It accepts React Native
...
The runnable [modal UI example](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/modal-ui)
...
`Icon` renders a [Lucide icon](https://lucide.dev/icons/) from Q8iDevAI's installed icon set. Plugin bundles do not import
...
registrations are client contributions. Q8iDevAI applies the transformer while building the render
model, including every live streaming update.

```tsx
import type { PluginClientContext, PluginTimelineItemProps } from "@q8idevai/plugin/client";
import { Text } from "react-native";
import { z } from "zod";
...
paragraph separators. Q8iDevAI invokes transformers before splitting native Markdown or grouping
...
Each replacement may set an optional plugin-local `id`; otherwise Q8iDevAI uses its index within that
source item's output.

Renderers receive `agentId`, `item`, `timestamp`, `theme`, `host`, and `layout`. Q8iDevAI validates
`item.data` with the registered schema before rendering. Keep transformers synchronous and
deterministic. Q8iDevAI memoizes results by source-item reference and derives replacement identity from
the source row, so updates to one streaming item do not remount its renderer. Use the exported
`useRevealedText(text, phase)` hook when a renderer should pace streaming text like Q8iDevAI's built-in
assistant rows.
...
```ts
import type { PluginHandlerContext } from "@q8idevai/plugin/server";

async function publishReview(agentId: string, { q8idevai }: PluginHandlerContext) {
  await q8idevai.agents.ref(agentId).timeline.append({
    type: "plugin",
    id: "review",
    kind: "review-result",
    version: 1,
...
same plugin and `id`. If its renderer is missing, Q8iDevAI shows the existing unavailable row. Daemons
...
Plugin UI runs on desktop, browser, iOS, and Android, across every Q8iDevAI theme. `theme` is a typed `PluginTheme` mapped from the active host theme. Color and spacing must come from those props. Hardcoded colors and unstyled `Text` break when the host theme changes.
...
Every color is a hex string; anything else fails to load. Q8iDevAI expands the palette into the full
...
`appearance` is `"light"` or `"dark"`. Q8iDevAI uses it to select the matching surface, status,
...
later disabled or removed, Q8iDevAI falls back to the default theme rather than leaving the app
unpainted.
...
The component receives `PluginSurfaceProps`. Q8iDevAI owns the header, back navigation, safe areas,
...
Import settings components from `@q8idevai/plugin/client/ui`. They work with your own state and RPCs;
no form wrapper or storage binding is required.

```tsx
import { useState } from "react";
import { SettingsCard, SettingsSection, SettingsSwitch } from "@q8idevai/plugin/client/ui";
...
| `SettingsSelect`                   | Row props plus required string `value`, `options: { label, value }[]`, `onValueChange`; optional `disabled`. Uses Q8iDevAI's adaptive menu. |
...
```ts
import { defineSettings } from "@q8idevai/plugin";
import { z } from "zod";
...
See the complete [settings example](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/settings)
...
```tsx
import { type PluginAgentPanelProps, useAgent, useWorkspace } from "@q8idevai/plugin/client";
import { useMemo } from "react";
import { Text, View } from "react-native";
...
Read cached state with `useWorkspace(workspaceId, selector)` and `useAgent(agentId, selector)`. A selector is required. Q8iDevAI compares its result shallowly, so selecting `{ name, status }` does not re-render when unrelated fields change. Select every field the component renders in one call; do not select the whole snapshot.
...
Q8iDevAI owns tab focus, splitting, closing, persistence, query state, the API/RPC providers, and the render error boundary. A restored tab whose plugin, panel, context, workspace, or agent is unavailable stays open with an unavailable message instead of crashing the workspace.
...
```tsx
import { defineRpc } from "@q8idevai/plugin";
import { z } from "zod";
...
client.addCommandCenterItem({
  ...
  keywords: ["inspect"],
  context: "agent",
  async onSelect({ q8idevai, rpc, workspace, agent, openPanel }) {
    await q8idevai.workspaces.ref(workspace.id).setTitle(`Review ${agent.id}`);
...
| `q8idevai`                   | All                 | Selected host's existing `Q8iDevAIApi`.                                                                            |
...
An agent callback may open either an agent panel or a workspace panel. A workspace callback may open only a workspace panel. Unknown surface and panel IDs fail visibly. Use `q8idevai` for normal workspace, agent, provider, and daemon-config operations. Use `rpc` for plugin-specific filesystem, credential, vendor, or daemon-local work.

## Slash commands

Register a command that runs in the Q8iDevAI client when the user submits `/name args` from the
...
`args` is `"src"`; Q8iDevAI trims only the remainder's leading and trailing whitespace and leaves
parsing to the plugin. Q8iDevAI owns the autocomplete row, input clearing, and the error toast. It
...
Try the [button example](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/buttons) for
...
Compact header buttons use icons without labels or chevrons. Q8iDevAI moves excess contributions
...
The [local plugin example](https://github.com/Q8iDevAI/Q8iDevAI/blob/main/plugin-examples/local-plugin/client/main.tsx)
replaces registrations on each snapshot and aborts the observation during entry cleanup, including pending bootstrap.

## Button descriptor

These contracts are exported from `@q8idevai/plugin/client`.
...
An action runs on the client. Q8iDevAI marks the button busy until its promise settles, blocks repeated
presses, and shows failures in a toast. A failed action can be retried. Use the client's `q8idevai` for
...
Q8iDevAI removes leading, trailing, and consecutive separators after filtering hidden items.
...
Render a React Native icon or indicator within the supplied size. Q8iDevAI bounds the icon slot and
...
Render the body only; Q8iDevAI owns anchoring, scrolling, padding, and sheet presentation. Content can
use `useQ8iDevAI`, `useRpc`, `useWorkspace`, `useAgent`, and the installation's React Query cache.
...
`remove()` is idempotent. Updates after removal do nothing. Q8iDevAI removes outstanding buttons when
...
## Use the Q8iDevAI SDK

Use `useQ8iDevAI()` for ordinary Q8iDevAI operations from a surface. It borrows the selected host's existing connection; do not create another client.

```tsx
import { type PluginSurfaceProps, useQ8iDevAI } from "@q8idevai/plugin/client";
import { Pressable, Text } from "react-native";

function PullRequestAction({ theme }: PluginSurfaceProps) {
  const q8idevai = useQ8iDevAI();
...
The returned API covers projects, workspaces, agents, terminals, providers, and daemon config. See the [SDK API reference](/docs/sdk/reference) for its methods. Connection lifecycle methods are intentionally absent because Q8iDevAI owns the connection.

## Add plugin-specific backend behavior

Use plugin RPC only for work that is not a normal Q8iDevAI operation: reading a vendor API, accessing daemon-local resources, or keeping credentials off the client.
...
```tsx
import { useRpc } from "@q8idevai/plugin/client";
...
```ts
import type { RpcInput } from "@q8idevai/plugin";
import { greeting } from "../shared/greeting";
...
Q8iDevAI adds `[q8idevai]` entries when the plugin starts loading, becomes ready, starts stopping, and has
stopped. It records compilation and load failures as stderr entries, including failures that happen
before the plugin subprocess starts. Q8iDevAI also captures output emitted during initialization, RPC
...
q8idevai plugin logs my-plugin --json
q8idevai --host <url> plugin logs my-plugin
```
...
Q8iDevAI retains up to 500 entries and 256 KiB per plugin in memory. Individual lines are capped at
...
also written to the daemon log at `$Q8IDEVAI_HOME/daemon.log`.
...
```ts
import { defineAttachmentSource, defineRpc } from "@q8idevai/plugin";
import { z } from "zod";
...
Q8iDevAI owns the composer menu, search picker, selected pill, draft state, and submission. The `text` value is the complete snapshot sent to the agent.

## Hosts and lifecycle

Plugins are installed per daemon. When the same contribution exists on several connected hosts, Q8iDevAI shows one sidebar item and adds a host picker. The selected host supplies the bundle, Q8iDevAI API, RPC transport, and query cache. Calls never fall through to another host when the selected host is offline.
...
q8idevai plugin install /absolute/path/to/plugin --id another-runtime-id
q8idevai plugin add owner/repository
q8idevai plugin add https://git.example.com/owner/repository.git --ref main
q8idevai plugin add owner/monorepo:plugins/review
q8idevai plugin ls [id]
q8idevai plugin update <id>
...
Use `update` when you want Q8iDevAI to contact a tracked Git remote and install an available update.
...
> **Trust every plugin you add.** `q8idevai plugin add` and `q8idevai plugin install` mean “I trust this codebase.” Server code and Git preparation commands run unsandboxed with the daemon user's access on the daemon host; client contributions run inside Q8iDevAI. Dependencies and future updates are part of that decision. With the global `--host` option, commands run on the remote daemon host.
...
that Q8iDevAI does not provide, generate source or assets, or perform another required preparation
step:
...
`build` is a list of non-empty argv arrays. Q8iDevAI runs each executable directly, without a shell,
...
The switch is the root `pluginsEnabled` field in `config.json`. After changing it, run `q8idevai reload --json`. Enabling starts every configured plugin whose own `enabled` value is not `false`; disabling tears down all plugins. No daemon restart is required. Manual edits to plugin source entries are not reloaded; use the plugin lifecycle commands for those.

## Load failures

Use `q8idevai plugin ls` to read the current status and error.

| Symptom                                                               | Check                                                                                                                                   |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `This plugin was made for an older version of Q8iDevAI`                  | The directory has only an `index.ts` entry. Follow the [migration guide](/docs/plugins/v0.8/migration).                                 |
...
| Edited code does not appear                                           | Run `npm run typecheck`, then `q8idevai plugin reload <id>`.                                                                               |
| Reload fails                                                          | Read `q8idevai plugin ls` and `q8idevai plugin logs <id>`, fix the source error, then reload; Q8iDevAI does not restore the previous bundle.     |
| Plugin exits unexpectedly                                             | Read `q8idevai plugin logs <id>` for retained initialization, cleanup, stderr, and final crash output.                                     |
