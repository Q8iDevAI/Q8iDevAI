---
title: Voice
description: Q8iDevAI voice architecture, local-first model execution, and provider configuration.
nav: Voice
order: 41
...
Q8iDevAI has first-class voice support for dictation and voice mode conversations with your coding environment.

## Philosophy

Voice is local-first. You can run speech fully on-device, or choose OpenAI for speech features. For voice reasoning/orchestration, Q8iDevAI reuses agent providers already installed and authenticated on your machine.
...
Missing models are downloaded at daemon startup into `$Q8IDEVAI_HOME/models/local-speech`. Downloads happen only for missing files.
...
```json
{
  ...
  "providers": {
    "local": {
      "modelsDir": "~/.q8idevai/models/local-speech"
    }
  }
}
```
...
The `language` field applies only to the OpenAI STT provider: set `features.dictation.stt.language` for dictation and `features.voiceMode.stt.language` for voice mode. If voice language is omitted, Q8iDevAI uses the dictation language before falling back to `en`. It has no effect on the local Parakeet models.
...
- `Q8IDEVAI_VOICE_LLM_PROVIDER`, voice agent provider override
- `Q8IDEVAI_DICTATION_STT_PROVIDER`, `Q8IDEVAI_VOICE_STT_PROVIDER`, `Q8IDEVAI_VOICE_TTS_PROVIDER`, speech provider selection (`local` or `openai`)
...
- `Q8IDEVAI_LOCAL_MODELS_DIR`, local model storage directory
- `Q8IDEVAI_DICTATION_LOCAL_STT_MODEL`, local dictation STT model ID
- `Q8IDEVAI_VOICE_LOCAL_STT_MODEL`, `Q8IDEVAI_VOICE_LOCAL_TTS_MODEL`, local voice STT/TTS model IDs
- `Q8IDEVAI_DICTATION_LANGUAGE`, dictation STT language (OpenAI STT only; ignored by local Parakeet)
- `Q8IDEVAI_VOICE_LANGUAGE`, voice mode STT language; falls back to `Q8IDEVAI_DICTATION_LANGUAGE` when unset (OpenAI STT only; ignored by local Parakeet)
