...
By default, Q8iDevAI uses `~/.q8idevai` as its home directory. The configuration file is:

```bash
~/.q8idevai/config.json
```

You can change the home directory by setting `Q8IDEVAI_HOME` or selecting `--home` on a CLI command.
...
```json
{
  "$schema": "https://q8idevai.sh/schemas/q8idevai.config.v1.json",
  "version": 1,
  "daemon": {
...
```bash
q8idevai daemon config get daemon.listen --home ~/q8idevai-test
q8idevai daemon config set daemon.listen 127.0.0.1:6800 --home ~/q8idevai-test
q8idevai daemon config unset features.webUi.enabled --home ~/q8idevai-test
```
...
Agent providers, both the first-class ones Q8iDevAI ships with and custom entries you add under `agents.providers`, are documented on their own page.

See [Providers](/docs/providers) for the mental model and [Supported providers](/docs/supported-providers) for the full list of agents Q8iDevAI can launch. For pointing Claude at Anthropic-compatible endpoints (Z.AI, Alibaba/Qwen), multiple profiles, custom binaries, ACP agents, and the `additionalModels` merge behavior, see [Custom providers](/docs/custom-providers). The full field reference lives on GitHub at [docs/custom-providers.md](https://github.com/Q8iDevAI/Q8iDevAI/blob/main/docs/custom-providers.md).

## Worktrees

New worktrees are created under `$Q8IDEVAI_HOME/worktrees` by default. To place new worktrees somewhere else, set `worktrees.root`:

```json
{
  "worktrees": {
    "root": "/mnt/fast/q8idevai-worktrees"
  }
}
```

Relative paths are resolved against `Q8IDEVAI_HOME`. Existing worktrees remain where they are; changing this setting only changes where Q8iDevAI creates and discovers Q8iDevAI-managed worktrees going forward.
...
- File (`$Q8IDEVAI_HOME/daemon.log`): `trace` and above
...
```bash
q8idevai daemon set-password
```

This prompts for a password, writes the bcrypt hash to `config.json`, and tells you to restart the daemon. Authentication is a startup setting, so `q8idevai reload` will also report it as restart-required.

Alternatively, set the `Q8IDEVAI_PASSWORD` environment variable (plaintext, hashed automatically at startup):

```bash
Q8IDEVAI_PASSWORD=my-secret q8idevai daemon start
```
...
1. The `password` query parameter on a `tcp://` host URI:
   ...
   ```bash
   q8idevai --host "tcp://192.168.1.10:6767?password=my-secret" ls
   ```

2. The `Q8IDEVAI_PASSWORD` environment variable, used as a fallback when the host carries no embedded password (works for `localhost:6767`, bare `host:port`, or `tcp://` hosts without a `password=` query):

   ```bash
   Q8IDEVAI_PASSWORD=my-secret q8idevai ls
   Q8IDEVAI_PASSWORD=my-secret q8idevai --host 192.168.1.10:6767 ls
   ```

A `password=` in the URI always wins over the env var, so you can keep `Q8IDEVAI_PASSWORD` set globally and still target a different daemon by spelling its password into the URI.

In the mobile app, enter the password in the direct connection setup screen.

## Relay

New homes write `daemon.relay.enabled: false`. Q8iDevAI asks before enabling relay when you pair a device; existing homes keep their saved value. See [Connectivity](/docs/connectivity) to choose and configure a connection method, and [Security](/docs/security) for the relay encryption model.
...
`Q8IDEVAI_RELAY_ENABLED=true|false` overrides the file for a foreground deployment. Managed `start` uses the file. End and relaunch a deployment to remove its override before changing relay from the app or `q8idevai daemon pair --relay`.

## Common env vars

- `Q8IDEVAI_HOME`, set Q8iDevAI home directory
- `Q8IDEVAI_HOST`, set the daemon target for CLI commands
- `Q8IDEVAI_PASSWORD`, on the daemon, the password to require (plaintext, hashed at startup); on the CLI, the password used to connect when the host URI doesn't include one
- `Q8IDEVAI_LISTEN`, override `daemon.listen`
- `Q8IDEVAI_RELAY_ENABLED`, enable or disable the outbound relay for this daemon launch
- `Q8IDEVAI_HOSTNAMES`, override/extend `daemon.hostnames`
- `Q8IDEVAI_ALLOWED_HOSTS`, deprecated alias for `Q8IDEVAI_HOSTNAMES`
- `Q8IDEVAI_WEB_UI_ENABLED`, enable or disable the daemon-served web UI
- `Q8IDEVAI_WEB_UI_DIST_DIR`, override the daemon web UI build directory
- `Q8IDEVAI_TRUSTED_PROXIES`, configure trusted reverse proxy ranges for `X-Forwarded-*` headers
- `Q8IDEVAI_LOG_CONSOLE_LEVEL`, override `log.console.level`
- `Q8IDEVAI_LOG_FILE_LEVEL`, override `log.file.level`
- `Q8IDEVAI_LOG_FILE_PATH`, override `log.file.path`
- `Q8IDEVAI_LOG_FILE_ROTATE_SIZE`, override `log.file.rotate.maxSize`
- `Q8IDEVAI_LOG_FILE_ROTATE_COUNT`, override `log.file.rotate.maxFiles`
- `Q8IDEVAI_LOG`, `Q8IDEVAI_LOG_FORMAT`, legacy log overrides (still supported)
...
- `Q8IDEVAI_VOICE_LLM_PROVIDER`, override voice LLM provider (`claude`, `codex`, `opencode`)
- `Q8IDEVAI_DICTATION_STT_PROVIDER`, `Q8IDEVAI_VOICE_STT_PROVIDER`, `Q8IDEVAI_VOICE_TTS_PROVIDER`, override voice provider selection (`local` or `openai`)
- `Q8IDEVAI_LOCAL_MODELS_DIR`, control local model directory
- `Q8IDEVAI_DICTATION_LOCAL_STT_MODEL`, override local dictation STT model
- `Q8IDEVAI_VOICE_LOCAL_STT_MODEL`, `Q8IDEVAI_VOICE_LOCAL_TTS_MODEL`, override local voice STT/TTS models
- `Q8IDEVAI_DICTATION_LANGUAGE`, `Q8IDEVAI_VOICE_LANGUAGE`, override dictation and voice STT language
- `Q8IDEVAI_VOICE_LOCAL_TTS_SPEAKER_ID`, `Q8IDEVAI_VOICE_LOCAL_TTS_SPEED`, optional local voice TTS tuning
...
```
https://q8idevai.sh/schemas/q8idevai.config.v1.json
```
