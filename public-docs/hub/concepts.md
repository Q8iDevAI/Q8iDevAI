---
title: How Hub works
description: How a provider event reaches a workflow and a Q8iDevAI daemon.
nav: How it works
order: 62
category: Hub
---
...
- A **daemon** is a registered machine running the Q8iDevAI daemon.
...
The configuration lives in `.q8idevai/hub.yml` plus convention-discovered `.q8idevai/workflows/*.yml` files when the project uses a GitHub source. A project has one active configuration revision at a time.
...
- Prompt partials must resolve below `.q8idevai/workflows/partials/` at the exact commit.
