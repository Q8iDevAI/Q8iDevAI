...
Use semantic GitHub events to start a workflow from one GitHub action. Add the workflow below to a repository whose `.q8idevai/hub.yml` defines the `dev` environment and `codex` agent, then activate the bundle.

## Triage new issues

`.q8idevai/workflows/triage-issue.yml`:
...
`.q8idevai/workflows/review-pull-request.yml`:
...
`.q8idevai/workflows/respond-to-issue-comment.yml`:
...
`.q8idevai/workflows/respond-to-pull-request-comment.yml`:
...
`.q8idevai/workflows/implement-ready-issue.yml`:
