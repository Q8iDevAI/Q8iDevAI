...
`.q8idevai/workflows/slack-help.yml`:
...
```text
@Q8iDevAI repo=project agent=claude investigate the failed sync
```

Hub consumes consecutive declared headers and exposes the remainder as `${{ q8idevai.prompt }}`. See [Workflows](/docs/hub/workflows).
