...
A workflow file contains one trigger and the ordered steps it starts. Files are discovered from `.q8idevai/workflows/*.yml`.

## Your first workflow

Assume `.q8idevai/hub.yml` defines an environment named `dev` and an agent named `codex`. Add `.q8idevai/workflows/slack-help.yml`:
...
Hub removes the mention and declared input headers before exposing the remaining text as `${{ q8idevai.prompt }}`. The reply capability is explicit beside the Slack trigger. Discord uses `discord.reply`; GitHub uses a step-scoped [`github` block](/docs/hub/github), not `hub.reply`.
...
steps:
  - id: work
    environment: q8idevai
    max_runtime: 30m
    idle_timeout: 5m
    agent: ${{ q8idevai.inputs.agent }}
    prompt:
      - text: ${{ q8idevai.prompt }}
```

If `codex-safe` contains structured sandbox options in `hub.yml`, selecting it carries those options unchanged. A dynamic inline object such as `provider: ${{ q8idevai.inputs.agent }}` is rejected.
...
`.q8idevai/workflows/partials/classify.md`:
...
- `${{ q8idevai.prompt }}` is normalized request text. It is always explicit in the authored prompt.
- `${{ q8idevai.context }}` opts this step into provider context materialization and inserts JSON. Without that expression, Hub does not fetch or inject ambient context.
