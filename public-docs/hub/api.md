...
organization. Set the Hub origin in `Q8IDEVAI_HUB_URL` below, for example
`https://hub.example.com`.

## API reference

- [Interactive API reference](https://hub.q8idevai.sh/api/reference)
- [OpenAPI 3.1 document](https://hub.q8idevai.sh/api/openapi.json)

These are the canonical reference endpoints for the hosted Q8iDevAI Hub. A self-hosted Hub exposes the same `/api/reference` and `/api/openapi.json` paths on its own origin.

## Authentication

Run `q8idevai hub login [origin]` for interactive CLI access. After browser approval, Q8iDevAI stores a durable, revocable organization credential under `Q8IDEVAI_HOME` for that exact origin. Without an explicit origin, the CLI uses `Q8IDEVAI_HUB_URL`, then the active stored login, then `https://hub.q8idevai.sh`.
...
```http
Authorization: Bearer q8idevai_pk_...
...
CLI credentials have the current CLI operation scopes and remain revocable independently of daemon relationships. `q8idevai hub logout` deletes the active local CLI credential; it does not revoke or disconnect the daemon identity.
...
```json
{
  "type": "https://q8idevai.sh/problems/unauthorized",
  "title": "Authentication required",
  "status": 401,
  "detail": "Provide an active Q8iDevAI organization credential in the Authorization: Bearer header.",
  "code": "unauthorized",
...
`q8idevai hub deploy --dry-run` validates each `.q8idevai/triggers/*.yml` file through `POST /api/v1/triggers/validate`. `q8idevai hub deploy` validates all files first, then installs each through `POST /api/v1/triggers/install`.
...
`GET /api/v1/projects` returns active projects in the bearer credential's organization. `q8idevai hub projects` renders the projects as a table. With `--json`, it returns `{ "origin": "...", "projects": [...] }` so even an empty result records the resolved Hub.
...
`q8idevai hub deploy --project <slug> --dry-run` calls this endpoint with the identical locally resolved payload that a deployment would send.
...
```json
{
  ...
  "files": [
    {
      "path": ".q8idevai/hub.yml",
      "content": "environments:\n  production:\n    kind: daemon\n    daemon: build-server\n    cwd: /workspace\nagents:\n  codex:\n    provider: codex\n"
    },
    {
      "path": ".q8idevai/workflows/deploy.yml",
      "content": "name: deploy\non: manual.run\nmax_runtime: 2h\nfilters:\n  from_users: [automation]\nsteps:\n  - id: deploy\n    environment: production\n    max_runtime: 90m\n    idle_timeout: 10m\n    agent: codex\n    prompt:\n      - include: partials/safety.md\n"
    },
    {
      "path": ".q8idevai/workflows/partials/safety.md",
...
`projectSlug` picks the target project; the bearer credential fixes the organization. `files` contains `.q8idevai/hub.yml`, every direct workflow `.yml`, and each referenced workflow partial. Hub rejects missing, extra, duplicate, unsafe, or noncanonical paths.
...
```bash
curl --fail-with-body -sS -X POST "$Q8IDEVAI_HUB_URL/api/v1/configurations/install" \
  -H "Authorization: Bearer $Q8IDEVAI_HUB_API_KEY" \
  -H "Content-Type: application/json" \
  --data @configuration-install.json
```

`q8idevai hub deploy -p <project>` selects this legacy endpoint with the discovered local bundle. The command uses an exact-origin stored login when flags and environment credentials are absent. See [Deploy from the CLI](/docs/hub/configuration#deploy-from-the-cli).
...
- `input` is the same string a provider message uses: leading `key=value` tokens are parsed as declared inputs, and the remainder becomes `${{ q8idevai.prompt }}`.
...
```bash
curl --fail-with-body -sS -X POST "$Q8IDEVAI_HUB_URL/api/v1/manual-runs" \
  -H "Authorization: Bearer $Q8IDEVAI_HUB_API_KEY" \
  -H "Content-Type: application/json" \
  --data '{
    "projectSlug": "my-project",
...
`q8idevai hub connect [origin]` performs this request with `--api-key`, `Q8IDEVAI_HUB_API_KEY`, or the matching stored login, then passes the one-time token to the daemon's enrollment operation. The daemon generates and keeps its own relationship credential.

```bash
curl --fail-with-body -sS -X POST \
  "$Q8IDEVAI_HUB_URL/api/v1/daemons/enrollment-tokens" \
  -H "Authorization: Bearer $Q8IDEVAI_HUB_API_KEY"
```

Direct API consumers can pass the returned token to the daemon enrollment protocol. The Q8iDevAI CLI intentionally does not accept raw enrollment tokens; `connect` owns the authenticated single-flow exchange.
