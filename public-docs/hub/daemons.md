...
A daemon is one of your machines running the Q8iDevAI daemon. Enroll it once with your Hub organization, then triggers can reference it.
...
```sh
Q8IDEVAI_HUB_URL=https://hub.example.com Q8IDEVAI_HUB_API_KEY=q8idevai_pk_... q8idevai hub connect
```

Origin precedence is explicit `[origin]`, `Q8IDEVAI_HUB_URL`, active stored login, then `https://hub.q8idevai.sh`. An explicit `--api-key <secret>` takes precedence over the environment and an exact-origin stored login.

Check and undo:

```sh
q8idevai hub status
q8idevai hub disconnect
q8idevai hub disconnect --force   # drop local authority when Hub is unreachable
```

One daemon has one Hub relationship. Connecting a daemon that already has one is refused.

`q8idevai hub logout` removes the active CLI login. The daemon's relationship is a separate identity and stays connected.
...
```sh
q8idevai hub logout --disconnect-daemon           # remove both identities
q8idevai hub logout --disconnect-daemon --force   # drop local authority when Hub is unreachable
```
...
`${{ q8idevai.execution.id }}` renders the execution's UUID, so every execution gets its own branch off `origin/main`.
