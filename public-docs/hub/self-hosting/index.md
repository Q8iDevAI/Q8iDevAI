...
Without `DATABASE_URL`, Hub stores an embedded PGlite database and its generated authentication secret under `$XDG_DATA_HOME/q8idevai-hub`. If `XDG_DATA_HOME` is not set to an absolute path, Hub uses `~/.local/share/q8idevai-hub`. Both survive restarts.

Set a different location explicitly with:

```sh
Q8IDEVAI_HUB_DATA_DIR=/path/to/q8idevai-hub-data npx @q8idevai/hub
```
...
```sh
Q8IDEVAI_HUB_APP_URL=https://hub.example.com npx @q8idevai/hub
```
...
```sh
DATABASE_URL=postgres://q8idevai:password@localhost:5432/q8idevai_hub \
  npx @q8idevai/hub
```
...
```dotenv
Q8IDEVAI_BOOTSTRAP_ORGANIZATION=My organization
Q8IDEVAI_BOOTSTRAP_OWNER_EMAIL=me@example.com
Q8IDEVAI_BOOTSTRAP_OWNER_PASSWORD=replace-with-a-temporary-password
```

The password must be at least 12 characters. Sign in once, replace it in the dashboard, then remove `Q8IDEVAI_BOOTSTRAP_OWNER_PASSWORD`. Hub keeps the account and organization.
...
```sh
git clone https://github.com/Q8iDevAI/hub.git
cd hub
cp .env.example .env
docker compose up -d
```

Open <http://localhost:3000> and complete browser setup. For a public deployment, set `Q8IDEVAI_HUB_APP_URL` and any reverse-proxy settings in `.env` before starting the stack.

The stack publishes Hub on port `3000` and stores PostgreSQL data in a named volume. The Hub image is `ghcr.io/getq8idevai/hub:latest`.
...
```dotenv
Q8IDEVAI_HUB_APP_URL=https://hub.example.com
Q8IDEVAI_HUB_TRUSTED_CLIENT_IP_HEADER=x-forwarded-for
```
...
fly deploy -a your-hub \
  -e Q8IDEVAI_HUB_APP_URL=https://your-hub.fly.dev
```
