...
On a local HTTP Hub, the Apps guide lets you configure repository access and explains that event setup is unavailable. Reopen Hub at its public address after setting `Q8IDEVAI_HUB_APP_URL` to add the webhook secret and events.

GitHub uses these Hub URLs:

| Setting      | Hub URL                                                |
| ------------ | ------------------------------------------------------ |
| Homepage URL | `<Q8IDEVAI_HUB_APP_URL>`                                  |
| Callback URL | `<Q8IDEVAI_HUB_APP_URL>/api/integrations/github/callback` |
| Setup URL    | `<Q8IDEVAI_HUB_APP_URL>/api/integrations/github/setup`    |
| Webhook URL  | `<Q8IDEVAI_HUB_APP_URL>/webhook`                          |
...
The connection appears with a slug derived from the account. An installation on `getq8idevai`, for example, becomes `getq8idevai-github`. Connect as many installations as the Hub organization needs.
...
- **Configuration sync:** a repository can hold the canonical `.q8idevai` bundle. See [Configuration](/docs/hub/configuration).
