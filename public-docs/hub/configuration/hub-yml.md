...
Only direct `.yml` children of `.q8idevai/workflows/` are workflows. Each file contains one trigger and its ordered steps. There is no manifest, `includes`, `uses`, reusable step, workflow call, or inheritance.

## `hub.yml`

`.q8idevai/hub.yml` contains named project resources. Names are map keys and are not repeated inside each object.
...
environments:
  review:
    ...
    worktree:
      mode: branch-off
      newBranch: trigger-${{ q8idevai.execution.id }}
      base: origin/main
```

`newBranch` is a branch-name string. Embed `${{ q8idevai.execution.id }}`, which renders the execution's UUID, so every execution branches off `base` on its own branch and keeps it when Hub retries or recovers that execution.

One execution is one step run, so two steps selecting the same environment get separate branches.

`${{ q8idevai.execution.id }}` is the only expression `newBranch` accepts. `q8idevai.prompt`, `q8idevai.context`, `q8idevai.inputs.*`, `values.*`, `steps.<id>.outputs.*`, and provider event fields are unavailable here, and each one fails bundle activation at the authored field, such as `.q8idevai/hub.yml.environments.review.worktree.newBranch`.

`${{ q8idevai.execution.id }}` fails activation the same way anywhere else in a bundle. `branch` and `prNumber` take literal values.
...
Hub passes `model`, `mode`, `thinkingOptionId`, and `options` to the Q8iDevAI daemon without renaming or flattening provider fields. The selected daemon validates them against its current provider schema; Hub does not translate provider-native options.

## Workflow files

`.q8idevai/workflows/review.yml`:
...
inputs:
  repo:
    type: string
    required: true
    choices: [q8idevai, hub]
steps:
  - id: inspect
    environment: ${{ q8idevai.inputs.repo }}
    max_runtime: 30m
    idle_timeout: 5m
    agent: codex-safe
    prompt:
      - text: ${{ q8idevai.prompt }}
```
...
Expressions may read declared `q8idevai.inputs`, earlier `steps.<id>.outputs`, and `values`. The grammar supports paths, JSON literals, parentheses, `!`, `==`, `!=`, `&&`, `||`, and `??`.
...
`${{ q8idevai.prompt }}` is the normalized request after the provider marker and declared leading `key=value` inputs are removed. It is not rewritten or augmented with event context.

`${{ q8idevai.context }}` opts that step into provider context materialization and renders the result as JSON in the prompt. It is available only in prompt text. Hub does not inject it unless the workflow authors that expression.

Includes resolve relative to `.q8idevai/workflows/`, so shared partials use `partials/<name>.md`. Missing files, absolute or traversing paths, symlinks, content mismatches, and files outside the partial tree are rejected.
...
Keep `environments` in `hub.yml`, convert the environment list to a named map, and move each former trigger into its own `.q8idevai/workflows/<name>.yml` file. Move shared prompt files to `.q8idevai/workflows/partials/`. Define complete named agent configurations under `agents` and replace dynamic provider fields with finite named-agent selection.
...
| `{mode: key, key: "support-${{ q8idevai.inputs.ticket }}"}` | Reuse the project's agent for the evaluated custom key.                                                                 |
...
Upgrade the connected Q8iDevAI daemons before enabling the new Hub version. Hub requires the daemon's ordinary agent RPC and request receipt capabilities; an older host produces an actionable dispatch error.
