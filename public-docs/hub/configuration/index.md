...
Each organization trigger is one self-contained YAML file. Keep triggers in your repository and deploy them with `q8idevai hub deploy`:
...
Run `q8idevai hub init` from the repository the agent should work in. Setup selects an app connection and an available agent runtime, asks which user may trigger it, validates the result, and writes one file. It then asks whether to deploy. Interactive `q8idevai hub login` connects the daemon and points to this command; it does not write trigger files.

For a Slack connection named `my-team`, the generated document looks like this:

```yaml
# .q8idevai/triggers/slack-help.yml
name: slack-help
enabled: true
on:
  slack.mention:
...
A Discord starter uses `discord.mention`, your Discord user ID, and `discord.reply`. A GitHub starter uses `github.issue_comment`, restricts the repository to the current GitHub remote, and requires both `@q8idevai` and your GitHub username. GitHub's starter has no explicit reply output declaration.
...
```sh
q8idevai hub login https://hub.example.com
q8idevai hub deploy --dry-run
q8idevai hub deploy
```

Both deploy commands discover direct `.q8idevai/triggers/*.yml` files in deterministic path order. The CLI rejects nested files, `.yaml` extensions, symlinked trigger paths, and unreadable files. It does not search parent directories.
...
4. `https://hub.q8idevai.sh`

Credential precedence:

1. `--api-key`
2. `Q8IDEVAI_HUB_API_KEY`
...
Existing project bundles use `.q8idevai/hub.yml`, direct `.q8idevai/workflows/*.yml` files, and referenced files below `.q8idevai/workflows/partials/`. `hub.yml` owns named environments and agents; each workflow owns its trigger and ordered steps.

Select the legacy deployment path explicitly:

```sh
q8idevai hub deploy --project my-project --dry-run
q8idevai hub deploy --project my-project
```

These commands send the complete bundle through the project configuration API. Dry-run validates without recording or activating a revision. `q8idevai hub init` does not create or migrate these bundles.
