...
The host, provider credentials, filesystem, network, and resulting actions remain under your control. See [Q8iDevAI security](/docs/security) for daemon authentication, pairing, and relay boundaries.
...
`${{ q8idevai.prompt }}` contains normalized request text. Hub does not automatically add provider event context. A step that needs it must author `${{ q8idevai.context }}` in prompt text; that opt-in materializes provider context as JSON.

## Protect configuration authority

Protect push access to the repository containing the `.q8idevai` bundle. A change can select connections, daemons, working directories, complete named agents, and output capabilities.
...
Hub defines no common sandbox abstraction. Put provider-owned settings in a complete named agent under `.q8idevai/hub.yml`:
...
- Prompts distinguish instructions, `${{ q8idevai.prompt }}`, and explicit `${{ q8idevai.context }}`.
