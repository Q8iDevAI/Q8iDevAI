...
When creating an agent, open the model picker and select a saved profile under **Profiles**. Q8iDevAI applies its settings together; you can still adjust them before sending your prompt.
...
The **When to use** field is the profile's notes. Q8iDevAI exposes these notes through its tools so an orchestrating agent can read them before choosing how to launch a worker.
...
After [enabling Q8iDevAI tools](/docs/orchestration#get-started), ask:

> Check my Q8iDevAI profiles and their notes. Choose one for the settings-page UI change, then use a review profile for an independent review of the diff.
...
The bundled [Q8iDevAI skill](/docs/skills#q8idevai-q8idevai-reference) instructs agents to check profiles and read their notes before delegating. If none fit, it directs the agent to discover configured providers and models and tell you about the fallback.
