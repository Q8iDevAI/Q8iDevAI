...
description: Serve the Q8iDevAI web app from your own daemon and reach it over your own LAN, VPN, reverse proxy, or tunnel.
nav: Web UI
order: 6
...
Q8iDevAI's daemon can serve the browser web app itself, from the same address it already uses for the API. You don't need the hosted app at [app.q8idevai.sh](https://app.q8idevai.sh): point a browser at your own daemon and you get the full UI, connected to your own agents, on infrastructure you control.
...
```bash
q8idevai daemon config set daemon.listen 0.0.0.0:6767
q8idevai daemon start
```
...
2. **Allow your hostname** so the daemon's DNS-rebinding protection accepts requests for your domain:
   ...
   ```bash
   q8idevai daemon config set daemon.hostnames '[".example.com"]'
   ```
...
server {
  ...
  ssl_certificate     /etc/letsencrypt/live/q8idevai.example.com/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/q8idevai.example.com/privkey.pem;
...
`Q8IDEVAI_TRUSTED_PROXIES` accepts the same comma-separated values:

```bash
Q8IDEVAI_TRUSTED_PROXIES=loopback,172.16.0.0/12 Q8IDEVAI_WEB_UI_ENABLED=true q8idevai daemon run
```
...
For the remote/relay path (driving a daemon through the Q8iDevAI relay rather than a reverse proxy), the relay has its own public-vs-internal TLS settings, see [Security](/docs/security).
...
For the full threat model, relay encryption, and DNS-rebinding details, see [Security](/docs/security) and [SECURITY.md](https://github.com/Q8iDevAI/Q8iDevAI/blob/main/SECURITY.md).

## Troubleshooting

- **Blank page or 404 at `/`.** The web UI isn't enabled. Start the daemon with `--web-ui` and confirm with `q8idevai daemon status` that it's the daemon you're hitting.
...
- [CLI](/docs/cli), the `q8idevai daemon` commands.
