...
description: Public configuration, methods, handles, results, defaults, and lifecycle behavior for @q8idevai/client.
nav: API reference
order: 58
...
Import every supported runtime value and TypeScript type from `@q8idevai/client`.

## `createQ8iDevAIClient(config)`
...
| `logger`                | `Q8iDevAILogger` | Unset          | Debug, info, warning, and error sink.            |

Relay E2EE clients can also pass `e2ee.enabled` and `e2ee.daemonPublicKeyB64`. `appVersion`, `runtimeGeneration`, and runtime-metrics options exist for Q8iDevAI client surfaces; ordinary integrations can omit them.
...
| Method               | Result                 | Behavior                                                                                                     |
| -------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------ |
| `list(options?)`     | `Q8iDevAIAgentListResult` | Lists a page of agents. `scope`, `filter`, `sort`, `page`, and `subscribe` match the daemon directory query. |
| `create(options)`    | `Q8iDevAIAgentHandle`     | Creates an agent and a fresh workspace for `cwd`. Requires `config`.                                         |
| `ref(agentOrId)`     | `Q8iDevAIAgentHandle`     | Creates a local handle without fetching.                                                                     |
...
| `current()`                    | `Q8iDevAIAgent \| null`              | Current detailed value observed by this handle; never fetches.                                          |
| `refresh(requestId?)`          | `Q8iDevAIAgentRefetchResult \| null` | Fetches the current agent and project placement.                                                        |
...
| `run(text, options?)`          | `Q8iDevAIAgentRunResult`             | Sends a prompt and waits for that turn. `timeoutMs` controls the wait; it defaults to 10 minutes.       |
| `waitForFinish(timeoutMs?)`    | `Q8iDevAIAgentRunResult`             | Waits for the active turn, including an initial prompt. Default timeout: 10 minutes.                    |
| `commands(options?)`           | `Q8iDevAIAgentCommandsResult`        | Asks the live session for its slash commands and skills, including built-in ones. Options: `requestId`. |
...
`Q8iDevAIAgentRunResult` contains `status`, `final`, `error`, and `lastMessage`. `final` refreshes the handle when present.

`Q8iDevAIAgentCommandsResult` contains `agentId`, `commands`, and `error`. Each command has `name`, `description`, `argumentHint`, and an optional `kind` of `"command"` or `"skill"`. A provider that cannot answer reports it in `error` rather than rejecting; providers that expose no command list at all return an empty array.
...
| Method               | Result                   | Behavior                                                                                       |
| -------------------- | ------------------------ | ---------------------------------------------------------------------------------------------- |
| `list(options?)`     | `Q8iDevAIProjectListResult` | Lists every registered project, including projects with no active workspaces.                  |
...
| Method                   | Result                        | Behavior                                                                                       |
| ------------------------ | ----------------------------- | ---------------------------------------------------------------------------------------------- |
| `list(options?)`         | `Q8iDevAIWorkspaceListResult`    | Lists, filters, pages, or subscribes to the workspace directory.                               |
| `open(cwd)`              | `Q8iDevAIWorkspaceHandle`        | Reuses the active workspace for a directory or creates one.                                    |
| `create(options)`        | `Q8iDevAIWorkspaceHandle`        | Always creates a fresh directory-backed or Q8iDevAI-worktree workspace.                           |
| `ref(workspaceOrId)`     | `Q8iDevAIWorkspaceHandle`        | Creates a local handle.                                                                        |
| `archive(workspaceOrId)` | `Q8iDevAIWorkspaceArchiveResult` | Archives without first creating a handle.                                                      |
| `subscribe(handler)`     | Unsubscribe function          | Local listener for this API instance. Requires an owned `list({ subscribe: {} })` observation. |
...
| Method              | Result                             | Behavior                                                                           |
| ------------------- | ---------------------------------- | ---------------------------------------------------------------------------------- |
| `create(options)`   | `Promise<Q8iDevAITerminalHandle>`     | Creates a terminal owned by the required `workspaceId`.                            |
| `list(options?)`    | `Promise<Q8iDevAITerminalListResult>` | Returns `{ entries, requestId }`. Omit filters to list all terminals on this host. |
| `ref(terminalOrId)` | `Q8iDevAITerminalHandle`              | Creates a local handle without fetching or attaching a terminal stream.            |
...
| Method              | Result                                | Behavior                                                                                         |
| ------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `current()`         | `Q8iDevAITerminal \| null`               | Last snapshot from creation, `ref(snapshot)`, or refresh. A handle made from an ID starts empty. |
| `refresh(options?)` | `Promise<Q8iDevAITerminal \| null>`      | Fetches the terminal snapshot, or `null` when it no longer exists. Accepts `requestId`.          |
...
| `capture(options?)` | `Promise<Q8iDevAITerminalCaptureResult>` | Returns `{ terminalId, lines, totalLines, requestId }`.                                          |
...
Use `workspace.terminals.create(options?)` and `workspace.terminals.list(options?)` to supply the workspace ID from a handle. Creation accepts the same options except `workspaceId`; listing accepts only `requestId`. Plugins get these methods through `useQ8iDevAI()` and the handler's `q8idevai` context.

## `client.providers`

| Method                           | Result                        | Behavior                                                                                                                                                 |
| -------------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `waitForReady(options?)`         | `Q8iDevAIProviderSnapshotResult` | Waits until no provider is loading. Default timeout: 60 seconds. Rejects with an update-host error when the daemon cannot correlate workspace snapshots. |
| `snapshot(options?)`             | `Q8iDevAIProviderSnapshotResult` | Returns the current catalog immediately.                                                                                                                 |
...
| `listUsage(options?)`            | `Q8iDevAIProviderUsageResult`    | Returns normalized subscription windows, balances, and provider details. Rejects with an update-host error when unsupported. Options: `requestId`.       |
...
Connection, validation, rejection, and timeout failures reject their promise. Turn outcomes are returned through `Q8iDevAIAgentRunResult.status` because permission and provider errors are expected agent states.
