...
description: Connect to a Q8iDevAI daemon, run one coding agent, and read its reply.
nav: Quickstart
order: 51
...
`cwd` is the directory the agent works in. Q8iDevAI creates the workspace behind it. [Workspaces](/docs/sdk/workspaces) covers reusing one instead.
...
| `permission` | The agent needs a person to answer a permission request in Q8iDevAI. |
...
The agent keeps running on the daemon and stays visible in Q8iDevAI. A later program reaches it again by ID:
