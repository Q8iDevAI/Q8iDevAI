...
// one interpreter every Q8iDevAI checkout already depends on.

const sourceRoot = process.env.Q8IDEVAI_SOURCE_CHECKOUT_PATH;
const targetRoot = process.env.Q8IDEVAI_WORKTREE_PATH || process.cwd();
...
function seedQ8iDevAIHome() {
  const source = process.env.Q8IDEVAI_DEV_SEED_HOME || join(sourceRoot, ".dev/q8idevai-home");
  const target = join(targetRoot, ".dev/q8idevai-home");
  ...
  if (process.env.Q8IDEVAI_DEV_RESET_HOME === "1") {
