...
seed_worktree_q8idevai_home() {
  local source_home="${Q8IDEVAI_DEV_SEED_HOME:-$HOME/.q8idevai}"
  local target_home="$1"
  ...
  if [ "${Q8IDEVAI_DEV_RESET_HOME:-0}" = "1" ]; then
    rm -rf "$target_home"
...
configure_dev_daemon_config() {
  if [ -z "${Q8IDEVAI_LISTEN:-}" ]; then
    return
  fi

  mkdir -p "$Q8IDEVAI_HOME"
  node -e '
const fs = require("fs");
...
' "$Q8IDEVAI_HOME/config.json" "$Q8IDEVAI_LISTEN"
}

resolve_dev_daemon_endpoint() {
  if [ -n "${Q8IDEVAI_DEV_DAEMON_ENDPOINT:-}" ]; then
    echo "$Q8IDEVAI_DEV_DAEMON_ENDPOINT"
    return
  fi

  case "${Q8IDEVAI_LISTEN:-127.0.0.1:6768}" in
    0.0.0.0:*) echo "localhost:${Q8IDEVAI_LISTEN#0.0.0.0:}" ;;
    127.0.0.1:*) echo "localhost:${Q8IDEVAI_LISTEN#127.0.0.1:}" ;;
    *) echo "$Q8IDEVAI_LISTEN" ;;
  esac
}

configure_dev_q8idevai_home() {
  if [ -n "${Q8IDEVAI_HOME:-}" ]; then
    export Q8IDEVAI_HOME
    if [ -n "${Q8IDEVAI_DEV_SEED_HOME:-}" ]; then
      seed_worktree_q8idevai_home "$Q8IDEVAI_HOME"
    fi
    mkdir -p "$Q8IDEVAI_HOME"
    if [ "${Q8IDEVAI_DEV_MANAGED_HOME:-0}" = "1" ] || [ -n "${Q8IDEVAI_DEV_SEED_HOME:-}" ]; then
      configure_dev_daemon_config
    fi
    return
  fi
  ...
  Q8IDEVAI_HOME="$dev_root/.dev/q8idevai-home"
  export Q8IDEVAI_DEV_MANAGED_HOME=1
...
configure_dev_command_env() {
  if [ -z "${Q8IDEVAI_LISTEN:-}" ]; then
    if [ -n "${Q8IDEVAI_SERVICE_DAEMON_PORT:-}" ]; then
      export Q8IDEVAI_LISTEN="0.0.0.0:${Q8IDEVAI_SERVICE_DAEMON_PORT}"
    else
      export Q8IDEVAI_LISTEN="127.0.0.1:6768"
    fi
  fi
