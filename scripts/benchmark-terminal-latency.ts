 ...
 *
 * Boots its OWN isolated daemon subprocess (fresh mkdtemp Q8IDEVAI_HOME, random
 ...
 *
 * Output: pretty table to stdout + JSON to /tmp/q8idevai-terminal-bench/<ts>.json
 *
...
async function bootDaemon(): Promise<BootedDaemon> {
  ...
  const q8idevaiHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-bench-home-"));
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      Q8IDEVAI_SERVER_ID: "srv_terminal_bench",
  ...
  return { child, port, q8idevaiHome, pid: child.pid ?? -1 };
}
...
async function main(): Promise<void> {
  ...
  console.log("Booting isolated daemon (random port, fresh Q8IDEVAI_HOME)...");
  ...
  try {
    ...
    console.log(`Daemon ready: pid=${daemon.pid} port=${daemon.port} home=${daemon.q8idevaiHome}`);

    const ctor = await loadDaemonClientCtor();
    client = await connectClient(ctor, daemon.port);

    workspaceDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-bench-ws-"));
    ...
    const outDir = "/tmp/q8idevai-terminal-bench";
    ...
    if (daemon) {
      ...
      await rm(daemon.q8idevaiHome, { recursive: true, force: true }).catch(() => undefined);
    }
    if (workspaceDir) {
