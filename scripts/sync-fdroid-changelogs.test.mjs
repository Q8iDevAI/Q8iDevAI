...
function withTempRepo(fn, { changelog, version = "0.2.3" }) {
  const tempDir = mkdtempSync(path.join(tmpdir(), "q8idevai-fdroid-changelogs-"));
  try {
...
test("stays inside the 500 character limit and keeps the full-notes link", () => {
  ...
  withTempRepo(
    (dir) => {
      ...
      assert.match(contents, /Full notes: https:\/\/q8idevai\.sh\/changelog\n$/);
...
test("preserves a blockquoted notice ahead of the bullets", () => {
  const contents = formatFdroidChangelog([
    ...
    ">",
    "> If you installed Q8iDevAI Desktop 0.1.108, you need to [reinstall manually](https://q8idevai.sh/download).",
    "",
    "### Fixed",
    "",
  ...
  assert.match(
    contents,
    /^Important update notice\nIf you installed Q8iDevAI Desktop 0\.1\.108, you need to reinstall manually\.\n\nFixed\n- Desktop no longer gets stuck connecting\./,
  );
  assert.ok(contents.length <= CHANGELOG_CHARACTER_LIMIT);
});
...
test("keeps the notice whole and cuts bullets when the budget runs out", () => {
  ...
  const contents = formatFdroidChangelog([
    "> Reinstall Q8iDevAI manually before updating, the old updater cannot install this build.",
    "",
    "### Fixed",
    "",
    ...bullets,
...
test("renders the real 0.1.109 notice, which is the entry's whole point", () => {
  ...
  withTempRepo(
    (dir) => {
      ...
      assert.match(contents, /download and reinstall Q8iDevAI manually/);
      assert.match(contents, /Users on 0\.1\.107 or earlier can update normally\./);
      assert.match(contents, /\nFixed\n- Q8iDevAI Desktop no longer gets stuck connecting/);
