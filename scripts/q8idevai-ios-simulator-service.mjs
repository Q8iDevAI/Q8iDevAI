...
const appProductName = "Q8iDevAIDebug";
const appScheme = "q8idevai";
const preferredSimulatorType = process.env.Q8IDEVAI_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const q8idevaiPort = requiredEnv("Q8IDEVAI_PORT");
const worktreePath = process.env.Q8IDEVAI_WORKTREE_PATH || rootDir;
const worktreeName = process.env.Q8IDEVAI_BRANCH_NAME || basename(worktreePath);
const worktreeHash = createHash("sha1").update(worktreePath).digest("hex").slice(0, 8);
const simulatorName =
  process.env.Q8IDEVAI_IOS_SIMULATOR_NAME || `Q8iDevAI ${worktreeName} ${worktreeHash}`;
const daemonEndpoint =
  process.env.Q8IDEVAI_DEV_DAEMON_ENDPOINT ||
  `localhost:${process.env.Q8IDEVAI_SERVICE_DAEMON_PORT || "6768"}`;
...
async function main() {
  ...
  metro = startMetro();
  await waitForUrl(`http://127.0.0.1:${q8idevaiPort}/.sim`);
  console.log(`iOS preview: ${process.env.Q8IDEVAI_URL || `http://127.0.0.1:${q8idevaiPort}`}/.sim`);
...
function launchApp() {
  const metroUrl = encodeURIComponent(`http://127.0.0.1:${q8idevaiPort}`);
  run(
    "xcrun",
...
function startMetro() {
  const child = spawn("npx", ["expo", "start", "--port", q8idevaiPort, "--localhost"], {
    cwd: appDir,
    env: {
      ...env,
      Q8IDEVAI_SERVE_SIM_PREVIEW: "1",
      Q8IDEVAI_SERVE_SIM_DEVICE_UDID: simulatorUdid,
      BROWSER: "none",
    },
    stdio: "inherit",
  });
...
function requiredEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required; run this as a Q8iDevAI service.`);
  return value;
}
