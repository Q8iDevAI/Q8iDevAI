...
if (process.env.Q8IDEVAI_SKIP_IOS_NATIVE_CACHE === "1") {
  process.exit(0);
}
...
function simulatorSlug() {
  const worktreeName = process.env.Q8IDEVAI_BRANCH_NAME || basename(targetRoot);
  const worktreeHash = createHash("sha1").update(targetRoot).digest("hex").slice(0, 8);
  const simulatorName = `Q8iDevAI ${worktreeName} ${worktreeHash}`;
