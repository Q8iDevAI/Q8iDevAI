...
export const Route = createFileRoute("/changelog")({
  head: () =>
    pageMeta(
      "Changelog - Q8iDevAI",
      "Product updates, bug fixes, and improvements shipped in each Q8iDevAI release. Track new agent providers, mobile features, and daemon changes over time.",
      "/changelog",
    ),
  component: Changelog,
});
...
function HeadingAnchor({ version }: { version: string }) {
  return (
    <a
      ...
      aria-label={`Link to Q8iDevAI ${version}`}
    >
    ...
    <article className="changelog-release">
      <div id={releaseAnchor(group.version)} className="changelog-release-heading">
        ...
        <h2 className="changelog-release-title">
          Q8iDevAI <span>{group.version}</span>
        </h2>
      </div>
