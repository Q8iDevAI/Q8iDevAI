...
description: Q8iDevAI is an Apache-2.0 Superset alternative that runs locally without an account and supports application plugins across its server and clients.
nav: Superset
order: 51
---

# Q8iDevAI vs Superset
...
Q8iDevAI orchestrates coding agents from desktop, mobile, web, and the CLI. Open source under Apache-2.0.
...
Q8iDevAI runs locally without an account, is licensed under Apache 2.0, provides structured interfaces for supported agent harnesses, and supports application plugins with server-side behavior and native client components.

## License

Q8iDevAI is open source under Apache-2.0. You can audit, fork, modify, and redistribute it.
...
Q8iDevAI's daemon runs independently of its clients. Desktop, web, mobile, CLI, SDK, and MCP clients can connect directly, and local use does not require an account.
...
Q8iDevAI runs Claude Code, Codex, OpenCode, and Pi through native structured harnesses, plus 30+ agents through its ACP catalog and any custom CLI agent. See [all supported providers](/agents).
...
Q8iDevAI gives supported providers a structured chat interface with modes, slash commands, tool calls, and file attachments. Superset can run any CLI agent in terminal panes and adds lifecycle status for supported agents.

Q8iDevAI also gives each worktree its own service URL, such as `web.fix-auth.my-app.localhost`, so parallel development servers do not compete for ports.
...
Q8iDevAI is free with no seat limits.
...
See also: [Q8iDevAI vs Conductor](/alternatives/conductor), [Q8iDevAI vs OpenChamber](/alternatives/openchamber), [Q8iDevAI vs Happy Coder](/alternatives/happy-coder).
