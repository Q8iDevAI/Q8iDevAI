...
function release({
  ...
  return {
    tag_name: tag,
    assets: hasApk ? [{ name: `q8idevai-${tag}-android.apk` }] : [],
    prerelease,
    draft: false,
  };
}
...
function desktopRelease({
  ...
  return {
    tag_name: `v${version}`,
    assets: [
      { name: `Q8iDevAI-${version}-arm64.dmg` },
      { name: "Q8iDevAI-x86_64.AppImage" },
      { name: `Q8iDevAI-Setup-${version}-x64.exe` },
      { name: `Q8iDevAI-Setup-${version}-arm64.exe` },
    ],
    prerelease,
    draft,
  };
...
describe("selectReleaseChannels", () => {
  it("offers the newest prerelease when it leads stable", () => {
    ...
    expect(channels.beta?.windowsArm64Asset).toBe("Q8iDevAI-Setup-0.3.0-beta.2-arm64.exe");
  });
