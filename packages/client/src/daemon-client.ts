...
import type { CreationSnapshot } from "@q8idevai/protocol/messages";
import type { z } from "zod";
import type { SessionEventSubscription } from "@q8idevai/protocol/messages";
import type { ClientCapability } from "@q8idevai/protocol/client-capabilities";
import type { AgentAttentionNotificationPayload } from "@q8idevai/protocol/agent-attention-notification";
import { parsePluginSourceReference } from "@q8idevai/protocol/plugin-source-reference";
import {
...
import { validateWSOutboundMessage } from "@q8idevai/protocol/validation/ws-outbound";
import type {
  ...
  Q8iDevAIWorktreeListResponse,
  Q8iDevAIWorktreeArchiveResponse,
  ProjectIconSource,
  ProjectIconResponse,
...
import type {
  ...
  AgentProvider,
  AgentSessionConfig,
} from "@q8idevai/protocol/agent-types";
import type {
  AgentConfigApply,
  MutableDaemonConfig,
  MutableDaemonConfigPatch,
} from "@q8idevai/protocol/messages";
import { isRelayClientWebSocketUrl } from "@q8idevai/protocol/daemon-endpoints";
import {
  asUint8Array,
...
} from "@q8idevai/protocol/browser-automation/rpc-schemas";
...
export interface CreateQ8iDevAIWorktreeInput extends Pick<
  CreateQ8iDevAIWorktreeRequest,
  | "cwd"
  | "projectId"
  | "worktreeSlug"
...
type Q8iDevAIWorktreeListPayload = Q8iDevAIWorktreeListResponse["payload"];
type Q8iDevAIWorktreeArchivePayload = Q8iDevAIWorktreeArchiveResponse["payload"];
type CreateQ8iDevAIWorktreePayload = Extract<
  SessionOutboundMessage,
  { type: "create_q8idevai_worktree_response" }
>["payload"];
...
export class DaemonClient {
  ...
  private attemptConnect(): void {
    ...
    const protocols = password ? [`q8idevai.bearer.${password}`] : undefined;
  ...
  private sendJsonMessage(envelopeType: string, messageType: string, message: unknown): void {
    this.traceInstant("q8idevai.ws.message.outbound", {
      envelopeType,
      messageType,
    });
  ...
  private sendTransportFrame(frame: string | Uint8Array | ArrayBuffer): void {
    ...
    const isOpen = this.beginTraceSection("q8idevai.ws.frame.outbound", {
  ...
  async appendAgentTimelineItem(
    agentId: string,
    item: Omit<import("@q8idevai/protocol/agent-types").PluginTimelineItem, "pluginId">,
  ...
  async stashList(
    ...
    return this.sendCorrelatedSessionRequest({
      ...
      message: {
        ...
        cwd,
        q8idevaiOnly: options?.q8idevaiOnly,
      },
      responseType: "stash_list_response",
    });
  }

  async getQ8iDevAIWorktreeList(
    input: { cwd?: string; repoRoot?: string },
    requestId?: string,
  ): Promise<Q8iDevAIWorktreeListPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "q8idevai_worktree_list_response",
    });
  }

  async archiveQ8iDevAIWorktree(
    input: {
      worktreePath?: string;
      repoRoot?: string;
      branchName?: string;
    ...
    requestId?: string,
  ): Promise<Q8iDevAIWorktreeArchivePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "q8idevai_worktree_archive_response",
    });
  }

  async createQ8iDevAIWorktree(
    input: CreateQ8iDevAIWorktreeInput,
    requestId?: string,
  ): Promise<CreateQ8iDevAIWorktreePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "create_q8idevai_worktree_response",
    });
  }
  ...
  private handleJsonPayload(payload: string, rawBytesLength: number | undefined): void {
    ...
    const parseTraceOpen = this.beginTraceSection("q8idevai.ws.json.parse", {
      size: String(bytes),
    });
    try {
    ...
    if (parsed.data.type === "pong") {
      this.traceInstant("q8idevai.ws.message.inbound", {
        envelopeType: "pong",
        messageType: "pong",
      });
