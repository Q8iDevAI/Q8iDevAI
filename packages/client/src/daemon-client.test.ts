...
import { BROWSER_AUTOMATION_COMMAND_NAMES } from "@q8idevai/protocol/browser-automation/rpc-schemas";
import {
...
} from "@q8idevai/protocol/terminal-stream-protocol";
...
test("traces WebSocket frames, message types, and JSON parse duration", async () => {
  ...
  expect(recorder.records).toEqual([
    {
      phase: "begin",
      name: "q8idevai.ws.message.outbound",
      args: { envelopeType: "hello", messageType: "hello" },
    },
    { phase: "end" },
    {
      phase: "begin",
      name: "q8idevai.ws.frame.outbound",
      args: { kind: "text", size: expect.any(String) },
    },
    { phase: "end" },
    {
      phase: "begin",
      name: "q8idevai.ws.frame.inbound",
      args: { kind: "text", size: expect.any(String) },
    },
    {
      phase: "begin",
      name: "q8idevai.ws.json.parse",
      args: { size: expect.any(String) },
    },
    { phase: "end" },
    {
      phase: "begin",
      name: "q8idevai.ws.message.inbound",
...
test("does not infer browser automation capabilities from Electron runtime", async () => {
  vi.stubGlobal("navigator", {
    userAgent:
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) Q8iDevAI/0.1.89 Chrome/146 Electron/41.2.0 Safari/537.36",
  });
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
...
test("passes password as HTTP bearer header and WebSocket subprotocol", async () => {
  ...
  expect(transportFactory).toHaveBeenCalledWith({
    ...
    protocols: ["q8idevai.bearer.shared-secret"],
  });
});
    ...
    wrapSessionMessage({
      ...
      payload: {
        ...
        file: {
          ...
          mimeType: "text/plain",
          size: 11,
          path: "/tmp/q8idevai-uploads/upload_req-upload/notes.txt",
        },
        error: null,
      },
    }),
  ...
  mock.triggerMessage(
    wrapSessionMessage({
      ...
      payload: {
        ...
        detail: {
          type: "worktree_setup",
          worktreePath: "/tmp/project/.q8idevai/worktrees/feature-a",
          ...
          commands: [
            {
              index: 1,
              command: "npm install",
              cwd: "/tmp/project/.q8idevai/worktrees/feature-a",
              log: "phase-one\n",
              status: "running",
              exitCode: null,
            },
...
test("sends structured first-agent context attachments with create_q8idevai_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  const createPromise = client.createQ8iDevAIWorktree({
    cwd: "/tmp/project",
    worktreeSlug: "review-pr-123",
    firstAgentContext: {
      attachments: [
...
test("sends worktree base-ref fields in create_q8idevai_worktree_request", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
...
test("omitting create_q8idevai_worktree_request worktree base-ref fields preserves legacy wire shape", async () => {
  const logger = createMockLogger();
  const mock = createMockTransport();
  ...
  mock.triggerMessage(
    JSON.stringify({
      ...
      message: {
        ...
        payload: {
          directories: ["/Users/test/projects/q8idevai"],
