...
import { createQ8iDevAIApi, createQ8iDevAIClient } from "./index.js";
import { DaemonClient } from "./daemon-client.js";
import type { Q8iDevAIAgent, Q8iDevAIClient, Q8iDevAIWorkspace } from "./index.js";
...
async function connectClient(
...
): Promise<{ client: Q8iDevAIClient; ws: FakeWebSocket }> {
  const client = createQ8iDevAIClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^q8idevai-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
async function observeAgents(client: Q8iDevAIClient, ws: FakeWebSocket): Promise<string> {
  const ready = client.agents.list({ subscribe: {} });
  acknowledgeObservation(ws, "agents-sdk");
  return (await ready).subscriptionId!;
}

function createWorkspace(input: Partial<Q8iDevAIWorkspace> = {}): Q8iDevAIWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<Q8iDevAIAgent> = {}): Q8iDevAIAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createQ8iDevAIClient exposes workspace list through the daemon client", async () => {
...
test("createQ8iDevAIApi borrows daemon capabilities without exposing connection ownership", () => {
  ...
  const q8idevai = createQ8iDevAIApi(daemonClient);

  expect(Object.keys(q8idevai).sort()).toEqual([
    "agents",
    "config",
    "dispose",
    "observeEvents",
  ...
  expect("connect" in q8idevai).toBe(false);
  expect("close" in q8idevai).toBe(false);
  expect("skills" in q8idevai.agents).toBe(false);
});
...
test("canceled timeline handles and captured state are collectible while their API stays alive", async () => {
  const source = `
    ...
    import { createQ8iDevAIApi } from ${JSON.stringify(new URL("./index.ts", import.meta.url).href)};
    ...
    async function batch(mode) {
      ...
      const api = createQ8iDevAIApi(driver, { signal: scope.signal });
  ...
  const env = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => !/^(Q8IDEVAI_|EXPO_|E2E_|AGENT_BROWSER_)/.test(key)),
  );
