...
test("SDK timelines have independent lifetimes and fresh IDs on reconnect", async () => {
  const { createQ8iDevAIApi } = await import("./index");
  const h = connection();
  const api = createQ8iDevAIApi(h.client);
  try {
...
test("SDK subscribers receive timeline replacement instead of silently losing history", async () => {
  ...
  try {
    ...
    h.open();
    await ready;
    const off = createQ8iDevAIApi(h.client)
      .agents.ref("agent")
...
test.each([
  ...
  const api = createQ8iDevAIApi(h.client);
  const received: import("./index").Q8iDevAIAgentTimelineEvent[][] = [[], [], []];
  try {
