...
describe("plugin selector shallow equality", () => {
  it("keeps equivalent selector objects and arrays stable", () => {
    ...
    expect(shallowEqual({ name: "Q8iDevAI", shared }, { name: "Q8iDevAI", shared })).toBe(true);
    expect(shallowEqual(["Q8iDevAI", shared], ["Q8iDevAI", shared])).toBe(true);
    expect(shallowEqual({ name: "Q8iDevAI" }, { name: "Changed" })).toBe(false);
  });
