# AGENTS.md — Q8iDevAI Server Development Guide

For AI coding agents working in `packages/server`. Supplements [CLAUDE.md](../CLAUDE.md) at the repo root.

## Project Overview

Q8iDevAI is a mobile + CLI app for monitoring and controlling local AI coding agents (Claude Code, Codex, GitHub Copilot, OpenCode, Pi). The daemon runs on your machine, manages agent processes, and streams their output over WebSocket to clients.
...
Agent state persists to `$Q8IDEVAI_HOME/agents/{cwd-with-dashes}/{agent-id}.json`  
Daemon logs: `$Q8IDEVAI_HOME/daemon.log`
...
```bash
tail -f $Q8IDEVAI_HOME/daemon.log      # Daemon logs
