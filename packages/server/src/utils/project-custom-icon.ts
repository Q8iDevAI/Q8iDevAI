...
import type { ProjectIconSource } from "@q8idevai/protocol/messages";
...
export async function setProjectCustomIcon(input: {
  ...
  return serialize(input.projectId, async () => {
    ...
    if (input.source.type === "automatic") {
      ...
      validateIcon(bytes);
      await writeFileAtomic(cachePath(input.q8idevaiHome, input.projectId), bytes);
...
export class ProjectIconReader {
  ...
  constructor(private readonly q8idevaiHome: string) {}

  async snapshot(project: PersistedProjectRecord): Promise<ProjectIconSnapshot> {
    const snapshot = await readProjectIconSnapshot({ q8idevaiHome: this.q8idevaiHome, project });
...
export async function readProjectIconSnapshot(input: {
  ...
  if (input.project.customIconRevision) {
    ...
    try {
      icon = validateIcon(await readFile(cachePath(input.q8idevaiHome, input.project.projectId)));
    } catch {
...
export async function removeProjectCustomIcon(input: {
  ...
  await rm(cachePath(input.q8idevaiHome, input.projectId), { force: true });
}
...
function cachePath(q8idevaiHome: string, projectId: string): string {
  const key = createHash("sha256").update(projectId).digest("hex");
  return join(q8idevaiHome, "projects", "icons", `${key}.bin`);
}
