...
async function project() {
  const rootPath = await tempDir("q8idevai-project-root-");
  const q8idevaiHome = await tempDir("q8idevai-home-");
  ...
  const reader = new ProjectIconReader(q8idevaiHome);

  return {
    q8idevaiHome,
    rootPath,
    set: (source: ProjectIconSource) =>
      setProjectCustomIcon({ q8idevaiHome, projectId: "project-a", source, projects }),
    read: () => readProjectIcon({ q8idevaiHome, project: record }),
    snapshot: () => readProjectIconSnapshot({ q8idevaiHome, project: record }),
    advertisedSnapshot: () => reader.snapshot(record),
    readAdvertised: () => reader.read(record),
    revision: () => record.customIconRevision,
    remove: () => removeProjectCustomIcon({ q8idevaiHome, projectId: "project-a" }),
  };
}
...
describe("project custom icon", () => {
  ...
  it("drops the stored image when the project is removed", async () => {
    ...
    await expect(
      readProjectIcon({ q8idevaiHome: target.q8idevaiHome, project: stored }),
    ).resolves.toBeNull();
  });
