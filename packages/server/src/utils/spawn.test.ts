...
import { buildSelfNodeCommand } from "../server/q8idevai-env.js";
...
describe("execCommand", () => {
  ...
  test("treats env as the replacement base and finalizes external command env", async () => {
    ...
    expect(parsePrintedEnv(result.stdout)).toEqual({
      ...
      Q8IDEVAI_DESKTOP_MANAGED: null,
      Q8IDEVAI_NODE_ENV: null,
      Q8IDEVAI_SUPERVISED: null,
    });
  });

  test("does not inherit process.env when env replacement is supplied", async () => {
    process.env.Q8IDEVAI_TEST_SHOULD_NOT_LEAK = "leaked";
    try {
      const result = await execCommand(
        process.execPath,
        [
          "-e",
          "console.log(JSON.stringify({ leaked: process.env.Q8IDEVAI_TEST_SHOULD_NOT_LEAK ?? null }))",
        ],
        {
          env: {
            PATH: process.env.PATH,
      ...
      delete process.env.Q8IDEVAI_TEST_SHOULD_NOT_LEAK;
    }
  });
  ...
  test("internal env mode preserves Q8iDevAI-owned launcher env", async () => {
    const result = await execCommand(process.execPath, ["-e", printEnvScript], {
      envMode: "internal",
      baseEnv: {
