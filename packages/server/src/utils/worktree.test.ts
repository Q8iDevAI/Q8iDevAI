...
describe("q8idevai worktree manager", () => {
  ...
  it("treats a worktree as q8idevai-owned even when its .git admin is missing", async () => {
    ...
    const ownership = await isQ8iDevAIOwnedWorktreeCwd(created.worktreePath, { q8idevaiHome });
    expect(ownership.allowed).toBe(true);
    await expect(
      isQ8iDevAIOwnedWorktreeCwd(join(created.worktreePath, "packages", "app"), { q8idevaiHome }),
  ...
  it("rejects paths that are not under the q8idevai worktrees root", async () => {
    const outsidePath = join(tempDir, "outside-q8idevai-home");
    mkdirSync(outsidePath, { recursive: true });

    const ownership = await isQ8iDevAIOwnedWorktreeCwd(outsidePath, { q8idevaiHome });
  ...
  it("rejects the worktrees root itself and the per-repo hash dir", async () => {
    ...
    const worktreesRoot = join(q8idevaiHome, "worktrees");
    const projectHashDir = join(worktreesRoot, projectHash);
    mkdirSync(projectHashDir, { recursive: true });

    await expect(isQ8iDevAIOwnedWorktreeCwd(worktreesRoot, { q8idevaiHome })).resolves.toMatchObject({
      allowed: false,
    });
    await expect(isQ8iDevAIOwnedWorktreeCwd(projectHashDir, { q8idevaiHome })).resolves.toMatchObject({
      allowed: false,
    });
  });
