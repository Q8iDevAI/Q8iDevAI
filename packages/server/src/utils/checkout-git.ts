...
import type { CheckoutCommit, CheckoutCommitFile } from "@q8idevai/protocol/messages";
import { parseGitHubRemoteIdentity, parseGitRemoteLocation } from "@q8idevai/protocol/git-remote";
import { maxBase64EncryptedPlaintextByteLength } from "@q8idevai/relay";
...
import { isQ8iDevAIOwnedWorktreeCwd, resolveQ8iDevAIWorktreesBaseRoot } from "./worktree.js";
import {
  branchNameFromRef,
  getQ8iDevAIWorktreeChangeRequestHintForBranch,
  type Q8iDevAIWorktreeMetadata,
  readQ8iDevAIWorktreeMetadata,
  rebindQ8iDevAIWorktreeChangeRequestHint,
...
export interface CheckoutStatusGitNonQ8iDevAI {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitQ8iDevAI {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonQ8iDevAI | CheckoutStatusGitQ8iDevAI;
...
export type CheckoutSnapshotFacts =
  ...
  | {
      ...
      q8idevaiWorktree: Q8iDevAIWorktreeForCwd;
...
async function getMainRepoRootFromCommonDir(
  ...
  const nonBareNonQ8iDevAI = worktrees.filter(
    (wt) =>
      !wt.isBare &&
      !isQ8iDevAIWorktreePath(wt.path, {
        q8idevaiHome: context?.q8idevaiHome,
        worktreesRoot: context?.worktreesRoot,
      }),
  );
  const childrenOfBareRepo = nonBareNonQ8iDevAI.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonQ8iDevAI[0]?.path ?? normalized;
}
...
/** Check whether a path is under Q8iDevAI's worktree root. */
export function isQ8iDevAIWorktreePath(
  p: string,
  options?: { q8idevaiHome?: string; worktreesRoot?: string },
): boolean {
  if (options?.worktreesRoot || options?.q8idevaiHome) {
    return isDescendantPath(p, resolveQ8iDevAIWorktreesBaseRoot(options));
  }
  return /[/\\]\.q8idevai[/\\]worktrees[/\\]/.test(p);
}
...
export async function renameCurrentBranch(
  ...
  if (currentBranch) {
    rebindQ8iDevAIWorktreeChangeRequestHint(worktreeRoot, previousBranch, currentBranch);
  }
  return { previousBranch, currentBranch };
}

type Q8iDevAIWorktreeForCwd =
  | { isQ8iDevAIOwnedWorktree: false }
  | { isQ8iDevAIOwnedWorktree: true; worktreeRoot: string };

interface Q8iDevAIWorktreeLookupOptions {
  context?: CheckoutContext;
  knownWorktreeRoot?: string | null;
  knownGitCommonDir?: string | null;
}

async function getQ8iDevAIWorktreeForCwd(
  cwd: string,
  options: Q8iDevAIWorktreeLookupOptions = {},
): Promise<Q8iDevAIWorktreeForCwd> {
  // Fast-path reject: non-worktree paths do not need expensive ownership checks.
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { isQ8iDevAIOwnedWorktree: false };
  }

  const ownership = await isQ8iDevAIOwnedWorktreeCwd(cwd, {
    q8idevaiHome: options.context?.q8idevaiHome,
...
function storedBaseRefFromMetadata(metadata: Q8iDevAIWorktreeMetadata | null): string | null {
  return metadata?.baseRef ?? metadata?.baseRefName ?? null;
}

function readQ8iDevAIWorktreeBaseRef(worktreeRoot: string): string | null {
  return storedBaseRefFromMetadata(readQ8iDevAIWorktreeMetadata(worktreeRoot));
}
...
async function getStoredBaseRefForCwd(
  ...
  const q8idevaiWorktree = await getQ8iDevAIWorktreeForCwd(cwd, { context });
  if (!q8idevaiWorktree.isQ8iDevAIOwnedWorktree) {
    return null;
  }

  return readQ8iDevAIWorktreeBaseRef(q8idevaiWorktree.worktreeRoot);
}
...
async function inspectCheckoutContext(
  ...
  const q8idevaiWorktree = await getQ8iDevAIWorktreeForCwd(cwd, {
    context,
...
function buildPullRequestLookupTargetFromMetadata(
  ...
  const target = getQ8iDevAIWorktreeChangeRequestHintForBranch(metadata, currentBranch);
  if (!target) {
    return null;
  }
  return {
...
export async function getCheckoutSnapshotFacts(
  ...
  const q8idevaiWorktreeMetadata = inspected.q8idevaiWorktree.isQ8iDevAIOwnedWorktree
    ? readQ8iDevAIWorktreeMetadata(inspected.q8idevaiWorktree.worktreeRoot)
    : null;
  const storedBaseRef = storedBaseRefFromMetadata(q8idevaiWorktreeMetadata);
  ...
  return {
    ...
    q8idevaiWorktree: inspected.q8idevaiWorktree,
    storedBaseRef,
    resolvedBaseRef,
    mainRepoRoot,
    comparisonBaseRef,
...
// Temporary until diffs load lazily per file. The Q8iDevAI relay's 32 MiB frame limit is
...
export async function getCheckoutStatus(
  ...
  const q8idevaiWorktree = facts.q8idevaiWorktree;
  ...
  if (q8idevaiWorktree.isQ8iDevAIOwnedWorktree && baseRef) {
    return {
      isGit: true,
...
function getUnavailablePullRequestStatus(
  ...
  if (
    facts?.isGit === true &&
    facts.q8idevaiWorktree.isQ8iDevAIOwnedWorktree &&
