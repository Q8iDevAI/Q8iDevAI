...
import {
  readQ8iDevAIConfigForEdit,
  statQ8iDevAIConfigPath,
  writeQ8iDevAIConfigForEdit,
} from "./q8idevai-config-file.js";

describe("q8idevai config file substrate", () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = realpathSync(mkdtempSync(join(tmpdir(), "q8idevai-config-file-test-")));
  });
  ...
  it("returns null config and revision when q8idevai.json is missing", () => {
    const result = readQ8iDevAIConfigForEdit(tempDir);

    expect(result).toEqual({ ok: true, config: null, revision: null });
  });

  it("returns invalid_project_config for invalid JSON", () => {
    writeFileSync(join(tempDir, "q8idevai.json"), "{ invalid json\n");
  ...
  it("preserves raw lifecycle string and array forms with a revision token", () => {
    ...
    expect(result).toEqual({
      ...
      revision: statQ8iDevAIConfigPath(tempDir),
    });
  });
  ...
  it("writes pretty JSON with a trailing newline when revision matches", () => {
    writeFileSync(join(tempDir, "q8idevai.json"), JSON.stringify({ worktree: { setup: "old" } }));
    const expectedRevision = statQ8iDevAIConfigPath(tempDir);

    const result = writeQ8iDevAIConfigForEdit({
      repoRoot: tempDir,
    ...
    expect(readFileSync(join(tempDir, "q8idevai.json"), "utf8")).toBe(
      '{\n  "worktree": {\n    "setup": "npm install"\n  }\n}\n',
    );
  });
  ...
  it.skipIf(isPlatform("win32"))(
    ...
    () => {
      writeFileSync(join(tempDir, "q8idevai.json"), JSON.stringify({ worktree: { setup: "old" } }));
      const expectedRevision = statQ8iDevAIConfigPath(tempDir);
      writeFileSync(join(tempDir, "q8idevai.json"), JSON.stringify({ worktree: { setup: "new" } }));
      const currentRevision = statQ8iDevAIConfigPath(tempDir);
  ...
  it("round-trips unknown top-level, worktree, and script-entry fields", () => {
    ...
    expect(readQ8iDevAIConfigForEdit(tempDir)).toEqual({
      ok: true,
      config,
      revision: statQ8iDevAIConfigPath(tempDir),
    });
  ...
  it("creates q8idevai.json when the file is still missing and expected revision is null", () => {
    ...
    expect(result).toEqual({
      ok: true,
      config: { scripts: { dev: { command: "npm run dev" } } },
      revision: statQ8iDevAIConfigPath(join(tempDir, "nested")),
    });
  });
});
