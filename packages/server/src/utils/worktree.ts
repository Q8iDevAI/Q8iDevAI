...
import { readQ8iDevAIConfigJson, resolveQ8iDevAIConfigPath } from "./q8idevai-config-file.js";
export {
  Q8iDevAIConfigRawSchema,
  Q8iDevAILifecycleCommandRawSchema,
  Q8iDevAIScriptEntryRawSchema,
  Q8iDevAIWorktreeConfigRawSchema,
  Q8iDevAIConfigSchema,
  type Q8iDevAIConfig,
  type Q8iDevAIConfigRaw,
} from "@q8idevai/protocol/q8idevai-config-schema";
import { Q8iDevAIConfigSchema, type Q8iDevAIConfig } from "@q8idevai/protocol/q8idevai-config-schema";
import {
  createQ8iDevAIWorktreeChangeRequestHint,
  normalizeBaseRefName,
  type Q8iDevAIWorktreeChangeRequestHint,
  readQ8iDevAIWorktreeMetadata,
  readQ8iDevAIWorktreeRuntimePort,
  writeQ8iDevAIWorktreeMetadata,
  writeQ8iDevAIWorktreeRuntimeMetadata,
} from "./worktree-metadata.js";
import { runGitCommand } from "./run-git-command.js";
import { spawnProcess } from "./spawn.js";
import { resolveQ8iDevAIHome } from "../server/q8idevai-home.js";
import { createExternalProcessEnv } from "../server/q8idevai-env.js";
...
export { slugify, validateBranchSlug } from "@q8idevai/protocol/branch-slug";
...
export interface WorktreeRuntimeEnv {
  [key: string]: string;
  Q8IDEVAI_SOURCE_CHECKOUT_PATH: string;
  Q8IDEVAI_ROOT_PATH: string;
  Q8IDEVAI_WORKTREE_PATH: string;
  Q8IDEVAI_BRANCH_NAME: string;
  Q8IDEVAI_WORKTREE_PORT: string;
}
...
export interface Q8iDevAIWorktreeInfo {
  path: string;
  createdAt: string;
  branchName?: string;
  head?: string;
...
export interface Q8iDevAIWorktreeOwnership {
  allowed: boolean;
  repoRoot?: string;
  worktreeRoot?: string;
  worktreePath?: string;
...
export interface Q8iDevAIWorktreeOwnershipOptions extends WorktreeRootOptions {
...
export type ReadQ8iDevAIConfigResult =
  | { ok: true; config: Q8iDevAIConfig | null }
  | { ok: false; configPath: string; error: unknown };

export function readQ8iDevAIConfig(repoRoot: string): ReadQ8iDevAIConfigResult {
  try {
    const json = readQ8iDevAIConfigJson(repoRoot);
    if (json === null) {
      return { ok: true, config: null };
    }
    return { ok: true, config: Q8iDevAIConfigSchema.parse(json) };
  } catch (error) {
    return { ok: false, configPath: resolveQ8iDevAIConfigPath(repoRoot), error };
  }
}

export function q8idevaiConfigParseError(failure: { configPath: string; error: unknown }): Error {
  const detail = failure.error instanceof Error ? failure.error.message : String(failure.error);
  return new Error(`Failed to parse q8idevai.json at ${failure.configPath}: ${detail}`, {
    cause: failure.error,
  });
}

function readQ8iDevAIConfigOrThrow(repoRoot: string): Q8iDevAIConfig | null {
  const result = readQ8iDevAIConfig(repoRoot);
  if (!result.ok) {
    throw q8idevaiConfigParseError(result);
  }
  return result.config;
}

export function getWorktreeSetupCommands(repoRoot: string): string[] {
  return readQ8iDevAIConfigOrThrow(repoRoot)?.worktree?.setup ?? [];
}

export function getWorktreeTeardownCommands(repoRoot: string): string[] {
  return readQ8iDevAIConfigOrThrow(repoRoot)?.worktree?.teardown ?? [];
}

export function getWorktreeTerminalSpecs(repoRoot: string): WorktreeTerminalConfig[] {
  const terminals = readQ8iDevAIConfigOrThrow(repoRoot)?.worktree?.terminals;
...
export function getScriptConfigs(config: Q8iDevAIConfig | null): Map<string, ScriptConfig> {
...
export async function runWorktreeSetupCommands(options: {
  ...
  // Read q8idevai.json from the worktree (it will have the same content as the source repo)
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  let worktreePort = readQ8iDevAIWorktreeRuntimePort(options.worktreePath);
  if (worktreePort === null) {
    worktreePort = await getAvailablePort();
    const metadata = readQ8iDevAIWorktreeMetadata(options.worktreePath);
    if (metadata) {
      writeQ8iDevAIWorktreeRuntimeMetadata(options.worktreePath, { worktreePort });
    }
  } else {
  ...
  return {
    ...
    // source checkout.
    Q8IDEVAI_SOURCE_CHECKOUT_PATH: repoRootPath,
    // Backward-compatible alias.
    Q8IDEVAI_ROOT_PATH: repoRootPath,
    Q8IDEVAI_WORKTREE_PATH: options.worktreePath,
    Q8IDEVAI_BRANCH_NAME: branchName,
    Q8IDEVAI_WORKTREE_PORT: String(worktreePort),
  };
}
...
export async function runWorktreeTeardownCommands(options: {
  ...
  const branchName =
    options.branchName ?? (await resolveBranchNameForWorktreePath(options.worktreePath));
  const worktreePort = readQ8iDevAIWorktreeRuntimePort(options.worktreePath);
  ...
  const teardownEnv: NodeJS.ProcessEnv = createStringCommandShellEnv(
    createExternalProcessEnv(process.env, {
      ...
      Q8IDEVAI_WORKTREE_PATH: options.worktreePath,
      Q8IDEVAI_BRANCH_NAME: branchName,
      ...(worktreePort !== null ? { Q8IDEVAI_WORKTREE_PORT: String(worktreePort) } : {}),
    }),
  );
...
export async function seedQ8iDevAIConfigFile(options: {
  sourceCwd: string;
  targetCwd: string;
}): Promise<void> {
  const sourceConfigPath = join(options.sourceCwd, "q8idevai.json");
  const targetConfigPath = join(options.targetCwd, "q8idevai.json");
...
export function resolveQ8iDevAIWorktreesBaseRoot(options?: WorktreeRootOptions): string {
  if (options?.worktreesRoot) {
    ...
    const home = options.q8idevaiHome ? resolve(options.q8idevaiHome) : resolveQ8iDevAIHome();
...
export async function getQ8iDevAIWorktreesRoot(
  cwd: string,
  q8idevaiHome?: string,
  worktreesRoot?: string,
): Promise<string> {
  const baseRoot = resolveQ8iDevAIWorktreesBaseRoot({ q8idevaiHome, worktreesRoot });
...
export async function computeWorktreePath(
  ...
  const projectWorktreesRoot = await getQ8iDevAIWorktreesRoot(cwd, q8idevaiHome, worktreesRoot);
...
export async function isQ8iDevAIOwnedWorktreeCwd(
  cwd: string,
  options?: Q8iDevAIWorktreeOwnershipOptions,
): Promise<Q8iDevAIWorktreeOwnership> {
  ...
  const worktreesBaseRoot = resolveQ8iDevAIWorktreesBaseRoot(options);
  ...
  // The <hash>/<slug> prefix is Q8iDevAI-private — nothing else writes there — so the
...
type ParsedQ8iDevAIWorktreeInfo = Omit<Q8iDevAIWorktreeInfo, "createdAt">;

function parseWorktreeList(output: string): ParsedQ8iDevAIWorktreeInfo[] {
  const entries: ParsedQ8iDevAIWorktreeInfo[] = [];
  let current: ParsedQ8iDevAIWorktreeInfo | null = null;
...
export async function listQ8iDevAIWorktrees({
  cwd,
  q8idevaiHome,
  worktreesRoot,
}: {
...
export interface DeleteQ8iDevAIWorktreeOptions {
  cwd: string | null;
  worktreePath?: string;
  teardownCwds?: string[];
  worktreeSlug?: string;
...
export async function deleteQ8iDevAIWorktree({
  cwd,
  worktreePath,
  teardownCwds,
  worktreeSlug,
  ...
  q8idevaiHome,
  worktreesBaseRoot,
}: DeleteQ8iDevAIWorktreeOptions): Promise<void> {
  ...
  if (worktreesRoot) {
    ...
    resolvedWorktreesRoot = await getQ8iDevAIWorktreesRoot(cwd, q8idevaiHome, worktreesBaseRoot);
  } else {
    throw new Error("cwd or worktreesRoot is required to delete a Q8iDevAI worktree");
  }
  ...
  const ownership = await isQ8iDevAIOwnedWorktreeCwd(requestedPath, {
    q8idevaiHome,
  ...
  if (relativeWorktreePath === null || relativeWorktreePath === "") {
    throw new Error("Refusing to delete non-Q8iDevAI worktree");
  }
...
export async function rollbackCreatedQ8iDevAIWorktree(
  options: DeleteQ8iDevAIWorktreeOptions,
  cause: unknown,
): Promise<never> {
  let cleanupError: unknown;
  try {
    await deleteQ8iDevAIWorktree(options);
  } catch (error) {
    cleanupError = error;
  }
  if (cleanupError) {
...
export const createWorktree = async ({
  ...
  let worktreePath = join(await getQ8iDevAIWorktreesRoot(cwd, q8idevaiHome, worktreesRoot), worktreeSlug);
  ...
  writeQ8iDevAIWorktreeMetadata(worktreePath, {
  ...
  await seedQ8iDevAIConfigFile({ sourceCwd: cwd, targetCwd: worktreePath });
  ...
  switch (source.kind) {
    case "branch-off": {
      ...
      return {
        ...
        changeRequestLookupTarget: createQ8iDevAIWorktreeChangeRequestHint({
    ...
    case "checkout-github-pr": {
      ...
      if (source.pushRemoteUrl) {
        const remoteName = `q8idevai-pr-${changeRequestNumber}`;
        ...
        if (originUrl) {
          remotePlan.pushRemote = {
            name: `q8idevai-pr-${changeRequestNumber}`,
            url: originUrl,
            headRef: source.headRef,
            track: false,
          };
...
function normalizeRequiredBaseBranch(baseBranch: string): string {
  ...
  if (!normalizedBaseBranch) {
    throw new Error("Base branch is required when creating a Q8iDevAI worktree");
  }
  if (normalizedBaseBranch === "HEAD") {
    throw new Error("Base branch cannot be HEAD when creating a Q8iDevAI worktree");
  }
