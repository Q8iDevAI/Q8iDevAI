...
describe("searchDirectoryEntries", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(searchRoot, "projects", "q8idevai-desktop"), { recursive: true });
  ...
  it("applies result paths and entry kinds as parameters of one search", async () => {
    ...
    expect({ directories, files }).toEqual({
      directories: [
        {
          path: path.join(searchRoot, "projects", "q8idevai-desktop"),
          kind: "directory",
        },
      ],
      files: [
  ...
  it("matches rooted queries against the full path at any depth", async () => {
    ...
    const expected = [
      ...
      { path: "projects/q8idevai-desktop", kind: "directory" },
    ];
  ...
  it("does not spend the scan budget on excluded entry kinds", async () => {
    ...
    const target = path.join(budgetRoot, "z-projects", "q8idevai-target");
    ...
    await expect(
      searchDirectoryEntries({
        root: budgetRoot,
        query: "q8idevai-target",
        pathFormat: "relative",
        includeFiles: false,
        includeDirectories: true,
        maxEntriesScanned: 2,
      }),
    ).resolves.toEqual([{ path: "z-projects/q8idevai-target", kind: "directory" }]);
  });
...
describe("absolute directory-path configuration", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(path.join(homeDir, "projects", "q8idevai"), { recursive: true });
  ...
  it("shares the scan budget fairly between nested sibling branches", async () => {
    ...
    const projectPath = path.join(budgetHome, "work", "client", "team", "q8idevai-desktop");
  ...
  it.skipIf(isWindows)("does not let a queued symlink hide the direct project branch", async () => {
    ...
    const projectPath = path.join(projectRoot, "q8idevai-desktop");
  ...
  it.skipIf(isWindows)("follows visible directory symlinks that stay inside home", async () => {
    ...
    const projectPath = path.join(symlinkHome, ".linked", "project-root", "q8idevai-desktop");
    mkdirSync(projectPath, { recursive: true });
  ...
  it.skipIf(isWindows)("matches the visible name of a directory symlink", async () => {
    ...
    const visibleProjectPath = path.join(projectsPath, "q8idevai");
  ...
  it("supports home-relative path query syntax", async () => {
    ...
    expect(result.map((entry) => realpathSync.native(entry))).toEqual([
      realpathSync.native(path.join(homeDir, "projects", "q8idevai")),
...
describe("relative typed-entry configuration", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(path.join(workspaceDir, "README.md"), "# q8idevai\n");
    writeFileSync(
  ...
  it("suffix mode matches whole path segment suffixes without fuzzy matches", async () => {
    ...
    writeFileSync(path.join(workspaceDir, "src", "q8idevai-config-file.ts"), "");
  ...
  it("suffix mode resolves explicit hidden file paths without broad hidden traversal", async () => {
    const targetPath = path.join(workspaceDir, ".dev", "q8idevai-home", "daemon.log");
    mkdirSync(path.dirname(targetPath), { recursive: true });
    writeFileSync(targetPath, "daemon log\n");

    const results = await searchRelativeDirectoryEntries({
      cwd: workspaceDir,
      query: ".dev/q8idevai-home/daemon.log",
      limit: 20,
      includeFiles: true,
      includeDirectories: false,
      matchMode: "suffix",
    ...
    expect(results).toEqual([{ path: ".dev/q8idevai-home/daemon.log", kind: "file" }]);
  });
  ...
  it("matches a path fragment anywhere in the full file path", async () => {
    ...
    expect(results).toEqual([
      {
        path: "something/something-else/skills/q8idevai-advisor/SKILL.md",
        kind: "file",
      },
    ]);
  });
