...
import { base64EncryptedWireByteLength } from "@q8idevai/relay";
import {
...
import {
  getQ8iDevAIWorktreeMetadataPath,
  readQ8iDevAIWorktreeMetadata,
  writeQ8iDevAIWorktreeMetadata,
...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
function createPullRequestStatus(overrides?: Partial<CurrentPullRequestStatus>) {
  return {
    url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/123",
    title: "Ship feature",
    state: "open",
    baseRefName: "main",
    headRefName: "feature",
...
async function readPullRequestLookupTargetFromFacts(
  ...
  const facts = await getCheckoutSnapshotFacts(repoDir, { q8idevaiHome });
  if (!facts.isGit) {
...
describe("checkout git utilities", () => {
  ...
  it("reuses checkout snapshot facts across status, shortstat, and PR status reads", async () => {
    ...
    const facts = await getCheckoutSnapshotFacts(repoDir, { q8idevaiHome });
    const status = await getCheckoutStatus(repoDir, { q8idevaiHome, facts });
    const shortstat = await getCheckoutShortstat(repoDir, { q8idevaiHome, facts }, { force: true });
    ...
    __resetPullRequestStatusCacheForTests();
    startGitCommandMetrics();
    const statusWithFacts = await getCheckoutStatus(repoDir, { q8idevaiHome, facts });
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isQ8iDevAIOwnedWorktree).toBe(false);
  ...
  it("reports a PR worktree as not ahead when its branch is pushed to the configured PR remote", async () => {
    ...
    commitFile(repoDir, "feature.txt", "feature\n", "feature commit");
    execFileSync("git", ["remote", "add", "q8idevai-pr-1285", prRemoteDir], { cwd: repoDir });
    execFileSync(
      "git",
      ["push", "q8idevai-pr-1285", "HEAD:refs/heads/open-button-targets-active-file"],
      { cwd: repoDir },
    );
    execFileSync(
      "git",
      ["config", "branch.aaronzhongg/open-button-targets-active-file.remote", "q8idevai-pr-1285"],
      {
        cwd: repoDir,
      },
    );
  ...
  it("reports a PR worktree as behind when its configured PR remote has newer commits", async () => {
    ...
    execFileSync("git", ["fetch", "q8idevai-pr-1285"], { cwd: repoDir });
  ...
  it("does not report full history as unpushed for fresh no-track Q8iDevAI worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { q8idevaiHome });
    expect(status).toMatchObject({
      isGit: true,
      isQ8iDevAIOwnedWorktree: true,
      baseRef: "main",
  ...
  it("resolves the Git common directory once when reading Q8iDevAI worktree facts", async () => {
    ...
    startGitCommandMetrics();
    const facts = await getCheckoutSnapshotFacts(result.worktreePath, { q8idevaiHome });
    const metrics = stopGitCommandMetrics();
  ...
  it("handles status/diff/commit in a .q8idevai worktree", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    expect(status.isDirty).toBe(true);
    expect(status.isQ8iDevAIOwnedWorktree).toBe(true);
    expect(realpathSync.native(status.mainRepoRoot ?? "")).toBe(realpathSync.native(repoDir));

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { q8idevaiHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { q8idevaiHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .q8idevai worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("detects plain git worktrees from git alone", async () => {
    ...
    const status = await getCheckoutStatus(worktreeDir, { q8idevaiHome });
    expect(status.isGit).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { q8idevaiHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { q8idevaiHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { q8idevaiHome });
  ...
  it("pushes the current branch to its configured upstream", async () => {
    ...
    execFileSync("git", ["remote", "add", "q8idevai-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["fetch", "q8idevai-pr-526", "main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.remote", "q8idevai-pr-526"], {
      cwd: repoDir,
    });
    ...
    expect(upstream).toBe("q8idevai-pr-526/main");
  });
  ...
  it("pushes the current branch to its configured push remote", async () => {
    ...
    execFileSync("git", ["remote", "add", "q8idevai-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.pushRemote", "q8idevai-pr-526"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.q8idevai-pr-526.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    writeFileSync(join(repoDir, "fork-pr.txt"), "fork pr edit\n");
    execFileSync("git", ["add", "fork-pr.txt"], { cwd: repoDir });
    ...
    const trackedPrRemoteHead = execFileSync(
      "git",
      ["rev-parse", "refs/remotes/q8idevai-pr-526/main"],
      {
        cwd: repoDir,
      },
    )
    ...
    expect(prRemoteMain).toBe(localHead);
    expect(getBranchUpstream(repoDir)).toBe("q8idevai-pr-526/main");
  ...
  it("refreshes the tracked ref after pushing through a configured push remote", async () => {
    ...
    execFileSync("git", ["checkout", "-b", "feature"], { cwd: repoDir });
    execFileSync("git", ["push", "-u", "origin", "feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "q8idevai-pr-1790", originDir], { cwd: repoDir });
    execFileSync("git", ["config", "branch.feature.pushRemote", "q8idevai-pr-1790"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.q8idevai-pr-1790.push", "HEAD:refs/heads/feature"], {
      cwd: repoDir,
    });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/Q8iDevAI/Q8iDevAI.git"], {
      cwd: repoDir,
    });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/Q8iDevAI/Q8iDevAI/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("uses an origin tracked head when the local branch name differs", async () => {
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(repoDir, q8idevaiHome);
  ...
  it.each([
    ...
    async ({ state, isMerged }) => {
      ...
      const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "pr-worktree");
      mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
      execFileSync("git", ["worktree", "add", workspaceDir, "contributor/old-change"], {
        cwd: repoDir,
      });
      writeQ8iDevAIWorktreeMetadata(workspaceDir, {
        baseRefName: "main",
      ...
      const facts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
  ...
  it("shows the PR after an agent renames the branch directly with git", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "renamed-by-agent");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
  ...
  it("does not replace a managed branch pin from shared push config", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "pinned-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(await readPullRequestLookupTargetFromFacts(workspaceDir, q8idevaiHome)).toMatchObject({
      headRef: "feature/pinned",
    });
  });
  ...
  it("uses the checked-out branch when a managed worktree has no metadata", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "unpinned-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
  ...
  it("uses the checked-out branch instead of ambiguous legacy PR metadata", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "legacy-pr-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, q8idevaiHome);
  ...
  it("does not apply a legacy fork hint to an ownerless branch with the same head", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "legacy-fork-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    execFileSync("git", ["worktree", "add", workspaceDir, "contributor/old-change"], {
      cwd: repoDir,
    });
    ...
    const forkFacts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "legacy-fork-branch" },
      { q8idevaiHome, facts: forkFacts },
    );
    ...
    const ownerlessFacts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "ownerless-same-head" },
      { q8idevaiHome, facts: ownerlessFacts },
    );
  ...
  it("recognizes a normalized GitHub owner branch from legacy Enterprise metadata", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "legacy-enterprise-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    ...
    const repointedFacts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "repointed-enterprise-owner" },
      { q8idevaiHome, facts: repointedFacts },
    );
  ...
  it("keeps a ref-only change request across rename and follows a later branch switch", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "gitlab-mr-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(readQ8iDevAIWorktreeMetadata(workspaceDir)?.changeRequestLookupTarget).toMatchObject({
      localBranchName: "feature/renamed",
    });
    const renamedFacts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
    const renamedStatus = await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "renamed-change-request" },
      { q8idevaiHome, facts: renamedFacts },
    );
    ...
    const switchedFacts = await getCheckoutSnapshotFacts(workspaceDir, { q8idevaiHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "switched-after-rename" },
      { q8idevaiHome, facts: switchedFacts },
    );
  ...
  it("moves a managed branch identity pin when its branch is renamed", async () => {
    ...
    const workspaceDir = join(q8idevaiHome, "worktrees", "repo", "renamed-worktree");
    mkdirSync(join(q8idevaiHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(readQ8iDevAIWorktreeMetadata(workspaceDir)?.changeRequestLookupTarget).toEqual({
  ...
  it("keeps fork identity when the local and tracked branch names match", async () => {
    ...
    execFileSync("git", ["remote", "add", "contributor", "git@github.com:contributor/q8idevai.git"], {
      cwd: repoDir,
    });
  ...
  it.each([
    "git@github.com:contributor/q8idevai.git",
    "https://github.com/contributor/q8idevai.git",
    "ssh://git@github.com/contributor/q8idevai.git",
  ])("preserves fork PR identity with branch.remote=%s", async (branchRemote) => {
    execFileSync("git", ["remote", "add", "origin", "git@github.com:getq8idevai/q8idevai.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["checkout", "-b", "topic"], { cwd: repoDir });
    ...
    expect(await readPullRequestLookupTargetFromFacts(repoDir, q8idevaiHome)).toEqual({
      headRef: "topic",
      headRepositoryOwner: "contributor",
      headSha,
    });
  ...
  it("does not attach an owner when the tracked remote is the same GitHub repository", async () => {
    ...
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/Q8iDevAI/Q8iDevAI.git"], {
      cwd: repoDir,
    });
  ...
  it("keeps the fork owner when same-repo comparison is indeterminate", async () => {
    ...
    execFileSync("git", ["remote", "add", "q8idevai-pr-345", "git@github.com:chethanuk/q8idevai.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "branch.chethanuk/main.remote", "q8idevai-pr-345"], {
      cwd: repoDir,
    });
  ...
  it("uses the configured push remote for fork PR lookup when upstream is absent", async () => {
    ...
    execFileSync("git", ["config", "branch.chethanuk/main.pushRemote", "q8idevai-pr-345"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.q8idevai-pr-345.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    const requestedTargets: RequestedPullRequestTarget[] = [];
    const github = createGitHubServiceRecordingPullRequestTargets({ requestedTargets });

    const factsTarget = await readPullRequestLookupTargetFromFacts(repoDir, q8idevaiHome);
    await getPullRequestStatus(
      repoDir,
      github,
  ...
  it("keeps the local branch lookup when a fork tracks the upstream base branch", async () => {
    execFileSync("git", ["checkout", "-b", "local-feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "origin", "git@github.com:contributor/q8idevai.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/Q8iDevAI/Q8iDevAI.git"], {
      cwd: repoDir,
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    const github = createGitHubServiceRecordingPullRequestTargets({
      ...
      statusOverrides: {
        number: 345,
        url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/345",
      },
    });
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/999",
  ...
  it("does not reuse a PR status cache entry after HEAD changes on the same branch", async () => {
    ...
    github.getCurrentPullRequestStatus = async (options) => {
      ...
      return createPullRequestStatus({
        url: `https://github.com/Q8iDevAI/Q8iDevAI/pull/${requestedShas.length}`,
      });
    };
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/Q8iDevAI/Q8iDevAI/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Q8iDevAI worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { q8idevaiHome });
  ...
  it("names both refs when a requested base ref does not match the stored one", async () => {
    ...
    await expect(
      getCheckoutDiff(worktree.worktreePath, { mode: "base", baseRef: "other" }, { q8idevaiHome }),
    ).rejects.toThrow("Base ref mismatch: stored refs/heads/main, requested other");
  });

  it("excludes dirty working tree changes from Q8iDevAI worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Q8iDevAI worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { q8idevaiHome });
  ...
  it("falls back to the repository default branch for base-dependent operations when metadata is missing", async () => {
    ...
    const metadataPath = getQ8iDevAIWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { q8idevaiHome });
    expect(baseDiff.diff).toContain("feature.txt");

    const shortstat = await getCheckoutShortstat(worktree.worktreePath, { q8idevaiHome });
    expect(shortstat).toEqual({ additions: 1, deletions: 0 });
  });

  it("falls back to plain git checkout status when Q8iDevAI worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.q8idevai/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.q8idevai/worktrees/feature",
  ...
  describe("isQ8iDevAIWorktreePath", () => {
    it("matches Unix .q8idevai/worktrees/ paths", () => {
      expect(isQ8iDevAIWorktreePath("/home/user/.q8idevai/worktrees/feature")).toBe(true);
    });

    it("matches Windows .q8idevai\\worktrees\\ paths", () => {
      expect(isQ8iDevAIWorktreePath("C:\\Users\\dev\\.q8idevai\\worktrees\\feature")).toBe(true);
    });

    it("matches worktrees under a custom Q8IDEVAI_HOME", () => {
      const customQ8iDevAIHome = process.platform === "win32" ? "C:\\q8idevai" : "/var/lib/q8idevai";
      const worktreePath =
        process.platform === "win32"
          ? win32.join(customQ8iDevAIHome, "worktrees", "project", "feature")
          : `${customQ8iDevAIHome}/worktrees/project/feature`;

      expect(
        isQ8iDevAIWorktreePath(worktreePath, {
          q8idevaiHome: customQ8iDevAIHome,
        }),
      ).toBe(true);
    });

    it("rejects paths without .q8idevai/worktrees segment", () => {
      expect(isQ8iDevAIWorktreePath("/home/user/repo")).toBe(false);
      expect(isQ8iDevAIWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
