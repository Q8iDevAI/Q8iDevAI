...
describe("OpenCode terminal agent hooks", () => {
  it("installs a self-contained OpenCode plugin idempotently", () => {
    const configDir = createTempDir("q8idevai-opencode-config-");
    ...
    expect(firstInstall.configPath).toBe(join(configDir, "plugins", "q8idevai-terminal-activity.js"));
  ...
  it.each(["succeeded", "failed", "interrupted"])(
    ...
    async (outcome) => {
      ...
      expect(commands).toEqual([
        ["q8idevai", "hooks", "opencode", "session.status.busy"],
        ["q8idevai", "hooks", "opencode", "session.status.idle"],
      ]);
    },
  );
  ...
  it("reports OpenCode 1 status and permission events through server()", async () => {
    ...
    expect(plugin.id).toBe("q8idevai-terminal-activity");
    expect(commands).toEqual([
      ["q8idevai", "hooks", "opencode", "session.status.busy"],
      ["q8idevai", "hooks", "opencode", "permission.asked"],
      ["q8idevai", "hooks", "opencode", "permission.replied"],
      ["q8idevai", "hooks", "opencode", "session.status.retry"],
      ["q8idevai", "hooks", "opencode", "session.status.idle"],
    ]);
  });
  ...
  it("preserves activity order when the host dispatches events concurrently", async () => {
    ...
    expect(commands).toEqual([["q8idevai", "hooks", "opencode", "session.status.busy"]]);
  ...
  it("keeps both generations inert outside Q8iDevAI terminals", async () => {
    const { plugin, commands } = loadInstalledPlugin("");
  ...
  it("uninstalls the OpenCode plugin file", () => {
    const configDir = createTempDir("q8idevai-opencode-config-uninstall-");
  ...
  it("prefers OPENCODE_CONFIG_DIR over the XDG config home", () => {
    const homeDir = createTempDir("q8idevai-home-");
    const configDir = createTempDir("q8idevai-opencode-override-");
    const xdgConfigHome = createTempDir("q8idevai-xdg-config-");
    ...
    expect(configPath).toBe(join(configDir, "plugins", "q8idevai-terminal-activity.js"));
  });
  ...
  it("uses the XDG config home for the default OpenCode config dir", () => {
    ...
    expect(configPath).toBe(
      join(xdgConfigHome, "opencode", "plugins", "q8idevai-terminal-activity.js"),
    );
  });
  ...
  it("falls back to the home .config OpenCode dir without an XDG config home", () => {
    ...
    expect(configPath).toBe(
      join(homeDir, ".config", "opencode", "plugins", "q8idevai-terminal-activity.js"),
    );
  });
...
function loadInstalledPlugin(terminalId = "terminal-1", exited = Promise.resolve(0)) {
  const configDir = createTempDir("q8idevai-opencode-runtime-");
  ...
  const plugin: InstalledPlugin = runInNewContext(
    ...
    {
      AbortController,
      process: { env: { Q8IDEVAI_TERMINAL_ID: terminalId } },
      Bun: {
