...
function hookPaths(root: string) {
  return {
    ...
    opencode: join(root, "opencode", "plugins", "q8idevai-terminal-activity.js"),
  };
}

function createStore(q8idevaiHome: string, enableTerminalAgentHooks: boolean): DaemonConfigStore {
...
describe("applyTerminalAgentHookSetting", () => {
  it("leaves agent configs untouched when the setting is disabled", () => {
    const root = createTempDir("q8idevai-hook-setting-");
    const store = createStore(createTempDir("q8idevai-hook-setting-home-"), false);
  ...
  it("installs agent hooks when the setting is enabled", () => {
    const root = createTempDir("q8idevai-hook-setting-");
    const store = createStore(createTempDir("q8idevai-hook-setting-home-"), true);
