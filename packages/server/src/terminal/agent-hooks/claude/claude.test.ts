...
function createFakeCliBinDir(): string {
  const dir = createTempDir("q8idevai-cli-bin-");
  writeFileSync(join(dir, "q8idevai"), "");
  return dir;
}
...
describe("Claude terminal agent hooks", () => {
  it("installs registered provider hooks idempotently", () => {
    const configDir = createTempDir("q8idevai-claude-config-");
    ...
    for (const event of provider.events) {
      const q8idevaiCommands = hookCommands(settings, event.event).filter((command) =>
        command.includes(install.hookMarker),
      );
      expect(q8idevaiCommands).toHaveLength(1);
      expect(q8idevaiCommands[0]).toBe(
        `if [ -n "$Q8IDEVAI_TERMINAL_ID" ]; then "\${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks ${provider.id} ${event.event}; fi`,
      );
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("q8idevai-claude-config-preserve-");
    writeFileSync(
  ...
  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("q8idevai-claude-config-uninstall-");
    installRegisteredAgentHooks({ configDir });
    const settings = readSettings(configDir);
    settings.hooks = {
      ...settings.hooks,
  ...
  it("builds a minimal gated hook command", () => {
    ...
    expect(command).toBe(
      'if [ -n "$Q8IDEVAI_TERMINAL_ID" ]; then "${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks claude UserPromptSubmit; fi',
    );
  });

  it.skipIf(isPlatform("win32")).each(AGENT_HOOK_PROVIDERS.claude.events)(
    "$event hook command exits 0 when Q8IDEVAI_TERMINAL_ID is unset",
    (event) => {
      const provider = AGENT_HOOK_PROVIDERS.claude;
      const command = buildAgentHookShellCommand(provider, event);

      const result = spawnSync("/bin/sh", ["-c", command], {
        env: { PATH: process.env.PATH ?? "", Q8IDEVAI_HOOK_CLI: "q8idevai" },
        stdio: "ignore",
      });
  ...
  it("prepends the q8idevai CLI directory and injects the hook CLI path", () => {
    const cliBinDir = createFakeCliBinDir();
    const hookCliPath = join(cliBinDir, "q8idevai");

    const env = buildTerminalEnvironment({
      shell: "/bin/sh",
      env: { PATH: ["/usr/bin", "/bin"].join(delimiter) },
      q8idevaiCliBinDir: cliBinDir,
      q8idevaiHookCliPath: hookCliPath,
    });

    expect(env.PATH?.split(delimiter)).toEqual([cliBinDir, "/usr/bin", "/bin"]);
    expect(env.Q8IDEVAI_HOOK_CLI).toBe(hookCliPath);
  });
