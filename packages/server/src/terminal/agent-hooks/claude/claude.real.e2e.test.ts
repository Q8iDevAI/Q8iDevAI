...
import { createTerminal, resolveQ8iDevAICliBinDir, type TerminalSession } from "../../terminal.js";
...
describe.skipIf(!claudeAvailability.available)(
  ...
  () => {
    it("posts running then idle through the installed Claude config hooks", async () => {
      ...
      const configDir = createTempDir("q8idevai-real-claude-config-");
      const cwd = createTempDir("q8idevai-real-claude-cwd-");
      const q8idevaiCliBinDir = resolveQ8iDevAICliBinDir();
      if (!q8idevaiCliBinDir) {
        throw new Error("Could not resolve q8idevai CLI bin directory");
      }
      ...
      try {
        const claude = pty.spawn(
          ...
          ["--settings", join(configDir, "settings.json"), "-p", "reply with: done"],
          {
            ...
            env: {
              ...process.env,
              Q8IDEVAI_TERMINAL_ID: terminalId,
              Q8IDEVAI_ACTIVITY_TOKEN: token,
              Q8IDEVAI_TERMINAL_ACTIVITY_URL: recorder.url,
              PATH: [q8idevaiCliBinDir, process.env.PATH].filter(isString).join(delimiter),
            },
          },
        );
    ...
    it("clears activity for an interrupted interactive Claude turn through TerminalSession", async () => {
      ...
      const configDir = createTempDir("q8idevai-real-claude-interrupt-config-");
      const q8idevaiCliBinDir = resolveQ8iDevAICliBinDir();
      if (!q8idevaiCliBinDir) {
        throw new Error("Could not resolve q8idevai CLI bin directory");
      }
