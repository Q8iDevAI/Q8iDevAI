...
describe("Codex terminal agent hooks", () => {
  it("installs POSIX and Windows hook commands idempotently", () => {
    const configDir = createTempDir("q8idevai-codex-config-");
    ...
    for (const event of codexAgentHookProvider.events) {
      expect(commandHooks(config, event.event)).toEqual([
        {
          command: `if [ -n "$Q8IDEVAI_TERMINAL_ID" ]; then "\${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks codex ${event.event}; fi`,
          commandWindows: `if defined Q8IDEVAI_TERMINAL_ID (if defined Q8IDEVAI_HOOK_CLI ("%Q8IDEVAI_HOOK_CLI%" hooks codex ${event.event}) else (q8idevai hooks codex ${event.event})) else (exit /b 0)`,
        },
      ]);
    }
  ...
  it("preserves unrelated user hooks", () => {
    const configDir = createTempDir("q8idevai-codex-config-preserve-");
    writeFileSync(
    ...
    expect(stopCommands).toEqual([
      "say codex done",
      'if [ -n "$Q8IDEVAI_TERMINAL_ID" ]; then "${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks codex Stop; fi',
    ]);
  });

  it("uninstalls only marker-matched hooks", () => {
    const configDir = createTempDir("q8idevai-codex-config-uninstall-");
    installAgentHooks(codexAgentHookProvider, { configDir });
    const config = readHooksFile(configDir);
    config.hooks = {
      ...config.hooks,
