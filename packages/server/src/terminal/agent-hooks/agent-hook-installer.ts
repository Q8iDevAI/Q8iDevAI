...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${Q8IDEVAI_HOOK_CLI:-q8idevai}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$Q8IDEVAI_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined Q8IDEVAI_TERMINAL_ID (if defined Q8IDEVAI_HOOK_CLI ("%Q8IDEVAI_HOOK_CLI%" ${hookArgs}) else (q8idevai ${hookArgs})) else (exit /b 0)`;
}
