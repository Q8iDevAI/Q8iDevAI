...
import type { TerminalCell, TerminalState } from "@q8idevai/protocol/messages";
import { TerminalInputModeTracker } from "@q8idevai/protocol/terminal-input-mode";
import { TerminalActivityTracker } from "./activity/terminal-activity-tracker.js";
import type { TerminalActivity, TerminalActivityState } from "@q8idevai/protocol/terminal-activity";

const { Terminal } = xterm;
const require = createRequire(import.meta.url);
const Q8IDEVAI_CLI_BIN_ENTRY = "@q8idevai/cli/bin/q8idevai";
...
interface BuildTerminalEnvironmentInput {
  ...
  q8idevaiCliBinDir?: string | null;
  q8idevaiHookCliPath?: string | null;
}
...
export function resolveQ8iDevAICliBinDir(): string | null {
  const cliExecutable = resolveQ8iDevAICliExecutablePath();
  return cliExecutable ? dirname(cliExecutable) : null;
}

export function resolveQ8iDevAICliExecutablePath(): string | null {
  const configuredCli = process.env.Q8IDEVAI_CLI?.trim();
  if (configuredCli) {
    return resolvePath(configuredCli);
  }

  const cliEntrypoint = resolveQ8iDevAICliBinEntrypoint();
  if (!cliEntrypoint) {
    return null;
  }
  ...
  if (npmBinDir) {
    const shim = resolveQ8iDevAICliShim(npmBinDir);
    if (shim) {
      return shim;
    }
  }
...
function resolveQ8iDevAICliBinEntrypoint(): string | null {
  try {
    return require.resolve(Q8IDEVAI_CLI_BIN_ENTRY);
  } catch {
    return null;
  }
}
...
function findNpmBinDir(startPath: string): string | null {
  ...
  while (true) {
    ...
    if (hasQ8iDevAICliShim(candidate)) {
      return candidate;
    }
...
function hasQ8iDevAICliShim(binDir: string): boolean {
  return resolveQ8iDevAICliShim(binDir) !== null;
}

function resolveQ8iDevAICliShim(binDir: string): string | null {
  for (const name of q8idevaiCliShimNames()) {
...
function q8idevaiCliShimNames(): string[] {
  return process.platform === "win32" ? ["q8idevai.cmd", "q8idevai.exe", "q8idevai"] : ["q8idevai"];
}
...
function resolveZshShellIntegrationRuntimeDir(): string {
  ...
  return join(tmpdir(), `${username}-q8idevai-zsh-${process.pid}`);
}
...
function prepareZshShellIntegrationRuntimeDir(sourceDir = resolveZshShellIntegrationDir()): string {
  ...
  writePrivateFileAtomicSync(
    join(runtimeDir, "q8idevai-integration.zsh"),
    readFileSync(join(readableSourceDir, "q8idevai-integration.zsh")),
  );
  return runtimeDir;
}
...
export function buildTerminalEnvironment(
  ...
  const envWithAgentHooks = prependQ8iDevAICliToPath(
    baseEnv,
    input.q8idevaiCliBinDir === undefined ? resolveQ8iDevAICliBinDir() : input.q8idevaiCliBinDir,
  );
  const envWithHookCli = injectQ8iDevAIHookCli(
    envWithAgentHooks,
    input.q8idevaiHookCliPath === undefined ? resolveQ8iDevAICliExecutablePath() : input.q8idevaiHookCliPath,
  );
...
function injectQ8iDevAIHookCli(
  ...
  return {
    ...env,
    Q8IDEVAI_HOOK_CLI: resolvePath(resolveExternalProcessPath(cliPath)),
  };
}

function prependQ8iDevAICliToPath(
  env: Record<string, string>,
  ...
  const ptyProcess = pty.spawn(spawnCommand, spawnArgs, {
    ...
    env: buildTerminalEnvironment({
      ...
      env: {
        ...env,
        ...activityEnv,
        Q8IDEVAI_WORKSPACE_ID: workspaceId,
      },
    }),
  });
