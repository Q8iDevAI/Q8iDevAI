...
import {
  resolveQ8iDevAICliBinDir,
  resolveQ8iDevAICliExecutablePath,
  type TerminalSession,
} from "./terminal.js";
import type { TerminalState } from "@q8idevai/protocol/messages";
...
it("delivers rapid small writes complete and in order through worker coalescing", async () => {
  ...
  const session = trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      cwd,
      env: { Q8IDEVAI_TERMINAL_BURST_GATE: burstGatePath },
      ...nodeTerminalCommand(`
      const fs = require("node:fs");
      const gatePath = process.env.Q8IDEVAI_TERMINAL_BURST_GATE;
...
it("keeps registered cwd env inheritance behind the worker manager interface", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { Q8IDEVAI_WORKER_TERMINAL_TEST: "worker-env" },
  });
  trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      ...
      require("node:fs").writeFileSync(
        ...
        process.env.Q8IDEVAI_WORKER_TERMINAL_TEST ?? "",
      );
      setInterval(() => {}, 1000);
    `),
    }),
...
it("injects parent-minted terminal activity env through the worker", async () => {
    ...
    await manager.createTerminal({
      ...
      ...nodeTerminalCommand(`
        require("node:fs").writeFileSync(
          ...
          JSON.stringify({
            terminalId: process.env.Q8IDEVAI_TERMINAL_ID,
            token: process.env.Q8IDEVAI_ACTIVITY_TOKEN,
            url: process.env.Q8IDEVAI_TERMINAL_ACTIVITY_URL,
            hookCli: process.env.Q8IDEVAI_HOOK_CLI,
  ...
  const q8idevaiCliBinDir = resolveQ8iDevAICliBinDir();
  const q8idevaiCliPath = resolveQ8iDevAICliExecutablePath();
  expect(q8idevaiCliBinDir).not.toBeNull();
  expect(q8idevaiCliPath).not.toBeNull();
  ...
  expect(env.hookCli).toBe(q8idevaiCliPath);
  ...
  expect(env.path?.split(delimiter)[0]).toBe(q8idevaiCliBinDir);
});
