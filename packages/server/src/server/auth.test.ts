...
describe("daemon bearer validator", () => {
  ...
  test("extracts WebSocket q8idevai bearer subprotocol tokens", () => {
    const protocol = extractWsBearerProtocol("chat, q8idevai.bearer.secret.with.dots");

    expect(protocol).toBe("q8idevai.bearer.secret.with.dots");
    expect(extractWsBearerToken(protocol)).toBe("secret.with.dots");
    expect(extractWsBearerToken("q8idevai.other.secret")).toBeNull();
  });
