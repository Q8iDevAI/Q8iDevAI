...
export type ActiveWorkspaceRef = Pick<
  PersistedWorkspaceRecord,
  "workspaceId" | "cwd" | "kind" | "worktreeRoot" | "isQ8iDevAIOwnedWorktree" | "mainRepoRoot"
>;
...
interface BackingDirectory {
  ...
  q8idevaiWorktreesRoot: string | null;
}
...
// Q8iDevAI-owned AND no active workspace still references it.
...
async function resolveWorkspaceBackingDirectory(
  ...
  dependencies: Pick<ArchiveDependencies, "q8idevaiHome" | "q8idevaiWorktreesBaseRoot">,
): Promise<BackingDirectory> {
  if (workspace.isQ8iDevAIOwnedWorktree && workspace.worktreeRoot && workspace.mainRepoRoot) {
    return {
      path: resolve(workspace.worktreeRoot),
      isQ8iDevAIOwnedWorktree: true,
      mainRepoRoot: workspace.mainRepoRoot,
      q8idevaiWorktreesRoot: null,
    };
  }
...
async function resolveBackingDirectory(
  ...
  const ownership = await isQ8iDevAIOwnedWorktreeCwd(cwd, options);
  return {
    path: resolve(ownership.allowed && ownership.worktreePath ? ownership.worktreePath : cwd),
    isQ8iDevAIOwnedWorktree: ownership.allowed,
    mainRepoRoot: ownership.repoRoot ?? null,
    q8idevaiWorktreesRoot: ownership.worktreeRoot ?? null,
  };
}
...
async function maybeRemoveDirectory(
  ...
  if (!backing?.isQ8iDevAIOwnedWorktree) {
    return false;
  }
  ...
  try {
    await deleteQ8iDevAIWorktree({
      ...
      worktreePath: backing.path,
      teardownCwds: [],
      worktreesRoot: backing.q8idevaiWorktreesRoot ?? undefined,
      q8idevaiHome: dependencies.q8idevaiHome,
      worktreesBaseRoot: dependencies.q8idevaiWorktreesBaseRoot,
    });
