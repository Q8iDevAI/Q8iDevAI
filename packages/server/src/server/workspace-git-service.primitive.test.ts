...
function createCheckoutFacts(
  ...
  return {
    ...
    q8idevaiWorktree: { isQ8iDevAIOwnedWorktree: false },
    storedBaseRef: null,
...
interface CreateServiceOptions {
  ...
  listQ8iDevAIWorktrees?: ReturnType<typeof vi.fn>;
  github?: ForgeService;
...
function buildDefaultServiceDeps() {
  return {
    ...
    listQ8iDevAIWorktrees: vi.fn(async () => []),
  ...
  test("attributes Git commands submitted by a workspace refresh", async () => {
    ...
    execFileSync(
      ...
      [
        ...
        "-c",
        "user.name=Q8iDevAI Test",
        "-c",
        "user.email=q8idevai@example.test",
        "commit",
        "-m",
        "initial",
      ],
  ...
  test("GitHub self-heal polling uses the fork PR head branch instead of the owner-prefixed local branch", async () => {
    ...
    const getCheckoutSnapshotFacts = vi.fn(async (cwd: string) =>
      createCheckoutFacts(cwd, {
        ...
        branchRemoteName: "q8idevai-pr-1285",
...
describe("WorkspaceGitServiceImpl D2 read methods", () => {
  ...
  test("listStashes cold-loads, warms, forces, and coalesces per cwd", async () => {
    let nowMs = 0;
    const stashOutput = "stash@{0}\u0000q8idevai-auto-stash: feature\n";
    ...
    const first = service.listStashes(REPO_CWD, { q8idevaiOnly: true });
    const second = service.listStashes(join(REPO_CWD, "."), { q8idevaiOnly: true });
    await flushPromises();
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([
      [{ index: 0, message: "q8idevai-auto-stash: feature", branch: "feature", isQ8iDevAI: true }],
      [{ index: 0, message: "q8idevai-auto-stash: feature", branch: "feature", isQ8iDevAI: true }],
    ]);

    nowMs = 1_000;
    await service.listStashes(REPO_CWD, { q8idevaiOnly: true });
    expect(runGitCommand).toHaveBeenCalledTimes(1);

    await service.listStashes(REPO_CWD, { q8idevaiOnly: true }, { force: true, reason: "test" });
    expect(runGitCommand).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees cold-loads, warms, forces, and coalesces per repo root", async () => {
    ...
    const listQ8iDevAIWorktrees = vi.fn().mockResolvedValue(worktrees);
    const service = createService({
    ...
    expect(listQ8iDevAIWorktrees).toHaveBeenCalledTimes(1);
    ...
    expect(listQ8iDevAIWorktrees).toHaveBeenCalledTimes(2);
  ...
  test("listWorktrees shares one repo-root scoped read across sibling workspace cwds", async () => {
    ...
    const worktrees = [
      {
        path: join(tempDir, "q8idevai-home", "worktrees", "repo", "feature"),
        createdAt: "2026-04-12T00:00:00.000Z",
        branchName: "feature",
      },
    ];
    const listQ8iDevAIWorktrees = vi.fn(async () => worktrees);
    const service = createService({
      getCheckoutSnapshotFacts: getCheckoutSnapshotFactsUncached as never,
      getCheckoutStatus: getCheckoutStatusUncached as never,
      listQ8iDevAIWorktrees,
    ...
    try {
      ...
      expect(listQ8iDevAIWorktrees).toHaveBeenCalledTimes(1);
      expect(listQ8iDevAIWorktrees).toHaveBeenCalledWith({
  ...
  test("resolveRepoRemoteUrl reads remote URL through the snapshot cache", async () => {
    ...
    await expect(service.resolveRepoRemoteUrl(REPO_CWD)).resolves.toBe(
      "https://github.com/Q8iDevAI/Q8iDevAI.git",
    );
    nowMs = 1_000;
  ...
  test("getProjectSlug derives the slug from the snapshot cache", async () => {
    ...
    await expect(service.getProjectSlug(REPO_CWD)).resolves.toBe("q8idevai");
