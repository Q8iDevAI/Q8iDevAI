...
import { WorkspaceScriptPayloadSchema } from "@q8idevai/protocol/messages";
import type { ScriptHealthState } from "./script-health-monitor.js";
import { WorkspaceScriptRuntimeStore } from "./workspace-script-runtime-store.js";
import { readQ8iDevAIConfig } from "../utils/worktree.js";
...
function buildPayloads(input: {
  ...
  q8idevaiConfig?: Q8iDevAIConfig | null;
  ...
  const q8idevaiConfig =
    input.q8idevaiConfig !== undefined ? input.q8idevaiConfig : loadConfig(input.workspaceDirectory);
...
function loadConfig(repoRoot: string): Q8iDevAIConfig | null {
...
describe("script-status-projection", () => {
  ...
  it("readQ8iDevAIConfig fails with configPath and error when q8idevai.json is malformed", () => {
    const workspace = createWorkspaceRepo();
    const configPath = path.join(workspace.repoDir, "q8idevai.json");
    writeFileSync(
      configPath,
      '{\n<<<<<<< HEAD\n  "scripts": {}\n=======\n  "scripts": {}\n>>>>>>> origin/main\n}\n',
    );
    ...
    try {
      const result = readQ8iDevAIConfig(workspace.repoDir);
  ...
  it("buildWorkspaceScriptPayloads given q8idevaiConfig=null still surfaces orphaned runtime scripts", () => {
