...
import type { ProjectCheckoutLitePayload } from "@q8idevai/protocol/messages";
...
import { listQ8iDevAIWorktrees, type Q8iDevAIWorktreeInfo } from "../utils/worktree.js";
...
export interface WorkspaceGitRuntimeSnapshot {
  ...
  git: {
    ...
    isQ8iDevAIOwnedWorktree: boolean;
...
export type WorkspaceGitWorktreeInfo = Q8iDevAIWorktreeInfo;
...
interface WorkspaceGitServiceDependencies {
  ...
  listQ8iDevAIWorktrees: typeof listQ8iDevAIWorktrees;
  /**
...
export class WorkspaceGitServiceImpl implements WorkspaceGitService {
  ...
  constructor(options: WorkspaceGitServiceOptions) {
    ...
    this.q8idevaiHome = options.q8idevaiHome;
  ...
  async getCheckout(cwd: string): Promise<ProjectCheckoutLitePayload> {
    ...
    return checkoutLiteFromGitSnapshot(normalizedCwd, {
      ...
      isQ8iDevAIOwnedWorktree: status.isQ8iDevAIOwnedWorktree,
  ...
  listStashes(
    ...
    this.assertNotDisposed();
    const normalizedCwd = resolve(cwd);
    const q8idevaiOnly = options?.q8idevaiOnly !== false;
    const key = JSON.stringify(["stashes", normalizedCwd, q8idevaiOnly]);
    return this.readAuxiliaryCache(this.stashListCache, key, readOptions, async () => {
      ...
      return parseWorkspaceGitStashList(stdout, { q8idevaiOnly });
    });
  }
  ...
  async listWorktrees(
    ...
    return this.readAuxiliaryCache(this.worktreeListCache, key, options, () =>
      this.deps.listQ8iDevAIWorktrees({
        cwd: repoRoot,
        q8idevaiHome: this.q8idevaiHome,
        worktreesRoot: this.worktreesRoot,
      }),
  ...
  async resolveRepoRoot(cwd: string, options?: WorkspaceGitReadOptions): Promise<string> {
    ...
    return snapshot.git.isQ8iDevAIOwnedWorktree
  ...
  private resolveForgePrStatusPollTarget(
    ...
    const lookupTarget =
      target.latestFacts?.isGit && target.latestFacts.currentBranch === git.currentBranch
        ...
        : null;
    if (target.latestFacts?.isGit && target.latestFacts.q8idevaiWorktree.isQ8iDevAIOwnedWorktree) {
      return lookupTarget;
    }
    if (lookupTarget) {
      return lookupTarget;
  ...
  private async refreshGitSnapshot(
    ...
    target.latestGit = {
      ...
      isQ8iDevAIOwnedWorktree: checkoutStatus.isQ8iDevAIOwnedWorktree,
...
function parseWorkspaceGitStashList(
  ...
  for (const line of lines) {
    ...
    const prefix = "q8idevai-auto-stash:";
    const prefixIdx = subject.indexOf(prefix);
    const isQ8iDevAI = prefixIdx >= 0;
    const branch = isQ8iDevAI ? subject.slice(prefixIdx + prefix.length).trim() || null : null;

    if (options.q8idevaiOnly && !isQ8iDevAI) {
      continue;
    }

    entries.push({ index, message: subject, branch, isQ8iDevAI });
  }
