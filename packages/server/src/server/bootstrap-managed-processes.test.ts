...
describe("daemon managed process bootstrap", () => {
  test("reaps stale helper process records during daemon bootstrap", async () => {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-managed-bootstrap-"));
    staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const q8idevaiHome = path.join(tempRoot, ".q8idevai");
