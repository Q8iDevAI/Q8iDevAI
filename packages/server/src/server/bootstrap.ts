...
import { createQ8iDevAIWorktree as createRegisteredQ8iDevAIWorktree } from "./q8idevai-worktree-service.js";
import { createWorkspaceProvisioningService } from "./session/workspace-provisioning/workspace-provisioning-service.js";
import { createQ8iDevAIWorktreeWorkflow } from "./worktree-session.js";
...
import {
  createQ8iDevAIToolCatalog,
  type Q8iDevAIToolHostDependencies,
} from "./agent/tools/q8idevai-tools.js";
import type { Q8iDevAIToolRuntimeContext } from "./agent/tools/types.js";
...
import { resolveQ8iDevAIToolPolicy } from "./agent/q8idevai-tool-policy.js";
...
export type Q8iDevAIOpenAIConfig = OpenAiSpeechProviderConfig;
export type Q8iDevAILocalSpeechConfig = LocalSpeechProviderConfig;

export interface Q8iDevAISpeechSttLanguages {
  dictation: string;
  voice: string;
}

export interface Q8iDevAISpeechConfig {
  providers: RequestedSpeechProviders;
  sttLanguages?: Q8iDevAISpeechSttLanguages;
  local?: Q8iDevAILocalSpeechConfig;
}
...
export interface Q8iDevAIDaemonConfig {
  listen: string;
  q8idevaiHome: string;
  ...
  openai?: Q8iDevAIOpenAIConfig;
  speech?: Q8iDevAISpeechConfig;
...
export interface Q8iDevAIDaemon {
  config: Q8iDevAIDaemonConfig;
...
export interface Q8iDevAIDaemonDependencies {
...
function createBootstrapManagedProcessRegistry(
  config: Pick<Q8iDevAIDaemonConfig, "q8idevaiHome" | "managedProcesses">,
  logger: Logger,
  ...
  return createManagedProcessRegistry({
    q8idevaiHome: config.q8idevaiHome,
...
function mountWebUi(app: express.Application, config: Q8iDevAIDaemonConfig, logger: Logger): void {
  app.use(
...
function resolveExpressTrustProxySetting(config: Q8iDevAIDaemonConfig): true | string[] {
  return config.trustedProxies ?? ["loopback"];
}

function createInitialMutableDaemonConfig(config: Q8iDevAIDaemonConfig): MutableDaemonConfig {
  ...
  const initialConfig: MutableDaemonConfig = {
    ...
    app: { baseUrl: config.appBaseUrl ?? "https://app.q8idevai.sh" },
...
export async function createQ8iDevAIDaemon(
  config: Q8iDevAIDaemonConfig,
  rootLogger: Logger,
  dependencies: Q8iDevAIDaemonDependencies = {},
): Promise<Q8iDevAIDaemon> {
  configureGitProcessPolicy(config.git ?? resolveGitProcessPolicy({ env: process.env }));
  const logger = rootLogger.child({ module: "bootstrap" });
  const obsoleteTimelineDirectory = path.join(config.q8idevaiHome, "agent-timelines");
  ...
  const daemonConfigStore = new DaemonConfigStore(config.q8idevaiHome, initialMutableConfig, logger, {
    relayEnabledMutable: config.relayEnabledMutable ?? true,
    startupPersisted: config.configReload?.startupPersisted,
    reloadSource: {
      resolve: (persisted) => {
        const reloaded = resolveConfigFromPersisted(config.q8idevaiHome, persisted, {
  ...
  const pluginRuntime = new PluginService(logger, daemonConfigStore, daemonVersion, {
    managedSources: new ManagedPluginSources(config.q8idevaiHome),
    settingsDirectory: path.join(config.q8idevaiHome, "plugin-settings"),
  });

  const serverId = getOrCreateServerId(config.q8idevaiHome, { logger });
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(config.q8idevaiHome, logger);
  ...
  let appBaseUrl = config.appBaseUrl ?? "https://app.q8idevai.sh";
  ...
  daemonConfigStore.onFieldChange("app.baseUrl", (value) => {
    appBaseUrl = typeof value === "string" ? value : "https://app.q8idevai.sh";
  });
  ...
  const fixedAllowedOrigins = [
    // Packaged desktop renderers use the custom q8idevai:// protocol scheme.
    "q8idevai://app",
  ...
  const projectRegistry = new FileBackedProjectRegistry(
    path.join(config.q8idevaiHome, "projects", "projects.json"),
    logger,
  );
  workspaceRegistry = new FileBackedWorkspaceRegistry(
    path.join(config.q8idevaiHome, "projects", "workspaces.json"),
    logger,
  );
  ...
  const agentManager = new AgentManager({
    ...
    resolveQ8iDevAIToolPolicy: (provider) =>
      resolveQ8iDevAIToolPolicy(provider, daemonConfigStore.get().providers),
    logger,
  });
  ...
  setupAutoArchiveOnMerge({
    q8idevaiHome: config.q8idevaiHome,
    q8idevaiWorktreesBaseRoot: config.worktreesRoot,
    daemonConfigStore,
    workspaceGitService,
    github,
    agentManager,
  ...
  const createQ8iDevAIWorktreeForTools = async (
    input: Parameters<typeof createQ8iDevAIWorktreeWorkflow>[1],
    serviceOptions?: Parameters<typeof createQ8iDevAIWorktreeWorkflow>[2],
  ) => {
    return createQ8iDevAIWorktreeWorkflow(
      {
        q8idevaiHome: config.q8idevaiHome,
        worktreesRoot: config.worktreesRoot,
        createQ8iDevAIWorktree: async (workflowInput, workflowOptions) => {
          return createRegisteredQ8iDevAIWorktree(workflowInput, {
            github,
  ...
  const createAgentCommandDependencies: CreateAgentCommandDependencies = {
    ...
    terminalManager,
    providerSnapshotManager,
    createQ8iDevAIWorktree: createQ8iDevAIWorktreeForTools,
  ...
  const hubAgentLifecycle = new CreateAgentLifecycleDispatch({
    ...
    github,
    workspaceGitService,
    createQ8iDevAIWorktreeWorkflow: createQ8iDevAIWorktreeForTools,
  ...
  const createScheduleQ8iDevAIWorktreeExternal = async (input: {
    cwd: string;
    firstAgentContext: FirstAgentContext;
  }) => {
    const result = await createQ8iDevAIWorktreeForTools({
      cwd: input.cwd,
      firstAgentContext: input.firstAgentContext,
    });
  ...
  const scheduleService = new ScheduleService({
    ...
    createQ8iDevAIWorktreeWorkspace: createScheduleQ8iDevAIWorktreeExternal,
  ...
  const createAgentToolHostDependencies = (
    runtime: Q8iDevAIToolRuntimeContext,
  ): Q8iDevAIToolHostDependencies => ({
    agentManager,
    agentStorage,
    terminalManager,
    getDaemonTcpPort: () => (boundListenTarget?.type === "tcp" ? boundListenTarget.port : null),
    ...
    workspaceScripts: createWorkspaceScriptsService({
      ...
      globalServicePorts: loadPersistedConfig(config.q8idevaiHome).worktrees?.servicePorts,
    }),
    markWorkspaceArchiving: markWorkspaceArchivingExternal,
    clearWorkspaceArchiving: clearWorkspaceArchivingExternal,
    ensureWorkspaceForCreate: createAgentCommandDependencies.ensureWorkspaceForCreate,
    createQ8iDevAIWorktree: createAgentCommandDependencies.createQ8iDevAIWorktree,
    browserToolsEnabled: browserToolsPolicy.isEnabled(),
    browserToolsBroker,
    q8idevaiToolPolicy:
      runtime.q8idevaiToolPolicy ??
      (runtime.callerAgentId ? agentManager.getQ8iDevAIToolPolicy(runtime.callerAgentId) : undefined),
    q8idevaiHome: config.q8idevaiHome,
    worktreesRoot: config.worktreesRoot,
  ...
  const createAgentToolCatalog = (runtime: Q8iDevAIToolRuntimeContext) =>
    createQ8iDevAIToolCatalog(createAgentToolHostDependencies(runtime));
  const setAgentProviderToolsEnabled = (enabled: boolean) => {
    agentProviderRuntime.setQ8iDevAIToolCatalog(enabled ? createAgentToolCatalog({}) : null);
  };
  agentManager.setQ8iDevAIToolCatalogFactory(createAgentToolCatalog);
  agentManager.setQ8iDevAIToolsEnabled(config.mcpInjectIntoAgents !== false);
    ...
    const createAgentMcpSession = async (callerAgentId?: string) => {
      const agentMcpServer = await createAgentMcpServer(
        createAgentToolHostDependencies({
          callerAgentId,
          q8idevaiToolPolicy: callerAgentId
            ? agentManager.getQ8iDevAIToolPolicy(callerAgentId)
            : undefined,
        }),
      );
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const onListening = () => {
          ...
          const logAndResolve = async () => {
            ...
            agentManager.setQ8iDevAIToolsEnabled(mcpEnabled && config.mcpInjectIntoAgents !== false);
            ...
            const relayEndpoint = config.relayEndpoint ?? "relay.q8idevai.sh:443";
            const relayPublicEndpoint = config.relayPublicEndpoint ?? relayEndpoint;
            const relayUseTls = config.relayUseTls ?? relayEndpoint === "relay.q8idevai.sh:443";
            ...
            pluginRuntime.bindQ8iDevAISessionHost(wsServer);
