...
async function runLoggerFixture(source: string): Promise<{ stdout: string; stderr: string }> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-fixture-"));
...
describe("resolveLogConfig", () => {
  const q8idevaiHome = "/tmp/q8idevai-logger-tests";

  it("defaults to stdout JSON without file logging", () => {
    const result = resolveLogConfig(undefined, { q8idevaiHome });
  ...
  it("keeps legacy level and format as stdout configuration", () => {
    const result = resolveLogConfig({ level: "warn", format: "pretty" }, { q8idevaiHome });
  ...
  it("enables file output only when log.file is present", () => {
    ...
    expect(resolveLogConfig(config, { q8idevaiHome })).toEqual({
      level: "debug",
      console: {
        level: "warn",
        format: "json",
      ...
      file: {
        level: "debug",
        path: path.resolve(q8idevaiHome, "logs", "programmatic.log"),
      },
    });
  });
  ...
  it("defaults file output to info when log.file is present without a level", () => {
    ...
    expect(resolveLogConfig(config, { q8idevaiHome })).toEqual({
      ...
      file: {
        level: "info",
        path: path.resolve(q8idevaiHome, "daemon.log"),
      },
    });
  });
});
...
describe("loadConfig logger config", () => {
  it("applies log format env at the config boundary", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-config-"));
    ...
    const config = loadConfig(q8idevaiHome, {
      env: { Q8IDEVAI_LOG_FORMAT: "pretty" },
    });

    expect(config.log?.format).toBe("pretty");
    expect(resolveLogConfig(config, { q8idevaiHome }).console.format).toBe("pretty");
  });
});

describe("createRootLogger", () => {
  it("includes the daemon version in child logger entries", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-version-"));

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(undefined, { q8idevaiHome: ${JSON.stringify(q8idevaiHome)} });
  ...
  it("writes JSON to stdout by default and does not initialize file logging", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-default-"));
    const missingLogDir = path.join(q8idevaiHome, "logs");
    ...
    expect(existsSync(path.join(q8idevaiHome, "daemon.log"))).toBe(false);
    expect(existsSync(missingLogDir)).toBe(false);
  });

  it("writes to an explicit file target without creating rotation files", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-file-"));
    const logPath = path.join(q8idevaiHome, "logs", "programmatic.log");

    await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { q8idevaiHome: ${JSON.stringify(q8idevaiHome)} },
      );
  ...
  it("can disable file output for supervised workers", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-no-worker-file-"));
    const logPath = path.join(q8idevaiHome, "daemon.log");

    const { stdout } = await runLoggerFixture(`
      const logger = createRootLogger(
        { log: { file: { path: ${JSON.stringify(logPath)} } } },
        { q8idevaiHome: ${JSON.stringify(q8idevaiHome)}, file: false },
      );
  ...
  it("keeps pretty output available as a format choice", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-logger-pretty-"));
