...
import { BrowserAutomationHostCapabilitySchema } from "@q8idevai/protocol/browser-automation/capabilities";
import type { SessionEventSubscription } from "@q8idevai/protocol/messages";
import { relative } from "node:path";
import { isAbsolute } from "node:path";
import { CreationService } from "./creation/index.js";
import type { CreationSnapshot, AgentCreateRequest } from "@q8idevai/protocol/messages";
...
import { CLIENT_CAPS, type ClientCapability } from "@q8idevai/protocol/client-capabilities";
import { formatPluginSourceReference } from "@q8idevai/protocol/plugin-source-reference";
import {
...
import type { TerminalActivity } from "@q8idevai/protocol/terminal-activity";
import type { BinaryFrame } from "@q8idevai/protocol/binary-frames/index";
...
import { getErrorMessage, getErrorMessageOr } from "@q8idevai/protocol/error-utils";
import { getAgentStatusPriority } from "@q8idevai/protocol/agent-state-bucket";
import { getParentAgentIdFromLabels } from "@q8idevai/protocol/agent-labels";
...
import {
  createQ8iDevAIWorktree,
  type CreateQ8iDevAIWorktreeInput,
  type CreateQ8iDevAIWorktreeResult,
} from "./q8idevai-worktree-service.js";
import { WorkspaceAutoName } from "./workspace-auto-name.js";
import {
  buildAgentSessionConfig as buildWorktreeAgentSessionConfig,
  createQ8iDevAIWorktreeWorkflow as createWorktreeWorkflow,
  type CreateQ8iDevAIWorktreeSetupContinuationInput,
  type CreateQ8iDevAIWorktreeWorkflowResult,
  handleCreateQ8iDevAIWorktreeRequest as handleCreateWorktreeRequest,
  handleQ8iDevAIWorktreeArchiveRequest as handleWorktreeArchiveRequest,
  handleQ8iDevAIWorktreeListRequest as handleWorktreeListRequest,
...
import { parseGitRemoteLocation } from "@q8idevai/protocol/git-remote";
import {
...
export interface SessionOptions {
  ...
  pluginRuntime?: {
    ...
    listPlugins(): import("@q8idevai/protocol/messages").PluginListItem[];
    getLogs(pluginId: string): import("@q8idevai/protocol/messages").PluginLogEntry[];
    installDirectory(input: {
      path: string;
      id?: string;
    }): Promise<import("@q8idevai/protocol/messages").PluginListItem>;
    ...
    statusSources(
      pluginId?: string,
    ): Promise<import("@q8idevai/protocol/messages").PluginSourceStatusItem[]>;
    updateSources(
      pluginId?: string,
    ): Promise<import("@q8idevai/protocol/messages").PluginSourceUpdateItem[]>;
    reloadPlugin(pluginId: string): Promise<import("@q8idevai/protocol/messages").PluginListItem>;
    enablePlugin(pluginId: string): Promise<import("@q8idevai/protocol/messages").PluginListItem>;
    disablePlugin(pluginId: string): Promise<import("@q8idevai/protocol/messages").PluginListItem>;
...
export class Session {
  ...
  private readonly q8idevaiHome: string;
  ...
  constructor(options: SessionOptions) {
    ...
    this.workspaceRecovery = createWorkspaceRecoveryService({
      q8idevaiHome: this.q8idevaiHome,
    ...
    this.createAgentLifecycleDispatch = new CreateAgentLifecycleDispatch({
      ...
      github: this.github,
      workspaceGitService: this.workspaceGitService,
      createQ8iDevAIWorktreeWorkflow: (input, workflowOptions) =>
        this.createQ8iDevAIWorktreeWorkflow(input, workflowOptions),
    ...
    this.workspaceScripts = createWorkspaceScriptsService({
      ...
      globalServicePorts: loadPersistedConfig(this.q8idevaiHome).worktrees?.servicePorts,
    });
  ...
  private dispatchWorkspaceAndProjectMessage(
    ...
    switch (msg.type) {
      ...
      case "q8idevai_worktree_list_request":
        return this.handleQ8iDevAIWorktreeListRequest(msg);
      case "q8idevai_worktree_archive_request":
        return this.handleQ8iDevAIWorktreeArchiveRequest(msg);
      case "create_q8idevai_worktree_request":
        return this.handleCreateQ8iDevAIWorktreeRequest(msg);
  ...
  private async createSessionAgent(
    ...
    let createdWorktreeForCleanup: CreateQ8iDevAIWorktreeWorkflowResult | null = null;
  ...
  private async resolveSessionCreateAgentIntent(input: {
    ...
    createdWorktree: CreateQ8iDevAIWorktreeWorkflowResult | null;
  ...
  private async buildAgentSessionConfig(
    ...
    setupContinuation?: CreateQ8iDevAIWorktreeWorkflowResult["setupContinuation"];
    ...
    return buildWorktreeAgentSessionConfig(
      {
        ...
        sessionLogger: this.sessionLogger,
        workspaceGitService: this.workspaceGitService,
        createQ8iDevAIWorktree: (input, serviceOptions) =>
          this.createQ8iDevAIWorktreeWorkflow(input, {
            ...serviceOptions,
            setupContinuation: {
              kind: "agent",
              terminalManager: this.terminalManager,
  ...
  private async handleQ8iDevAIWorktreeListRequest(
    msg: Extract<SessionInboundMessage, { type: "q8idevai_worktree_list_request" }>,
  ): Promise<void> {
  ...
  private async handleQ8iDevAIWorktreeArchiveRequest(
    msg: Extract<SessionInboundMessage, { type: "q8idevai_worktree_archive_request" }>,
  ): Promise<void> {
    return handleWorktreeArchiveRequest(
      {
        q8idevaiHome: this.q8idevaiHome,
        q8idevaiWorktreesBaseRoot: this.worktreesRoot,
        github: this.github,
        workspaceGitService: this.workspaceGitService,
        agentManager: this.agentManager,
        agentStorage: this.agentStorage,
  ...
  private async describeWorkspaceRecord(
    ...
    const worktreeSlug =
      workspace.isQ8iDevAIOwnedWorktree && workspace.worktreeRoot
  ...
  private buildWorkspaceGitRuntimePayload(
    ...
    return {
      ...
      isQ8iDevAIOwnedWorktree: snapshot.git.isQ8iDevAIOwnedWorktree,
  ...
  private async describeCreatedWorktreeWorkspace(
    result: CreateQ8iDevAIWorktreeResult,
  ): Promise<WorkspaceDescriptorPayload> {
  ...
  private async createQ8iDevAIWorktree(
    input: CreateQ8iDevAIWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    },
  ): Promise<CreateQ8iDevAIWorktreeResult> {
    const result = await createQ8iDevAIWorktree(input, {
      github: this.github,
  ...
  private async listActiveWorkspaceRefs(): Promise<ActiveWorkspaceRef[]> {
    ...
    return workspaces
      ...
      .map((workspace) => ({
        ...
        isQ8iDevAIOwnedWorktree: workspace.isQ8iDevAIOwnedWorktree,
  ...
  private async handleWorkspaceCreateWorktree(
    ...
    const result = await this.createQ8iDevAIWorktreeWorkflow(
      {
        cwd: sourceCwd,
        workspaceId,
  ...
  private async handleProjectGithubCloneRequest(
    ...
    try {
      ...
      const cloneStagingPath = await mkdtemp(resolve(targetParent, ".q8idevai-clone-"));
      try {
  ...
  private async handleCreateQ8iDevAIWorktreeRequest(
    request: Extract<SessionInboundMessage, { type: "create_q8idevai_worktree_request" }>,
  ): Promise<void> {
    return handleCreateWorktreeRequest(
      {
        ...
        emit: (message) => this.emit(message),
        sessionLogger: this.sessionLogger,
        createQ8iDevAIWorktreeWorkflow: (input) => this.createQ8iDevAIWorktreeWorkflow(input),
      },
      request,
    );
  }

  private async createQ8iDevAIWorktreeWorkflow(
    input: CreateQ8iDevAIWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
      setupContinuation?: CreateQ8iDevAIWorktreeSetupContinuationInput;
    },
  ): Promise<CreateQ8iDevAIWorktreeWorkflowResult> {
    return createWorktreeWorkflow(
      {
        q8idevaiHome: this.q8idevaiHome,
        worktreesRoot: this.worktreesRoot,
        createQ8iDevAIWorktree: (workflowInput, serviceOptions) =>
          this.createQ8iDevAIWorktree(workflowInput, serviceOptions),
              ...
              : [
                  {
                    ...
                    item: {
                      ...
                      text: "Please upgrade the Q8iDevAI app to view this subagent conversation.",
                    },
                  },
                ],
            error: null,
  ...
  private workspaceSetupMessageForClient(
    ...
    const legacySnapshot = {
      ...
      error:
        "Workspace setup is blocked pending approval of code from a fork pull request. Update Q8iDevAI to review and run setup.",
    };
