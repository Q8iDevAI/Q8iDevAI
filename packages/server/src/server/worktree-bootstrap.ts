...
import type { Q8iDevAIServicePortAllocation } from "@q8idevai/protocol/q8idevai-config-schema";
...
function buildTerminalTimelineItem(input: {
  ...
  if (input.status === "running") {
    return {
      type: "tool_call",
      name: "q8idevai_worktree_terminals",
      callId: input.callId,
      status: "running",
      detail: {
        type: "unknown",
...
export interface SpawnWorkspaceScriptOptions {
  ...
  globalServicePorts?: Q8iDevAIServicePortAllocation;
  logger?: Logger;
...
async function setupServiceScriptRoute(params: {
  ...
  serviceProxy: ServiceProxySubsystem;
  servicePortAllocation: Q8iDevAIServicePortAllocation | undefined;
...
export async function spawnWorkspaceScript(
  ...
  const configResult = readQ8iDevAIConfig(repoRoot);
  if (!configResult.ok) {
    throw q8idevaiConfigParseError(configResult);
  }
  const scriptConfigs = getScriptConfigs(configResult.config);
  const config = scriptConfigs.get(scriptName);
  if (!config) {
    throw new Error(`Script '${scriptName}' is not configured in q8idevai.json`);
  }
