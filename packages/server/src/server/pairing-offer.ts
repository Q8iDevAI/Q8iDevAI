...
export async function generateLocalPairingOffer(args: {
  ...
  const relayEndpoint = args.relayEndpoint ?? "relay.q8idevai.sh:443";
  const relayPublicEndpoint = args.relayPublicEndpoint ?? relayEndpoint;
  const relayUseTls = args.relayUseTls ?? relayEndpoint === "relay.q8idevai.sh:443";
  const relayPublicUseTls = args.relayPublicUseTls ?? relayUseTls;
  const appBaseUrl = args.appBaseUrl ?? "https://app.q8idevai.sh";
  const serverId = getOrCreateServerId(args.q8idevaiHome, { logger: args.logger });
  const daemonKeyPair = await loadOrCreateDaemonKeyPair(args.q8idevaiHome, args.logger);
