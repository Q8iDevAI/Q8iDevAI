...
import { Q8iDevAIServicePortAllocationSchema } from "@q8idevai/protocol/q8idevai-config-schema";
...
const DEFAULT_PERSISTED_CONFIG = PersistedConfigSchema.parse({
  ...
  daemon: {
    ...
    cors: {
      allowedOrigins: ["https://app.q8idevai.sh"],
    },
    relay: {
      enabled: false,
    },
...
function getConfigPath(q8idevaiHome: string): string {
  return path.join(q8idevaiHome, CONFIG_FILENAME);
}
...
export function loadPersistedConfig(q8idevaiHome: string, logger?: LoggerLike): PersistedConfig {
  const log = getLogger(logger);
  const configPath = getConfigPath(q8idevaiHome);
...
export function readPersistedConfig(
  ...
  let raw: string;
  try {
    raw = readFileSync(getConfigPath(q8idevaiHome), "utf8");
  } catch (error) {
...
export function editPersistedConfig(
  ...
  const config = readPersistedConfig(q8idevaiHome, { defaultsIfMissing: true });
  ...
  savePersistedConfig(q8idevaiHome, config);
  return config;
}
