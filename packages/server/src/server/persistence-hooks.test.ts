...
describe("persistence hooks", () => {
  test("buildConfigOverrides preserves the complete private launch config", () => {
    const record = createRecord({
      ...
      config: {
        ...
        toolPolicy: {
          preapproved: [{ kind: "mcp", server: "q8idevai", tool: "report_status" }],
        },
  ...
  test("buildConfigOverrides drops persisted internal q8idevai MCP server", () => {
    const record = createRecord({
      config: {
        modeId: "default",
        model: "gpt-5.4-mini",
  ...
  test("buildConfigOverrides preserves user-provided q8idevai MCP server", () => {
    const record = createRecord({
      config: {
        modeId: "default",
        model: "gpt-5.4-mini",
