...
import {
  ...
  type CreateQ8iDevAIWorktreeDeps,
} from "./q8idevai-worktree-service.js";
import { readQ8iDevAIWorktreeMetadata } from "../utils/worktree-metadata.js";
import { createWorktree, getQ8iDevAIWorktreesRoot } from "../utils/worktree.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createQ8iDevAIWorktree(
    {
      cwd: repoDir,
...
test("seeds an uncommitted exact-project config into the mapped worktree directory", async () => {
  ...
  writeFileSync(path.join(sourceDir, "q8idevai.json"), config);
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "q8idevai.json"), "utf8")).toBe(config);
  expect(existsSync(path.join(result.worktree.worktreePath, "q8idevai.json"))).toBe(false);
});
...
test("does not overwrite a committed exact-project config with source checkout edits", async () => {
  ...
  writeFileSync(path.join(sourceDir, "q8idevai.json"), committedConfig);
  commitAll(repoDir, "add subproject config");
  writeFileSync(
    path.join(sourceDir, "q8idevai.json"),
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "q8idevai.json"), "utf8")).toBe(committedConfig);
  expect(
...
test("removes a new worktree when its ref does not contain the selected project directory", async () => {
  ...
  const worktreePath = path.join(
    await getQ8iDevAIWorktreesRoot(repoDir, q8idevaiHome),
    "missing-subproject",
  );
...
test("maps a nested cwd from an existing Q8iDevAI worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const q8idevaiHome = path.join(tempDir, ".q8idevai");
  ...
  commitAll(repoDir, "add subproject");
  const deps = createDeps();
  const source = await createQ8iDevAIWorktree(
    {
      cwd: repoDir,
  ...
  const created = await createQ8iDevAIWorktree(
    {
      cwd: sourceCwd,
...
test("rejects source checkout planning before creating a worktree", async () => {
  ...
  expect(existsSync(path.join(q8idevaiHome, "worktrees"))).toBe(false);
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createQ8iDevAIWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createQ8iDevAIWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  expect(readQ8iDevAIWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "q8idevai-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
// Worktree restore (Unit 3): recreate a deleted Q8iDevAI-owned worktree from its
...
// repoRoot=null, so deleteQ8iDevAIWorktree's `git worktree remove`/`prune` is
...
interface TestDeps extends CreateQ8iDevAIWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isQ8iDevAIOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
