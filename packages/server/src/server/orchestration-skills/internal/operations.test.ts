...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "q8idevai-skills-"));
...
async function writeCurrentBundle(sourceDir: string): Promise<void> {
  await writeBundleSkill(sourceDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });
  await writeBundleSkill(sourceDir, "q8idevai-loop", { "SKILL.md": "loop-v1" });
}
...
describe("getSkillsStatus", () => {
  ...
  it("reports every bundled skill as available", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeBundleSkill(sandbox.targets.sourceDir, "q8idevai-advisor", {
      "SKILL.md": "advisor-v1",
    });

    const status = await getSkillsStatus(sandbox.targets, only("q8idevai"));

    expect(status.available).toEqual(["q8idevai", "q8idevai-advisor", "q8idevai-loop"]);
  });

  it("reports a skill present in only one target as installed", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });

    const status = await getSkillsStatus(sandbox.targets, ALL_SKILLS);

    // `add` means "missing from at least one target", so it cannot answer
    // "is there a directory here to delete". `installed` answers that.
    expect(status.installed).toEqual(["q8idevai"]);
    expect(status.ops).toEqual([
      { kind: "add", name: "q8idevai" },
      { kind: "add", name: "q8idevai-loop" },
    ]);
  ...
  it("reports legacy skill directories left on disk as installed", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai-chat", { "SKILL.md": "chat-old" });

    expect((await getSkillsStatus(sandbox.targets, ALL_SKILLS)).installed).toEqual(["q8idevai-chat"]);
  });
  ...
  it("returns up-to-date when every bundled skill matches on disk", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai-loop", { "SKILL.md": "loop-v1" });
    ...
    expect(status).toEqual({
      state: "up-to-date",
      ops: [],
      available: ["q8idevai", "q8idevai-loop"],
  ...
  it("ignores user-added files inside current managed skill dirs in every target", async () => {
    ...
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai", {
      "SKILL.md": "q8idevai-v1",
      "my-context.md": "user context",
    });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai", {
      "SKILL.md": "q8idevai-v1",
      "commands/local.md": "user command",
    });
    await writeOnDiskSkill(sandbox.targets.codexDir, "q8idevai", {
      "SKILL.md": "q8idevai-v1",
  ...
  it("returns drift with a single update op when one bundled file diverges", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai-loop", { "SKILL.md": "loop-v1" });

    const status = await getSkillsStatus(sandbox.targets, ALL_SKILLS);

    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "update", name: "q8idevai" }]);
  });

  it("returns drift when a secondary agent target is stale", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai-loop", { "SKILL.md": "loop-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "q8idevai", { "SKILL.md": "q8idevai-v1" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "q8idevai-loop", { "SKILL.md": "loop-v1" });
  ...
  it("returns drift with add ops for the bundled skills missing from disk", async () => {
    ...
    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "add", name: "q8idevai-loop" }]);
  });
  ...
  it("returns drift with a delete op for a legacy skill name still on disk", async () => {
    ...
    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "delete", name: "q8idevai-chat" }]);
  });
  ...
  it("emits add + update + delete ops sorted by name when state is mixed", async () => {
    ...
    expect(status.ops).toEqual([
      ...
      { kind: "delete", name: "q8idevai-chat" },
      { kind: "add", name: "q8idevai-loop" },
    ]);
  });
});
...
describe("custom skill selection", () => {
  ...
  it("installs only the selected skills", async () => {
    const status = await installSkills(sandbox.targets, only("q8idevai", "q8idevai-loop"));

    expect(status).toEqual({
      state: "up-to-date",
      ops: [],
      available: ["q8idevai", "q8idevai-advisor", "q8idevai-loop"],
      installed: ["q8idevai", "q8idevai-loop"],
    });
    expect(await installedIn(sandbox.targets, "q8idevai")).toEqual([true, true, true]);
    expect(await installedIn(sandbox.targets, "q8idevai-loop")).toEqual([true, true, true]);
    expect(await installedIn(sandbox.targets, "q8idevai-advisor")).toEqual([false, false, false]);
  });

  it("reports up-to-date while an unselected bundled skill is absent", async () => {
    await installSkills(sandbox.targets, only("q8idevai"));
  ...
  it("removes a previously installed skill once it leaves the selection", async () => {
    ...
    const status = await installSkills(sandbox.targets, only("q8idevai"));

    expect(status.state).toBe("up-to-date");
    expect(await installedIn(sandbox.targets, "q8idevai")).toEqual([true, true, true]);
    expect(await installedIn(sandbox.targets, "q8idevai-loop")).toEqual([false, false, false]);
    expect(await installedIn(sandbox.targets, "q8idevai-advisor")).toEqual([false, false, false]);
  });
  ...
  it("reports a delete op for a deselected skill before it is applied", async () => {
    ...
    const status = await getSkillsStatus(sandbox.targets, only("q8idevai", "q8idevai-advisor"));

    expect(status.state).toBe("drift");
    expect(status.ops).toEqual([{ kind: "delete", name: "q8idevai-loop" }]);
  });
  ...
  it("treats an empty custom selection as no managed skills installed", async () => {
    ...
    expect(await installedIn(sandbox.targets, "q8idevai")).toEqual([false, false, false]);
    expect(await installedIn(sandbox.targets, "q8idevai-loop")).toEqual([false, false, false]);
  });

  it("ignores selected names that the bundle does not ship", async () => {
    const status = await installSkills(sandbox.targets, only("q8idevai", "not-a-skill"));
  ...
  it("still deletes legacy skill names that are not selectable", async () => {
    await writeOnDiskSkillToAllTargets(sandbox.targets, "q8idevai-orchestrator", {
      "SKILL.md": "orchestrator-old",
    });

    await installSkills(sandbox.targets, only("q8idevai"));

    expect(await installedIn(sandbox.targets, "q8idevai-orchestrator")).toEqual([false, false, false]);
  });
});
...
describe("installSkills / updateSkills", () => {
  ...
  it("installs from a clean machine, populates all three targets, and leaves user dirs alone", async () => {
    ...
    for (const name of ["q8idevai", "q8idevai-loop"]) {
      expect(
        await fs.readFile(path.join(sandbox.targets.agentsDir, name, "SKILL.md"), "utf-8"),
      ).toBe(name === "q8idevai" ? "q8idevai-v1" : "loop-v1");
      expect(
  ...
  it("repairs missing and edited skills without deleting a legacy directory", async () => {
    ...
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai", { "SKILL.md": "stale" });
    await writeOnDiskSkill(sandbox.targets.agentsDir, "q8idevai-chat", { "SKILL.md": "chat-old" });
    await writeOnDiskSkill(sandbox.targets.claudeDir, "q8idevai-chat", { "SKILL.md": "chat-old" });
    await writeOnDiskSkill(sandbox.targets.codexDir, "q8idevai-chat", { "SKILL.md": "chat-old" });

    const status = await updateSkills(sandbox.targets, ALL_SKILLS);

    expect(status).toEqual({
      state: "drift",
      ops: [{ kind: "delete", name: "q8idevai-chat" }],
      available: ["q8idevai", "q8idevai-loop"],
      installed: ["q8idevai", "q8idevai-chat", "q8idevai-loop"],
    });
    expect(
      await fs.readFile(path.join(sandbox.targets.agentsDir, "q8idevai", "SKILL.md"), "utf-8"),
    ).toBe("q8idevai-v1");
    expect(
      await fs.readFile(path.join(sandbox.targets.agentsDir, "q8idevai-loop", "SKILL.md"), "utf-8"),
    ).toBe("loop-v1");
    for (const dir of [
      ...
      expect(await pathExists(path.join(dir, "q8idevai-chat"))).toBe(true);
    }
  });
  ...
  it("defines updated as the state reached after preserving user files", async () => {
    ...
    expect(
      await fs.readFile(
        path.join(sandbox.targets.agentsDir, "q8idevai", "hooks", "guard.sh"),
        "utf-8",
      ),
    ).toBe("user guard");
    expect(
      await fs.readFile(
        path.join(sandbox.targets.claudeDir, "q8idevai", "notes", "local.md"),
        "utf-8",
      ),
    ).toBe("claude notes");
    expect(
      await fs.readFile(
        path.join(sandbox.targets.codexDir, "q8idevai", "prompts", "local.md"),
        "utf-8",
      ),
    ).toBe("codex prompt");
  });
  ...
  it("repairs secondary agent targets even when agents skills are current", async () => {
    ...
    expect(status.state).toBe("up-to-date");
    expect(
      await fs.readFile(path.join(sandbox.targets.claudeDir, "q8idevai-loop", "SKILL.md"), "utf-8"),
    ).toBe("loop-v1");
  });
...
describe("uninstallSkills", () => {
  ...
  it("removes every Q8iDevAI skill from all three targets and preserves user dirs", async () => {
    await writeCurrentBundle(sandbox.targets.sourceDir);
    await installSkills(sandbox.targets, ALL_SKILLS);
    for (const name of ["unslop", "tdd", "devbox"]) {
      await writeOnDiskSkill(sandbox.targets.agentsDir, name, { "SKILL.md": `user-${name}` });
    ...
    expect(status.state).toBe("not-installed");
    for (const name of ["q8idevai", "q8idevai-loop", ...LEGACY_SKILL_NAMES]) {
  ...
  it("cleans up legacy skill names that linger in agents, claude, and codex", async () => {
    ...
    for (const dir of [
      ...
      await writeOnDiskSkill(dir, "q8idevai-chat", { "SKILL.md": "chat-old" });
    }

    const status = await uninstallSkills(sandbox.targets, ALL_SKILLS);

    expect(status.state).toBe("not-installed");
    expect(await installedIn(sandbox.targets, "q8idevai-chat")).toEqual([false, false, false]);
  });
});
