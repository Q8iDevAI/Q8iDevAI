...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "q8idevai-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "q8idevai", {
      "SKILL.md": "new q8idevai content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "q8idevai");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old q8idevai content");
    ...
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "q8idevai", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new q8idevai content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "q8idevai", "SKILL.md"),
      "utf-8",
    );
    expect(claudeContent).toBe("new q8idevai content");
    const codexContent = await fs.readFile(
      path.join(sandbox.codexDir, "q8idevai", "SKILL.md"),
      "utf-8",
    );
    expect(codexContent).toBe("new q8idevai content");
  });

  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "q8idevai-committee", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["q8idevai-committee"],
    });

    expect(
      await fs.readFile(path.join(sandbox.agentsDir, "q8idevai-committee", "SKILL.md"), "utf-8"),
    ).toBe("committee content");
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "q8idevai-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
      await fs.readFile(
        path.join(sandbox.codexDir, "q8idevai-committee", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");

    const claudeSkillDir = path.join(sandbox.claudeDir, "q8idevai-committee");
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "q8idevai", { "SKILL.md": "new" });
  ...
  it("removes stale files previously written to managed skill dirs", async () => {
    ...
    await fs.rm(path.join(sandbox.sourceDir, "q8idevai"), { recursive: true, force: true });
    await writeBundleSkill(sandbox.sourceDir, "q8idevai", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "q8idevai", { "SKILL.md": "content" });
    // "q8idevai-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["q8idevai", "q8idevai-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "q8idevai", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "q8idevai-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "q8idevai", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "q8idevai-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["q8idevai", "q8idevai-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "q8idevai"), "blocking file");
    ...
    expect(errors).toContain("q8idevai");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("q8idevai", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "q8idevai"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "q8idevai"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "q8idevai"))).rejects.toThrow();
  });
