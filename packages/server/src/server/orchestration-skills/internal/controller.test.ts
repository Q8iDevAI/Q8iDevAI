...
const BUNDLED_SKILLS = ["q8idevai", "q8idevai-advisor", "q8idevai-loop"];

async function makeHarness(selectionStore?: SkillSelectionStore): Promise<Harness> {
  const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-skills-controller-"));
    ...
    [targets.agentsDir, targets.claudeDir, targets.codexDir].map(async (dir) => {
      ...
      return [
        ...parentEntries.filter(
          (entry) => entry !== path.basename(dir) && !entry.startsWith(".q8idevai-skills-recovered-"),
        ),
        ...rootEntries.filter((entry) => entry.startsWith(".q8idevai-skills-transaction-")),
      ].sort();
    }),
  );
}
...
async function waitForTransactionDirectory(parent: string): Promise<void> {
  ...
  try {
    for await (const event of events) {
      if (event.filename?.startsWith(".q8idevai-skills-transaction-")) return;
    }
  } finally {
    await events.return?.();
  }
...
describe("skills controller", () => {
  it("imports a legacy selection only while daemon selection is absent", async () => {
    ...
    await expect(
      harness.controller.importLegacySelectionIfUnset({
        mode: "custom",
        skills: ["q8idevai", "q8idevai-loop"],
      }),
    ).resolves.toEqual({
      imported: true,
      selection: { mode: "custom", skills: ["q8idevai", "q8idevai-loop"] },
    });
  ...
  it("reports one snapshot with catalog, selection, status, and pending work", async () => {
    expect(await harness.controller.status()).toEqual({
      state: "not-installed",
      ops: [
        { kind: "add", name: "q8idevai" },
        { kind: "add", name: "q8idevai-advisor" },
        { kind: "add", name: "q8idevai-loop" },
      ],
  ...
  it("installs every bundled skill while the selection is all", async () => {
    ...
    expect(await isInstalled(harness.targets, "q8idevai-advisor")).toBe(true);
  });

  it("does not remove deselected directories during install", async () => {
    await harness.controller.save({ mode: "custom", skills: ["q8idevai"] });
    await writeUserFile(harness.targets, "q8idevai-loop", "notes/mine.md", "keep this");

    await harness.controller.install();

    expect(await readUserFile(harness.targets, "q8idevai-loop", "notes/mine.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("saves a custom selection, converges disk, and returns the refreshed snapshot", async () => {
    ...
    expect(snapshot).toEqual({
      ...
      ops: [],
      available: BUNDLED_SKILLS,
      installed: ["q8idevai", "q8idevai-loop"],
      selection: { mode: "custom", skills: ["q8idevai", "q8idevai-loop"] },
      confirmationRequired: null,
    });
    expect(await isInstalled(harness.targets, "q8idevai")).toBe(true);
    expect(await isInstalled(harness.targets, "q8idevai-loop")).toBe(true);
    expect(await isInstalled(harness.targets, "q8idevai-advisor")).toBe(false);
  });
  ...
  it("removes a skill from disk when it is dropped from the selection", async () => {
    ...
    await harness.controller.save({
      mode: "custom",
      skills: ["q8idevai"],
      confirmedRemovals: ["q8idevai-advisor", "q8idevai-loop"],
    });

    expect(await isInstalled(harness.targets, "q8idevai")).toBe(true);
    expect(await isInstalled(harness.targets, "q8idevai-advisor")).toBe(false);
    expect(await isInstalled(harness.targets, "q8idevai-loop")).toBe(false);
  });
  ...
  it("keeps the saved selection after uninstall so a later install restores it", async () => {
    ...
    expect(afterUninstall).toEqual({
      state: "not-installed",
      ops: [{ kind: "add", name: "q8idevai" }],
      available: BUNDLED_SKILLS,
      installed: [],
      selection: { mode: "custom", skills: ["q8idevai"] },
    });
  ...
  it("treats an empty custom selection as uninstall while keeping the preference", async () => {
    ...
    expect(await isInstalled(harness.targets, "q8idevai")).toBe(false);
  });
  ...
  it("saves no selection at all when the very first save fails", async () => {
    ...
    await expect(harness.controller.save({ mode: "custom", skills: ["q8idevai"] })).rejects.toThrow();
    await rm(harness.targets.agentsDir, { force: true });
  ...
  it("restores deleted directories byte for byte when the selection cannot be committed", async () => {
    ...
    await writeUserFile(readOnly.targets, "q8idevai-loop", "notes/mine.md", "hand written");

    // Deselects q8idevai-loop and adds q8idevai-advisor, then fails to commit.
    await expect(
      readOnly.controller.save({
        mode: "custom",
        skills: ["q8idevai", "q8idevai-advisor"],
        confirmedRemovals: ["q8idevai-loop"],
      }),
    ...
    expect(await installedEverywhere(readOnly.targets)).toEqual([
      ["q8idevai", "q8idevai-loop"],
      ["q8idevai", "q8idevai-loop"],
      ["q8idevai", "q8idevai-loop"],
    ]);
    expect(await readUserFile(readOnly.targets, "q8idevai-loop", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("preserves files added by another writer before rollback", async () => {
    const selection: SkillSelection = { mode: "custom", skills: ["q8idevai"] };
    const gated = createGatedUnwritableSelectionStore(selection);
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeFile(path.join(readOnly.targets.sourceDir, "q8idevai", "SKILL.md"), "q8idevai-v2");

    const save = readOnly.controller.save(selection);
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "q8idevai", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    expect(await readUserFile(readOnly.targets, "q8idevai", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("does not automatically delete files preserved from a rolled-back add", async () => {
    const previous: SkillSelection = { mode: "custom", skills: ["q8idevai"] };
    ...
    await gated.persistenceStarted;
    await writeUserFile(readOnly.targets, "q8idevai-loop", "notes/concurrent.md", "keep this");
    gated.failPersistence();
    await expect(save).rejects.toThrow("selection store is read-only");

    await readOnly.controller.autoUpdate();
    expect(await readUserFile(readOnly.targets, "q8idevai-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it("merges a deleted directory backup into files another writer recreated", async () => {
    ...
    const readOnly = await makeHarness(gated.store);
    await readOnly.controller.install();
    await writeUserFile(readOnly.targets, "q8idevai-loop", "notes/before.md", "restore this");
    ...
    expect(await readUserFile(readOnly.targets, "q8idevai-loop", "notes/before.md")).toEqual([
      "restore this",
      "restore this",
      "restore this",
    ]);
  ...
  it("atomically stages a deletion before another writer can recreate its path", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["q8idevai"] };
    await harness.controller.save(previous);

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "q8idevai-loop" },
    ]);

    expect(await isInstalled(harness.targets, "q8idevai-loop")).toBe(false);
    await writeUserFile(harness.targets, "q8idevai-loop", "notes/concurrent.md", "keep this");
    await transaction.rollback();
    expect(await readUserFile(harness.targets, "q8idevai-loop", "notes/concurrent.md")).toEqual([
      "keep this",
      "keep this",
      "keep this",
    ]);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      ].map((root) => path.join(root, "q8idevai-loop"));
  ...
  it("finishes rollback when an external deletion leaves no live or staged path", async () => {
    ...
    const codexStage = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".q8idevai-skills-transaction-"),
    );
    expect(codexStage).toBeDefined();
    await rm(path.join(harness.targets.codexDir, codexStage!, "q8idevai-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(await readUserFile(harness.targets, "q8idevai-loop", "SKILL.md")).toEqual([
      "q8idevai-loop-v1",
      "q8idevai-loop-v1",
      null,
    ]);
  ...
  it("quarantines a staged directory when an external file takes its live path", async () => {
    ...
    const live = path.join(harness.targets.codexDir, "q8idevai-loop");
    ...
    const recovered = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".q8idevai-skills-recovered-"),
    );
  ...
  it("quarantines staged files that collide with a recreated directory", async () => {
    ...
    const next: SkillSelection = { mode: "custom", skills: ["q8idevai"] };
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "q8idevai-loop", "notes/mine.md", "staged notes");
    ...
    const recovered = (await readdir(harness.targets.codexDir)).find((entry) =>
      entry.startsWith(".q8idevai-skills-recovered-q8idevai-loop-"),
    );
    expect(recovered).toBeDefined();
    expect(
      await readFile(path.join(harness.targets.codexDir, recovered!, "SKILL.md"), "utf8"),
    ).toBe("q8idevai-loop-v1");
    expect(
      await readFile(path.join(harness.targets.codexDir, recovered!, "notes", "mine.md"), "utf8"),
    ).toBe("staged notes");
    expect(await backupArtifacts(harness.targets)).toEqual([[], [], []]);
  ...
  it("preserves incompatible live paths while rolling back adds and updates", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "q8idevai", "notes/mine.md", "keep this");

    const transaction = await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "update", name: "q8idevai" },
      { kind: "add", name: "q8idevai-advisor" },
    ]);
    const replacedUpdate = path.join(harness.targets.agentsDir, "q8idevai");
    await rm(replacedUpdate, { recursive: true, force: true });
    await writeFile(replacedUpdate, "external update replacement");
    const replacedAdd = path.join(harness.targets.codexDir, "q8idevai-advisor");
    ...
    const recovered = (await readdir(recoveryParent)).find((entry) =>
      entry.startsWith(".q8idevai-skills-recovered-q8idevai-"),
    );
    expect(recovered).toBeDefined();
    expect(await readFile(path.join(recoveryParent, recovered!, "notes", "mine.md"), "utf8")).toBe(
      "keep this",
    );
    expect(await readUserFile(harness.targets, "q8idevai", "notes/mine.md")).toEqual([
      null,
      "keep this",
      "keep this",
    ]);
  ...
  it("recovers after an update backup was quarantined before transaction cleanup", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "q8idevai", "notes/mine.md", "captured notes");
    ...
    const entry = manifest.entries.find(
      (candidate) => candidate.livePath === path.join(harness.targets.agentsDir, "q8idevai"),
    );
    expect(entry?.backupPath).toBeTruthy();
    const backup = path.join(transactionDir, entry!.backupPath!);
    const recovered = path.join(
      transactionParent,
      `.q8idevai-skills-recovered-q8idevai-${transactionName!.replace(".q8idevai-skills-transaction-", "")}`,
    );
  ...
  it("preserves a directory that replaces a captured file before recovery", async () => {
    ...
    await harness.controller.save(previous);
    const live = path.join(harness.targets.agentsDir, "q8idevai");
  ...
  it.skipIf(process.platform !== "linux")(
    ...
    async () => {
      const crossFilesystemRoot = await mkdtemp("/dev/shm/q8idevai-skills-controller-");
      try {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const next: SkillSelection = { mode: "custom", skills: ["q8idevai"] };
      await harness.controller.save(previous);
      const shared = path.join(harness.root, "home", "shared", "q8idevai-loop");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "shared target");
      const live = path.join(harness.targets.claudeDir, "q8idevai-loop");
      await rm(live, { recursive: true, force: true });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      const shared = path.join(readOnly.root, "home", "shared", "q8idevai");
      await mkdir(shared, { recursive: true });
      await writeFile(path.join(shared, "SKILL.md"), "q8idevai-v1");
      const live = path.join(readOnly.targets.claudeDir, "q8idevai");
      await rm(live, { recursive: true, force: true });
      ...
      expect((await lstat(live)).isSymbolicLink()).toBe(true);
      expect(await readFile(path.join(shared, "SKILL.md"), "utf8")).toBe("q8idevai-v1");
      await rm(readOnly.root, { recursive: true, force: true });
    },
  );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      for (const root of [
        ...
        const notes = path.join(root, "q8idevai-loop", "notes");
      ...
      for (const root of [
        ...
        const restored = path.join(root, "q8idevai-loop", "notes", "latest.md");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const readOnly = await makeHarness(gated.store);
      await readOnly.controller.install();
      await writeUserFile(readOnly.targets, "q8idevai-loop", "hooks/run.sh", "#!/bin/sh\n");
      for (const root of [
        readOnly.targets.agentsDir,
        readOnly.targets.claudeDir,
        readOnly.targets.codexDir,
      ]) {
        await chmod(path.join(root, "q8idevai-loop", "hooks", "run.sh"), 0o751);
      }
      ...
      for (const root of [
        ...
        const restored = await lstat(path.join(root, "q8idevai-loop", "hooks", "run.sh"));
  ...
  it("does not delete an unrelated file that resembles transaction staging", async () => {
    const unrelated = path.join(
      ...
      ".q8idevai-skills-transaction-my-notes",
    );
  ...
  it("recovers an interrupted save before the next controller operation", async () => {
    ...
    await harness.controller.save(previous);
    await writeUserFile(harness.targets, "q8idevai-loop", "notes/mine.md", "hand written");
    await beginSkillsTransaction(harness.targets, previous, next, [
      { kind: "delete", name: "q8idevai-loop" },
    ]);
    for (const root of [
      ...
      await rm(path.join(root, "q8idevai-loop"), { recursive: true, force: true });
    }
  ...
  it("asks for confirmation naming the directories a save would delete", async () => {
    await harness.controller.save({ mode: "custom", skills: ["q8idevai", "q8idevai-loop"] });
    // Something puts a managed directory back after the UI took its snapshot.
    await writeUserFile(harness.targets, "q8idevai-advisor", "SKILL.md", "external");
    ...
    expect(result.confirmationRequired).toEqual({ removals: ["q8idevai-advisor"] });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["q8idevai", "q8idevai-advisor", "q8idevai-loop"],
      ["q8idevai", "q8idevai-advisor", "q8idevai-loop"],
      ["q8idevai", "q8idevai-advisor", "q8idevai-loop"],
    ]);
    expect(result.selection).toEqual({ mode: "custom", skills: ["q8idevai", "q8idevai-loop"] });
  });
  ...
  it("applies the save once the removals are confirmed", async () => {
    ...
    expect(result.confirmationRequired).toBeNull();
    expect(result.selection).toEqual({ mode: "custom", skills: ["q8idevai"] });
    expect(await installedEverywhere(harness.targets)).toEqual([["q8idevai"], ["q8idevai"], ["q8idevai"]]);
  });

  it("asks again when another directory appears before the retry", async () => {
    await harness.controller.install();
    await writeUserFile(harness.targets, "q8idevai-chat", "SKILL.md", "retired but present");
    ...
    expect(result.confirmationRequired).toEqual({
      removals: ["q8idevai-advisor", "q8idevai-chat", "q8idevai-loop"],
    });
    expect(await installedEverywhere(harness.targets)).toEqual([
      ["q8idevai", "q8idevai-advisor", "q8idevai-chat", "q8idevai-loop"],
      ["q8idevai", "q8idevai-advisor", "q8idevai-chat", "q8idevai-loop"],
      ["q8idevai", "q8idevai-advisor", "q8idevai-chat", "q8idevai-loop"],
    ]);
  });
  ...
  it("does not commit when a new removal appears while the frozen plan is applying", async () => {
    ...
    await writeFile(path.join(harness.targets.sourceDir, "q8idevai", "SKILL.md"), "q8idevai-v2");
    ...
    await writeUserFile(harness.targets, "q8idevai-chat", "notes/mine.md", "hand written");

    const result = await save;

    expect(result.confirmationRequired).toEqual({ removals: ["q8idevai-chat"] });
    expect(result.selection).toEqual(selection);
    expect(await readUserFile(harness.targets, "q8idevai-chat", "notes/mine.md")).toEqual([
      "hand written",
      "hand written",
      "hand written",
    ]);
  ...
  it("saves without asking when nothing would be deleted", async () => {
    const result = await harness.controller.save({ mode: "custom", skills: ["q8idevai"] });
  ...
  it("preserves a regular file at a skill path when save convergence fails", async () => {
    ...
    const collision = path.join(harness.targets.agentsDir, "q8idevai");
  ...
  it("serializes startup convergence with an interactive save", async () => {
    ...
    await rm(path.join(harness.targets.claudeDir, "q8idevai-loop"), {
      recursive: true,
      force: true,
    });
    ...
    expect(saved.selection).toEqual({ mode: "custom", skills: ["q8idevai"] });
    expect(await installedEverywhere(harness.targets)).toEqual([["q8idevai"], ["q8idevai"], ["q8idevai"]]);
    expect(await harness.controller.status()).toEqual({
      state: "up-to-date",
      ops: [],
  ...
  it("updates a drifted install without touching the saved selection", async () => {
    await harness.controller.save({ mode: "custom", skills: ["q8idevai"] });
    await writeFile(path.join(harness.targets.agentsDir, "q8idevai", "SKILL.md"), "stale");
