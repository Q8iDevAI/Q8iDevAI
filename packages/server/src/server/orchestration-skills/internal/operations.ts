...
export const LEGACY_SKILL_NAMES = [
  "q8idevai-chat",
  "q8idevai-epic",
  "q8idevai-orchestrate",
  "q8idevai-orchestrator",
] as const;
...
/** Every name Q8iDevAI owns on disk: what it ships now plus what it used to ship. */
...
function hasInstalledQ8iDevAISkill(disks: readonly TargetSkills[]): boolean {
...
export async function getSkillsStatus(
  ...
  if (!hasInstalledQ8iDevAISkill(disks)) return { state: "not-installed", ops, available, installed };
