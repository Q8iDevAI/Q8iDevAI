...
import { SessionInboundMessageSchema, WSOutboundMessageSchema } from "@q8idevai/protocol/messages";
...
import type { CreationSnapshot, SessionOutboundMessage } from "@q8idevai/protocol/messages";
...
test("legacy keyed creation preserves checkout error codes", async () => {
  ...
  const daemon = await createTestQ8iDevAIDaemon({ agentClients: createTestAgentClients() });
  const peer = await connectCreationPeer(daemon.port);
  try {
