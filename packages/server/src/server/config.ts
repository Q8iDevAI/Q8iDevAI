...
import { resolveQ8iDevAINodeEnv } from "./q8idevai-env.js";
import { z } from "zod";
import { expandTilde } from "../utils/path.js";

import type { Q8iDevAIDaemonConfig } from "./bootstrap.js";
import {
  loadPersistedConfig,
  LogFormatSchema,
  LogLevelSchema,
...
import { AgentProviderSchema } from "@q8idevai/protocol/provider-manifest";
...
const DEFAULT_RELAY_ENDPOINT = "relay.q8idevai.sh:443";
const DEFAULT_APP_BASE_URL = "https://app.q8idevai.sh";
...
): NonNullable<Q8iDevAIDaemonConfig["git"]> {
...
function resolveLogConfigFromEnv(
  ...
  const level = parseLogLevelEnv(env.Q8IDEVAI_LOG_LEVEL ?? env.Q8IDEVAI_LOG);
  const format = parseLogFormatEnv(env.Q8IDEVAI_LOG_FORMAT);
...
function resolveConsoleLogConfigFromEnv(
  ...
  const level = parseLogLevelEnv(env.Q8IDEVAI_LOG_CONSOLE_LEVEL);
  const format = parseLogFormatEnv(env.Q8IDEVAI_LOG_CONSOLE_FORMAT);
...
function resolveFileLogConfigFromEnv(
  ...
  const level = parseLogLevelEnv(env.Q8IDEVAI_LOG_FILE_LEVEL);
  const filePath = nonEmptyEnv(env.Q8IDEVAI_LOG_FILE_PATH);
  const maxSize = nonEmptyEnv(env.Q8IDEVAI_LOG_FILE_ROTATE_SIZE);
  const maxFiles = parsePositiveIntegerEnv(env.Q8IDEVAI_LOG_FILE_ROTATE_COUNT);
...
function resolveRelayConfig(input: ResolveRelayInput): ResolvedRelay {
  const environmentEnabled = parseBooleanEnv(input.env.Q8IDEVAI_RELAY_ENABLED);
...
function resolveServiceProxyPublicBaseUrl(value: string | null): string | null {
  ...
  try {
    ...
    throw new Error(`Invalid Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL: ${value}`);
  }
}
...
function resolveServiceProxyConfig(
  ...
  const enabledShim =
    parseBooleanEnv(env.Q8IDEVAI_SERVICE_PROXY_ENABLED) ?? persisted.daemon?.serviceProxy?.enabled;
  ...
  const publicBaseUrl = optionalLayersEnabled
    ? resolveServiceProxyPublicBaseUrl(
        env.Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL ??
          persisted.daemon?.serviceProxy?.publicBaseUrl ??
          null,
      )
    : null;
  const standaloneListen = optionalLayersEnabled
    ? (env.Q8IDEVAI_SERVICE_PROXY_LISTEN ?? persisted.daemon?.serviceProxy?.listen ?? null)
    : null;
...
function resolveWebUiConfig(
  ...
  const enabled =
    cli?.webUiEnabled ??
    parseBooleanEnv(env.Q8IDEVAI_WEB_UI_ENABLED) ??
    persisted.features?.webUi?.enabled ??
    false;
  const rawDistDir = env.Q8IDEVAI_WEB_UI_DIST_DIR ?? persisted.features?.webUi?.distDir;
  const trimmedDistDir = rawDistDir?.trim();
  const distDir = trimmedDistDir
    ? path.resolve(path.isAbsolute(trimmedDistDir) ? trimmedDistDir : q8idevaiHome, trimmedDistDir)
...
function resolveVoiceLlmConfig(
  ...
  const envVoiceLlmProvider = parseOptionalVoiceLlmProvider(env.Q8IDEVAI_VOICE_LLM_PROVIDER);
...
function resolveCorsAllowedOrigins(
  ...
  const envCorsOrigins = env.Q8IDEVAI_CORS_ORIGINS
    ? env.Q8IDEVAI_CORS_ORIGINS.split(",").map((s) => s.trim())
    : [];
...
function resolveTrustedProxiesConfig(
  ...
  return (
    parseTrustedProxiesEnv(env.Q8IDEVAI_TRUSTED_PROXIES) ??
    persisted.daemon?.trustedProxies ??
    DEFAULT_TRUSTED_PROXIES
  );
}

// Q8IDEVAI_LISTEN can be:
// - host:port (TCP)
...
function resolveAuthConfig(
  env: NodeJS.ProcessEnv,
  persisted: ReturnType<typeof loadPersistedConfig>,
): Q8iDevAIDaemonConfig["auth"] {
  const envPassword = env.Q8IDEVAI_PASSWORD?.trim();
  if (envPassword) {
...
function resolveWorktreesRoot(
  ...
  return path.isAbsolute(expandedRoot)
    ...
    : path.resolve(q8idevaiHome, expandedRoot);
}
...
function resolveStaticLoadConfigSettings(
  ...
  return {
    ...
    hostnames: mergeHostnames([
      ...
      parseHostnamesEnv(env.Q8IDEVAI_HOSTNAMES ?? env.Q8IDEVAI_ALLOWED_HOSTS),
      cli?.hostnames,
    ]),
    trustedProxies: resolveTrustedProxiesConfig(env, persisted),
    appBaseUrl: env.Q8IDEVAI_APP_BASE_URL ?? persisted.app?.baseUrl ?? DEFAULT_APP_BASE_URL,
  };
}
...
export function resolveConfigFromPersisted(
  ...
  const webUi = resolveWebUiConfig(q8idevaiHome, env, cli, persisted);
  ...
  return {
    listen,
    q8idevaiHome,
    desktopManaged: env.Q8IDEVAI_DESKTOP_MANAGED === "1",
    worktreesRoot: resolveWorktreesRoot(q8idevaiHome, persisted),
    ...
    isDev: resolveQ8iDevAINodeEnv(env) === "development",
    agentStoragePath: path.join(q8idevaiHome, "agents"),
    staticDir: "public",
    agentClients: {},
...
export function loadConfig(
  ...
  return resolveConfigFromPersisted(q8idevaiHome, persisted, options);
}
...
function resolveCoreDaemonOverridePaths(
  ...
  const paths: string[] = [];
  if (cli?.listen !== undefined || env.Q8IDEVAI_LISTEN !== undefined) {
  ...
  if (parseTrustedProxiesEnv(env.Q8IDEVAI_TRUSTED_PROXIES) !== undefined) {
    paths.push("daemon.trustedProxies");
  }
  if (parsePositiveGitOverride(env.Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND)) {
    paths.push("daemon.git.maxProcessesPerSecond");
  }
  if (
    parsePositiveGitOverride(env.Q8IDEVAI_GIT_MAX_PROCESS_CONCURRENCY ?? env.Q8IDEVAI_GIT_CONCURRENCY)
  ) {
    paths.push("daemon.git.maxProcessConcurrency");
  }
  if (env.Q8IDEVAI_APP_BASE_URL !== undefined) paths.push("app.baseUrl");
  if (env.Q8IDEVAI_PASSWORD?.trim()) paths.push("daemon.auth.password");
  return paths;
}
...
function resolveRelayOverridePaths(
  ...
  const paths: string[] = [];
  if (cli?.relayEnabled !== undefined || parseBooleanEnv(env.Q8IDEVAI_RELAY_ENABLED) !== undefined) {
    paths.push("daemon.relay.enabled");
  }
  if (env.Q8IDEVAI_RELAY_ENDPOINT !== undefined) paths.push("daemon.relay.endpoint");
  if (env.Q8IDEVAI_RELAY_PUBLIC_ENDPOINT !== undefined) {
    paths.push("daemon.relay.publicEndpoint");
  }
  if (cli?.relayUseTls !== undefined || env.Q8IDEVAI_RELAY_USE_TLS !== undefined) {
    paths.push("daemon.relay.useTls");
  }
  if (env.Q8IDEVAI_RELAY_PUBLIC_USE_TLS !== undefined) {
...
function resolveServiceAndWebUiOverridePaths(
  ...
  const paths: string[] = [];
  const serviceProxyEnabled = parseBooleanEnv(env.Q8IDEVAI_SERVICE_PROXY_ENABLED);
  if (serviceProxyEnabled !== undefined) paths.push("daemon.serviceProxy.enabled");
  if (env.Q8IDEVAI_SERVICE_PROXY_LISTEN !== undefined || serviceProxyEnabled === false) {
    paths.push("daemon.serviceProxy.listen");
  }
  if (env.Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL !== undefined || serviceProxyEnabled === false) {
    paths.push("daemon.serviceProxy.publicBaseUrl");
  }

  if (cli?.webUiEnabled !== undefined || parseBooleanEnv(env.Q8IDEVAI_WEB_UI_ENABLED) !== undefined) {
    paths.push("features.webUi.enabled");
  }
  if (env.Q8IDEVAI_WEB_UI_DIST_DIR !== undefined) paths.push("features.webUi.distDir");
  return paths;
}

function resolveLogOverrideControlledPaths(env: NodeJS.ProcessEnv): string[] {
  const paths: string[] = [];
  if (parseLogLevelEnv(env.Q8IDEVAI_LOG_LEVEL ?? env.Q8IDEVAI_LOG) !== undefined) {
    paths.push("log.level");
  }
  if (parseLogFormatEnv(env.Q8IDEVAI_LOG_FORMAT) !== undefined) paths.push("log.format");
  if (parseLogLevelEnv(env.Q8IDEVAI_LOG_CONSOLE_LEVEL) !== undefined) {
    paths.push("log.console.level");
  }
  if (parseLogFormatEnv(env.Q8IDEVAI_LOG_CONSOLE_FORMAT) !== undefined) {
    paths.push("log.console.format");
  }
  if (parseLogLevelEnv(env.Q8IDEVAI_LOG_FILE_LEVEL) !== undefined) paths.push("log.file.level");
  if (nonEmptyEnv(env.Q8IDEVAI_LOG_FILE_PATH) !== undefined) paths.push("log.file.path");
  if (nonEmptyEnv(env.Q8IDEVAI_LOG_FILE_ROTATE_SIZE) !== undefined) {
    paths.push("log.file.rotate.maxSize");
  }
  if (parsePositiveIntegerEnv(env.Q8IDEVAI_LOG_FILE_ROTATE_COUNT) !== undefined) {
...
function resolveSpeechOverrideControlledPaths(
  ...
  add("Q8IDEVAI_DICTATION_ENABLED", "features.dictation.enabled");
  add("Q8IDEVAI_DICTATION_STT_PROVIDER", "features.dictation.stt.provider");
  if (
    env.Q8IDEVAI_DICTATION_LOCAL_STT_MODEL !== undefined &&
    isEnabledSpeechProvider(providers.dictationStt, "local")
  ) {
    paths.push("features.dictation.stt.model");
  }
  add("Q8IDEVAI_DICTATION_LANGUAGE", "features.dictation.stt.language");
  add("Q8IDEVAI_VOICE_MODE_ENABLED", "features.voiceMode.enabled");
  add("Q8IDEVAI_VOICE_LLM_PROVIDER", "features.voiceMode.llm.provider");
  add("Q8IDEVAI_VOICE_STT_PROVIDER", "features.voiceMode.stt.provider");
  if (
    env.Q8IDEVAI_VOICE_LOCAL_STT_MODEL !== undefined &&
    isEnabledSpeechProvider(providers.voiceStt, "local")
  ) {
    paths.push("features.voiceMode.stt.model");
  }
  add("Q8IDEVAI_VOICE_LANGUAGE", "features.voiceMode.stt.language");
  add("Q8IDEVAI_VOICE_TURN_DETECTION_PROVIDER", "features.voiceMode.turnDetection.provider");
  add("Q8IDEVAI_VOICE_TTS_PROVIDER", "features.voiceMode.tts.provider");
  if (
    env.Q8IDEVAI_VOICE_LOCAL_TTS_MODEL !== undefined &&
    isEnabledSpeechProvider(providers.voiceTts, "local")
  ) {
    paths.push("features.voiceMode.tts.model");
  }
  add("Q8IDEVAI_VOICE_LOCAL_TTS_SPEAKER_ID", "features.voiceMode.tts.speakerId");
  add("Q8IDEVAI_VOICE_LOCAL_TTS_SPEED", "features.voiceMode.tts.speed");
  add("Q8IDEVAI_LOCAL_MODELS_DIR", "providers.local.modelsDir");
  ...
  if (env.Q8IDEVAI_DICTATION_LANGUAGE !== undefined && env.Q8IDEVAI_VOICE_LANGUAGE === undefined) {
