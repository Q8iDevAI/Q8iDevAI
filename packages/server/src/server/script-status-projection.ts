...
import { getScriptConfigs, isServiceScript, readQ8iDevAIConfig } from "../utils/worktree.js";
...
export function readQ8iDevAIConfigForProjection(
  workspaceDirectory: string,
  logger: Logger,
): Q8iDevAIConfig | null {
  const result = readQ8iDevAIConfig(workspaceDirectory);
  if (result.ok) {
    return result.config;
  }
  logger.warn(
    { configPath: result.configPath, workspaceDirectory, err: result.error },
    "Failed to parse q8idevai.json; treating workspace as having no scripts",
  );
  return null;
}
...
export function buildWorkspaceScriptPayloads(
  ...
  const scriptConfigs = getScriptConfigs(options.q8idevaiConfig);
  ...
  return (workspaceId, scripts) => {
    void (async () => {
      ...
      const projected = buildWorkspaceScriptPayloads({
        workspaceId,
        workspaceDirectory,
        q8idevaiConfig: readQ8iDevAIConfigForProjection(workspaceDirectory, logger),
        serviceProxy,
        runtimeStore,
