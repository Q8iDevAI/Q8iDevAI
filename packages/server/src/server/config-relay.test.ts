...
async function createQ8iDevAIHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-config-relay-"));
  roots.push(root);
...
describe("daemon relay config", () => {
  ...
  test("preserves implicit relay-on for a legacy config without enabled", async () => {
    const home = await createQ8iDevAIHome({ version: 1, daemon: { relay: {} } });
  ...
  test("marks environment relay overrides immutable", async () => {
    ...
    const config = loadConfig(home, { env: { Q8IDEVAI_RELAY_ENABLED: "true" } });
  ...
  test.each(["", "treu"])(
    ...
    async (value) => {
      ...
      const config = loadConfig(home, { env: { Q8IDEVAI_RELAY_ENABLED: value } });
      expect(config.relayEnabled).toBe(false);
      expect(config.relayEnabledMutable).toBe(true);
    },
  );
  ...
  test("loads relay TLS from env, persisted config, and hosted relay fallback", async () => {
    const persistedHome = await createQ8iDevAIHome({
      version: 1,
      daemon: {
        relay: {
    ...
    const envHome = await createQ8iDevAIHome({
      version: 1,
      daemon: {
        relay: {
          endpoint: "relay.example.com:443",
    ...
    expect(loadConfig(envHome, { env: { Q8IDEVAI_RELAY_USE_TLS: "true" } }).relayUseTls).toBe(true);

    const hostedHome = await createQ8iDevAIHome({
      version: 1,
      daemon: { relay: {} },
    });
  ...
  test("Q8IDEVAI_RELAY_PUBLIC_USE_TLS overrides relayUseTls for public side", async () => {
    const home = await createQ8iDevAIHome({ version: 1, daemon: { relay: {} } });
    const config = loadConfig(home, {
      env: { Q8IDEVAI_RELAY_USE_TLS: "false", Q8IDEVAI_RELAY_PUBLIC_USE_TLS: "true" },
    });
    expect(config.relayUseTls).toBe(false);
    expect(config.relayPublicUseTls).toBe(true);
  });

  test("relayPublicUseTls falls back to relayUseTls when only Q8IDEVAI_RELAY_USE_TLS is set", async () => {
    const home = await createQ8iDevAIHome({ version: 1, daemon: { relay: {} } });
    const config = loadConfig(home, { env: { Q8IDEVAI_RELAY_USE_TLS: "false" } });
    expect(config.relayUseTls).toBe(false);
...
describe("daemon service proxy config", () => {
  ...
  test("loads public base URL from env before persisted config", async () => {
    ...
    const config = loadConfig(home, {
      env: { Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL: "https://env.example.com/" },
    });
  ...
  test("rejects invalid Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL values", async () => {
    const home = await createQ8iDevAIHome({ version: 1 });

    expect(() =>
      loadConfig(home, {
        env: { Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL: "not-a-url" },
      }),
    ).toThrow("Invalid Q8IDEVAI_SERVICE_PROXY_PUBLIC_BASE_URL: not-a-url");
  });
});
...
describe("daemon trusted proxy config", () => {
  ...
  test("Q8IDEVAI_TRUSTED_PROXIES overrides persisted config", async () => {
    const home = await createQ8iDevAIHome({
      version: 1,
      daemon: {
    ...
    const config = loadConfig(home, {
      env: { Q8IDEVAI_TRUSTED_PROXIES: "loopback,172.16.0.0/12" },
    });

    expect(config.trustedProxies).toEqual(["loopback", "172.16.0.0/12"]);
  });

  test("Q8IDEVAI_TRUSTED_PROXIES supports explicit trust-all and trust-none modes", async () => {
    const trustAllHome = await createQ8iDevAIHome({ version: 1 });
    expect(
      loadConfig(trustAllHome, { env: { Q8IDEVAI_TRUSTED_PROXIES: "true" } }).trustedProxies,
    ).toBe(true);

    const trustNoneHome = await createQ8iDevAIHome({ version: 1 });
    expect(
      loadConfig(trustNoneHome, { env: { Q8IDEVAI_TRUSTED_PROXIES: "false" } }).trustedProxies,
    ).toEqual([]);
  });
});
...
describe("daemon worktree root config", () => {
  ...
  test("resolves relative worktrees.root against Q8IDEVAI_HOME", async () => {
    const home = await createQ8iDevAIHome({
      version: 1,
  ...
  test("keeps absolute worktrees.root absolute", async () => {
    const home = await createQ8iDevAIHome({
      version: 1,
      worktrees: { root: path.join(os.tmpdir(), "q8idevai-custom-worktrees") },
    });

    expect(loadConfig(home, { env: {} }).worktreesRoot).toBe(
      path.join(os.tmpdir(), "q8idevai-custom-worktrees"),
    );
  });
});
