...
import { PARENT_AGENT_ID_LABEL } from "@q8idevai/protocol/agent-labels";
import { CLIENT_CAPS } from "@q8idevai/protocol/client-capabilities";
import type { WorkspaceDescriptorPayload } from "@q8idevai/protocol/messages";
import {
  decodeFileTransferFrame,
  encodeFileTransferFrame,
  FileTransferOpcode,
  type FileTransferFrame,
} from "@q8idevai/protocol/binary-frames/index";
...
interface SessionHandlerInternals {
  ...
  createQ8iDevAIWorktree(params: unknown): Promise<unknown>;
...
const q8idevaiWorktreeServiceMocks = vi.hoisted(() => ({
  createQ8iDevAIWorktree: vi.fn(),
}));
...
vi.mock("./q8idevai-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./q8idevai-worktree-service.js")>();
  return {
    ...actual,
    createQ8iDevAIWorktree: q8idevaiWorktreeServiceMocks.createQ8iDevAIWorktree,
  };
});
...
function createSessionForTest(options: SessionForTestOptions = {}): Session {
  ...
  const sessionOptions: SessionOptions = {
    ...
    q8idevaiHome: options.q8idevaiHome ?? "/tmp/q8idevai-home",
...
describe("project command-center RPCs", () => {
    ...
    const searchRepositories = vi.fn().mockResolvedValue([
      {
        id: "R_q8idevai",
        name: "q8idevai",
        nameWithOwner: "getq8idevai/q8idevai",
        description: "Development environment in your pocket",
        visibility: "public",
        updatedAt: "2026-07-15T10:00:00Z",
        cloneUrl: "git@github.com:getq8idevai/q8idevai.git",
      },
    ]);
  ...
  test("creates a directory and returns its normalized Project descriptor", async () => {
    const parentDirectory = realpathSync(mkdtempSync(join(tmpdir(), "q8idevai-project-session-")));
    ...
    const session = createSessionForTest({
      ...
      workspaceGitService: {
        getCheckout: vi.fn(async (cwd: string) => ({
          ...
          remoteUrl: null,
          worktreeRoot: null,
          isQ8iDevAIOwnedWorktree: false as const,
          mainRepoRoot: null,
        })),
      },
    });
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file upload round-trips bytes through binary frames", async () => {
    const q8idevaiHome = makeDir("file-access-upload-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, q8idevaiHome });
...
describe("project config RPC authorization", () => {
  ...
  test("read_project_config_request accepts the same root with a trailing slash", async () => {
    ...
    writeFileSync(join(repoRoot, "q8idevai.json"), JSON.stringify({ worktree: { setup: "npm ci" } }));
    const messages: unknown[] = [];
    const session = createSessionForTest({
      messages,
  ...
  test("write_project_config_request emits stale and write-failed inline domain failures", async () => {
    ...
    writeFileSync(join(staleRoot, "q8idevai.json"), JSON.stringify({ worktree: { setup: "old" } }));
  ...
  test("daemon.get_pairing_offer.request returns an empty offer when relay is disabled", async () => {
    ...
    const session = createSessionForTest({
      ...
      daemonRuntimeConfig: {
        ...
        getRelayConfig: () => ({
          enabled: false,
          endpoint: "relay.q8idevai.sh:443",
          publicEndpoint: "relay.q8idevai.sh:443",
          useTls: true,
          publicUseTls: true,
        }),
      },
...
function createWorkspaceGitSnapshot(
  ...
  return {
    ...
    git: {
      ...
      remoteUrl: "https://github.com/Q8iDevAI/Q8iDevAI.git",
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "q8idevai.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "q8idevai.json"), config);
  ...
  test.each([
    ["q8idevai.json missing", undefined],
    ["q8idevai.json exists but invalid JSON", "{ nope"],
    ["q8idevai.json valid but missing metadataGeneration", {}],
...
`;
  ...
  test("keeps the PR fallback when structured generation fails", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/9",
      number: 9,
    });
    const session = createSessionForTest({ workspaceGitService, messages });
    ...
    expect(checkoutGitMocks.createPullRequest).toHaveBeenCalledWith(
      "/tmp/request-worktree",
      {
        ...
        body: "Automated PR generated by Q8iDevAI.",
        base: "main",
      },
      expect.anything(),
    );
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "q8idevai-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "q8idevai-auto-stash: feature",
        branch: "feature",
        isQ8iDevAI: true,
      },
    ];
...
describe("session stash mutation handling", () => {
  test("forces a workspace git snapshot refresh after pushing a stash", async () => {
    ...
    expect(gitCommandMocks.runGitCommand).toHaveBeenCalledWith(
      ["stash", "push", "--include-untracked", "-m", "q8idevai-auto-stash: feature"],
...
describe("session q8idevai worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    q8idevaiWorktreeServiceMocks.createQ8iDevAIWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/q8idevai/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/q8idevai/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createQ8iDevAIWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/q8idevai/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
...
describe("session workspace script handling", () => {
  test("passes the project slug and cached branch into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getProjectSlug: vi.fn().mockResolvedValue("q8idevai"),
    };
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "getq8idevai", repoName: "q8idevai" },
    { prNumber: -1, repoOwner: "getq8idevai", repoName: "q8idevai" },
    { prNumber: 42, repoOwner: "get q8idevai", repoName: "q8idevai" },
    { prNumber: 42, repoOwner: "getq8idevai/cli", repoName: "q8idevai" },
    { prNumber: 42, repoOwner: "get$q8idevai", repoName: "q8idevai" },
    { prNumber: 42, repoOwner: "getq8idevai", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "getq8idevai", repoName: "q8idevai/app" },
    { prNumber: 42, repoOwner: "getq8idevai", repoName: "q8idevai!" },
  ...
  test("emits GitHub check details responses", async () => {
    ...
    const checkDetails: CheckDetails = {
      ...
      status: "completed",
      conclusion: "failure",
      url: "https://github.com/Q8iDevAI/Q8iDevAI/actions/runs/456/job/789",
      detailsUrl: "https://github.com/Q8iDevAI/Q8iDevAI/actions/runs/456/job/789",
