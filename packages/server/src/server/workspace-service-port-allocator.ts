...
export interface AllocateWorkspaceServicePortOptions {
  allocation: Q8iDevAIServicePortAllocation | undefined;
  cwd: string;
  scriptName: string;
  workspaceId: string;
  ...
  try {
    result = await execCommand(
      ...
      [options.scriptName, options.workspaceId, options.branchName ?? "", options.cwd],
      {
        cwd: options.cwd,
        envOverlay: {
          Q8IDEVAI_SCRIPTNAME: options.scriptName,
          Q8IDEVAI_WORKSPACE_ID: options.workspaceId,
          Q8IDEVAI_BRANCH_NAME: options.branchName ?? "",
          Q8IDEVAI_WORKTREE_PATH: options.cwd,
        },
