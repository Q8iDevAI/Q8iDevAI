...
function resolveFilePath(q8idevaiHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(q8idevaiHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(q8idevaiHome, configuredPath);
}
...
function resolveConfiguredQ8iDevAIHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.q8idevaiHome) {
    return options.q8idevaiHome;
  }
  return resolveQ8iDevAIHome();
}
...
export function resolveLogConfig(
  ...
  const q8idevaiHome = resolveConfiguredQ8iDevAIHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(q8idevaiHome, persistedLog.file.path),
        }
      : undefined;
