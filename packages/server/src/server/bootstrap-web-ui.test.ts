...
function readInjectedConnectionHint(html: string): InitialDaemonConnectionHint {
  const match = html.match(
    /window\.__Q8IDEVAI_INITIAL_DAEMON_CONNECTION__=(?<json>\{[^<]+})<\/script>/,
  );
...
describe("daemon web UI bootstrap", () => {
  ...
  let daemonHandle: TestQ8iDevAIDaemon | null = null;

  async function createWebUiDist(): Promise<string> {
    tempRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-bootstrap-web-ui-"));
