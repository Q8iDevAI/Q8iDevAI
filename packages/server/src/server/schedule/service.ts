...
import type { CreateQ8iDevAIWorktreeWorkflowResult } from "../worktree-session.js";
...
export interface ScheduleServiceOptions {
  ...
  createQ8iDevAIWorktreeWorkspace: (
    input: ScheduleWorkspaceCreateInput,
...
export class ScheduleService {
  ...
  private readonly createQ8iDevAIWorktreeWorkspace: (
    input: ScheduleWorkspaceCreateInput,
  ) => Promise<CreateQ8iDevAIWorktreeWorkflowResult>;
  ...
  constructor(options: ScheduleServiceOptions) {
    this.store = new ScheduleStore(join(options.q8idevaiHome, "schedules"));
    ...
    this.createQ8iDevAIWorktreeWorkspace = options.createQ8iDevAIWorktreeWorkspace;
  ...
  private async executeSchedule(
    ...
    try {
      ...
      const created = await this.createAgent({
        ...
        labels: {
          "q8idevai.schedule-id": schedule.id,
          "q8idevai.schedule-run": runId,
        },
        mode: config.modeId,
  ...
  private async createScheduleRunWorkspace(
    ...
    switch (config.isolation ?? "local") {
      ...
      case "worktree":
        return (await this.createQ8iDevAIWorktreeWorkspace({ cwd: config.cwd, firstAgentContext }))
          .workspace;
    }
  }
