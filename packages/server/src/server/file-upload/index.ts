...
import { FileTransferOpcode, type FileTransferFrame } from "@q8idevai/protocol/binary-frames/index";
...
export class FileUploadStore {
  ...
  beginUpload(
    ...
    const uploadDir = join(this.q8idevaiHome, "uploads", id);
  ...
  private async startWriting(upload: PendingUpload): Promise<void> {
    await mkdir(join(this.q8idevaiHome, "uploads", upload.id), { recursive: true });
  ...
  private async removeUploadDirectory(upload: PendingUpload): Promise<void> {
    await rm(join(this.q8idevaiHome, "uploads", upload.id), { recursive: true, force: true });
  }
}
