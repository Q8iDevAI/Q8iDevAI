...
describe("file uploads", () => {
  ...
  it("stores chunked upload bytes and returns an uploaded-file attachment", async () => {
    const q8idevaiHome = makeQ8iDevAIHome();
    const uploads = new FileUploadStore({ q8idevaiHome });
    ...
    const path = uploadedPath(q8idevaiHome, "notes.txt");
  ...
  it("preserves chunk order when frames arrive before earlier disk writes finish", async () => {
    ...
    expect(readFileSync(uploadedPath(q8idevaiHome, "notes.txt"), "utf8")).toBe("hello world");
  });
  ...
  it("replaces duplicate upload starts without letting the old stale timeout evict the replacement", async () => {
    ...
    const q8idevaiHome = makeQ8iDevAIHome();
    const uploads = new FileUploadStore({ q8idevaiHome, staleUploadTimeoutMs: 50 });
    ...
    const path = uploadedPath(q8idevaiHome, "new.txt");
...
function makeQ8iDevAIHome(): string {
...
function uploadedPath(q8idevaiHome: string, fileName: string): string {
  const root = join(q8idevaiHome, "uploads");
