...
describe("allocateWorkspaceServicePort", () => {
  ...
  function createContextPortScript(tempDir: string, port: number): string {
    const contents =
      process.platform === "win32"
        ? `@echo off\r\n<nul set /p "=%CD%" > cwd\r\n<nul set /p "=%~1|%~2|%~3|%~4" > argv\r\n<nul set /p "=%Q8IDEVAI_SCRIPTNAME%|%Q8IDEVAI_WORKSPACE_ID%|%Q8IDEVAI_BRANCH_NAME%|%Q8IDEVAI_WORKTREE_PATH%" > env\r\necho ${port}\r\n`
        : `#!/bin/sh\nprintf '%s' "$PWD" > cwd\nprintf '%s|%s|%s|%s' "$1" "$2" "$3" "$4" > argv\nprintf '%s|%s|%s|%s' "$Q8IDEVAI_SCRIPTNAME" "$Q8IDEVAI_WORKSPACE_ID" "$Q8IDEVAI_BRANCH_NAME" "$Q8IDEVAI_WORKTREE_PATH" > env\nprintf '${port}\\n'\n`;
    return writePortScript(tempDir, contents);
  }
