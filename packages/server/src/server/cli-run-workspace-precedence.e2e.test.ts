...
// The daemon-level workspace contract that `q8idevai run` depends on: each
...
// precedence (--workspace > $Q8IDEVAI_WORKSPACE_ID > --worktree > bare) is covered
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "q8idevai-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $Q8IDEVAI_WORKSPACE_ID land a run) creates no new workspace record: the
