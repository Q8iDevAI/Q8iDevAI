...
const RUN = process.env.Q8IDEVAI_GIT_DIFF_BOTTLENECK_E2E === "1";
const LARGE_CHANGESET_SIZE = Number.parseInt(
  process.env.Q8IDEVAI_GIT_DIFF_BOTTLENECK_FILE_COUNT ?? "1200",
  10,
);

function tmpRepo(): string {
  return mkdtempSync(path.join(tmpdir(), "q8idevai-git-diff-bottleneck-"));
}
