...
async function createHarness(): Promise<{
  client: DaemonClient;
  daemon: Awaited<ReturnType<typeof createTestQ8iDevAIDaemon>>;
}> {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "q8idevai-e2e-custom": {
            description: "Custom agent for Q8iDevAI daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "q8idevai-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for Q8iDevAI daemon E2E test");
