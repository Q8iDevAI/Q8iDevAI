...
export async function createOpenCodeOmoRealRuntime(): Promise<OpenCodeOmoRealRuntime> {
  ...
  const openCodeVersion =
    process.env.Q8IDEVAI_REAL_OPENCODE_VERSION?.trim() || PINNED_OPENCODE_VERSION;
  const omoVersion = process.env.Q8IDEVAI_REAL_OMO_VERSION?.trim() || PINNED_OMO_VERSION;
  ...
  if (model === NO_AUTH_MODEL) {
    await runCommand({
      command: openCodeCommand,
      args: ["run", "--model", model, "Reply with exactly: Q8IDEVAI_BIG_PICKLE_PROBE_OK"],
      cwd: paths.workspace,
      env: runtimeEnv,
      artifactName: "big-pickle-probe.log",
      artifacts: paths.artifacts,
      secrets,
      requiredOutput: "Q8IDEVAI_BIG_PICKLE_PROBE_OK",
    });
  }

  const previousQ8iDevAIHome = process.env.Q8IDEVAI_HOME;
  ...
  let daemon: TestQ8iDevAIDaemon | null = null;
  let client: DaemonClient | null = null;
  try {
    process.env.Q8IDEVAI_HOME = path.join(paths.q8idevaiHomeRoot, ".q8idevai");
    ...
    daemon = await createTestQ8iDevAIDaemon({
      agentClients: { opencode: openCodeClient },
      logger,
      q8idevaiHomeRoot: paths.q8idevaiHomeRoot,
    ...
    return {
      ...
      close: async (passed) => {
        try {
          ...
          restoreEnvironment("Q8IDEVAI_HOME", previousQ8iDevAIHome);
        }
      },
    };
  } catch (error) {
...
function createRuntimePaths(): RuntimePaths {
  const root = mkdtempSync(path.join(tmpdir(), "q8idevai-real-opencode-omo-"));
  const paths: RuntimePaths = {
    root,
    home: path.join(root, "home"),
    q8idevaiHomeRoot: path.join(root, "q8idevai-home"),
...
function resolveModel(openRouterApiKey: string | null): string {
  const explicitModel = process.env.Q8IDEVAI_REAL_OPENCODE_MODEL?.trim();
  if (explicitModel) {
...
function buildRuntimeEnv(paths: RuntimePaths, openRouterApiKey: string | null): NodeJS.ProcessEnv {
  return {
    ...
    Q8IDEVAI_HOME: path.join(paths.q8idevaiHomeRoot, ".q8idevai"),
