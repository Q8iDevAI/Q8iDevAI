...
githubTest(
  ...
  async () => {
    const repository = realpathSync(mkdtempSync(path.join(tmpdir(), "q8idevai-github-merge-")));
    cleanupPaths.add(repository);
    run(repository, "git", ["init", "-b", "main"]);
    run(repository, "git", ["config", "user.email", "q8idevai-test@example.com"]);
    run(repository, "git", ["config", "user.name", "Q8iDevAI Test"]);
    ...
    const merged = await createWorktree({
      ...
      runSetup: false,
      q8idevaiHome: context.daemon.q8idevaiHome,
    });
