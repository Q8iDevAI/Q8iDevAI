...
test("an empty project becomes Git without changing its identity or creating a workspace", async () => {
  const projectRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "q8idevai-project-becomes-git-")),
  );
  const q8idevaiHomeRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "q8idevai-project-becomes-git-home-")),
  );
  ...
  const persistedProjects = JSON.parse(
    await readFile(path.join(daemon.q8idevaiHome, "projects", "projects.json"), "utf8"),
