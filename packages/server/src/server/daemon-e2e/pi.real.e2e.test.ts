...
async function withConnectedPiDaemon(
  run: (context: { client: DaemonClient; daemon: TestQ8iDevAIDaemon }) => Promise<void>,
): Promise<void> {
...
test(
  "real Pi daemon composes project and Q8iDevAI system prompts",
  async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          "When the user says Q8IDEVAI_SYSTEM_PROMPT_PROBE, reply with exactly two tokens:",
          "PROJECT_PROMPT followed by the value of Q8IDEVAI_PROMPT_TOKEN from later system instructions.",
          "If no Q8IDEVAI_PROMPT_TOKEN exists, use MISSING as the second token.",
        ].join("\n"),
      );
      ...
      await withConnectedPiDaemon(async ({ client }) => {
        const agent = await client.createAgent({
          ...
          systemPrompt:
            "Q8IDEVAI_PROMPT_TOKEN is Q8IDEVAI_PROMPT. Follow the project instruction for Q8IDEVAI_SYSTEM_PROMPT_PROBE.",
        });

        await client.sendMessage(agent.id, "Q8IDEVAI_SYSTEM_PROMPT_PROBE");
        ...
        const response = items
          ...
          .join("")
          .trim();
        expect(response).toBe("PROJECT_PROMPT Q8IDEVAI_PROMPT");
      });
    } finally {
...
test(
  "real Pi daemon lists Q8iDevAI-handled compact slash commands",
  async () => {
...
test(
  ...
  async () => {
    ...
    const firstPrompt = "Q8IDEVAI_PI_ENTRY_ID_FIRST. Reply exactly: first-ok";
    const secondPrompt = "Q8IDEVAI_PI_ENTRY_ID_SECOND. Reply exactly: second-ok";
