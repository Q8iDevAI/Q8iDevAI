...
beforeEach(async () => {
  ...
  sdk = createQ8iDevAIClient({ url: `ws://127.0.0.1:${daemon.port}/ws` });
  await sdk.connect();
});
...
test("plugin handlers operate terminals through their host-owned Q8iDevAI API", async () => {
...
export default function contribute(server) {
  server.handle(operate, async ({ workspaceId, command }, { q8idevai }) => {
    const workspace = q8idevai.workspaces.ref(workspaceId);
    ...
    try {
      ...
      await q8idevai.terminals.ref(terminal.id).kill();
