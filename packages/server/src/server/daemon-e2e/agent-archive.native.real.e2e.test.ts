...
async function expectContextAfterWorkspaceRestore(legacyNativeArchive: boolean): Promise<void> {
  const root = mkdtempSync(path.join(tmpdir(), "q8idevai-archive-codex-"));
  ...
  let daemon: TestQ8iDevAIDaemon | undefined;
  ...
  try {
    ...
    // Older daemons marked Q8iDevAI archived while native archive failed against its writer.
...
test.runIf(process.env.Q8IDEVAI_NATIVE_ARCHIVE_QA === "1").each([false, true])(
