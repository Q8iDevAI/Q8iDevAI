...
test("openProject preserves a worktree's exact-root project without rehoming it", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "q8idevai-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "q8idevai-open-project-worktree-")),
    );
    const q8idevaiHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "q8idevai-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(q8idevaiHome, "projects", "workspaces.json");
    ...
    process.env.Q8IDEVAI_SUPERVISED = previousSupervised;
  }
}, 30_000);
