...
import { renderTerminalSnapshotToAnsi } from "@q8idevai/protocol/terminal-snapshot";
import type { TerminalStreamEvent } from "@q8idevai/client/internal/daemon-client";
import { DaemonClient } from "../test-utils/daemon-client.js";
import { createTestQ8iDevAIDaemon } from "../test-utils/q8idevai-daemon.js";

const BYTE_DONE_MARKER = "__Q8IDEVAI_BYTE_PACKAGE_LOCK_DONE__";
...
async function createPackageLockTerminalCwd(): Promise<PackageLockTerminalCwd> {
  const cwd = await mkdtemp(path.join(tmpdir(), "q8idevai-byte-package-lock-"));
  return {
    path: cwd,
