...
async function getPairingOfferUrl(args: {
  ...
  const pairing = await generateLocalPairingOffer({
    q8idevaiHome: args.q8idevaiHome,
...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    process.env.Q8IDEVAI_PRIMARY_LAN_IP = "192.168.1.12";
    ...
    try {
      const offerUrl = await getPairingOfferUrl({
        q8idevaiHome: daemon.q8idevaiHome,
      ...
      expect(offerUrl.startsWith("https://app.q8idevai.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.q8idevai.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestQ8iDevAIDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      q8idevaiHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      const offerUrl1 = await getPairingOfferUrl({
        q8idevaiHome: daemon1.q8idevaiHome,
      ...
      const daemon2 = await createTestQ8iDevAIDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        q8idevaiHomeRoot: tempHomeRoot,
      ...
      try {
        const offerUrl2 = await getPairingOfferUrl({
          q8idevaiHome: daemon2.q8idevaiHome,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      Q8IDEVAI_HOME: tempHome,
      Q8IDEVAI_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
