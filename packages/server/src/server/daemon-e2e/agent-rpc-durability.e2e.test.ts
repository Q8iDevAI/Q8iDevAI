...
test("agent fetch RPCs tolerate an agent whose workspace project record is gone", async () => {
  ...
  try {
    daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot: fixture.q8idevaiHomeRoot, cleanup: false });
...
function seedStaleAgentFixture(): StaleAgentFixture {
  const healthyCwd = mkdtempSync(path.join(os.tmpdir(), "q8idevai-healthy-agent-"));
  const orphanCwd = mkdtempSync(path.join(os.tmpdir(), "q8idevai-orphan-agent-"));
  const q8idevaiHomeRoot = mkdtempSync(path.join(os.tmpdir(), "q8idevai-orphan-agent-home-"));
  const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
  const projectsDir = path.join(q8idevaiHome, "projects");
  const agentsDir = path.join(q8idevaiHome, "agents");
  const healthyProjectId = "proj-healthy-agent-rpc";
  const healthyWorkspaceId = "ws-healthy-agent-rpc";
  const orphanWorkspaceId = "c:\\Users\\q8idevai\\stale-project";
  ...
  return {
    ...
    orphanAgentId,
    q8idevaiHomeRoot,
    cleanupPaths: [healthyCwd, orphanCwd, q8idevaiHomeRoot],
  };
}
