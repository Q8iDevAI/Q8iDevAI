...
beforeEach(() => {
  previousSupervised = process.env.Q8IDEVAI_SUPERVISED;
...
function restoreSupervisedEnv(): void {
  if (previousSupervised === undefined) {
    delete process.env.Q8IDEVAI_SUPERVISED;
    return;
  }
...
test("openProject on a nonexistent directory does not broadcast an upsert that is immediately removed", async () => {
  ...
  const tempParent = await mkdtemp(path.join(os.tmpdir(), "q8idevai-open-project-"));
...
test("openProject expands tilde before creating the workspace", async () => {
  ...
  const workspacePath = await mkdtemp(path.join(home, ".q8idevai-open-project-"));
