...
function initGitRepo(repoDir: string): void {
  ...
  execSync("git config user.email 'q8idevai-test@example.com'", {
    cwd: repoDir,
    stdio: "pipe",
  });
  execSync("git config user.name 'Q8iDevAI Test'", {
    cwd: repoDir,
    stdio: "pipe",
  });
    ...
    async () => {
      ...
      try {
        ...
        const worktree = await createLegacyWorktreeForTest({
          ...
          q8idevaiHome: ctx.daemon.q8idevaiHome,
        });
        ...
        const worktreeList = await ctx.client.getQ8iDevAIWorktreeList({
          cwd: repoDir,
        });
        ...
        const archiveResult = await ctx.client.archiveQ8iDevAIWorktree({
        ...
        const worktreeListAfter = await ctx.client.getQ8iDevAIWorktreeList({
          cwd: repoDir,
        });
        expect(
