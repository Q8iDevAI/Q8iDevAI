...
import { createTestQ8iDevAIDaemon, type TestQ8iDevAIDaemon } from "../test-utils/q8idevai-daemon.js";
import { type PersistedProjectRecord } from "../workspace-registry.js";

const cleanupPaths = new Set<string>();
const cleanupDaemons = new Set<TestQ8iDevAIDaemon>();
...
test("project.add creates a project without creating a workspace", async () => {
  const previousSupervised = process.env.Q8IDEVAI_SUPERVISED;
  process.env.Q8IDEVAI_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "q8idevai-add-project-repo-")));
    const q8idevaiHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "q8idevai-add-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(q8idevaiHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@getq8idevai.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Q8iDevAI Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot, cleanup: false });
    ...
    restoreEnv("Q8IDEVAI_SUPERVISED", previousSupervised);
  }
}, 30_000);
...
test("archiving the last workspace leaves the project parent with no workspaces", async () => {
  ...
  process.env.Q8IDEVAI_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "q8idevai-empty-project-repo-")));
    const q8idevaiHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "q8idevai-empty-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(q8idevaiHomeRoot);
    ...
    const projectsPath = path.join(q8idevaiHome, "projects", "projects.json");
