...
import { createClientChannel, type Transport } from "@q8idevai/relay/e2ee";
import {
  deriveSharedKey,
  decrypt,
  encrypt,
...
import { ConnectionOfferSchema } from "@q8idevai/protocol/connection-offer";
import { WSOutboundMessageSchema } from "@q8idevai/protocol/messages";
...
(shouldRunRelayE2e ? describe : describe.skip)("Relay transport (E2EE) - daemon E2E", () => {
  ...
  const startRelay = async (options: { useLocalRelay?: boolean } = {}) => {
    ...
    if (options.useLocalRelay) {
      relayArgs.push("--var", "Q8IDEVAI_RELAY_UPSTREAM:");
    }
