...
import { createQ8iDevAIDaemon, formatListenTarget } from "./bootstrap.js";
...
process.title = "Q8iDevAI Daemon";

type SupervisorLifecycleMessage =
  | {
      type: "q8idevai:shutdown";
      reason: string;
    }
  | {
      type: "q8idevai:ready";
...
function bootstrapFromEnvironment(): BootstrapResult {
  try {
    const q8idevaiHome = resolveQ8iDevAIHome();
    const config = loadConfig(q8idevaiHome);
    const logger = createRootLogger({ log: config.log }, { q8idevaiHome, file: false });
    return { q8idevaiHome, logger, config };
  } catch (err) {
...
async function main() {
  const { q8idevaiHome, logger, config } = bootstrapFromEnvironment();
  let daemon: Awaited<ReturnType<typeof createQ8iDevAIDaemon>> | null = null;
  ...
  const handleLifecycleIntent = (intent: DaemonLifecycleIntent) => {
    if (intent.type === "shutdown") {
      ...
      if (sendSupervisorLifecycleMessage({ type: "q8idevai:shutdown", reason: intent.reason })) {
        return;
      }
  ...
  const installSupervisorLivenessGuard = () => {
    ...
    const exitAfterSupervisorLoss = (reason: string) => {
      ...
      writeWorkerLifecycleLog(q8idevaiHome, "Supervisor liveness lost; worker exiting", {
        reason,
    ...
    process.on("message", (message: unknown) => {
      ...
      if (type === "q8idevai:supervisor-heartbeat") {
        lastSupervisorHeartbeatAt = Date.now();
        return;
      }
      if (type === "q8idevai:graceful-shutdown") {
  ...
  try {
    daemon = await createQ8iDevAIDaemon(
      {
        ...config,
  ...
  try {
    ...
    sendSupervisorLifecycleMessage({ type: "q8idevai:ready", listen });
  } catch (err) {
