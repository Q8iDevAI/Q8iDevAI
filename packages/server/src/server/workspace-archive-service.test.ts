...
function createGitRepo(): { tempDir: string; repoDir: string } {
  ...
  execFileSync("git", ["config", "user.name", "Q8iDevAI Test"], {
    cwd: repoDir,
    stdio: "pipe",
  });
...
async function createQ8iDevAIOwnedWorktree(
  repoDir: string,
  q8idevaiHome: string,
  worktreeSlug: string,
...
interface ArchiveDepsInput {
  ...
  q8idevaiWorktreesBaseRoot?: string;
...
function createArchiveDeps(input: ArchiveDepsInput): ArchiveTestDependencies {
  ...
  return {
    q8idevaiHome: input.q8idevaiHome,
    q8idevaiWorktreesBaseRoot: input.q8idevaiWorktreesBaseRoot,
...
describe("archiveByScope", () => {
  test("workspace scope archives the record and removes the directory on last reference", async () => {
    ...
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "last-ref-workspace");
  ...
  test("workspace scope runs teardown while keeping a directory referenced by a sibling", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.Q8IDEVAI_SOURCE_CHECKOUT_PATH + '/shared-teardown.log', 'ok')\"",
          ],
        },
      }),
    );
    ...
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "sibling-workspace");
  ...
  test("workspace scope skips teardown while repository automation is blocked", async () => {
    ...
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "blocked-teardown");
  ...
  test("workspace scope keeps a worktree for an active workspace in a subdirectory", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "subdirectory-sibling");
  ...
  test("archiving a subdirectory workspace keeps its active worktree root", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "subdirectory-target");
  ...
  test("workspace scope runs teardown from the exact nested workspace before deleting its worktree", async () => {
    ...
    writeFileSync(
      path.join(sourceNested, "q8idevai.json"),
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.Q8IDEVAI_SOURCE_CHECKOUT_PATH + '/nested-teardown.log', process.cwd())\"",
          ],
        },
      }),
    );
    ...
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "nested-teardown");
  ...
  test("worktree scope archives root and subdirectory workspaces before removing the backing worktree", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"const fs=require('fs');const out=process.env.Q8IDEVAI_SOURCE_CHECKOUT_PATH+'/root-scope-teardown.log';if(fs.existsSync(out))process.exit(2);fs.writeFileSync(out,'ok')\"",
          ],
        },
      }),
    );
    writeFileSync(
      ...
      JSON.stringify({
        worktree: {
          teardown: [
            "node -e \"require('fs').writeFileSync(process.env.Q8IDEVAI_SOURCE_CHECKOUT_PATH+'/nested-scope-teardown.log','ok')\"",
          ],
        },
      }),
    );
    ...
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "worktree-scope");
  ...
  test("workspace scope never removes a non-Q8iDevAI-owned directory", async () => {
  ...
  test("worktree scope keeps the directory when one record teardown fails", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "partial-failure");
  ...
  test("worktree scope removes an owned directory with zero matching records", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "zero-records");
  ...
  test("marks archiving, emits an upsert carrying the archiving state, then clears it and emits a remove", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "lifecycle");
  ...
  test("archives stored snapshots only for the target workspace", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "snapshot-scope");
  ...
  test("worktree scope archives three workspaces on the directory and removes it", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "worktree-scope-n3");
