...
import { DaemonClient, type WebSocketLike } from "@q8idevai/client/internal/daemon-client";
...
test("blocked setup remains readable on mixed-capability sockets sharing a session", async () => {
  await daemon.close();
  const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-blocked-compat-"));
  const projects = path.join(root, ".q8idevai", "projects");
  ...
  daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot: root });
...
test("plugin items are gated in provider child streams, child fetches, and rewind replay", async () => {
  ...
  daemon = await createTestQ8iDevAIDaemon({ isDev: true, agentClients: { mock: provider } });
...
async function createAttentionWorkspace(client: DaemonClient): Promise<string> {
  const result = await client.createWorkspace({
    source: { kind: "directory", path: daemon.q8idevaiHome },
  });
