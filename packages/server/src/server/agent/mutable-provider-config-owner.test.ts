...
function mutableConfig(persisted: PersistedConfig): MutableDaemonConfig {
  return {
    ...
    app: { baseUrl: persisted.app?.baseUrl ?? "https://app.q8idevai.sh" },
  };
}
...
describe("mutable provider config owner", () => {
  test("publishes provider changes after commit and emits nothing on rollback", () => {
    const q8idevaiHome = mkdtempSync(path.join(tmpdir(), "q8idevai-provider-config-owner-"));
    tempDirs.push(q8idevaiHome);
    const store = new DaemonConfigStore(q8idevaiHome, mutableConfig({ version: 1 }));
  ...
  test("does no provider work for unrelated CORS, Git, and app URL reloads", () => {
    ...
    const store = new DaemonConfigStore(q8idevaiHome, mutableConfig(initial), undefined, {
