...
let tempRoot: string;
let daemonHandle: TestQ8iDevAIDaemon;
...
beforeAll(async () => {
  ...
  daemonHandle = await createTestQ8iDevAIDaemon({
...
describe("Suite A: Core Fixes", () => {
  ...
  test("agentManager.createAgent injects q8idevai MCP using the daemon listen target", async () => {
    let agentId: string | null = null;
    try {
      ...
      expect(snapshot.config.mcpServers?.q8idevai).toBeUndefined();

      const liveAgent = daemonHandle.daemon.agentManager.getAgent(agentId);
      expect(liveAgent?.config.mcpServers?.q8idevai).toBeUndefined();
    } finally {
      await archiveAgentIfPresent(agentId);
    }
  });
