...
import { isQ8iDevAIOwnedWorktreeCwd } from "../../utils/worktree.js";
...
interface CreateAgentLifecycleDispatchDependencies {
  ...
  createQ8iDevAIWorktreeWorkflow: CreateQ8iDevAIWorktreeWorkflowFn;
...
type AutoArchiveTarget =
  | { kind: "agent-only" }
  | { kind: "created-worktree"; result: CreateQ8iDevAIWorktreeWorkflowResult };
...
export class CreateAgentLifecycleDispatch {
  ...
  }): Promise<CreateQ8iDevAIWorktreeWorkflowResult | null> {
  ...
  private async createWorktreeForTarget(
    ...
    const baseInput = {
      ...
      firstAgentContext,
      runSetup: false,
      q8idevaiHome: this.dependencies.q8idevaiHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    } as const;

    switch (target.mode) {
      case "branch-off":
        return this.dependencies.createQ8iDevAIWorktreeWorkflow(
          {
            ...baseInput,
  ...
  private async archiveAutoCreatedWorktree(options: {
    ...
    const ownership = await isQ8iDevAIOwnedWorktreeCwd(worktreePath, {
      q8idevaiHome: this.dependencies.q8idevaiHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    });
    if (!ownership.allowed) {
      throw new Error("Auto-created worktree is not a Q8iDevAI-owned worktree");
    }

    await archiveByScope(
      {
        q8idevaiHome: this.dependencies.q8idevaiHome,
        q8idevaiWorktreesBaseRoot: this.dependencies.worktreesRoot,
