...
function fakeWorktreeCreator(args: { repoRoot: string; createdWorkspaceId: string }) {
  ...
  return async (): Promise<CreateQ8iDevAIWorktreeWorkflowResult> =>
    ({
    ...
    }) as unknown as CreateQ8iDevAIWorktreeWorkflowResult;
}
...
test("session create forwards clientMessageId to the initial prompt run options", async () => {
  ...
  await createAgentCommand(dependencies, {
    kind: "session",
    config: { provider: "codex", cwd: "/tmp/q8idevai-create-test" },
...
test("session create validates the requested mode against the provider's modes", async () => {
  ...
  await expect(
    createAgentCommand(dependencies, {
      kind: "session",
      config: { provider: "opencode", cwd: "/tmp/q8idevai-create-test", modeId: "plan" },
      workspaceId: "ws-create-test",
      labels: {},
      provisionalTitle: null,
      firstAgentContext: { attachments: [] },
...
test("session create applies the resolved mode from the provider create config", async () => {
  ...
  await createAgentCommand(dependencies, {
    kind: "session",
    config: { provider: "opencode", cwd: "/tmp/q8idevai-create-test", modeId: "build" },
    workspaceId: "ws-create-test",
    labels: {},
    provisionalTitle: null,
    firstAgentContext: { attachments: [] },
    ...
    const { snapshot: child } = await createAgentCommand(
      {
        ...
        logger,
        providerSnapshotManager,
        createQ8iDevAIWorktree: fakeWorktreeCreator({
          repoRoot: workdir,
          createdWorkspaceId: "ws-new-worktree",
        }),
      },
    ...
    await createAgentCommand(
      {
        ...
        createQ8iDevAIWorktree: async () => createdWorktree,
      },
      {
        kind: "mcp",
        provider: "codex",
