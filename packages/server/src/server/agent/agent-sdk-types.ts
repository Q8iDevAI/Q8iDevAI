...
export interface AgentCapabilityFlags {
  ...
  supportsNativeQ8iDevAITools?: boolean;
...
export interface AgentLaunchContext {
  ...
  env?: Record<string, string>;
  /**
   * Runtime-only internal Q8iDevAI tools. This must never be persisted into
...
export interface AgentClient {
   ...
   * Called when Q8iDevAI archives an agent so the provider's own UI reflects the same state.
   */
  archiveNativeSession?(handle: AgentPersistenceHandle): Promise<void>;
  /**
   * Unarchive a durable native session in the provider.
   * Called before Q8iDevAI clears its archived flag so provider resume can succeed.
   */
