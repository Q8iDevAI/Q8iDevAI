...
describe("STTManager", () => {
  ...
  it("uses Q8IDEVAI_VOICE_LANGUAGE over Q8IDEVAI_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        Q8IDEVAI_VOICE_LANGUAGE: "pt",
        Q8IDEVAI_DICTATION_LANGUAGE: "es",
  ...
  it("uses Q8IDEVAI_DICTATION_LANGUAGE when Q8IDEVAI_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.Q8IDEVAI_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.Q8IDEVAI_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.Q8IDEVAI_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.Q8IDEVAI_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
