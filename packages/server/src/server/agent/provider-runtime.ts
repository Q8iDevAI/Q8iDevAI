...
export interface AgentProviderRuntime {
  ...
  setQ8iDevAIToolCatalog(catalog: Q8iDevAIToolCatalog | null): void;
...
export async function createAgentProviderRuntime(
  ...
  const bridge = new OpenCodeBridge({ q8idevaiHome: options.q8idevaiHome, logger: options.logger });
  try {
    await bridge.start();
    ...
    return {
      snapshotManager,
      setQ8iDevAIToolCatalog: (catalog) => bridge.setManifestCatalog(catalog),
      shutdown: () => {
