  ...
  it("renders terminal tool calls as one-line command summaries", () => {
    const timeline: AgentTimelineItem[] = [
      toolCallItem({
        ...
        detail: {
          type: "plain_text",
          label: `skills/q8idevai-chat/bin/chat.sh post --room storage-revamp --body $'first line
...
second line'`,
    ...
    expect(result).toContain(
      "[Terminal] skills/q8idevai-chat/bin/chat.sh post --room storage-revamp --body $'first line second line'",
    );
  ...
  it("falls back to input json for likely external tools", () => {
    ...
    expect(result).toBe('[q8idevai__create_agent] {"cwd":"/tmp/repo","initialPrompt":"do the thing"}');
  });
  ...
  it("builds fork context from user messages, assistant messages, and tool summaries", () => {
    ...
    expect(result.attachment.text).toContain("[q8idevai__create_agent]");
