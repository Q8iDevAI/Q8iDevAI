import { normalizeAgentModelCatalog } from "@q8idevai/protocol/agent-types";
...
import { BUILTIN_PROVIDER_IDS } from "@q8idevai/protocol/provider-manifest";
...
import type { HubExecutionAgentValidationIssue } from "@q8idevai/protocol/messages";
import {
...
const PROVIDER_REFRESH_DEADLINE_ENV = "Q8IDEVAI_PROVIDER_REFRESH_TIMEOUT_MS";
export const GLOBAL_PROVIDER_SNAPSHOT_KEY = "q8idevai:global";
...
function identifyEntry(entry: ProviderSnapshotEntry): ProviderSnapshotRecord {
  ...
  const contentHash = createHash("sha256")
    .update(JSON.stringify(["q8idevai.provider-result/1", content]))
    .digest("base64url");
