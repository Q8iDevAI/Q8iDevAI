...
import { AgentProfileSchema } from "@q8idevai/protocol/messages";
...
import { everyMsToFiveFieldCron } from "@q8idevai/protocol/schedule/cadence";
import { expandUserPath, isSameOrDescendantPath, resolvePathFromBase } from "../../path-utils.js";
import type { TerminalManager } from "../../../terminal/terminal-manager.js";
import type { CreateQ8iDevAIWorktreeWorkflowFn } from "../../worktree-session.js";
...
import {
  ...
  type CreateQ8iDevAIWorktreeCommandInput,
  createQ8iDevAIWorktreeCommand,
...
import type {
  Q8iDevAIToolCatalog,
  Q8iDevAIToolConfig,
  Q8iDevAIToolDefinition,
  Q8iDevAIToolExecutionContext,
  Q8iDevAIToolResult,
} from "./types.js";
import type { ProviderQ8iDevAIToolsPolicy } from "@q8idevai/protocol/provider-config";
import { isQ8iDevAIToolEnabled } from "../q8idevai-tool-policy.js";

export interface Q8iDevAIToolHostDependencies {
  ...
  createQ8iDevAIWorktree?: CreateQ8iDevAIWorktreeWorkflowFn;
...
type WorkspaceWorktreeTarget = Pick<
  CreateQ8iDevAIWorktreeCommandInput,
...
export function createQ8iDevAIToolCatalog(options: Q8iDevAIToolHostDependencies): Q8iDevAIToolCatalog {
  const {
    agentManager,
    agentStorage,
    terminalManager,
  ...
  const childLogger = logger.child({ module: "agent", component: "q8idevai-tool-catalog" });
  const callerContext = callerAgentId ? (resolveCallerContext?.(callerAgentId) ?? null) : null;

  const parseToolInput = async (tool: Q8iDevAIToolDefinition, input: unknown): Promise<unknown> => {
  ...
  const tools = new Map<string, Q8iDevAIToolDefinition>();
  const registerTool = (
    name: string,
    ...
    handler: (input: any, context: Q8iDevAIToolExecutionContext) => Promise<Q8iDevAIToolResult>,
  ) => {
    if (!isQ8iDevAIToolEnabled(options.q8idevaiToolPolicy, name)) {
      return;
    }
    tools.set(name, {
      name,
      ...
      handler: handler as Q8iDevAIToolDefinition["handler"],
    });
  };
  const toCatalog = (): Q8iDevAIToolCatalog => ({
    tools,
    ...
    async executeTool(
      ...
      if (!tool) {
        throw new Error(`Q8iDevAI tool not found: ${name}`);
      }
    ...
    z
      .object({
        ...
        worktreeSlug: z
          ...
          .min(1)
          .optional()
          .describe("Optional worktree slug/path label. Omit to let Q8iDevAI generate one."),
        branchName: z
          .string()
          .min(1)
          .optional()
      ...
      .strict()
      .describe("Create a new branch in a new Q8iDevAI worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out an existing branch in a new Q8iDevAI worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out a GitHub pull request in a new Q8iDevAI worktree."),
  ]);
  ...
  registerTool(
    "create_workspace",
    {
      ...
      description:
        "Create a workspace using an existing local checkout or a new Q8iDevAI-managed worktree.",
      inputSchema: {
        isolation: z.enum(["local", "worktree"]),
        path: z
          .string()
    ...
    async ({
      ...
      if (isolation === "local") {
        ...
        const result = await createQ8iDevAIWorktreeCommand(
          {
            q8idevaiHome: options.q8idevaiHome,
            worktreesRoot: options.worktreesRoot,
            createQ8iDevAIWorktreeWorkflow: options.createQ8iDevAIWorktree,
          },
          {
            cwd,
            ...(projectId ? { projectId } : {}),
    ...
    async (args: unknown) => {
      ...
      const {
      ...
      } = await createAgentCommand(
        {
          ...
          terminalManager,
          providerSnapshotManager,
          createQ8iDevAIWorktree: options.createQ8iDevAIWorktree,
  ...
  registerTool(
    "start_workspace_script",
    {
      ...
      description:
        "Start one configured workspace script through Q8iDevAI's managed workspace-script launcher.",
      inputSchema: {
        workspaceId: z.string().describe("Workspace ID containing the configured script."),
        scriptName: z.string().min(1).describe("Configured q8idevai.json script name to start."),
      },
      outputSchema: {
  ...
  registerTool(
    "stop_workspace_script",
    {
      ...
      inputSchema: {
        ...
        scriptName: z.string().min(1).describe("Configured q8idevai.json script name to stop."),
      },
      outputSchema: {
        script: WorkspaceScriptPayloadSchema,
      },
...
function archiveWorktreeDependencies(
  options: Q8iDevAIToolHostDependencies,
  ...
  return {
    q8idevaiHome: options.q8idevaiHome,
    q8idevaiWorktreesBaseRoot: options.worktreesRoot,
