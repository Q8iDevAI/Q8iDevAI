...
import { addModelVisibleStructuredContent } from "./tools/q8idevai-tool-serialization.js";
import { createQ8iDevAIToolCatalog, type Q8iDevAIToolHostDependencies } from "./tools/q8idevai-tools.js";
import type { Q8iDevAIToolResult } from "./tools/types.js";

export type AgentMcpServerOptions = Q8iDevAIToolHostDependencies;

type McpToolContext = RequestHandlerExtra<ServerRequest, ServerNotification>;

function toMcpToolResult(result: Q8iDevAIToolResult): CallToolResult {
...
export async function createAgentMcpServer(options: AgentMcpServerOptions): Promise<McpServer> {
  const catalog = await createQ8iDevAIToolCatalog(options);
