...
import { createQ8iDevAIDaemon, type Q8iDevAIDaemonConfig } from "../bootstrap.js";
...
class RecordingAgentClient implements AgentClient {
  ...
  constructor(
    ...
    this.capabilities = {
      ...
      supportsNativeQ8iDevAITools: false,
    };
  }
...
describe("agent MCP end-to-end (offline)", () => {
  test("create_agent runs initial prompt and affects filesystem", async () => {
    const q8idevaiHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-agent-cwd-"));
    const port = await getAvailablePort();

    const daemonConfig: Q8iDevAIDaemonConfig = {
    ...
    const daemon = await createQ8iDevAIDaemon(daemonConfig, pino({ level: "silent" }));
    await daemon.start();
  ...
  test("create_agent auto-injects q8idevai MCP by default and can be disabled", async () => {
    const q8idevaiHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-agent-cwd-"));
    const port = await getAvailablePort();
    ...
    const disabledQ8iDevAIHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-home-disabled-"));
    const disabledStaticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-disabled-"));
    const disabledAgentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-agent-cwd-disabled-"));
    const disabledPort = await getAvailablePort();
    const disabledRecorder: LaunchRecorder = { recordedLaunches: [] };
    const disabledDaemonConfig: Q8iDevAIDaemonConfig = {
      listen: `127.0.0.1:${disabledPort}`,
      q8idevaiHome: disabledQ8iDevAIHome,
      corsAllowedOrigins: [],
      hostnames: true,
      mcpEnabled: true,
      ...
      agentStoragePath: path.join(disabledQ8iDevAIHome, "agents"),
    };
    const disabledDaemon = await createQ8iDevAIDaemon(disabledDaemonConfig, pino({ level: "silent" }));
    ...
    try {
      ...
      expect(injectedAgent?.config.mcpServers?.q8idevai).toBeUndefined();
      ...
      expect(disabledRecorder.recordedLaunches.at(-1)?.mcpServers?.q8idevai).toBeUndefined();
      const disabledAgent = disabledDaemon.agentManager.getAgent(disabledAgentId!);
      expect(disabledAgent?.config.mcpServers?.q8idevai).toBeUndefined();
    } finally {
      if (agentId) {
        await client.callTool({ name: "kill_agent", args: { agentId } });
      }
      ...
      await rm(disabledQ8iDevAIHome, { recursive: true, force: true });
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    const q8idevaiHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-home-"));
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-worktree-repo-"));
    const port = await getAvailablePort();
    ...
    try {
      ...
      const setupCommand =
        'while [ ! -f "$Q8IDEVAI_WORKTREE_PATH/allow-setup" ]; do sleep 0.05; done; echo "done" > "$Q8IDEVAI_WORKTREE_PATH/setup-done.txt"';
      await writeFile(
        path.join(repoRoot, "q8idevai.json"),
        JSON.stringify({
          worktree: {
            setup: [setupCommand],
            terminals: [
      ...
      execSync("git add q8idevai.json", { cwd: repoRoot, stdio: "pipe" });
