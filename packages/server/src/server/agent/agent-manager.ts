...
import type { PluginSessionOpenRequest } from "@q8idevai/plugin/server";
...
} from "@q8idevai/protocol/agent-lifecycle";
import {
...
} from "@q8idevai/protocol/agent-labels";
import type { Logger } from "pino";
import type { ProviderOptions, ToolPolicy } from "@q8idevai/protocol/agent-types";
...
import { stripInternalQ8iDevAIMcpServer, withRuntimeQ8iDevAIMcpServer } from "./runtime-mcp-config.js";
import { resolveCreateAgentTitles } from "./create-agent-title.js";
import type { Q8iDevAIToolCatalogFactory } from "./tools/types.js";
import { isQ8iDevAIToolPolicyEnabled } from "./q8idevai-tool-policy.js";
import {
...
interface PreparedSessionConfig {
  ...
  q8idevaiToolPolicy: ProviderQ8iDevAIToolsPolicy | undefined;
}
...
function buildStoredAgentConfig(record: StoredAgentRecord): AgentSessionConfig {
  ...
  return stripInternalQ8iDevAIMcpServer(config);
}
...
export interface AgentManagerOptions {
  ...
  mcpBaseUrl?: string;
  mcpAuthToken?: string;
  q8idevaiToolsEnabled?: boolean;
  q8idevaiToolCatalogFactory?: Q8iDevAIToolCatalogFactory;
  resolveQ8iDevAIToolPolicy?: (provider: AgentProvider) => ProviderQ8iDevAIToolsPolicy | undefined;
...
export class AgentManager {
  ...
  private q8idevaiToolsEnabled = true;
  private q8idevaiToolCatalogFactory: Q8iDevAIToolCatalogFactory | null = null;
  private readonly q8idevaiToolPolicies = new Map<string, ProviderQ8iDevAIToolsPolicy | undefined>();
  private readonly resolveQ8iDevAIToolPolicy: (
    provider: AgentProvider,
  ...
  constructor(options: AgentManagerOptions) {
    ...
    this.configureQ8iDevAITools(options);
    this.resolveQ8iDevAIToolPolicy = options.resolveQ8iDevAIToolPolicy ?? (() => undefined);
  ...
  private configureQ8iDevAITools(options: AgentManagerOptions): void {
    this.q8idevaiToolsEnabled = options.q8idevaiToolsEnabled ?? true;
    this.q8idevaiToolCatalogFactory = options.q8idevaiToolCatalogFactory ?? null;
  }
  ...
  setQ8iDevAIToolsEnabled(enabled: boolean): void {
    this.q8idevaiToolsEnabled = enabled;
  }

  setQ8iDevAIToolCatalogFactory(factory: Q8iDevAIToolCatalogFactory | null): void {
    this.q8idevaiToolCatalogFactory = factory;
  }

  getQ8iDevAIToolPolicy(agentId: string): ProviderQ8iDevAIToolsPolicy | undefined {
    return this.q8idevaiToolPolicies.get(agentId);
  }
  ...
  private async createAgentInternal(
    ...
    const { storedConfig, launchConfig, q8idevaiToolPolicy } = await this.prepareSessionConfig(
      config,
      resolvedAgentId,
      options?.env,
    );
    ...
    this.q8idevaiToolPolicies.set(resolvedAgentId, q8idevaiToolPolicy);
  ...
  private async importProviderSessionInternal(input: {
    ...
    try {
      const importedConfig = await this.normalizeConfig(
        stripInternalQ8iDevAIMcpServer(imported.config),
      );
  ...
  private async reloadAgentSessionInternal(
    ...
    const hadPreviousQ8iDevAIToolPolicy = this.q8idevaiToolPolicies.has(agentId);
    const previousQ8iDevAIToolPolicy = this.q8idevaiToolPolicies.get(agentId);
    const launchContext = await this.buildLaunchContext(
      agentId,
      client,
      storedConfig.cwd,
    ...
    try {
      ...
      this.q8idevaiToolPolicies.set(agentId, q8idevaiToolPolicy);
      session = handle
      ...
      if (!handedToRegistration) {
        if (hadPreviousQ8iDevAIToolPolicy) {
          this.q8idevaiToolPolicies.set(agentId, previousQ8iDevAIToolPolicy);
        } else {
          this.q8idevaiToolPolicies.delete(agentId);
        }
        if (session) {
  ...
  private async prepareSessionConfig(
    ...
    const storedConfig = await this.normalizeConfig(stripInternalQ8iDevAIMcpServer(config), { env });
    const q8idevaiToolPolicy = this.q8idevaiToolsEnabled
      ? this.resolveQ8iDevAIToolPolicy(storedConfig.provider)
      : { enabled: false };
    const launchConfig = this.applyDaemonAppendSystemPrompt(
      withRuntimeQ8iDevAIMcpServer({
        config: storedConfig,
        agentId,
        mcpBaseUrl:
          this.q8idevaiToolsEnabled && isQ8iDevAIToolPolicyEnabled(q8idevaiToolPolicy)
            ? this.mcpBaseUrl
            : null,
        mcpAuthToken: this.mcpAuthToken,
      }),
    );
    return { storedConfig, launchConfig, q8idevaiToolPolicy };
  }
  ...
  private async buildLaunchContext(
    ...
    const context: AgentLaunchContext = {
      ...
      env: {
        ...env,
        Q8IDEVAI_AGENT_ID: agentId,
        Q8IDEVAI_AGENT_CWD: cwd,
      },
    };
    if (
      this.q8idevaiToolsEnabled &&
      isQ8iDevAIToolPolicyEnabled(q8idevaiToolPolicy) &&
      client.capabilities.supportsNativeQ8iDevAITools &&
      this.q8idevaiToolCatalogFactory
    ) {
      context.q8idevaiTools = await this.q8idevaiToolCatalogFactory({
  ...
  private resolveProviderLaunchConfig(
    ...
    return launchContext.q8idevaiTools ? stripInternalQ8iDevAIMcpServer(launchConfig) : launchConfig;
  }
