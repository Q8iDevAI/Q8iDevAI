...
import type { FetchRecentProviderSessionsRequestMessage } from "@q8idevai/protocol/messages";
...
test("listImportableProviderSessions includes a provider session after its Q8iDevAI agent is archived", async () => {
  const cwd = "/tmp/project";
...
test("listImportableProviderSessions keeps realpath-equivalent cwd matches", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "q8idevai-import-cwd-"));
