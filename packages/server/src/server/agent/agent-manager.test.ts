...
import { getOpenAgentTabLabel, PARENT_AGENT_ID_LABEL } from "@q8idevai/protocol/agent-labels";
...
import type { Q8iDevAIToolCatalog } from "./tools/types.js";
  ...
  override async createSession(
    ...
    const script = `
      process.stdout.write(JSON.stringify({
        ...
        agentId: process.env.Q8IDEVAI_AGENT_ID ?? null
      }));
    `;
...
test("createAgent passes daemon launch env through the provider launch context", async () => {
  ...
  expect(client.lastLaunchContext).toEqual({
    agentId: snapshot.id,
    env: {
      Q8IDEVAI_AGENT_ID: snapshot.id,
      Q8IDEVAI_AGENT_CWD: workdir,
    },
  });
});
...
test("createAgent injects q8idevai MCP server only into provider launch config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
...
test("reloadAgentSession preserves the live session when its replacement cannot honor external MCP", async () => {
  ...
  let q8idevaiToolPolicy = { disabledTools: ["list_agents"] };
  const manager = new AgentManager({
    clients: { codex: new UnsupportedReloadClient() },
    registry: new AgentStorage(join(workdir, "agents"), logger),
    logger,
    resolveQ8iDevAIToolPolicy: () => q8idevaiToolPolicy,
  });
  ...
  try {
    ...
    q8idevaiToolPolicy = { disabledTools: ["create_agent"] };
    ...
    expect(manager.getQ8iDevAIToolPolicy(created.id)).toEqual({
...
test("createAgent passes native Q8iDevAI tools through launch context without internal MCP", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);

  const q8idevaiTools: Q8iDevAIToolCatalog = {
    tools: new Map(),
    getTool: () => undefined,
    executeTool: async () => {
  ...
  class NativeToolsClient extends TestAgentClient {
    override readonly capabilities = {
      ...TEST_CAPABILITIES,
      supportsMcpServers: true,
      supportsNativeQ8iDevAITools: true,
    };
    lastConfig: AgentSessionConfig | null = null;
    lastLaunchContext: AgentLaunchContext | undefined;
  ...
  const manager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    q8idevaiToolCatalogFactory: () => q8idevaiTools,
  ...
  expect(client.lastLaunchContext?.q8idevaiTools).toBe(q8idevaiTools);
  expect(client.lastConfig?.mcpServers).toEqual({
    custom: {
      type: "stdio",
      command: "custom-mcp",
...
test("createAgent allows best-effort internal MCP when the provider session reports no support", async () => {
  ...
  expect(client.lastConfig?.mcpServers?.q8idevai).toEqual({
    type: "http",
    url: `http://127.0.0.1:6767/mcp/agents?callerAgentId=${snapshot.id}`,
...
test("uses each provider's current policy for new sessions and snapshots it by agent", async () => {
  ...
  const policyInputs: Array<{ callerAgentId?: string; q8idevaiToolPolicy?: unknown }> = [];
  const q8idevaiTools: Q8iDevAIToolCatalog = {
    tools: new Map(),
    getTool: () => undefined,
    executeTool: async () => {
  ...
  const manager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    resolveQ8iDevAIToolPolicy: (provider) => policies.get(provider),
    q8idevaiToolCatalogFactory: async (context) => {
      policyInputs.push(context);
      return q8idevaiTools;
    },
  });
  ...
  expect(policyInputs).toEqual([
    { callerAgentId: codexAgent.id, q8idevaiToolPolicy: { disabledTools: ["list_agents"] } },
  ]);
  expect(codex.launchContexts[0]?.q8idevaiTools).toBe(q8idevaiTools);
  expect(claude.launchContexts[0]?.q8idevaiTools).toBeUndefined();
  expect(codex.configs[0]?.mcpServers?.q8idevai).toBeUndefined();
  expect(claude.configs[0]?.mcpServers).toBeUndefined();
  expect(manager.getQ8iDevAIToolPolicy(codexAgent.id)).toEqual({
    disabledTools: ["list_agents"],
  });
  expect(manager.getQ8iDevAIToolPolicy(claudeAgent.id)).toEqual({ enabled: false });
  ...
  expect(manager.getQ8iDevAIToolPolicy(nextCodexAgent.id)).toEqual({
  ...
  expect(manager.getQ8iDevAIToolPolicy(claudeAgent.id)).toBeUndefined();

  rmSync(workdir, { recursive: true, force: true });
});

test("keeps the global Q8iDevAI-tools gate outside provider policy and MCP injection", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storage = new AgentStorage(join(workdir, "agents"), logger);
  ...
  const enabledManager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    resolveQ8iDevAIToolPolicy: () => ({ disabledTools: ["list_agents"] }),
  });
  ...
  expect(enabledClient.lastConfig?.mcpServers?.q8idevai).toEqual({
    type: "http",
  ...
  const disabledManager = new AgentManager({
    ...
    logger,
    mcpBaseUrl: "http://127.0.0.1:6767/mcp/agents",
    q8idevaiToolsEnabled: false,
    resolveQ8iDevAIToolPolicy: () => ({ enabled: true }),
    q8idevaiToolCatalogFactory: () => {
  ...
  expect(disabledManager.getQ8iDevAIToolPolicy(disabledAgent.id)).toEqual({ enabled: false });

  rmSync(workdir, { recursive: true, force: true });
});

test("resumeAgentFromPersistence replaces stored internal q8idevai MCP with current runtime URL", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
...
test("resumeAgentFromPersistence drops stored internal q8idevai MCP when runtime injection is disabled", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  const client = new TestAgentClient();
...
test("createAgent preserves a user-provided q8idevai MCP config", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-test-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  ...
  const snapshot = await manager.createAgent(
    {
      ...
      mcpServers: {
        q8idevai: {
          type: "http",
          url: "https://example.com/custom-q8idevai",
        },
      },
    },
    undefined,
  ...
  const resumed = await manager.resumeAgentFromPersistence(handle, {
    ...
    mcpServers: {
      q8idevai: {
        type: "stdio",
        command: "node",
        args: ["/tmp/mcp-bridge.mjs", "--socket", "/tmp/q8idevai.sock"],
      },
    },
  });
...
test("user_message events wrapping a q8idevai-system envelope are not added to the timeline", async () => {
...
test("user_message events wrapping a q8idevai-system envelope are not restored during history replay", async () => {
