...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Q8iDevAI using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "q8idevai-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Q8iDevAI issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
