...
async function readNativeThreadPath(threadId: string): Promise<string> {
  ...
  try {
    await client.request("initialize", {
      clientInfo: { name: "q8idevai-archive-regression", version: "1.0.0" },
    });
...
// archived by older Q8iDevAI versions whose best-effort native archive failed.
test.runIf(process.env.Q8IDEVAI_NATIVE_ARCHIVE_QA === "1").each([true, false])(
...
test.runIf(process.env.Q8IDEVAI_NATIVE_ARCHIVE_QA === "1")(
