...
test("real OpenCode server shares one process while shell.env stays session-scoped", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-opencode-real-"));
  const firstCwd = path.join(root, "first");
  const secondCwd = path.join(root, "second");
  const logger = createTestLogger();
  const bridge = new OpenCodeBridge({ q8idevaiHome: root, logger });
  await bridge.start();
  ...
  try {
    ...
    first = await client.createSession(
      { provider: "opencode", cwd: firstCwd, model: "opencode/big-pickle", modeId: "build" },
      {
        ...
        env: { Q8IDEVAI_AGENT_ID: "real-agent-one", Q8IDEVAI_AGENT_CWD: firstCwd },
        q8idevaiTools: firstTools,
      },
    ...
    second = await client.createSession(
      { provider: "opencode", cwd: secondCwd },
      {
        ...
        env: { Q8IDEVAI_AGENT_ID: "real-agent-two", Q8IDEVAI_AGENT_CWD: secondCwd },
        q8idevaiTools: secondTools,
      },
      { persistSession: false },
    );
    ...
    const [firstShell, secondShell] = await Promise.all([
      sdk.session.shell({
        ...
        directory: firstCwd,
        agent: "build",
        command: 'printf "%s|%s" "$Q8IDEVAI_AGENT_ID" "$Q8IDEVAI_AGENT_CWD"',
      }),
      sdk.session.shell({
    ...
    const agentResult = await first.run(
      [
        "Use the bash tool to run: env | grep -E '^(Q8IDEVAI_AGENT_ID|Q8IDEVAI_AGENT_CWD)='",
    ...
    const callerResult = await first.run(
      [
        "Use the q8idevai_report_caller_agent_id tool to read your Q8iDevAI caller agent ID.",
    ...
    expect(callerResult.timeline).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "tool_call",
          name: "q8idevai_report_caller_agent_id",
          status: "completed",
        }),
      ]),
    );
...
function createCallerCatalog(callerAgentId: string): Q8iDevAIToolCatalog {
  const tool = {
    name: "report_caller_agent_id",
    title: "Report Q8iDevAI caller agent ID",
    description: "Returns the caller agent ID assigned by Q8iDevAI.",
    inputSchema: {},
    async handler() {
