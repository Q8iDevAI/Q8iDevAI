...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex q8idevai speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex q8idevai_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        id: "codex-speak-thread-2",
        status: "completed",
        server: "q8idevai_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via q8idevai_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via q8idevai_voice.",
      output: null,
    });
  });

  it("normalizes codex q8idevai speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
