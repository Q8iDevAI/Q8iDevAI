...
type Q8iDevAIExtensionListener = (event: unknown, context?: unknown) => unknown;

async function loadQ8iDevAIExtensionListeners(
  extensionPath: string,
): Promise<Map<string, Q8iDevAIExtensionListener>> {
  const listeners = new Map<string, Q8iDevAIExtensionListener>();
  const extension = (await import(pathToFileURL(extensionPath).href)) as {
    default: (piApi: {
      on: (event: string, listener: Q8iDevAIExtensionListener) => void;
...
async function applyQ8iDevAIExtensionSystemPrompt(
  extensionPath: string,
  systemPrompt: string,
): Promise<string | undefined> {
  const listeners = await loadQ8iDevAIExtensionListeners(extensionPath);
...
describe("PiRpcAgentSession", () => {
  ...
  test("reports the persisted Pi entry attached to the submitted message", async () => {
    ...
    expect(notifications).toEqual([
      'Q8IDEVAI_SUBMITTED_USER_ENTRY {"entry":{"id":"entry-new","parentId":"entry-old-assistant","text":"new prompt"}}',
    ]);
  ...
  test("appends agent and daemon prompts after Pi's discovered system prompt", async () => {
    ...
    await expect(
      applyQ8iDevAIExtensionSystemPrompt(actualLaunch.extensionPaths[0]!, "Pi project prompt"),
  ...
  test("materializes image prompts as text hints for text-only Pi models", async () => {
    ...
    try {
      ...
      expect(imagePath).toMatch(
        /q8idevai-attachments(?:-[^\\/]+)?[\\/](?:[^\\/]+[\\/])?[0-9a-f]{64}\.png$/,
      );
...
describe("PiRpcAgentClient", () => {
  test("lists JSONL persisted sessions from configured provider params", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "q8idevai-pi-sessions-"));
  ...
  test("lists JSONL persisted sessions from Pi's configured agent directory", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "q8idevai-pi-default-sessions-"));
    const cwd = path.join(root, "workspace");
  ...
  test("imports JSONL sessions with the recorded model and thinking level", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "q8idevai-pi-import-config-"));
    const cwd = path.join(root, "workspace");
    const sessionsDir = path.join(root, "sessions");
    mkdirSync(sessionsDir, { recursive: true });
  ...
  test("maps extension, prompt, and skill commands to Q8iDevAI slash commands", async () => {
    const { pi, session } = await createSession();
  ...
  test("injects MCP servers without replacing the Pi global MCP config", async () => {
    const agentDir = mkdtempSync(path.join(tmpdir(), "q8idevai-pi-agent-"));
  ...
  test("reports the path of a malformed Pi global MCP config", async () => {
    ...
    await expect(
      client.createSession(
        createConfig({
          mcpServers: {
            q8idevai: { type: "http", url: "http://127.0.0.1:6767/mcp/agents" },
          },
        }),
        { env: { PI_CODING_AGENT_DIR: agentDir } },
      ),
