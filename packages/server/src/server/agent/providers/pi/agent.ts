...
const Q8IDEVAI_PI_TREE_EXTENSION_COMMAND = "q8idevai_tree";
const Q8IDEVAI_PI_CAPTURE_EXTENSION_COMMAND = "q8idevai_capture_entries";
const Q8IDEVAI_PI_ENTRY_CAPTURE_MARKER = "Q8IDEVAI_ENTRY_CAPTURE";
const Q8IDEVAI_PI_SUBMITTED_USER_ENTRY_MARKER = "Q8IDEVAI_SUBMITTED_USER_ENTRY";
const Q8IDEVAI_PI_COMMAND_RESULT_MARKER = "Q8IDEVAI_COMMAND_RESULT";
...
function buildResumeStartInput(input: {
  ...
  q8idevaiExtension: PiTempFile | null;
}): PiStartSessionInput {
  return {
    ...
    extensionPaths: input.q8idevaiExtension ? [input.q8idevaiExtension.path] : undefined,
  };
}
...
function createPiMcpConfigFile(
  ...
  const dir = mkdtempSync(join(tmpdir(), "q8idevai-pi-mcp-"));
...
function createPiQ8iDevAIExtensionFile(systemPrompt?: string): PiTempFile {
  const dir = mkdtempSync(join(tmpdir(), "q8idevai-pi-extension-"));
  const filePath = join(dir, "q8idevai-integration.mjs");
  writeFileSync(
    filePath,
    `
	...
	function emitEntryCapture(ctx, reason, requestId) {
	  ctx.ui.notify(
	    "${Q8IDEVAI_PI_ENTRY_CAPTURE_MARKER} " +
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${Q8IDEVAI_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	    result.ok ? "info" : "error",
	  );
	}

	export default function q8idevaiIntegration(pi) {
	  ...
	  function emitSubmittedUserEntries(ctx) {
	    ...
	    for (let index = 0; index < submittedUserMessages.length; index += 1) {
	      ...
	      index -= 1;
	      ctx.ui.notify(
	        "${Q8IDEVAI_PI_SUBMITTED_USER_ENTRY_MARKER} " +
	  ...
	  pi.registerCommand("${Q8IDEVAI_PI_CAPTURE_EXTENSION_COMMAND}", {
	    description: "Internal Q8iDevAI entry capture bridge",
	  ...
	  pi.registerCommand("${Q8IDEVAI_PI_TREE_EXTENSION_COMMAND}", {
	    description: "Internal Q8iDevAI tree navigation bridge",
	    handler: async (args, ctx) => {
	      const payload = decodePayload(args.trim());
	      try {
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${Q8IDEVAI_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${Q8IDEVAI_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private handleSubmittedUserEntryMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, Q8IDEVAI_PI_SUBMITTED_USER_ENTRY_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, Q8IDEVAI_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, Q8IDEVAI_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
...
export class PiRpcAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    const q8idevaiExtension = createPiQ8iDevAIExtensionFile(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: q8idevaiExtension ? [q8idevaiExtension.path] : undefined,
      });
    } catch (error) {
      mcpConfig?.cleanup();
      q8idevaiExtension?.cleanup();
      throw error;
    }
    try {
      return new PiRpcAgentSession({
        ...
        cleanup: combineCleanup([mcpConfig?.cleanup, q8idevaiExtension?.cleanup]),
