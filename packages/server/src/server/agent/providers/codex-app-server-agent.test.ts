...
async function startPublicSteeringSession(
...
): Promise<{ session: AgentSession; q8idevaiTurnId: string }> {
  const session = new CodexAppServerAgentSession(
  ...
  return { session, q8idevaiTurnId: started.turnId };
}
...
describe("Codex active-turn steering admission", () => {
  test("a steer without the clearing contract leaves permissions open", async () => {
    ...
    const { session, q8idevaiTurnId } = await startPublicSteeringSession(appServer);
    castInternals<{ emitSyntheticPlanApprovalRequest: (planText: string) => void }>(
      session,
    ).emitSyntheticPlanApprovalRequest("Ship the thing");

    await expect(
      session.steerActiveTurn!("background notification", { expectedTurnId: q8idevaiTurnId }),
  ...
  test("does not steer B when A completes while command resolution is pending", async () => {
    ...
    const appServer = createFakeCodexAppServer();
    const { session, q8idevaiTurnId } = await startPublicSteeringSession(appServer, async (prompt) => {
    ...
    expect(startedB.turnId).not.toBe(q8idevaiTurnId);
...
const capturePath = process.env.Q8IDEVAI_FAKE_CODEX_CAPTURE;
let buffer = "";
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          Q8IDEVAI_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("initializes Codex app-server without making Q8iDevAI the request originator", async () => {
  ...
  test("unarchives Codex when an active Q8iDevAI agent resumes an archived thread", async () => {
    const threadRequests: string[] = [];
  ...
  test("resolves Codex skill slash commands into app-server skill input", async () => {
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "q8idevai-implement",
                  description: "Execute an existing Q8iDevAI plan.",
                  path: "/tmp/skills/q8idevai-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    await session.startTurn("/q8idevai-implement in a worktree, remember to use Claude for the UI");
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$q8idevai-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/q8idevai/SKILL.md",
      },
      {
        name: "q8idevai",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/q8idevai/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "q8idevai")).toEqual([
      {
        name: "q8idevai",
        description: "Shared orchestration skill.",
        argumentHint: "",
    ...
    const input = await codexAppServerTurnInputFromPrompt(
      [
        { type: "text", text: "Please review this" },
        {
          ...
          url: "https://github.com/Q8iDevAI/Q8iDevAI/issues/456",
        },
      ],
      logger,
    );
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        Q8IDEVAI_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        Q8IDEVAI_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          Q8IDEVAI_AGENT_ID: "runtime-value",
          Q8IDEVAI_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.Q8IDEVAI_AGENT_ID).toBe(launchContext.env?.Q8IDEVAI_AGENT_ID);
    expect(env.Q8IDEVAI_TEST_FLAG).toBe(launchContext.env?.Q8IDEVAI_TEST_FLAG);
  });
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](file:///tmp/q8idevai%20image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/q8idevai-attachments(?:-[^\\/]+)?[\\/].+\.png$/);
    expect(existsSync(source)).toBe(true);
    rmSync(source, { force: true });
  });
      ...
      expect(timelineEvents).toEqual([
        {
          ...
          item: {
            ...
            name: "q8idevai.browser_screenshot",
            status: "completed",
            error: null,
            detail: {
              type: "unknown",
