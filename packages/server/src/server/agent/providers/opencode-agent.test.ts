...
describe("OpenCodeAgentClient adapter smoke tests", () => {
  ...
  test("single turn completes with streaming deltas", async () => {
    ...
    // own default agent instead of Q8iDevAI assuming any particular agent exists.
  ...
  test("fetchCatalog returns models with required fields", async () => {
    ...
    runtime.enqueueClient(openCodeClient);
    const q8idevaiHome = tmpCwd();
    const opencodeHome = path.join(q8idevaiHome, "opencode-home");
    const client = new OpenCodeAgentClient(logger, undefined, {
      serverManager: runtime,
      createClient: runtime.createClient,
    ...
    rmSync(q8idevaiHome, { recursive: true, force: true });
  }, 60_000);
  ...
  test("custom agents defined in opencode.json appear in available modes", async () => {
    ...
    openCodeClient.appAgentsResponse = {
      data: [
        {
          name: "q8idevai-test-custom",
          description: "Custom agent defined for Q8iDevAI integration test",
          mode: "primary",
        },
    ...
    expect(modes.map((mode) => mode.id)).toEqual(["q8idevai-test-custom"]);

    const custom = modes.find((mode) => mode.id === "q8idevai-test-custom");
    expect(custom).toBeDefined();
    expect(custom!.label).toBe("Q8iDevAI-test-custom");
    expect(custom!.description).toBe("Custom agent defined for Q8iDevAI integration test");
    ...
    const parts = __openCodeInternals.buildOpenCodePromptParts([
      {
        ...
        url: "https://github.com/Q8iDevAI/Q8iDevAI/issues/55",
        body: "Issue body",
      },
    ]);
  ...
  test("fails the turn when OpenCode reports MCP add failure in data payload", async () => {
    ...
    try {
      ...
      await expect(collectTurnEvents(streamSession(session, "hello"))).rejects.toThrow(
        /Failed to add OpenCode MCP server 'q8idevai': SSE error/,
      );
...
describe("OpenCode provider subagent contract", () => {
  async function createAdoptedChildSession(
    ...
    const parent = await client.createSession(
      { provider: "opencode", cwd: "/workspace/repo" },
      { env: { Q8IDEVAI_AGENT_ID: "parent-agent" } },
    );
    ...
    const child = await client.resumeSession(
      ...
      undefined,
      { env: { Q8IDEVAI_AGENT_ID: "child-agent" } },
    );
  ...
  test("resumes an adopted child on the parent's registered OpenCode server", async () => {
    ...
    expect(runtime.acquisitions).toEqual([
      { kind: "dedicated", env: { Q8IDEVAI_AGENT_ID: "parent-agent" }, releaseCount: 1 },
  ...
  test("keeps an autonomous turn active until OpenCode reaches its terminal event", async () => {
    ...
    try {
      ...
      await expect(parent.startTurn("Continue from Q8iDevAI")).rejects.toThrow(
  ...
  test("does not adopt late output from an interrupted Q8iDevAI turn", async () => {
    ...
    try {
      await parent.startTurn("Start from Q8iDevAI");
...
describe("OpenCode snapshot summary false-idle regression", () => {
  test("user message.updated carrying snapshot diffs does not start a turn", async () => {
    ...
    const session = await client.createSession(
      { provider: "opencode", cwd: "/workspace/repo" },
      { env: { Q8IDEVAI_AGENT_ID: "snapshot-agent" } },
    );
    const events: AgentStreamEvent[] = [];
    session.subscribe((event) => events.push(event));
