...
import {
  ...
  serializeQ8iDevAIToolInputParameters,
} from "../../tools/q8idevai-tool-serialization.js";
...
export class OpenCodeBridge {
  ...
  private manifestCatalog: Q8iDevAIToolCatalog | null = null;
  ...
  setManifestCatalog(catalog: Q8iDevAIToolCatalog | null): void {
  ...
  decorateServerEnv(env: Record<string, string>): Record<string, string> {
    ...
    const withoutBridge = plugins.filter((entry) => {
      ...
      return !specifier.includes("/q8idevai-") || !specifier.endsWith(".mjs");
    });
    return {
      ...env,
  ...
  private async materializePlugin(): Promise<string> {
    ...
    const destination = path.join(this.q8idevaiHome, "runtime", "opencode", `q8idevai-${digest}.mjs`);
  ...
  private async route(request: IncomingMessage, response: ServerResponse): Promise<void> {
    try {
      ...
      if (request.method === "GET" && contextMatch) {
        ...
        if (!binding) {
          sendJson(response, 404, { error: "OpenCode session is not bound to a Q8iDevAI agent" });
          return;
        }
  ...
  private serializeManifest(): Array<Record<string, unknown>> {
    ...
    return [...catalog.tools.values()].map((tool) => {
      const definition: Record<string, unknown> = {
        ...
        inputSchema: serializeQ8iDevAIToolInputParameters(tool),
      };
  ...
  private async executeTool(input: {
    ...
    if (!binding) {
      sendJson(input.response, 404, { error: "OpenCode session is not bound to a Q8iDevAI agent" });
      return;
    }
    if (!binding.tools) {
      sendJson(input.response, 403, { error: "Q8iDevAI tools are disabled for this session" });
      return;
    }
