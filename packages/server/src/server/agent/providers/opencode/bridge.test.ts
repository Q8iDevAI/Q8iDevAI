...
function createCatalog(): Q8iDevAIToolCatalog {
  const tool = {
    name: "echo_context",
    title: "Echo context",
...
describe("OpenCodeBridge", () => {
  test("loads packaged bundle bytes without invoking source compilation", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "q8idevai-opencode-artifact-"));
  ...
  test("serves authenticated session context and caller-scoped tools", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-opencode-bridge-"));
    temporaryDirectories.push(q8idevaiHome);
    const catalog = createCatalog();
    const bridge = new OpenCodeBridge({ q8idevaiHome, logger: createTestLogger() });
    await bridge.start();
    bridge.setManifestCatalog(catalog);
    const release = bridge.bindSession({
      sessionId: "ses_one",
      env: {
        Q8IDEVAI_AGENT_ID: "agent-one",
        Q8IDEVAI_AGENT_CWD: "/workspace/one",
        CUSTOM_VALUE: "one",
      },
      tools: catalog,
    });
    ...
    try {
      ...
      await expect(
        hooks.tool.q8idevai_echo_context.execute(
      ...
      expect(pluginError).toHaveBeenCalledWith(
        "[q8idevai-opencode-plugin] shell.env failed",
  ...
  test("preserves user OpenCode config while installing one content-addressed plugin", async () => {
    const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-opencode-bridge-config-"));
    temporaryDirectories.push(q8idevaiHome);
    const bridge = new OpenCodeBridge({ q8idevaiHome, logger: createTestLogger() });
    await bridge.start();
    ...
    try {
      ...
      expect(config.plugin[1]?.[0]).toMatch(/q8idevai-[a-f0-9]{64}\.mjs$/);
    } finally {
      await bridge.close();
    }
  });
