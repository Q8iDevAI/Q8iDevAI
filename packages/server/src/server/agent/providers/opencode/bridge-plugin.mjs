...
export default async function q8idevaiPlugin(input, options) {
  const request = async (pathname, init) => {
    ...
    if (!response.ok) {
      const error = new Error(payload.error ?? `Q8iDevAI OpenCode bridge failed: ${response.status}`);
  ...
  for (const definition of manifest.tools ?? []) {
    tools[`q8idevai_${definition.name}`] = {
      ...
      execute: async (args, context) => {
        ...
        return {
          ...
          metadata: { q8idevaiTool: definition.name },
        };
      },
    };
  }
...
function logPluginError(stage, context, error) {
  console.error(`[q8idevai-opencode-plugin] ${stage} failed`, {
    ...context,
