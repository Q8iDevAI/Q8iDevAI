...
const NORMAL_SENTINEL = "Q8IDEVAI_NORMAL_RECOVERY_SENTINEL_7F31";
const ROOT_SENTINEL = "Q8IDEVAI_ROOT_RECOVERY_SENTINEL_8C42";
const CHILD_SENTINEL = "Q8IDEVAI_CHILD_RECOVERY_SENTINEL_5A19";
const PARENT_SENTINEL = "Q8IDEVAI_PARENT_RECOVERY_SENTINEL_6B20";
...
async function createRealHarness() {
  const artifactDir = mkdtempSync(path.join(os.tmpdir(), "q8idevai-opencode-recovery-"));
  const runtime = createDisposableRuntime();
  const { runtimeDir, home, q8idevaiHome, xdgConfig, xdgData, xdgCache, xdgState, workspace } =
    runtime;
...
function createDisposableRuntime(
  ...
  try {
    runtimeDir = mkdtempSync(path.join(os.tmpdir(), "q8idevai-opencode-runtime-"));
    onCreate?.(runtimeDir);
    ...
    for (const directory of [home, q8idevaiHome, xdgConfig, xdgData, xdgCache, xdgState, workspace]) {
    ...
    return { runtimeDir, home, q8idevaiHome, xdgConfig, xdgData, xdgCache, xdgState, workspace };
  } catch (error) {
    if (runtimeDir) {
      rmSync(runtimeDir, { recursive: true, force: true });
    }
