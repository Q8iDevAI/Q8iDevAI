...
describe("Claude SDK env", () => {
  test("forwards launch-context env through Claude process env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        Q8IDEVAI_AGENT_ID: "00000000-0000-4000-8000-000000000201",
        Q8IDEVAI_TEST_FLAG: "launch-value",
      },
    };
    ...
    try {
      ...
      expect(capturedEnv?.Q8IDEVAI_AGENT_ID).toBe(launchContext.env?.Q8IDEVAI_AGENT_ID);
      expect(capturedEnv?.Q8IDEVAI_TEST_FLAG).toBe(launchContext.env?.Q8IDEVAI_TEST_FLAG);
  ...
  test("forwards launch-context env through Claude resume env", async () => {
    ...
    const launchContext: AgentLaunchContext = {
      env: {
        Q8IDEVAI_AGENT_ID: "00000000-0000-4000-8000-000000000202",
        Q8IDEVAI_TEST_FLAG: "resume-launch-value",
      },
    };
    const queryFactory = vi.fn(({ options }: ClaudeQueryInput) => {
      capturedEnv = options.env;
