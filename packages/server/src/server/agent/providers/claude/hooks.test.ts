...
describe("mergeClaudeHooks", () => {
  ...
  it("keeps Q8iDevAI's hooks when the user configured none", () => {
  ...
  it("appends user hooks for the same event instead of replacing them", () => {
    ...
    // Both must survive: dropping Q8iDevAI's would silently stop effort being observed, and
    // dropping the user's would break their configuration.
    expect(merged.PreToolUse).toHaveLength(2);
  });

  it("carries through events Q8iDevAI does not register", () => {
