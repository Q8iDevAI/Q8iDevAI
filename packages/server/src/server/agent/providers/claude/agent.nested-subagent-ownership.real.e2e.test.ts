...
const ROOT_PROMPT = `You are ROOT_OWNER. Use Claude Code's native Agent tool exactly once, never Q8iDevAI tools.
...
test("attributes a nested Claude child and its background notification to their direct owners", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-claude-nested-ownership-"));
