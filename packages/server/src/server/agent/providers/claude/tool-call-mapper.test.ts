  ...
  it("maps Glob calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        name: "Glob",
        input: { pattern: "**/.claude/commands/q8idevai*" },
        output: {
          durationMs: 7,
          numFiles: 2,
    ...
    expect(item.detail).toEqual({
      type: "search",
      query: "**/.claude/commands/q8idevai*",
      toolName: "glob",
  ...
  it("maps Grep calls as search detail using pattern input", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        input: {
          pattern: '\\\\\\"cli\\\\\\""',
          path: "/workspaces/q8idevai/packages/desktop/src",
          output_mode: "content",
          "-n": true,
        },
        output: {
  ...
  it("normalizes namespaced voice MCP speak tools", () => {
    const item = expectMapped(
      mapClaudeCompletedToolCall({
        ...
        name: "mcp__q8idevai_voice__speak",
