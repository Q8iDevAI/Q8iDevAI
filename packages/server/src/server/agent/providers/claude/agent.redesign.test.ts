...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "Q8IDEVAI_ENV_SENTINEL_SECRET";
  const runtimeSecret = "Q8IDEVAI_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "Q8IDEVAI_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.Q8IDEVAI_TEST_SENTINEL_SECRET;
  process.env.Q8IDEVAI_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        Q8IDEVAI_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.Q8IDEVAI_TEST_SENTINEL_SECRET;
    } else {
      process.env.Q8IDEVAI_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
