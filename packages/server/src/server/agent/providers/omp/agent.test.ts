...
function createToolCatalog(): Q8iDevAIToolCatalog {
  return {
    tools: new Map([
      [
        "create_agent",
...
describe("OMP agent client and session", () => {
  test("owns launch configuration and registers native host tools", async () => {
    ...
    expect(omp.launchConfiguration()).toEqual({
      cwd: "/tmp/q8idevai-omp-agent-test",
      protocolMode: "rpc-ui",
      modeId: "ask",
  ...
  test("resumes an OMP session and replays its history", async () => {
    ...
    expect(omp.launchConfiguration()).toEqual({
      ...
      protocolMode: "rpc-ui",
      modeId: "ask",
      session: expect.stringMatching(/[\\/]q8idevai-omp-resume-.*[\\/]session\.jsonl$/),
      argv: [
        "omp",
        "--mode",
        "rpc-ui",
        ...
        "high",
        "--session",
        expect.stringMatching(/[\\/]q8idevai-omp-resume-.*[\\/]session\.jsonl$/),
      ],
    });
