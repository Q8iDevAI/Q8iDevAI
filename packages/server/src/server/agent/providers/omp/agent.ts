...
export class OmpAgentSession implements AgentSession {
  ...
  constructor(options: OmpAgentSessionOptions) {
    ...
    this.q8idevaiTools = options.q8idevaiTools;
  ...
  private readonly q8idevaiTools?: Q8iDevAIToolCatalog;
  ...
  private handleExtraRuntimeEvent(event: OmpRuntimeEvent): boolean {
    if (
      handleOmpHostToolRuntimeEvent(event, {
        runtimeSession: this.runtimeSession,
        q8idevaiTools: this.q8idevaiTools,
        logger: this.logger,
      })
    ) {
      return true;
  ...
  private handleSessionEvent(event: OmpAgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case "agent_end": {
        ...
        // so do not advertise Q8iDevAI idle until it reports that transition.
...
export class OmpAgentClient implements AgentClient {
  ...
  private async configureNativeQ8iDevAITools(
    runtimeSession: OmpRuntimeSession,
    catalog: Q8iDevAIToolCatalog | undefined,
  ): Promise<void> {
    if (!catalog) {
      return;
    }
  ...
  async createSession(
    ...
    try {
      await this.configureNativeQ8iDevAITools(runtimeSession, launchContext?.q8idevaiTools);
      return new OmpAgentSession({
        ...
        q8idevaiTools: launchContext?.q8idevaiTools,
      });
    } catch (error) {
