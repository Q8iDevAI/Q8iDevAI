...
import type { Q8iDevAIToolCatalog, Q8iDevAIToolResult } from "../../tools/types.js";
...
interface OmpHostToolRouterInput {
  ...
  catalog: Q8iDevAIToolCatalog;
  logger: Logger;
}

const routersByRuntimeSession = new WeakMap<OmpRuntimeSession, OmpHostToolRouter>();

export function serializeOmpHostTools(catalog: Q8iDevAIToolCatalog): OmpRpcHostToolDefinition[] {
  return [...catalog.tools.values()].map((tool) => {
    const definition: OmpRpcHostToolDefinition = {
      name: tool.name,
      description: tool.description,
      loadMode: "essential",
      parameters: serializeQ8iDevAIToolInputParameters(tool),
    };
    if (tool.title) {
...
export function handleOmpHostToolRuntimeEvent(
  ...
  input: {
    runtimeSession: OmpRuntimeSession;
    q8idevaiTools?: Q8iDevAIToolCatalog;
    logger: Logger;
  },
): boolean {
...
function sendMissingCatalogResult(
  ...
  runtimeSession.sendHostToolResult(
    toOmpHostToolErrorResult(
      request.id,
      `Host tool "${request.toolName}" was called before Q8iDevAI tools were registered`,
    ),
  );
}
...
class OmpHostToolRouter {
  ...
  private readonly catalog: Q8iDevAIToolCatalog;
  ...
  private sendUpdate(callId: string, result: Q8iDevAIToolResult): void {
...
function toOmpHostToolResult(id: string, result: Q8iDevAIToolResult): OmpRpcHostToolResult {
...
function toOmpAgentToolResult(result: Q8iDevAIToolResult): OmpAgentToolResult {
