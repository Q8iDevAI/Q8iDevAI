...
import type { Q8iDevAIToolCatalog, Q8iDevAIToolDefinition, Q8iDevAIToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: Q8iDevAIToolDefinition[]): Q8iDevAIToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: Q8iDevAIToolResult) => void) | null = null;
  ...
  static async withTools(tools: Q8iDevAIToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: Q8iDevAIToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Q8iDevAI agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<Q8iDevAIToolResult>((resolve) => {
  ...
  completeControlledCall(result: Q8iDevAIToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, q8idevaiTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped Q8iDevAI tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create a Q8iDevAI agent.",
