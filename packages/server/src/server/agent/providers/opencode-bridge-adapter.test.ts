...
describe("OpenCode bridge adapter", () => {
  ...
  test("shares the server and binds exact managed env per OpenCode session", async () => {
    const q8idevaiHome = await mkdtemp(path.join(os.tmpdir(), "q8idevai-opencode-adapter-"));
    ...
    const first = await client.createSession(
      ...
      {
        agentId: "agent-one",
        env: { Q8IDEVAI_AGENT_ID: "agent-one", Q8IDEVAI_AGENT_CWD: "/workspace/one" },
      },
    );
    const second = await client.createSession(
      ...
      {
        agentId: "agent-two",
        env: { Q8IDEVAI_AGENT_ID: "agent-two", Q8IDEVAI_AGENT_CWD: "/workspace/two" },
      },
    );

    expect(client.capabilities.supportsNativeQ8iDevAITools).toBe(true);
  ...
  test("keeps process-scoped env and directory-scoped MCP on dedicated servers", async () => {
    ...
    const customEnv = await client.createSession(
      { provider: "opencode", cwd: "/workspace/one" },
      { env: { Q8IDEVAI_AGENT_ID: "one", CUSTOM_TOKEN: "secret" } },
    );
