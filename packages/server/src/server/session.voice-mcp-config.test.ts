...
describe("voice MCP stdio config", () => {
  test("builds stdio MCP config for voice agent", () => {
    const config = buildVoiceAgentMcpServerConfig({
      ...
      socketPath: "/tmp/q8idevai-voice.sock",
      env: {
        ELECTRON_RUN_AS_NODE: "1",
        Q8IDEVAI_HOME: "/tmp/q8idevai-home",
      },
    });
...
describe("voice mode prompt instructions", () => {
  test("builds enabled voice instructions and preserves base prompt", () => {
    ...
    expect(prompt).toContain("<q8idevai_voice_mode>");
    expect(prompt).toContain("Q8iDevAI voice mode is now on.");
  ...
  test("builds disabled voice instructions and supersedes previous voice block", () => {
    ...
    expect(prompt).toContain("Base system prompt");
    expect(prompt).toContain("Q8iDevAI voice mode is now off.");
    expect(prompt).toContain("Ignore any earlier Q8iDevAI voice mode instructions in this thread.");
    expect(prompt.match(/<q8idevai_voice_mode>/g)?.length ?? 0).toBe(1);
  ...
  test("strips voice blocks from persisted prompt", () => {
    ...
    expect(
      stripVoiceModeSystemPrompt(
        ["<q8idevai_voice_mode>", "legacy voice instruction", "</q8idevai_voice_mode>"].join("\n\n"),
      ),
    ).toBeUndefined();
  });
});
