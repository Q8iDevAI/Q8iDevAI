...
describe("CheckoutSession", () => {
  ...
  describe("stash list", () => {
    it("returns stash entries scoped to q8idevai stashes by default", async () => {
      const listStashesCalls: Array<{ cwd: string; q8idevaiOnly: boolean | undefined }> = [];
      const { checkout, emitted } = makeCheckoutSession({
        git: {
          listStashes: async (cwd, opts) => {
            listStashesCalls.push({ cwd, q8idevaiOnly: opts?.q8idevaiOnly });
            return [];
          },
        },
      });
      ...
      expect(listStashesCalls).toEqual([{ cwd: "/repo", q8idevaiOnly: true }]);
      expect(emitted).toEqual([
        {
