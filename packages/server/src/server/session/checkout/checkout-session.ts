...
import { getForgeDefinitionOrNeutral } from "@q8idevai/protocol/forge-manifest";
import { validateBranchSlug } from "@q8idevai/protocol/branch-slug";
import type {
...
export class CheckoutSession {
  private static readonly Q8IDEVAI_STASH_PREFIX = "q8idevai-auto-stash:";
  ...
  async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${CheckoutSession.Q8IDEVAI_STASH_PREFIX} ${branchLabel}`
        : `${CheckoutSession.Q8IDEVAI_STASH_PREFIX} unnamed`;
  ...
  async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const q8idevaiOnly = msg.q8idevaiOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { q8idevaiOnly });
  ...
  async handleCheckoutMergeRequest(
    ...
    try {
      ...
      const mutatedCwd = await mergeToBase(
        ...
        { q8idevaiHome: this.q8idevaiHome, worktreesRoot: this.worktreesRoot },
      );
      await Promise.all([
