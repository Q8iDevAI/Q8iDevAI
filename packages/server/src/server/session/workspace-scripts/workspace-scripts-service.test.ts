...
describe("buildSnapshot", () => {
  ...
  test("returns no scripts for a workspace without a q8idevai.json", async () => {
  ...
  test("refuses to start a script while repository automation is blocked", async () => {
    const { service, emitted, spawnCalls } = buildService({
      automationError: new Error(
        "Scripts are blocked for PR #42 from contributor/q8idevai. Run setup to allow them.",
      ),
    });
    ...
    expect(emitted).toContainEqual({
      ...
      payload: {
        ...
        scriptName: "app",
        terminalId: null,
        error: "Scripts are blocked for PR #42 from contributor/q8idevai. Run setup to allow them.",
      },
    });
  });
