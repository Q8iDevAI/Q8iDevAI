...
} from "../../../utils/q8idevai-config-file.js";
...
/**
 * A client's read/write surface for a project's on-disk q8idevai.json. Resolves the
...
export class ProjectConfigSession {
  ...
  async handleReadProjectConfigRequest(
    ...
    const result = readQ8iDevAIConfigForEdit(repoRoot);
    if (!result.ok) {
      this.logger.warn(
