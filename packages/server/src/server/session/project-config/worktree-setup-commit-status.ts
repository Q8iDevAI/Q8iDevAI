...
export async function hasUncommittedWorktreeSetupChanges(input: {
  repoRoot: string;
  currentConfig: Q8iDevAIConfigRaw | null;
}): Promise<boolean> {
...
async function resolveConfigGitPath(repoRoot: string): Promise<string> {
  ...
  return `${stdout.trim()}q8idevai.json`;
}

async function readCommittedConfig(
  repoRoot: string,
  gitPath: string,
): Promise<Q8iDevAIConfigRaw | null> {
  ...
  return Q8iDevAIConfigRawSchema.parse(JSON.parse(stdout));
}
