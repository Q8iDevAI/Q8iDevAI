...
import type { Q8iDevAIConfigRaw } from "@q8idevai/protocol/q8idevai-config-schema";
...
function makeGitRepo(config?: Q8iDevAIConfigRaw): string {
...
function writeConfig(repoRoot: string, config: Q8iDevAIConfigRaw): void {
  writeFileSync(join(repoRoot, "q8idevai.json"), `${JSON.stringify(config, null, 2)}\n`);
}
...
describe("worktree setup commit status", () => {
  ...
  test("reports modified and staged setup as uncommitted", async () => {
    ...
    execFileSync("git", ["add", "q8idevai.json"], { cwd: repoRoot });
    await expect(hasUncommittedWorktreeSetupChanges({ repoRoot, currentConfig })).resolves.toBe(
      true,
    );
  });
  ...
  test("resolves q8idevai.json relative to a nested project root", async () => {
    const repoRoot = makeGitRepo();
    const projectRoot = join(repoRoot, "packages", "app");
    mkdirSync(projectRoot, { recursive: true });
    writeConfig(projectRoot, { worktree: { setup: "npm ci" } });
    execFileSync("git", ["add", "packages/app/q8idevai.json"], { cwd: repoRoot });
