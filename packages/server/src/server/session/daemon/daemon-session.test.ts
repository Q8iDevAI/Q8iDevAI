...
function makeSubsystem(overrides: {
  ...
  return { subsystem, emitted, q8idevaiHome, restartIntents };
}
...
describe("DaemonSession", () => {
  ...
  test("diagnostics includes a log tail and redacts connection secrets", async () => {
    const { subsystem, emitted, q8idevaiHome } = makeSubsystem({
      serverId: "srv-1",
      daemonVersion: "1.2.3",
      daemonRuntimeConfig: {
        listen: "127.0.0.1:6767",
    ...
    writeFileSync(
      join(q8idevaiHome, "daemon.log"),
      "first line\nrelay.secret.test:443 token=super-secret q8idevai://pairing-secret\n",
    );
  ...
  test("diagnostics includes the PATH and shell visible to the daemon", async () => {
    ...
    const originalCOMSPEC = process.env.COMSPEC;
    try {
      process.env.PATH = "/opt/q8idevai-test/bin:/usr/bin";
      process.env.SHELL = "/bin/q8idevai-test-shell";
      ...
      expect(message.payload.diagnostic).toContain("PATH: /opt/q8idevai-test/bin:/usr/bin");
      expect(message.payload.diagnostic).toContain("Shell: SHELL=/bin/q8idevai-test-shell");
    } finally {
