...
import { Q8IDEVAI_CLI_PACKAGE, type NpmGlobalQ8iDevAIInstall } from "./npm-global-cli.js";
...
export function validateDaemonInstallOrigin(
  install: NpmGlobalQ8iDevAIInstall,
  daemonVersion: string | null,
  runtime: DaemonInstallOriginRuntime = daemonInstallOriginRuntime,
): string | null {
  if (install.isLinked) {
    return `The global ${Q8IDEVAI_CLI_PACKAGE} install is linked; self-update only supports normal npm global installs.`;
  }

  if (daemonVersion && install.version !== daemonVersion) {
    return `This daemon is not running from the npm global ${Q8IDEVAI_CLI_PACKAGE} install (global npm has ${install.version}, daemon is ${daemonVersion}).`;
  }
  ...
  if (!isCurrentServerUnderNpmInstall(currentServerPackageRoot, install)) {
    return `This daemon is not running from the npm global ${Q8IDEVAI_CLI_PACKAGE} install.`;
  }
...
function resolveCurrentServerPackageRoot(): string | null {
  return resolvePackageRootFrom(fileURLToPath(import.meta.url), "@q8idevai/server");
}
