import { getErrorMessage } from "@q8idevai/protocol/error-utils";
import { z } from "zod";
import { execCommand } from "../../../utils/spawn.js";

export const Q8IDEVAI_CLI_PACKAGE = "@q8idevai/cli";
...
export interface NpmGlobalQ8iDevAIInstall {
  version: string;
  packagePath: string;
...
export interface NpmGlobalQ8iDevAICli {
  inspect(): Promise<NpmGlobalQ8iDevAIInstall>;
...
function parseNpmGlobalQ8iDevAIInstall(stdout: string): NpmGlobalQ8iDevAIInstall | null {
  ...
  const rawCliPackage = list.data.dependencies?.[Q8IDEVAI_CLI_PACKAGE];
...
export class DefaultNpmGlobalQ8iDevAICli implements NpmGlobalQ8iDevAICli {
  constructor(private readonly runCommand: CommandRunner = runExternalCommand) {}

  async inspect(): Promise<NpmGlobalQ8iDevAIInstall> {
    const result = await this.runCommand(
      "npm",
      ["-g", "ls", Q8IDEVAI_CLI_PACKAGE, "--json", "--depth=0", "--long"],
      {
    ...
    const install = parseNpmGlobalQ8iDevAIInstall(result.stdout);
    if (!install) {
      throw new Error(`${Q8IDEVAI_CLI_PACKAGE} is not installed with npm -g on this host`);
    }
    return install;
  }

  installLatest(): Promise<CommandResult> {
    return this.runCommand("npm", ["install", "-g", `${Q8IDEVAI_CLI_PACKAGE}@latest`], {
...
export const npmGlobalQ8iDevAICli = new DefaultNpmGlobalQ8iDevAICli();
