...
function createWorkspaceRepo(options?: {
  branchName?: string;
  q8idevaiConfig?: Record<string, unknown>;
...
describe("script-route-branch-handler", () => {
  it("updates routes on branch rename by removing old hostnames and registering new ones", () => {
    ...
    registerRoute(routeStore, {
      hostname: "api--feature-auth--q8idevai.localhost",
      port: 3001,
      scriptName: "api",
    });
    ...
    expect(routeStore.findRoute("api--feature-auth--q8idevai.localhost")).toBeNull();
    expect(routeStore.findRoute("api--feature-billing--q8idevai.localhost")).toEqual({
      hostname: "api--feature-billing--q8idevai.localhost",
      port: 3001,
    });
  });
  ...
  it("is a no-op when the resolved hostnames do not change", () => {
    const routeStore = new ScriptRouteStore();
    registerRoute(routeStore, {
      hostname: "api--q8idevai.localhost",
      port: 3001,
      scriptName: "api",
    });
  ...
  it("updates public route aliases from the stored public base URL", () => {
    ...
    registerRoute(routeStore, {
      hostname: "api--feature-auth--q8idevai.localhost",
      publicHostname: "api--feature-auth--q8idevai.services.example.com",
    ...
    expect(routeStore.findRoute("api--feature-auth--q8idevai.services.example.com")).toBeNull();
    expect(routeStore.findRoute("api--feature-billing--q8idevai.services.example.com")).toEqual({
      hostname: "api--feature-billing--q8idevai.localhost",
      port: 3001,
    });
    expect(routeStore.listRoutesForWorkspace("workspace-a")).toEqual([
      {
        hostname: "api--feature-billing--q8idevai.localhost",
        publicHostname: "api--feature-billing--q8idevai.services.example.com",
        publicBaseUrl: "https://services.example.com:8443",
        port: 3001,
        workspaceId: "workspace-a",
        projectSlug: "q8idevai",
  ...
  it("updates all services for a workspace when multiple routes are registered", () => {
    ...
    registerRoute(routeStore, {
      hostname: "web--feature-auth--q8idevai.localhost",
      port: 3002,
      scriptName: "web",
    });
    registerRoute(routeStore, {
    ...
    expect(routeStore.listRoutesForWorkspace("workspace-a")).toEqual([
      ...
      {
        hostname: "web--feature-billing--q8idevai.localhost",
        port: 3002,
        workspaceId: "workspace-a",
        projectSlug: "q8idevai",
        scriptName: "web",
  ...
  it("does not emit a status update when no changes are needed", () => {
    const routeStore = new ScriptRouteStore();
    registerRoute(routeStore, {
      hostname: "web--q8idevai.localhost",
      port: 3002,
      scriptName: "web",
    });
