...
import { createQ8iDevAIDaemon, type Q8iDevAIDaemon, type Q8iDevAIDaemonConfig } from "../../bootstrap.js";
...
export class HubRelationshipHarness {
  ...
  private daemon: Q8iDevAIDaemon | null = null;
  private config!: Q8iDevAIDaemonConfig;
  private root = "";
  private q8idevaiHome = "";
  private host = "";
  ...
  async relationshipStateBecomes(expected: string | null): Promise<void> {
    ...
    const watcher = watch(this.q8idevaiHome, () => {
  ...
  async durableOwnedAgentIdsOnDisk(): Promise<string[]> {
    const storage = new AgentStorage(
      path.join(this.q8idevaiHome, "agents"),
      pino({ level: "silent" }),
    );
  ...
  async hubExecutionIntentFiles(): Promise<string[]> {
    const directory = path.join(this.q8idevaiHome, "hub-executions");
  ...
  private workspaceArchivedAt(workspaceId: string): string | null {
    const records = JSON.parse(
      readFileSync(path.join(this.q8idevaiHome, "projects", "workspaces.json"), "utf8"),
  ...
  relationshipFile(): PersistedRelationship | null {
    const file = path.join(this.q8idevaiHome, "hub-relationship.json");
    if (!existsSync(file)) return null;
    return JSON.parse(readFileSync(file, "utf8")) as PersistedRelationship;
  }

  relationshipFileMode(): number {
    return statSync(path.join(this.q8idevaiHome, "hub-relationship.json")).mode & 0o777;
  }

  async corruptRelationshipFile(contents = "{not-json"): Promise<void> {
    await this.stopDaemon();
    await writeFile(path.join(this.q8idevaiHome, "hub-relationship.json"), contents, "utf8");
  }

  async quarantinedRelationshipFiles(): Promise<string[]> {
    return (await readdir(this.q8idevaiHome)).filter((file) =>
  ...
  private async createHome(): Promise<void> {
    this.root = await mkdtemp(path.join(tmpdir(), "q8idevai-hub-relationship-"));
    this.q8idevaiHome = path.join(this.root, ".q8idevai");
    const staticDir = path.join(this.root, "static");
    await Promise.all([mkdir(this.q8idevaiHome, { recursive: true }), mkdir(staticDir)]);
    ...
    this.config = {
      ...
      agentStoragePath: path.join(this.q8idevaiHome, "agents"),
      relayEnabled: false,
  ...
  private async startDaemon(): Promise<void> {
    ...
    this.daemon = await createQ8iDevAIDaemon(this.config, pino({ level: "trace" }, destination), {
