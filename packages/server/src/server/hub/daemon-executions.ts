...
export class DaemonExecutions implements HubExecutionAgents {
  ...
  private async createOrResolve(
    ...
    let createdWorktree: CreateQ8iDevAIWorktreeWorkflowResult | null = null;
...
function requireHubMcpNamespace(mcpServers: Record<string, McpServerConfig> | undefined): void {
  if (mcpServers && Object.hasOwn(mcpServers, "q8idevai")) {
    throw new Error('Hub execution MCP server name "q8idevai" is reserved by the daemon');
  }
}
...
function ownedCreatedWorktree(
  worktree: CreateQ8iDevAIWorktreeWorkflowResult | null,
): CreateQ8iDevAIWorktreeWorkflowResult | null {
