...
const openQ8iDevAIHomes: string[] = [];
...
afterEach(async () => {
  ...
  await Promise.all(openQ8iDevAIHomes.splice(0).map((home) => rm(home, { recursive: true })));
});
...
test.each([
  ...
  async ({ acknowledge, expected }) => {
    ...
    if (acknowledge) {
      webSockets.on("headers", (headers) => headers.push("x-q8idevai-session-protocol: 1"));
    }
    let offeredProtocol: string | string[] | undefined;
    server.on("upgrade", (request, socket, head) => {
      offeredProtocol = request.headers["x-q8idevai-session-protocol"];
...
async function connectController(
  ...
  const q8idevaiHome = await mkdtemp(path.join(tmpdir(), "q8idevai-hub-socket-"));
  openQ8iDevAIHomes.push(q8idevaiHome);
