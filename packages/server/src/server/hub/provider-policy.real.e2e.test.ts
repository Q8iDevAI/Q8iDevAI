...
import type { AgentStreamEvent } from "@q8idevai/protocol/agent-types";
import type { HubExecutionAgentCreateResponse } from "@q8idevai/protocol/messages";
...
const RUN_REAL_HUB_POLICY = process.env.Q8IDEVAI_REAL_HUB_POLICY === "1";
...
describe.skipIf(!RUN_REAL_HUB_POLICY)("Hub provider policy (real providers)", () => {
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-codex-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-codex-denied-"));
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-claude-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-claude-denied-"));
      const allowedFile = path.join(allowedRoot, "allowed.txt");
      const deniedFile = path.join(deniedRoot, "denied.txt");
      const providerOptions = {
