...
test("Hub MCP configuration reaches the provider alongside Q8iDevAI MCP without entering snapshots", async () => {
...
test("new Hub executions cannot override the daemon-owned Q8iDevAI MCP server", async () => {
  const hub = await launchRelationship();
  hub.beginOwnedCreate("reserved-mcp-create", "reserved-mcp-execution", {
    mcpServers: {
      q8idevai: { type: "http", url: "https://hub.test/replace-q8idevai" },
    },
  });
...
test("reserved Q8iDevAI MCP input does not invalidate replay of an owned execution", async () => {
  const hub = await launchRelationship();
