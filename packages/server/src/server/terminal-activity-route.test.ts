...
it("accepts terminalId and token reports through the route into the tracker", async () => {
  ...
  const session = await manager.createTerminal({
    ...
    args: [
      "-e",
      `require("node:fs").writeFileSync(${JSON.stringify(envPath)}, JSON.stringify({ terminalId: process.env.Q8IDEVAI_TERMINAL_ID, token: process.env.Q8IDEVAI_ACTIVITY_TOKEN, url: process.env.Q8IDEVAI_TERMINAL_ACTIVITY_URL })); setInterval(() => {}, 1000);`,
    ],
  });
