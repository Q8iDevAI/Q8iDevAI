...
import { isLikelyExternalToolName } from "@q8idevai/protocol/tool-name-normalization";
...
class FakeAgentSession implements AgentSession {
  ...
  constructor(options: FakeAgentSessionOptions) {
    ...
    this.historyPath = path.join(
      tmpdir(),
      "q8idevai-fake-provider-history",
      this.providerName,
      `${this.id}.jsonl`,
    );
  }
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `Q8IDEVAI_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `Q8IDEVAI_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }

    return {
      text: "Q8IDEVAI_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "Q8IDEVAI_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
