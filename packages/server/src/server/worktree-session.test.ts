...
import {
  ...
  createQ8iDevAIWorktreeWorkflow,
  handleQ8iDevAIWorktreeArchiveRequest,
  handleQ8iDevAIWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateQ8iDevAIWorktreeRequest,
...
import {
  createQ8iDevAIWorktree as createQ8iDevAIWorktreeService,
  type CreateQ8iDevAIWorktreeFn,
...
function createWorkflowForRequestTest(options: {
  q8idevaiHome: string;
  createQ8iDevAIWorktree?: CreateQ8iDevAIWorktreeFn;
  ...
  return async (input: Parameters<CreateQ8iDevAIWorktreeFn>[0]) => {
    const createQ8iDevAIWorktree =
      options.createQ8iDevAIWorktree ?? createQ8iDevAIWorktreeForTest({ q8idevaiHome: options.q8idevaiHome });
...
function createQ8iDevAIWorktreeForTest(options: {
  q8idevaiHome: string;
  events?: string[];
}): CreateQ8iDevAIWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createQ8iDevAIWorktreeService(input, {
...
describe("handleQ8iDevAIWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/q8idevai-home/worktrees/repo/feature",
    ...
    await handleQ8iDevAIWorktreeListRequest(
      {
        emit: (message) => emitted.push(message),
        q8idevaiHome: "/tmp/q8idevai-home",
        workspaceGitService: workspaceGitService as unknown as WorkspaceGitService,
      },
      {
        type: "q8idevai_worktree_list_request",
        cwd: "/tmp/repo",
    ...
    expect(emitted).toContainEqual({
      type: "q8idevai_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/q8idevai-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
...
describe("create-agent worktree setup boundary", () => {
  test("blocked worktrees keep their workspace but skip setup and automatic terminals", async () => {
    ...
    const emitted: SessionOutboundMessage[] = [];
    writeFileSync(
      path.join(repoDir, "q8idevai.json"),
      JSON.stringify({
        worktree: {
    ...
    try {
      const result = await createQ8iDevAIWorktreeWorkflow(
        {
          q8idevaiHome,
          createQ8iDevAIWorktree: createQ8iDevAIWorktreeForTest({ q8idevaiHome }),
          ...
          assertWorkspaceAutomationAllowed: async () => {
            throw new WorkspaceAutomationBlockedError({
              ...
              forge: "github",
              number: 42,
              headRepository: "contributor/q8idevai",
            });
          },
          emitWorkspaceUpdateForWorkspaceId: async () => {},
          cacheWorkspaceSetupSnapshot: () => {},
        ...
        { cwd: repoDir, worktreeSlug: "blocked-fork", runSetup: false, q8idevaiHome },
        {
          setupContinuation: {
            kind: "agent",
            terminalManager: createTerminalManagerStub().manager,
...
function createGitRepo(options?: { q8idevaiConfig?: Record<string, unknown> }) {
  ...
  if (options?.q8idevaiConfig) {
    writeFileSync(path.join(repoDir, "q8idevai.json"), JSON.stringify(options.q8idevaiConfig, null, 2));
  }
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("runs setup from an exact workspace subdirectory", async () => {
    ...
    writeFileSync(
      path.join(sourceWorkspaceCwd, "q8idevai.json"),
      JSON.stringify({
        worktree: {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "q8idevai.json"), "{ invalid json\n");
    execFileSync("git", ["add", "q8idevai.json"], { cwd: repoDir, stdio: "pipe" });
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse q8idevai\.json at .*q8idevai\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse q8idevai\.json at .*q8idevai\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.q8idevai/worktrees/feature-a",
  ...
  test("reports persisted fork provenance as blocked after restart", async () => {
    ...
    expect(emitted[0]).toMatchObject({
      ...
      payload: {
        snapshot: {
          status: "blocked",
          blockedSource: { number: 42, headRepository: "contributor/q8idevai" },
        },
      },
    });
  });
  ...
  test("run setup clears the block and starts the existing setup runtime once", async () => {
    ...
    const setupMarker = path.join(tempDir, "setup-ran");
    writeFileSync(
      path.join(tempDir, "q8idevai.json"),
      JSON.stringify({
        worktree: {
...
describe("handleCreateQ8iDevAIWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateQ8iDevAIWorktreeRequest(
      {
        q8idevaiHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createQ8iDevAIWorktreeWorkflow: createWorkflowForRequestTest({ q8idevaiHome }),
      },
      {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_q8idevai_worktree_response" }> =>
        message.type === "create_q8idevai_worktree_response",
    );
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createQ8iDevAIWorktree = vi.fn(async () => ({
      worktree: {
    ...
    expect(createQ8iDevAIWorktree).toHaveBeenCalledWith(
      expect.objectContaining({
        firstAgentContext,
      }),
      expect.anything(),
  ...
  test("createQ8iDevAIWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createQ8iDevAIWorktreeForTest({ q8idevaiHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateQ8iDevAIWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createQ8iDevAIWorktreeWorkflow: createWorkflowForRequestTest({
            q8idevaiHome,
            createQ8iDevAIWorktree: createQ8iDevAIWorktreeForTest({ q8idevaiHome, events }),
          }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateQ8iDevAIWorktreeRequest(
        {
          ...
          createQ8iDevAIWorktreeWorkflow: createWorkflowForRequestTest({
            q8idevaiHome,
            createQ8iDevAIWorktree: async (input) => {
              const result = await createQ8iDevAIWorktreeForTest({ q8idevaiHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_q8idevai_worktree_response",
        "workspace_update",
      ]);
...
describe("handleQ8iDevAIWorktreeArchiveRequest worktree scope", () => {
  const cleanupPaths: string[] = [];
  ...
  test("archives every active workspace on the directory and removes it", async () => {
    ...
    await handleQ8iDevAIWorktreeArchiveRequest(
      {
        q8idevaiHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
      ...
      {
        type: "q8idevai_worktree_archive_request",
    ...
    expect(
      emitted.find((message) => message.type === "q8idevai_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("ignores deleteWorktreeFromDisk:true and derives directory removal from remaining references", async () => {
    ...
    await handleQ8iDevAIWorktreeArchiveRequest(deps, {
      type: "q8idevai_worktree_archive_request",
    ...
    expect(existsSync(sharedCwd)).toBe(false);
    expect(
      emitted.filter((message) => message.type === "q8idevai_worktree_archive_response"),
    ).toHaveLength(2);
  });
});
