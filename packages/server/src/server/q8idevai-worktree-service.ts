...
import {
  ...
  rollbackCreatedQ8iDevAIWorktree,
  seedQ8iDevAIConfigFile,
  validateBranchSlug,
  type WorktreeConfig,
} from "../utils/worktree.js";
import { getCurrentBranch, localBranchExists, renameCurrentBranch } from "../utils/checkout-git.js";
import {
  markQ8iDevAIWorktreeFirstAgentBranchAutoNameAttempted,
...
import type { FirstAgentContext } from "@q8idevai/protocol/messages";
import { runWithGitCommandPriority } from "../utils/run-git-command.js";

export interface CreateQ8iDevAIWorktreeInput extends CreateWorktreeCoreInput {
  workspaceId?: string;
  projectId?: string;
  title?: string;
}

export interface CreateQ8iDevAIWorktreeResult {
...
export type CreateQ8iDevAIWorktreeFn = (
...
export interface CreateQ8iDevAIWorktreeDeps extends CreateWorktreeCoreDeps {
  workspaceGitService: WorkspaceGitService;
  workspaceProvisioning: Pick<WorkspaceProvisioningService, "createWorkspaceForWorktree">;
}

export async function createQ8iDevAIWorktree(
  input: CreateQ8iDevAIWorktreeInput,
  deps: CreateQ8iDevAIWorktreeDeps,
): Promise<CreateQ8iDevAIWorktreeResult> {
  return runWithGitCommandPriority("high", () => createQ8iDevAIWorktreeWithPriority(input, deps));
}

async function createQ8iDevAIWorktreeWithPriority(
  input: CreateQ8iDevAIWorktreeInput,
  deps: CreateQ8iDevAIWorktreeDeps,
): Promise<CreateQ8iDevAIWorktreeResult> {
  ...
  try {
    ...
    return rollbackCreatedQ8iDevAIWorktree(
      {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readQ8iDevAIWorktreeMetadata>;
  try {
    metadata = readQ8iDevAIWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markQ8iDevAIWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeQ8iDevAIWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
