...
const RUN = process.env.Q8IDEVAI_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const q8idevaiHomeRoot = mkdtempSync(path.join(tmpdir(), "q8idevai-speech-download-"));
    const modelsDir = path.join(q8idevaiHomeRoot, ".q8idevai", "models", "local-speech");
