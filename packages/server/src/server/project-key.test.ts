...
describe("deriveProjectKey", () => {
  ...
  test.each([
    ...
    "ssh://git@github.com/Q8iDevAI/Q8iDevAI.git",
    "git@github.com:Q8iDevAI/Q8iDevAI.git",
  ])("normalizes common remote form %s", (remoteUrl) => {
    expect(derive(remoteUrl)).toBe("remote:github.com/q8idevai/q8idevai");
  });

  test("normalizes GitHub casing", () => {
    expect(derive("git@github.com:Q8iDevAI/Q8iDevAI.git")).toBe("remote:github.com/q8idevai/q8idevai");
  });
  ...
  test("groups an SSH remote whether or not its default port is spelled out", () => {
    expect(derive("ssh://git@github.com:22/Q8iDevAI/Q8iDevAI.git")).toBe(
      derive("ssh://git@github.com/Q8iDevAI/Q8iDevAI.git"),
    );
  });
  ...
  test("keeps a repository root distinct from one of its subprojects", () => {
    ...
    const remoteUrl = "git@github.com:getq8idevai/q8idevai.git";

    expect(
      deriveProjectKey({ rootPath: worktreeRoot, remoteUrl, worktreeRoot, mainRepoRoot: null }),
    ).toBe("remote:github.com/Q8iDevAI/Q8iDevAI");
    expect(
      deriveProjectKey({
    ...
    ).toBe("remote:github.com/Q8iDevAI/Q8iDevAI#subdir:packages/app");
  });
