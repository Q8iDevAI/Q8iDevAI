...
// derived from scope + last-reference + Q8iDevAI ownership; there is no caller-
// supplied disk flag.
...
test("archiving the last reference to a worktree removes it from disk regardless of the disk flag", async () => {
  ...
  const keepArchive = await ctx.client.archiveQ8iDevAIWorktree({ worktreePath: keepDir });
  ...
  const deleteArchive = await ctx.client.archiveQ8iDevAIWorktree({ worktreePath: deleteDir });
...
test("worktree archive targets the explicit workspaceId when a directory backs multiple workspaces", async () => {
  ...
  await ctx.client.archiveQ8iDevAIWorktree({
    worktreePath: worktreeDir,
