...
import type { AgentSkillSelection } from "@q8idevai/protocol/messages";

export type { MutableDaemonConfig, MutableDaemonConfigPatch } from "@q8idevai/protocol/messages";

type MutableDaemonConfig = import("@q8idevai/protocol/messages").MutableDaemonConfig;
type MutableDaemonConfigPatch = import("@q8idevai/protocol/messages").MutableDaemonConfigPatch;
...
export function applyMutableProviderConfigToOverrides(
  ...
  for (const [providerId, providerConfig] of Object.entries(mutableProviders ?? {})) {
    ...
    nextOverrides[providerId] = {
      ...previousOverride,
      ...parsedOverride,
      ...(parsedOverride.q8idevaiTools
        ? {
            q8idevaiTools: {
              ...previousOverride?.q8idevaiTools,
              ...parsedOverride.q8idevaiTools,
            },
          }
        : {}),
...
export class DaemonConfigStore {
  ...
  constructor(
    ...
    this.startupPersisted = options.startupPersisted ?? loadPersistedConfig(q8idevaiHome, this.logger);
  ...
  private applySupportedPatch(parsedPatch: SupportedMutableConfigPatch): MutableDaemonConfig {
    if (parsedPatch.relay?.enabled !== undefined && !this.relayEnabledMutable) {
      throw new Error(
        "Relay is controlled by a daemon launch override. Remove Q8IDEVAI_RELAY_ENABLED or the relay CLI flag before changing it here.",
      );
    }
    ...
    try {
      ...
      savePersistedConfig(this.q8idevaiHome, persistedBeforePatch, this.logger);
      throw error;
    }
  ...
  public reload(): DaemonConfigReloadResult {
    ...
    const persisted = loadPersistedConfig(this.q8idevaiHome, this.logger);
  ...
  private persistConfig(
    ...
    savePersistedConfig(this.q8idevaiHome, nextPersisted, this.logger);
