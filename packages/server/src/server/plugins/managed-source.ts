...
import { z } from "zod";
import { PluginIdSchema, type PluginSourceStatusItem } from "@q8idevai/protocol/messages";
...
export class ManagedPluginSources {
  ...
  constructor(q8idevaiHome: string) {
    this.root = path.join(q8idevaiHome, "plugins");
...
async function clone(remote: string, checkoutRoot: string): Promise<void> {
  ...
  const cloneRemote = publicRemote === remote ? remote : "https://q8idevai.invalid/plugin.git";
  const envOverlay =
    cloneRemote === remote
      ? GIT_ENV
      : {
