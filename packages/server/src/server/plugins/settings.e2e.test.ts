...
import { z } from "zod";
import { defineRpc, settingsRpc } from "@q8idevai/plugin";
...
export default function(server) {
  ...
  const options = {
    q8idevaiHomeRoot: path.join(root, "daemon"),
  ...
  const first = await createTestQ8iDevAIDaemon(options);
  ...
  const restarted = await createTestQ8iDevAIDaemon(options);
...
test("two clients share settings, observe changes, and preserve values through plugin lifecycle", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(directory, "index.server.ts"),
      `import { defineRpc, defineSettings } from "@q8idevai/plugin";
import { z } from "zod";
