import type { PluginBeforeRequests, PluginLifecycleEvents } from "@q8idevai/plugin/server";
...
import type { PluginLogEntry } from "@q8idevai/protocol/messages";
import { compilePlugin } from "./compiler.js";
import { readPluginManifest } from "./manifest.js";
import type { PluginRequirements } from "@q8idevai/protocol/messages";
import { assertPluginCompatibility } from "@q8idevai/protocol/plugin-requirements";
import type {
...
interface PluginFrameInput {
  ...
  pluginId: string;
  child: PluginChild;
  sessionHost: PluginQ8iDevAISessionHost;
...
export interface PluginQ8iDevAISessionHost {
  attachPluginSocket(
    pluginId: string,
    socket: PluginSessionSocket,
...
export class PluginRuntime {
  ...
  private sessionHost: PluginQ8iDevAISessionHost | null;
  ...
  bindQ8iDevAISessionHost(sessionHost: PluginQ8iDevAISessionHost): void {
  ...
  async startPlugin(
    ...
    this.appendLog(pluginId, "stdout", "[q8idevai] Loading plugin");
    const loaded = await this.loadDirectoryPlugin(pluginId, configuredPath).catch((error) => {
      this.appendLog(pluginId, "stderr", `[q8idevai] Plugin failed to load: ${describeError(error)}`);
      throw error;
    });
    if (!canPublish()) {
    ...
    this.appendLog(pluginId, "stdout", "[q8idevai] Plugin ready");
  }
  ...
  private async loadDirectoryPlugin(
    ...
    if (!sessionHost) throw new Error("Plugin Q8iDevAI session host is not attached");
      ...
      ready = await new Promise<Extract<PluginProcessMessage, { type: "ready" }>>(
        (resolve, reject) => {
          ...
          child.on("message", (rawMessage) => {
            ...
            if (message.type === "q8idevai_frame") {
            ...
            } else if (message.type === "q8idevai_close") {
  ...
  private async reattachPluginSession(input: PluginFrameInput): Promise<void> {
    ...
    try {
      ...
      this.appendLog(pluginId, "stdout", "[q8idevai] Re-attached plugin session");
    } catch (error) {
      const reason = error instanceof Error ? error.message : String(error);
      this.appendLog(pluginId, "stderr", `[q8idevai] Failed to re-attach plugin session: ${reason}`);
  ...
  private handleMalformedChildMessage(
    ...
    if (!connectionId || !state) {
      this.appendLog(loaded.id, "stderr", `[q8idevai] ${error.message}`);
  ...
  private async stopPlugin(loaded: LoadedPlugin): Promise<void> {
    this.appendLog(loaded.id, "stdout", "[q8idevai] Stopping plugin");
    ...
    if (!child || !sessionSocket || !sessionClosed) {
      this.appendLog(loaded.id, "stdout", "[q8idevai] Plugin stopped");
      return;
    }
    if (child.killed) {
