...
test("plugin handlers create workspaces and agents through their Q8iDevAI API", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-api-plugin-"));
  const workspaceDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-api-workspace-"));
...
export default function contribute(server: PluginServerContext) {
  server.handle(create, async ({ path }, { q8idevai }) => {
  ...
  server.handle(list, async (_input, { q8idevai }) => {
    const result = await q8idevai.agents.list({ page: { limit: 100 } });
    return { agentIds: result.entries.map((entry) => entry.agent.id) };
  });
  server.handle(append, ({ agentId, status }, { q8idevai }) =>
    q8idevai.agents.ref(agentId).timeline.append({
      type: "plugin",
      id: "review-1",
      kind: "review",
      version: 1,
  ...
  const daemon = await createTestQ8iDevAIDaemon({
  ...
  try {
    ...
    const created = await client.invokePluginRpc("q8idevai-api", "create", {
    ...
    const listed = await client.invokePluginRpc("q8idevai-api", "list", {});
    expect(listed).toEqual({
      agentIds: expect.arrayContaining([Reflect.get(created, "agentId")]),
    });
    const agentId = Reflect.get(created, "agentId");
    await expect(
      client.invokePluginRpc("q8idevai-api", "append", { agentId, status: "running" }),
    ).resolves.toEqual({ seq: expect.any(Number), epoch: expect.any(String) });
    await client.invokePluginRpc("q8idevai-api", "append", { agentId, status: "complete" });
    ...
    expect(timeline.entries.filter((entry) => entry.item.type === "plugin")).toEqual([
      expect.objectContaining({
        item: expect.objectContaining({
          type: "plugin",
          id: "review-1",
          pluginId: "q8idevai-api",
          data: { status: "complete" },
        }),
      }),
    ]);
    await client.removePlugin("q8idevai-api");
...
test("daemon config reload enables and disables configured plugins without restarting", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-reload-plugin-"));
  const q8idevaiHomeRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-reload-home-"));
  const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
  roots.push(pluginDirectory, q8idevaiHomeRoot);
  await writeFile(
    path.join(pluginDirectory, "q8idevai-plugin.json"),
    JSON.stringify({
  ...
  const configPath = path.join(daemon.q8idevaiHome, "config.json");
...
test("plugin host APIs and observations recover after repeated daemon-side socket closes", async () => {
  const pluginDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-reconnecting-plugin-"));
  const workspaceDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-reconnecting-workspace-"));
  roots.push(pluginDirectory, workspaceDirectory);
  await writeFile(
    path.join(pluginDirectory, "q8idevai-plugin.json"),
    JSON.stringify({ id: "reconnecting", requirements: { q8idevai: ">=0.8.0" } }),
  );
  await writeFile(
    path.join(pluginDirectory, "index.server.ts"),
    `
...
export default function contribute(server: PluginServerContext) {
  ...
  server.handle(defineRpc({ name: "observe", input: z.object({}), output: z.string() }), async (_, { q8idevai }) => {
    const { subscription } = await q8idevai.workspaces.list({ subscribe: {} });
  ...
  server.handle(defineRpc({ name: "probe", input: z.object({}), output: z.object({ pid: z.number(), projectIds: z.array(z.string()) }) }), async (_, { q8idevai }) => {
    await reconnected;
    return { pid: process.pid, projectIds: (await q8idevai.projects.list()).projects.map((project) => project.projectId) };
  });
  ...
  server.handle(defineRpc({ name: "disconnect", input: z.object({}), output: z.null() }), () => new Promise((resolve) => {
    ...
    function closed(message: { type: string }) {
      if (message.type !== "q8idevai_close") return;
    ...
    reconnected = new Promise<void>((connected) => {
      function ready(message: { type: string; data?: unknown }) {
        if (message.type !== "q8idevai_frame" || typeof message.data !== "string") return;
    ...
    process.send!({ type: "q8idevai_frame", isBinary: false, data: JSON.stringify({
