...
import type { ProviderEvent, ProviderRegistration } from "@q8idevai/plugin/server/provider";
...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-"));
  temporaryDirectories.push(directory);
  await writeFile(path.join(directory, "q8idevai-plugin.json"), JSON.stringify({ id }), "utf8");
...
describe("PluginRuntime", () => {
  it.each([
    { specifier: "@q8idevai/plugin", moduleDirectory: "shared" },
    { specifier: "@q8idevai/plugin", moduleDirectory: "server" },
  ])(
...
export default function contribute(server: any) { server.registerProvider(provider); return () => undefined; }`,
  ...
  it("records host-owned plugin lifecycle events", async () => {
    ...
    expect(
      ...
      { stream: "stdout", message: "[q8idevai] Loading plugin" },
      { stream: "stdout", message: "[q8idevai] Plugin ready" },
      { stream: "stdout", message: "[q8idevai] Stopping plugin" },
      { stream: "stdout", message: "[q8idevai] Plugin stopped" },
    ]);
  });
  ...
  it("frames stdout and stderr, normalizes CRLF, and flushes final fragments once", async () => {
    ...
    expect(
      logs
        .filter((entry) => !entry.message.startsWith("[q8idevai]"))
  ...
  it("retains compilation failures in the plugin stderr tail", async () => {
    ...
    expect(
      ...
      {
        stream: "stdout",
        message: "[q8idevai] Loading plugin",
      },
      {
        stream: "stderr",
  ...
  it("waits for asynchronous plugin cleanup before stopping", async () => {
    const cleanupFile = path.join(tmpdir(), `q8idevai-plugin-cleanup-${Date.now()}`);
    const directory = await createPlugin(
      "async-cleanup",
...
export default function contribute(plugin: unknown) {
  ...
  it("closes a provider connection that resolves during plugin shutdown", async () => {
    ...
    const startedFile = path.join(tmpdir(), `q8idevai-provider-connect-started-${suffix}`);
    const closedFile = path.join(tmpdir(), `q8idevai-provider-connect-closed-${suffix}`);
    const directory = await createPlugin(
      "shutdown-connect",
      `import { writeFile } from "node:fs/promises";
...
export default function contribute(server: { registerProvider(provider: ProviderRegistration): void }) {
  ...
  it("explains that an index.ts plugin was made for an older Q8iDevAI version", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-"));
    temporaryDirectories.push(directory);
    await Promise.all([
      writeFile(path.join(directory, "q8idevai-plugin.json"), JSON.stringify({ id: "legacy" })),
    ...
    await expect(runtime.startPlugin("legacy", directory)).rejects.toThrow(
      "This plugin was made for an older version of Q8iDevAI and cannot run on Q8iDevAI v0.8. Ask its author to update it. Plugin authors can follow the migration guide: https://q8idevai.sh/docs/plugins/v0.8/migration",
    );
  });
  ...
  it("loads a client-only plugin without spawning a subprocess", async () => {
    ...
    temporaryDirectories.push(directory);
    await Promise.all([
      writeFile(path.join(directory, "q8idevai-plugin.json"), JSON.stringify({ id: "theme" })),
      writeFile(
  ...
  it("loads separate entries, exposes the client bundle, and invokes the server RPC", async () => {
    ...
    await writeFile(path.join(directory, "q8idevai-plugin.json"), JSON.stringify({ id: "hello" }));
    await writeFile(
      path.join(directory, "index.client.tsx"),
      `import React from "react";
import { Text } from "react-native";
import { defineAttachmentSource } from "@q8idevai/plugin";
...
export default function contribute(server: any) {
  ...
  return () => undefined;
    ...
    await expect(runtime.invoke("hello", "greet", { name: "Q8iDevAI" })).resolves.toMatchObject({
      message: "Hello, Q8iDevAI",
    });
...
export default function contribute(plugin: any) {
  ...
  it("attaches no replacement session when the plugin is stopped", async () => {
    ...
    expect(runtime.getLogs("stopping").map((entry) => entry.message)).not.toContain(
      "[q8idevai] Re-attached plugin session",
    );
    expect(sessions.active.size).toBe(0);
  });
