import type { AgentTimelineItem } from "@q8idevai/protocol/agent-types";
...
test.skipIf(process.platform !== "linux")(
  ...
  async () => {
    ...
    const fixture = await mkdtemp(path.join(tmpdir(), "q8idevai-real-hooks-"));
    ...
    try {
      ...
      const envCommand = 'printf \'%s|%s\' "$Q8IDEVAI_HOOK_CREATE_EXAMPLE" "$Q8IDEVAI_HOOK_OPEN_EXAMPLE"';
      await client.sendMessage(
        agent.id,
