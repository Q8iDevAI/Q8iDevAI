...
import { z } from "zod";
import { CreateAgentRequestMessageSchema } from "@q8idevai/protocol/messages";
import type {
  PluginHookAgent,
  PluginHookContext,
  PluginLifecycleRegistration,
} from "@q8idevai/plugin/server";
...
import { WorkspaceCreateRequestSchema } from "@q8idevai/protocol/messages";
...
export class PluginHookHandlers implements PluginLifecycleRegistration {
  ...
  async invoke(
    ...
    name: string,
    input: unknown,
    q8idevai: PluginHookContext["q8idevai"],
  ): Promise<unknown> {
    ...
    try {
      if (kind === "before") {
        if (
          !beforeHookNames.includes(
            name as keyof import("@q8idevai/plugin/server").PluginBeforeRequests,
          )
        ) {
          throw new Error(`Unknown before hook: ${name}`);
        }
        const hookName = name as keyof import("@q8idevai/plugin/server").PluginBeforeRequests;
        let request = validateBeforeRequest(hookName, input);
        for (const handler of this.transforms.get(name) ?? []) {
          controller.signal.throwIfAborted();
          const result = await handler(
            { request: structuredClone(request) },
            { q8idevai, signal: controller.signal },
          );
          if (result !== undefined) {
      ...
      for (const handler of this.events.get(name) ?? []) {
        controller.signal.throwIfAborted();
        try {
          await handler(structuredClone(input), { q8idevai, signal: controller.signal });
        } catch (error) {
