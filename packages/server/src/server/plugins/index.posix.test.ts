...
async function createPlugin(id: string, source: string): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-service-"));
  roots.push(directory);
  await writeFile(path.join(directory, "q8idevai-plugin.json"), JSON.stringify({ id }));
...
function bindTestSessionHost(service: PluginService): PluginService {
  service.bindQ8iDevAISessionHost({
...
function createPausedRuntime() {
  ...
  const runtime: TestPluginRuntime = {
    ...
    subscribe: () => () => undefined,
    bindQ8iDevAISessionHost: () => undefined,
  };
...
describe("PluginService", () => {
  it("resolves a provider icon path to sanitized inline SVG", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-home-"));
    roots.push(home);
  ...
  it("keeps the running commit when a Git update build command fails", async () => {
    const home = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-home-"));
    roots.push(home);
    const repository = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-repository-"));
    roots.push(repository);
    await runGitCommand(["init", "-b", "main"], { cwd: repository });
    await runGitCommand(["config", "user.name", "Q8iDevAI Tests"], { cwd: repository });
    await runGitCommand(["config", "user.email", "q8idevai@example.test"], { cwd: repository });
    await writeFile(
      path.join(repository, "q8idevai-plugin.json"),
...
export default function contribute(plugin: unknown) {
  ...
  it("reports invalid manifests, missing entries, and startup failures", async () => {
    ...
    await writeFile(path.join(invalid, "q8idevai-plugin.json"), JSON.stringify({}));
