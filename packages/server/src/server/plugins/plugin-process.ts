...
import * as pluginSharedRuntime from "@q8idevai/plugin";
import * as pluginProviderRuntime from "@q8idevai/plugin/server/provider";
import * as pluginAcpRuntime from "@q8idevai/plugin/server/acp";
import type { SettingsDefinition, PluginRpcContract } from "@q8idevai/plugin";
...
let q8idevai: Q8iDevAIApi | null = null;
let stopping = false;
...
function runtimeRequire(name: string): unknown {
  ...
  if (name === "@q8idevai/plugin") return pluginSharedRuntime;
  if (name === "@q8idevai/plugin/server") return {};
  if (name === "@q8idevai/plugin/server/provider") return pluginProviderRuntime;
  if (name === "@q8idevai/plugin/server/acp") return pluginAcpRuntime;
  if (name === "@q8idevai/plugin/client/host")
...
async function initialize(message: Extract<PluginProcessRequest, { type: "initialize" }>) {
  ...
  q8idevai = createQ8iDevAIApi(daemonClient);
...
async function shutdown(): Promise<void> {
  ...
  await sendAndWait({ type: "q8idevai_close" });
  daemonClient = null;
  q8idevai = null;
  process.disconnect();
}
...
process.on("message", (rawMessage: unknown) => {
  ...
  if (message.type === "q8idevai_frame" || message.type === "q8idevai_close") return;
  ...
  void registered.contract.input
    ...
    .then((input) => {
      if (!q8idevai) throw new Error("Plugin Q8iDevAI API is unavailable");
      return registered.handler(input, { q8idevai });
    })
  ...
  if (message.type === "hook") {
    if (!q8idevai) {
      send({
        type: "error",
        requestId: message.requestId,
        error: "Plugin Q8iDevAI API is unavailable",
      });
      return;
    }
    void hooks.invoke(message.requestId, message.kind, message.name, message.input, q8idevai).then(
      (output) => {
