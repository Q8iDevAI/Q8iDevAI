...
function createRuntimeBoundaryPlugin(target: PluginBuildTarget, pluginDirectory: string): Plugin {
  ...
  return {
    name: `q8idevai-plugin-${target}-runtime-boundary`,
    setup(buildContext) {
...
function runtimeSpecifierError(
  ...
  if (specifier === "@q8idevai/plugin/client/host") kind = "host-private";
  else if (
    (specifier === "@q8idevai/plugin" ||
      specifier.startsWith("@q8idevai/plugin/") ||
      specifier === "@q8idevai/plugin" ||
      specifier.startsWith("@q8idevai/plugin/")) &&
    !(PLUGIN_SDK_SPECIFIERS as readonly string[]).includes(specifier)
  )
