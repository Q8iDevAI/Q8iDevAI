import type { DaemonTransportFactory } from "@q8idevai/client/internal/daemon-client-transport-types";
...
export function createPluginDaemonTransportFactory(
  ...
  return () => {
    ...
    const unsubscribe = port.onMessage((message) => {
      ...
      if (message.type === "q8idevai_close") closeTransport();
    });
    ...
    return {
      send(data) {
        ...
        port.send({ type: "q8idevai_frame", data: frame, isBinary: typeof frame !== "string" });
      },
      close() {
        if (!isOpen) return;
        port.send({ type: "q8idevai_close" });
        closeTransport();
      },
      onMessage(handler) {
