...
async function createSplitPlugin(): Promise<{
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-compiler-"));
...
async function createRootAlias(directory: string): Promise<string> {
  const aliases = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-root-alias-"));
...
describe("plugin runtime entries", () => {
  ...
  it.each([
    ...
    { target: "server", dependency: "@q8idevai/plugin/server/provider" },
  ] as const)(
  ...
  it.each([
    'import type { PluginClientContext } from "@q8idevai/plugin/client"; export type Context = PluginClientContext;',
    'export type { PluginClientContext } from "@q8idevai/plugin/client";',
    'export type Context = import("@q8idevai/plugin/client").PluginClientContext;',
    'import { type PluginClientContext } from "@q8idevai/plugin/client"; export type Context = PluginClientContext;',
...
export type Value = string;`,
  ...
  it("resolves type-only path aliases from the plugin tsconfig", async () => {
    ...
    await writeFile(
      ...
      'export type { PluginClientContext as Value } from "@q8idevai/plugin/client";',
    );
    await expect(compilePlugin(entries)).rejects.toThrow("plugin shared");
  });

  it.each([
    { specifier: "@q8idevai/plugin/client", importKind: "import type" },
    { specifier: "@q8idevai/plugin/server", importKind: "import type" },
    { specifier: "@q8idevai/plugin/client", importKind: "import" },
    { specifier: "@q8idevai/plugin/server", importKind: "import" },
  ])(
  ...
  it.each([
    "@q8idevai/plugin",
    "@q8idevai/plugin/react-native",
    "@q8idevai/plugin/ui",
    "@q8idevai/plugin/provider",
    "@q8idevai/plugin/acp",
    "@q8idevai/plugin/host",
...
export default function contribute() { void value; return () => undefined; }`,
  ...
  it("rejects relative imports that escape the plugin root", async () => {
    const parent = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-compiler-parent-"));
...
export default function contribute() { void secret; return () => undefined; }`,
  ...
  it("rejects absolute imports from outside the plugin root", async () => {
    const entries = await createSplitPlugin();
    const outsideDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-outside-"));
...
export default function contribute() { void secret; return () => undefined; }`,
  ...
  it("rejects plugin-authored relative imports that escape into node_modules", async () => {
    const parent = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-node-modules-parent-"));
    temporaryDirectories.push(parent);
    const pluginDirectory = path.join(parent, "plugin");
    const server = path.join(pluginDirectory, "index.server.ts");
...
export default function contribute() { void handler; return () => undefined; }`,
  ...
  it("allows linked dependencies to resolve within their own package root", async () => {
    const entries = await createSplitPlugin();
    const linkedPackage = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-linked-dependency-"));
...
export default function contribute() { void secret; return () => undefined; }`,
  ...
  it("does not let remembered linked roots hide plugin-local runtime boundaries", async () => {
    const entries = await createSplitPlugin();
    const linkedPackage = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-linked-boundary-"));
    temporaryDirectories.push(linkedPackage);
    await mkdir(path.join(entries.directory, "node_modules"));
    await Promise.all([
      writeFile(
