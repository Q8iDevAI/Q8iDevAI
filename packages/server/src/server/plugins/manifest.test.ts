...
describe("plugin manifest", () => {
  ...
  it("reads and validates requirements before any plugin code runs", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-manifest-"));
    directories.push(directory);
    const manifest = path.join(directory, "q8idevai-plugin.json");
    await writeFile(manifest, JSON.stringify({ id: "example", requirements: { q8idevai: "^0.8.0" } }));
