...
import { ProviderEventSchema, ProviderInputSchema } from "@q8idevai/plugin/server/provider";
import { z } from "zod";
...
export type PluginProcessRequest =
  ...
  | { type: "shutdown" }
  | { type: "q8idevai_frame"; data: string | Uint8Array; isBinary: boolean }
  | { type: "q8idevai_close" };
...
export const PluginProcessRequestSchema: z.ZodType<PluginProcessRequest> = z.discriminatedUnion(
  ...
  [
    ...
    z.object({ type: z.literal("q8idevai_frame"), ...frameFields }).strict(),
    z.object({ type: z.literal("q8idevai_close") }).strict(),
  ],
);
