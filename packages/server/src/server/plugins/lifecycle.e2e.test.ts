...
test("a plugin transforms workspace creation once across receipt replays and observes its committed lifecycle", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-lifecycle-"));
  const daemon = await createTestQ8iDevAIDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "lifecycle", requirements: { q8idevai: ">=0.8.0" } }),
    );
    await writeFile(
...
test("plugins observe turns, answer permissions, and observe archive without blocking the agent", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-turn-hooks-"));
  const daemon = await createTestQ8iDevAIDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "turn-hooks", requirements: { q8idevai: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
export default function contribute(server) {
  ...
  server.on("agent.permission_requested", async (event, context) => {
    ...
    await context.q8idevai.agents.ref(event.agent.id).respondToPermission({
...
test("agent creation hooks change the provider and environment before the session opens", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-agent-hooks-"));
  const daemon = await createTestQ8iDevAIDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "agent-hooks", requirements: { q8idevai: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
...
test("invalid output from an untyped plugin rejects creation before later callbacks run", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-invalid-hook-"));
  const daemon = await createTestQ8iDevAIDaemon({ daemonVersion: "0.8.0" });
  const client = new DaemonClient({ url: `ws://127.0.0.1:${daemon.port}/ws`, appVersion: "0.8.0" });
  try {
    await writeFile(
      path.join(directory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "invalid-hook", requirements: { q8idevai: ">=0.8.0" } }),
    );
    await writeFile(
      path.join(directory, "index.server.ts"),
      `
