...
import { handleCreateQ8iDevAIWorktreeRequest } from "./worktree-session.js";
...
describe("wire compatibility", () => {
  ...
  test("legacy worktree request shape normalizes to the same internal input as the new shape", async () => {
    ...
    const dependencies = {
      ...
      createQ8iDevAIWorktreeWorkflow: workflow.create.bind(workflow),
    };
    ...
    if (legacyRequest.type !== "create_q8idevai_worktree_request") {
      throw new Error("Expected legacy worktree request");
    }
    if (newRequest.type !== "create_q8idevai_worktree_request") {
      throw new Error("Expected new worktree request");
    }

    await handleCreateQ8iDevAIWorktreeRequest(dependencies, legacyRequest);
    await handleCreateQ8iDevAIWorktreeRequest(dependencies, newRequest);
  ...
  expect(delivered.get(modern)).toEqual([
    {
      ...
      payload: {
        ...message.payload,
        status: "failed",
        error: expect.stringContaining("Update Q8iDevAI"),
