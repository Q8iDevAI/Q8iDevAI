...
describe("isVoicePermissionAllowed", () => {
  test("allows direct speak tool names across provider conventions", () => {
    ...
    expect(isVoicePermissionAllowed(buildRequest({ name: "q8idevai_voice.speak" }))).toBe(true);
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__q8idevai_voice__speak" }))).toBe(true);
  });

  test("denies non-speak tool names", () => {
    expect(isVoicePermissionAllowed(buildRequest({ name: "mcp__q8idevai__create_agent" }))).toBe(
      false,
    );
    expect(isVoicePermissionAllowed(buildRequest({ name: "q8idevai_create_agent" }))).toBe(false);
  });

  test("denies non-tool permission kinds", () => {
    const result = isVoicePermissionAllowed(
      buildRequest({ kind: "mode", name: "mcp__q8idevai__create_agent" }),
    );
  ...
  test("denies wrapper tools even when metadata references speak", () => {
    const denied = isVoicePermissionAllowed(
      buildRequest({
        name: "codextool",
        metadata: {
          questions: [{ question: "Allow codextool to call q8idevai_voice.speak for user feedback?" }],
        },
      }),
    );
