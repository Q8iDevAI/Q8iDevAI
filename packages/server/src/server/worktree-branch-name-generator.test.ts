...
describe("generateBranchNameFromFirstAgentContext", () => {
  ...
  test("keeps the branch slug validator fallback when instructions are present", async () => {
    const repoRoot = createTempDir("q8idevai-branch-config-");
    const worktreeRoot = createTempDir("q8idevai-branch-worktree-");
    mkdirSync(path.join(worktreeRoot, ".git"));
    writeQ8iDevAIWorktreeMetadata(worktreeRoot, { baseRefName: "main" });
    writeQ8iDevAIWorktreeFirstAgentBranchAutoNameMetadata(worktreeRoot, {
...
async function generateBranchPromptWithConfig(config: unknown): Promise<{ prompt: string }> {
  ...
  if (typeof config === "string") {
    writeFileSync(path.join(repoRoot, "q8idevai.json"), config);
...
function writeConfig(repoRoot: string, config: unknown): void {
  writeFileSync(path.join(repoRoot, "q8idevai.json"), `${JSON.stringify(config)}\n`);
}
