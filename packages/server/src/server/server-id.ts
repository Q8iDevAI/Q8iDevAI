...
function getServerIdPath(q8idevaiHome: string): string {
  return path.join(q8idevaiHome, SERVER_ID_FILENAME);
}
...
/**
 * Stable daemon identifier scoped to a given $Q8IDEVAI_HOME.
 *
 * - Persisted to `$Q8IDEVAI_HOME/server-id`
 * - Can be overridden via `Q8IDEVAI_SERVER_ID` (useful for tests)
 */
export function getOrCreateServerId(
  ...
  const serverIdPath = getServerIdPath(q8idevaiHome);

  const envOverride =
    typeof env.Q8IDEVAI_SERVER_ID === "string" && env.Q8IDEVAI_SERVER_ID.trim().length > 0
      ? env.Q8IDEVAI_SERVER_ID.trim()
      : null;
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted Q8IDEVAI_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist Q8IDEVAI_SERVER_ID override");
      }
    } else {
