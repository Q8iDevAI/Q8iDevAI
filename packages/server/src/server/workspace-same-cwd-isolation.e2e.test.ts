...
async function createSnapshotStormDaemon(clients: SnapshotStormProviderClient[]) {
  return createTestQ8iDevAIDaemon({
    mcpEnabled: false,
    isDev: true,
    agentClients: {
...
function seedSameCwdWorkspaces(): { q8idevaiHomeRoot: string; cwd: string } {
  const q8idevaiHomeRoot = mkdtempSync(path.join(tmpdir(), "q8idevai-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-same-cwd-dir-"));
  const projectsDir = path.join(q8idevaiHomeRoot, ".q8idevai", "projects");
  ...
  return { q8idevaiHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ q8idevaiHomeRoot: string; cwd: string }> {
  const q8idevaiHomeRoot = mkdtempSync(path.join(tmpdir(), "q8idevai-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-legacy-agent-dir-"));
  const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
  const projectsDir = path.join(q8idevaiHome, "projects");
  mkdirSync(projectsDir, { recursive: true });
  ...
  const agentStorage = new AgentStorage(path.join(q8idevaiHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { q8idevaiHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-agent-submit-title-"));
  const daemon = await createTestQ8iDevAIDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-running-same-cwd-create-"));
  const daemon = await createTestQ8iDevAIDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { q8idevaiHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
