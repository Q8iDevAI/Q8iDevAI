...
function createGitRepo(): { tempDir: string; repoDir: string; q8idevaiHome: string } {
  ...
  return { tempDir, repoDir, q8idevaiHome };
}

function createGitRepoWithDevBranch(): { tempDir: string; repoDir: string; q8idevaiHome: string } {
  const { tempDir, repoDir, q8idevaiHome } = createGitRepo();
...
function createGitRepoWithOriginMain(): { tempDir: string; repoDir: string; q8idevaiHome: string } {
  const { tempDir, repoDir, q8idevaiHome } = createGitRepo();
...
function createGitHubPrRemoteRepo(): { tempDir: string; repoDir: string; q8idevaiHome: string } {
  const { tempDir, repoDir, q8idevaiHome } = createGitRepo();
...
function createSameRepoGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, remoteDir, q8idevaiHome };
}
...
function createForkGitHubPrRemoteRepo(): {
  ...
  return { tempDir, repoDir, headRemoteDir, q8idevaiHome };
}
...
describe.skipIf(isPlatform("win32"))("worktree-core POSIX-only", () => {
  describe("createWorktreeCore", () => {
    ...
    test("creates branch-off worktrees from origin main without tracking origin main", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitRepoWithOriginMain();
      cleanupPaths.push(tempDir);
    ...
    test("checks out an explicit GitHub PR branch with legacy RPC fields", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("branches off an explicit refName base", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitRepoWithDevBranch();
      cleanupPaths.push(tempDir);
    ...
    test("tracks a same-repo PR branch from a single-branch clone", async () => {
      const { tempDir, repoDir, remoteDir, q8idevaiHome } = createSameRepoGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a GitLab MR source branch through the resolved forge service", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitRepoWithOriginFeatureBranch();
      cleanupPaths.push(tempDir);
    ...
    test("checks out a GitLab MR ref when the source branch is gone", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitLabMrRefOnlyRemoteRepo();
      cleanupPaths.push(tempDir);
      const gitlab: ForgeService = {
        ...createGitHubServiceStub(),
        getPullRequestCheckoutTarget: async ({ number }) => ({
    ...
    test("checks out the authoritative GitLab MR ref before a same-named origin branch", async () => {
      const { tempDir, repoDir, q8idevaiHome } = createGitLabMrWithConflictingOriginBranchRepo();
      cleanupPaths.push(tempDir);
    ...
    test("does not add a fallback push remote for a cross-repo MR without a push URL", async () => {
      ...
      const pushRefspec = getGitConfigValue(
        second.worktree.worktreePath,
        "remote.q8idevai-pr-14.push",
      );
      const pushRemoteUrl = getGitConfigValue(
        second.worktree.worktreePath,
        "remote.q8idevai-pr-14.url",
      );
      const metadata = readQ8iDevAIWorktreeMetadata(second.worktree.worktreePath);
      const facts = await getCheckoutSnapshotFacts(second.worktree.worktreePath, { q8idevaiHome });
    ...
    test("checks out a same-repo GitHub PR with valid origin tracking", async () => {
      ...
      const status = await getCheckoutStatus(result.worktree.worktreePath, { q8idevaiHome });
    ...
    test("pushes a deduplicated same-repo GitHub PR branch to the PR head", async () => {
      ...
      const pushRemote = execFileSync(
        ...
        .toString()
        .trim();
      const pushRefspec = execFileSync("git", ["config", "remote.q8idevai-pr-1790.push"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
        .trim();
      const pushRemoteUrl = execFileSync("git", ["config", "remote.q8idevai-pr-1790.url"], {
        cwd: second.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(pushRemote).toBe("q8idevai-pr-1790");
    ...
    test("derives the tracked PR lookup target when managed metadata has no target", async () => {
      ...
      writeFileSync(
        getQ8iDevAIWorktreeMetadataPath(second.worktree.worktreePath),
    ...
    test("checks out a fork PR whose head branch collides with local main", async () => {
      const { tempDir, repoDir, headRemoteDir, q8idevaiHome } = createForkGitHubPrRemoteRepo();
      cleanupPaths.push(tempDir);
      const github = {
        ...createGitHubServiceStub(),
        getPullRequestCheckoutTarget: async () => ({
      ...
      const worktreeBranch = execFileSync("git", ["branch", "--show-current"], {
        ...
        .trim();
      const readme = readFileSync(path.join(result.worktree.worktreePath, "README.md"), "utf8");
      const cleanStatus = await getCheckoutStatus(result.worktree.worktreePath, { q8idevaiHome });
      const pushRefspec = execFileSync("git", ["config", "remote.q8idevai-pr-526.push"], {
        cwd: result.worktree.worktreePath,
        stdio: "pipe",
      })
        .toString()
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("q8idevai-pr-526/main");
    ...
    test("pushes a fork PR when the contributor branch cannot be fetched at checkout", async () => {
      ...
      expect(getBranchUpstream(result.worktree.worktreePath)).toBe("q8idevai-pr-526/main");
      expect(pushRemote).toBe("q8idevai-pr-526");
      expect(pushRefspec).toBe("HEAD:refs/heads/main");
      expect(remotePrHead).toBe(localHead);
    ...
    test("uses a unique local branch when the same fork PR branch already exists", async () => {
      ...
      expect(getBranchUpstream(first.worktree.worktreePath)).toBe("q8idevai-pr-526/main");
      expect(getBranchUpstream(second.worktree.worktreePath)).toBe("q8idevai-pr-526/main");
    });
    ...
    test("creates a suffixed branch-off worktree for the same slug", async () => {
      ...
      const first = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", q8idevaiHome, runSetup: false },
        deps,
      );
      const second = await createCoreWorktree(
        { cwd: repoDir, worktreeSlug: "reused-worktree", q8idevaiHome, runSetup: false },
