...
import { createQ8iDevAIDaemon, parseListenString, type Q8iDevAIDaemonConfig } from "./bootstrap.js";
import { loadConfig } from "./config.js";
import { AgentManagerShuttingDownError } from "./agent/agent-manager.js";
import { hashDaemonPassword } from "./auth.js";
import { generateLocalPairingOffer } from "./pairing-offer.js";
import { createTestQ8iDevAIDaemon } from "./test-utils/q8idevai-daemon.js";
...
describe("q8idevai daemon bootstrap", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  test("starts and serves health endpoint", async () => {
    const daemonHandle = await createTestQ8iDevAIDaemon({
  ...
  test("keeps timeline activity in memory and removes obsolete timeline files at startup", async () => {
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-timeline-cleanup-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const obsoleteTimelineDirectory = path.join(q8idevaiHome, "agent-timelines");
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-timeline-agent-"));
    await mkdir(obsoleteTimelineDirectory, { recursive: true });
    await writeFile(path.join(obsoleteTimelineDirectory, "obsolete.json"), "{}\n", "utf-8");

    const daemonHandle = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot, cleanup: false });
    try {
      ...
      await daemonHandle.close();
      await Promise.all([
        rm(q8idevaiHomeRoot, { recursive: true, force: true }),
  ...
  test("does not create a timeline directory for live timeline activity", async () => {
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-timeline-memory-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-timeline-agent-"));
    const daemonHandle = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot, cleanup: false });
    const timelineDirectory = path.join(daemonHandle.q8idevaiHome, "agent-timelines");
    try {
      const agent = await daemonHandle.daemon.agentManager.createAgent(
        { provider: "codex", cwd: agentCwd },
        undefined,
  ...
  test("reload applies live HTTP, MCP, Git, provider, relay, and app policies", async () => {
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-config-reload-runtime-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-config-reload-agent-"));
    ...
    config.agentStoragePath = path.join(q8idevaiHome, "agents");
    config.isDev = true;
    config.speech = {
      providers: {
    ...
    const daemon = await createQ8iDevAIDaemon(config, pino({ level: "silent" }));
  ...
  test("rolls back daemon listener when standalone service proxy startup fails", async () => {
    ...
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-standalone-rollback-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    await mkdir(q8idevaiHome, { recursive: true });
    const config: Q8iDevAIDaemonConfig = {
      listen: "127.0.0.1:0",
      q8idevaiHome,
      corsAllowedOrigins: [],
      hostnames: true,
      ...
      relayEnabled: false,
      appBaseUrl: "https://app.q8idevai.sh",
      openai: undefined,
      speech: undefined,
      serviceProxy: {
    ...
    try {
      ...
      await rm(q8idevaiHomeRoot, { recursive: true, force: true });
  ...
  test("relay config changes during Hub enrollment reach the live runtime", async () => {
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-relay-startup-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    await mkdir(q8idevaiHome, { recursive: true });
    await writeFile(
      path.join(q8idevaiHome, "hub-relationship.json"),
      `${JSON.stringify({
        version: 1,
        state: "pending",
        relationship: {
  ...
  test("rolls back standalone listener without starting plugins when main listen fails", async () => {
    ...
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-main-rollback-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    const pluginDirectory = path.join(q8idevaiHomeRoot, "plugin");
    const pluginPidPath = path.join(pluginDirectory, "plugin.pid");
    await mkdir(q8idevaiHome, { recursive: true });
    if (!isPlatform("win32")) {
      await mkdir(pluginDirectory);
      await writeFile(
        path.join(pluginDirectory, "q8idevai-plugin.json"),
        JSON.stringify({
          id: "startup-rollback",
          requirements: { q8idevai: `>=${resolveDaemonVersion(import.meta.url)}` },
        }),
      );
      await writeFile(
...
export default function contribute(plugin: unknown) {
  ...
  test("starts when OpenAI speech provider is configured without credentials", async () => {
    const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-openai-config-"));
    const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
    await mkdir(q8idevaiHome, { recursive: true });
  ...
  test("does not block daemon start on local speech model downloads", async () => {
    ...
    const daemonHandle = await createTestQ8iDevAIDaemon({
      speech: {
        ...
        local: {
          modelsDir: path.join(os.tmpdir(), `q8idevai-missing-models-${Date.now()}`),
          models: {
  ...
  test("rejects Windows absolute paths that are not named pipes", () => {
    ...
    expect(() => parseListenString(String.raw`D:\Users\foo\.q8idevai\daemon.sock`)).toThrow();
  ...
  test("parses Windows named pipes as managed IPC listen targets", () => {
    expect(parseListenString(String.raw`\\.\pipe\q8idevai-managed-test`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\q8idevai-managed-test`,
    });
    expect(parseListenString(`pipe://${String.raw`\\.\pipe\q8idevai-managed-test`}`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\q8idevai-managed-test`,
    });
  });
  ...
  test.skipIf(isPlatform("win32"))(
    ...
    async () => {
      const q8idevaiHomeRoot = await mkdtemp(path.join(os.tmpdir(), "q8idevai-socket-relay-"));
      const q8idevaiHome = path.join(q8idevaiHomeRoot, ".q8idevai");
      const staticDir = await mkdtemp(path.join(os.tmpdir(), "q8idevai-static-"));
      const socketPath = path.join(q8idevaiHomeRoot, "run", "q8idevai.sock");
      ...
      const daemon = await createQ8iDevAIDaemon(config, logger);
      ...
      try {
        ...
        expect(pairing.url?.startsWith("https://app.q8idevai.sh/#offer=")).toBe(true);
      } finally {
        await daemon.stop().catch(() => undefined);
...
async function beginDaemonShutdownWithAgentClosing(): Promise<BlockedDaemonShutdown> {
  ...
  const agentCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-shutdown-agent-"));
  ...
  return {
    ...
    async finish() {
      ...
      await Promise.all([
        rm(path.dirname(daemonHandle.q8idevaiHome), { recursive: true, force: true }),
