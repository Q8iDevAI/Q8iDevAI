...
import { pinQ8iDevAIWorktreeBranchIdentityIfMissing } from "../utils/worktree-metadata.js";
...
export async function bootstrapWorkspaceRegistries(options: {
  ...
  for (const workspace of await options.workspaceRegistry.list()) {
    if (
      ...
      !workspace.isQ8iDevAIOwnedWorktree ||
    ...
    try {
      pinQ8iDevAIWorktreeBranchIdentityIfMissing(workspace.worktreeRoot, workspace.branch);
    } catch (error) {
      options.logger.warn(
  ...
  options.logger.info(
    {
      projectsFile: path.join(options.q8idevaiHome, "projects", "projects.json"),
      workspacesFile: path.join(options.q8idevaiHome, "projects", "workspaces.json"),
