...
const REPO_CWD = path.resolve("/tmp/q8idevai-observation-repo");
...
const WORKTREE_A = path.resolve("/tmp/q8idevai-observation-worktree-a");
const WORKTREE_B = path.resolve("/tmp/q8idevai-observation-worktree-b");
...
describe("WorkspaceGitService checkout observation", () => {
  ...
  test("ten worktrees share one repository metadata and fetch observer while retaining checkout state", async () => {
    ...
    const commonGitDir = path.resolve("/tmp/q8idevai-shared-repository.git");
    const worktrees = Array.from({ length: 10 }, (_, index) =>
      path.resolve(`/tmp/q8idevai-shared-worktree-${index}`),
    );
  ...
  test("repository watcher canary timeout closes the subscription and starts fallback polling", async () => {
    ...
    const service = createService(
      watcher,
      {
        getCheckoutStatus,
        createWatcherLivenessCanary: vi.fn(() => ({
          path: path.join(GIT_DIR, "q8idevai", ".watcher-canary-timeout"),
