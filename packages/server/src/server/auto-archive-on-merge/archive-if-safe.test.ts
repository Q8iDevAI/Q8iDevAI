...
const CWD = "/tmp/q8idevai/worktrees/repo/branch";
const Q8IDEVAI_HOME = "/tmp/q8idevai";
const WORKTREES_ROOT = "/tmp/q8idevai/worktrees/repo";
...
function createHarness(overrides?: {
  ...
  isQ8iDevAIOwnedWorktreeCwd?: ArchiveIfSafeDependencies["isQ8iDevAIOwnedWorktreeCwd"];
  ...
  const isQ8iDevAIOwnedWorktreeCwd = vi.fn(
    overrides?.isQ8iDevAIOwnedWorktreeCwd ??
      (async () => ({
        allowed: true,
        repoRoot: "/tmp/repo",
  ...
  ) as unknown as ArchiveIfSafeDependencies["isQ8iDevAIOwnedWorktreeCwd"];
...
describe("archiveIfSafe", () => {
  ...
  test("does nothing when the worktree is dirty", async () => {
    ...
    expect(harness.deps.isQ8iDevAIOwnedWorktreeCwd).not.toHaveBeenCalled();
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("does nothing when the cwd is not a Q8iDevAI-owned worktree", async () => {
    const harness = createHarness({
      isQ8iDevAIOwnedWorktreeCwd: async () => ({ allowed: false, worktreePath: CWD }),
    });

    await runArchiveIfSafe(harness);

    expect(harness.deps.isQ8iDevAIOwnedWorktreeCwd).toHaveBeenCalledWith(CWD, {
      q8idevaiHome: Q8IDEVAI_HOME,
    });
    expect(harness.deps.archiveByScope).not.toHaveBeenCalled();
  });
  ...
  test("archives a clean Q8iDevAI-owned worktree after merge", async () => {
    const harness = createHarness();
  ...
  test("real outcome: keeps sibling workspace and directory on last reference", async () => {
    ...
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "merged-with-sibling");
  ...
  test("real outcome: removes directory when no sibling workspace remains", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "merged-last-ref");
  ...
  test("real outcome: an unarchived workspace is not archived again for the same merged PR", async () => {
    const { tempDir, repoDir } = createGitRepo();
    const q8idevaiHome = path.join(tempDir, ".q8idevai");
    const worktree = await createQ8iDevAIOwnedWorktree(repoDir, q8idevaiHome, "merged-then-unarchived");
    const workspace = {
