...
export interface ArchiveIfSafeDependencies {
  ...
  isQ8iDevAIOwnedWorktreeCwd: typeof isQ8iDevAIOwnedWorktreeCwd;
...
export async function archiveIfSafe(input: {
  ...
  const ownership = await deps.isQ8iDevAIOwnedWorktreeCwd(cwd, {
    q8idevaiHome: options.q8idevaiHome,
    worktreesRoot: options.q8idevaiWorktreesBaseRoot,
  });
  ...
  try {
    ...
    await deps.archiveByScope(
      {
        q8idevaiHome: options.q8idevaiHome,
        q8idevaiWorktreesBaseRoot: options.q8idevaiWorktreesBaseRoot,
