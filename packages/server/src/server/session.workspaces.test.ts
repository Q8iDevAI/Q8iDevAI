...
import { CLIENT_CAPS } from "@q8idevai/protocol/client-capabilities";
import { createTestLogger } from "../test-utils/test-logger.js";
import { Session } from "./session.js";
import type { SessionOptions } from "./session.js";
import { OWNER_PERMISSIONS } from "./authorization/index.js";
import type { AgentUpdatesService } from "./session/agent-updates/agent-updates-service.js";
import type { AgentSnapshotPayload, SessionOutboundMessage } from "@q8idevai/protocol/messages";
...
import {
  ...
  writeQ8iDevAIWorktreeFirstAgentBranchAutoNameMetadata,
...
interface SessionTestAccess {
  ...
  handleCreateQ8iDevAIWorktreeRequest(params: unknown): Promise<unknown>;
...
function createSessionForWorkspaceTests(
  ...
  const session = asTestSession(
    new Session({
      ...
      q8idevaiHome: options.q8idevaiHome ?? "/tmp/q8idevai-test",
...
test("create_agent_request keeps requested child cwd when grouped under an existing parent workspace", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "q8idevai-create-agent-cwd-"));
  try {
    ...
    const session = asTestSession(
      new Session({
        ...
        downloadTokenStore: asDownloadTokenStore(),
        pushNotifications: asPushNotifications(),
        q8idevaiHome: path.join(workdir, "q8idevai-home"),
        agentManager,
        agentStorage,
        projectRegistry,
        workspaceRegistry,
...
test("create_agent_request launches from an exact subdirectory in a created worktree", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "q8idevai-create-agent-worktree-cwd-"));
  try {
    const parent = path.join(workdir, "parent");
    const child = path.join(parent, "packages", "app");
    mkdirSync(child, { recursive: true });
    execFileSync("git", ["init", "-b", "main"], { cwd: parent, stdio: "pipe" });
    execFileSync("git", ["config", "user.email", "test@getq8idevai.local"], {
      cwd: parent,
      stdio: "pipe",
    });
    execFileSync("git", ["config", "user.name", "Q8iDevAI Test"], { cwd: parent, stdio: "pipe" });
...
test("create_agent_request does not title an existing workspace from the agent prompt", async () => {
  vi.useFakeTimers();
  const workdir = mkdtempSync(path.join(tmpdir(), "q8idevai-create-agent-existing-title-"));
  try {
...
test("archive emits an authoritative agent_update upsert for subscribed clients", async () => {
  ...
  const session = asTestSession(
    new Session({
      ...
      downloadTokenStore: asDownloadTokenStore(),
      pushNotifications: asPushNotifications(),
      q8idevaiHome: "/tmp/q8idevai-test",
  ...
  const storedRecords = new Map<string, StoredAgentRecord>([
    ...
    [
      ...
      makeStoredAgent({
        ...
        labels: { "q8idevai.parent-agent-id": "root-agent" },
      }),
    ],
  ]);
...
test("workspace placements preserve checkout facts independently from the project", async () => {
  ...
  const q8idevaiSubdirectory = createPersistedWorkspaceRecord({
    workspaceId: "ws-q8idevai-subdirectory",
    projectId: "proj-manual-worktree",
    cwd: "/tmp/q8idevai-worktree/packages/app",
    kind: "worktree",
    displayName: "app",
  ...
  await expect(
    session.buildProjectPlacementForWorkspaceId(q8idevaiSubdirectory.workspaceId),
  ).resolves.toEqual(
    expect.objectContaining({
      checkout: expect.objectContaining({
        cwd: q8idevaiSubdirectory.cwd,
        worktreeRoot: "/tmp/q8idevai-worktree",
      }),
    }),
  );
});
...
test("create q8idevai worktree response preserves an explicit non-Git project", async () => {
  const emitted: SessionOutboundMessage[] = [];
  ...
  const q8idevaiHome = path.join(tempDir, "q8idevai-home");
  ...
  session.q8idevaiHome = q8idevaiHome;
  ...
  try {
    await session.handleCreateQ8iDevAIWorktreeRequest({
      type: "create_q8idevai_worktree_request",
      cwd: repoDir,
  ...
  const response = findByType(emitted, "create_q8idevai_worktree_response");
...
test("workspace updates stay scoped to the matching cwd", async () => {
  ...
  const missingRoot = path.join(tmpdir(), `q8idevai-scoped-workspace-${Date.now()}`);
...
test("open_project_request does not match a new child directory to an existing parent workspace", async () => {
  ...
  const worktree = path.join(home, ".q8idevai", "worktrees", "project-config-lifecycle-textarea");
...
test("open_project_request reclassifies an archived directory workspace when git metadata becomes available", async () => {
  ...
  const projects = new Map<string, ReturnType<typeof createPersistedProjectRecord>>();
  const workspaces = new Map<string, ReturnType<typeof createPersistedWorkspaceRecord>>();
  const repoRoot = path.resolve("/home/developer/dev/q8idevai");
  const cwd = path.join(
  ...
  session.workspaceGitService.getCheckout = async () => ({
    ...
    remoteUrl: "git@github.com:getq8idevai/q8idevai.git",
    worktreeRoot: cwd,
    isQ8iDevAIOwnedWorktree: false,
...
test("refresh_agent_request leaves workspace archival independent when its directory exists", async () => {
  ...
  const cwd = path.resolve("/tmp/q8idevai-unit2-existing-dir");
  session.filesystem.isDirectory = async () => true;
  const workspaceId = "ws-repo-archived";
...
test("refresh_agent_request leaves workspace archival independent when its directory is missing", async () => {
  ...
  const cwd = path.resolve("/tmp/q8idevai-missing-workspace-dir");
...
test("refresh_agent_request does not recreate or unarchive a deleted worktree", async () => {
  ...
  const cwd = path.resolve("/tmp/q8idevai-deleted-worktree-dir");
  session.filesystem.isDirectory = async () => false;
...
test("refresh_agent_request does not inspect an archived worktree branch", async () => {
  ...
  const cwd = path.resolve("/tmp/q8idevai-deleted-worktree-fail");
  session.filesystem.isDirectory = async () => false;
...
function createRecreateWorktreeRepo(): { tempDir: string; repoDir: string } {
  const tempDir = realpathSync(mkdtempSync(path.join(tmpdir(), "q8idevai-recreate-worktree-")));
  const repoDir = path.join(tempDir, "repo");
  ...
  execFileSync("git", ["config", "user.name", "Q8iDevAI Test"], { cwd: repoDir, stdio: "pipe" });
...
test("archive_workspace_request archives a worktree-kind workspace and removes the directory on last reference", async () => {
  ...
  const q8idevaiHome = path.join(tempDir, ".q8idevai");
...
test.skip("opening a new worktree reconciles older local workspaces into the remote project", async () => {
  ...
  const worktreeWorkspaceId = path.join(mainWorkspaceId, ".q8idevai", "worktrees", "feature-a");
  ...
  session.buildProjectPlacement = async (cwd: string) => ({
    ...
    checkout: {
      ...
      worktreeRoot: cwd,
      isQ8iDevAIOwnedWorktree: cwd !== mainWorkspaceId,
...
test("project.icon.set.request publishes a custom icon that project.icon.get serves back", async () => {
  ...
  const session = asTestSession(
    createSessionForWorkspaceTests({
      q8idevaiHome: path.join(tempDir, "q8idevai-home"),
      onMessage: (message) => emitted.push(message),
    }),
  );
...
test("title-only terminal change does not build workspace descriptors or emit workspace_update", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-title-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("terminal activity contribution change updates the correct workspace", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-activity-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("same-cwd terminal activity updates only the workspace that owns the terminal", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-same-cwd-"));
  const workspaceA = createPersistedWorkspaceRecord({
...
test("a worktree terminal updates only the workspace that owns it", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const rootCwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-nested-"));
...
test("removing an idle terminal does not update workspace status", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-remove-idle-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("removing a contributing terminal clears workspace status", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-session-remove-contrib-"));
  const workspace = createPersistedWorkspaceRecord({
...
function createWorkspaceCreatePrRepo(): WorkspaceCreatePrRepoFixture {
  ...
  return { tempDir, repoDir, q8idevaiHome, headRef, prFileName, prNumber };
}
...
test("workspace.create worktree source checks out a GitHub PR from githubPrNumber", async () => {
  ...
  const session = createSessionForWorkspaceTests({
    ...
    q8idevaiHome: fixture.q8idevaiHome,
    projectRegistry,
    workspaceRegistry,
    workspaceGitService: createNoopWorkspaceGitService({
...
test("workspace auto-name uses the backing root for a nested worktree", async () => {
  ...
  writeQ8iDevAIWorktreeMetadata(repoDir, { baseRefName: "main" });
  writeQ8iDevAIWorktreeFirstAgentBranchAutoNameMetadata(repoDir, {
  ...
  try {
    ...
    expect(readQ8iDevAIWorktreeMetadata(repoDir)).toMatchObject({
      version: 2,
