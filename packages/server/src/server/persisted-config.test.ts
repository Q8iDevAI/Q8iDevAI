...
function createTempHome(): string {
  return mkdtempSync(path.join(tmpdir(), "q8idevai-config-"));
}
...
describe("PersistedConfigSchema worktrees config", () => {
  test("accepts optional worktree root", () => {
    ...
    expect(parsed.worktrees?.root).toBe("/mnt/fast/q8idevai-worktrees");
  });
...
describe("loadPersistedConfig", () => {
  test("materializes relay disabled for a new Q8iDevAI home", () => {
