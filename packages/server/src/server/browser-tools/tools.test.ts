...
interface RegisteredTool {
  ...
  handler: (args: unknown, context: Q8iDevAIToolExecutionContext) => Promise<Q8iDevAIToolResult>;
}
...
class BrowserToolHarness {
  ...
  public async execute(name: string, input: unknown): Promise<Q8iDevAIToolResult> {
...
function schemaFor(inputSchema: Q8iDevAIToolConfig["inputSchema"]): z.ZodType {
  if (!inputSchema) {
...
const routedToolCases = [
  ...
  content: Q8iDevAIToolResult["content"];
}>;
...
describe("registerBrowserTools", () => {
  ...
  test("list tabs sends workspace in the request envelope", async () => {
    ...
    expect(response.content).toEqual([
      {
        type: "text",
        text: `Found 1 Q8iDevAI browser tab. Use these browserId values for tab-scoped browser tools.\n- browserId=${BROWSER_ID} active title="Example" url=https://example.com`,
      },
    ]);
  });
