...
function candidate(input: {
  ...
  return {
    ...
    project: {
      projectKey: "key",
      projectName: input.projectName ?? "getq8idevai/q8idevai",
...
describe("scoreAgentHistoryCandidate", () => {
  ...
  it("matches the project name", () => {
    expect(
      scoreAgentHistoryCandidate("q8idevai", candidate({ projectName: "getq8idevai/q8idevai" })),
    ).not.toBeNull();
  });
...
describe("rankAgentHistoryCandidates", () => {
  ...
  it("ranks the workspace name above the project name that every session shares", () => {
    const ranked = rankAgentHistoryCandidates(
      "q8idevai",
      [
        candidate({ title: "unrelated work", projectName: "getq8idevai/q8idevai" }),
        candidate({ workspaceName: "q8idevai", projectName: "getq8idevai/q8idevai" }),
      ],
      byUpdatedAtDesc,
    );
    expect(ranked[0].candidate.project.workspaceName).toBe("q8idevai");
  });
