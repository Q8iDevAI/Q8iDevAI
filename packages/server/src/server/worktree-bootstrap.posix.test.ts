  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("streams running setup updates live and persists only a final setup timeline row", async () => {
      ...
      const liveSetupItems = live.filter(
        (item) =>
          item.type === "tool_call" &&
          item.name === "q8idevai_worktree_setup" &&
          item.status === "running",
      );
      expect(liveSetupItems.length).toBeGreaterThan(0);

      const persistedSetupItems = persisted.filter(
        (item) => item.type === "tool_call" && item.name === "q8idevai_worktree_setup",
      );
    ...
    it("keeps only the final carriage-return-updated content in command logs", async () => {
      ...
      const persistedSetupItem = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" && item.name === "q8idevai_worktree_setup",
      );
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$Q8IDEVAI_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.Q8IDEVAI_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.Q8IDEVAI_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalWorkspaceIds).toEqual(["ws-shared-runtime-port"]);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "q8idevai_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.Q8IDEVAI_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.Q8IDEVAI_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(apiEnv.Q8IDEVAI_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.Q8IDEVAI_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.Q8IDEVAI_URL).toBe(apiEnv.Q8IDEVAI_SERVICE_API_URL);
      expect(apiEnv.Q8IDEVAI_PORT).toBe(apiEnv.Q8IDEVAI_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.Q8IDEVAI_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.Q8IDEVAI_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:6767",
      );
      expect(webEnv.Q8IDEVAI_SERVICE_API_PORT).toBe(apiEnv.Q8IDEVAI_SERVICE_API_PORT);
      expect(webEnv.Q8IDEVAI_SERVICE_WEB_PORT).toBe(apiEnv.Q8IDEVAI_SERVICE_WEB_PORT);
      expect(webEnv.Q8IDEVAI_URL).toBe(webEnv.Q8IDEVAI_SERVICE_WEB_URL);
      expect(webEnv.Q8IDEVAI_PORT).toBe(webEnv.Q8IDEVAI_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.Q8IDEVAI_SERVICE_API_PORT);
      const webPort = Number(apiEnv.Q8IDEVAI_SERVICE_WEB_PORT);
