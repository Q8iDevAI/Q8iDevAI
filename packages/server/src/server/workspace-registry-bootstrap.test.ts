...
describe("bootstrapWorkspaceRegistries", () => {
  ...
  beforeEach(() => {
    ...
    q8idevaiHome = path.join(tmpDir, ".q8idevai");
    agentStorage = new AgentStorage(path.join(q8idevaiHome, "agents"), logger);
    projectRegistry = new FileBackedProjectRegistry(
      path.join(q8idevaiHome, "projects", "projects.json"),
      logger,
    );
    workspaceRegistry = new FileBackedWorkspaceRegistry(
      path.join(q8idevaiHome, "projects", "workspaces.json"),
      logger,
    );
