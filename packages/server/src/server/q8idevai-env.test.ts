...
import {
  ...
  createQ8iDevAIInternalEnv,
  resolveQ8iDevAINodeEnv,
} from "./q8idevai-env.js";

describe("q8idevai env contract", () => {
  const ELECTRON_RUN_AS_NODE = "ELECTRON_RUN_AS_NODE";
  const Q8IDEVAI_NODE_ENV = "Q8IDEVAI_NODE_ENV";
  const baseEnv = {
    [ELECTRON_RUN_AS_NODE]: "1",
    ELECTRON_NO_ATTACH_CONSOLE: "1",
    NODE_ENV: "development",
    PATH: "/usr/bin",
    Q8IDEVAI_AGENT_ID: "agent-123",
    Q8IDEVAI_DESKTOP_MANAGED: "1",
    [Q8IDEVAI_NODE_ENV]: "production",
    Q8IDEVAI_SUPERVISED: "1",
    ESBUILD_BINARY_PATH: "/Applications/Q8iDevAI.app/Contents/Resources/app.asar.unpacked/esbuild",
  };
  ...
  test("builds internal daemon child env by preserving pass-through and control vars", () => {
    const env = createQ8iDevAIInternalEnv(baseEnv);
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.Q8IDEVAI_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.Q8IDEVAI_DESKTOP_MANAGED).toBeUndefined();
    expect(env[Q8IDEVAI_NODE_ENV]).toBeUndefined();
    expect(env.Q8IDEVAI_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.Q8IDEVAI_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[Q8IDEVAI_NODE_ENV]).toBeUndefined();
    expect(command.env.Q8IDEVAI_SUPERVISED).toBeUndefined();
  });
  ...
  test("does not use user NODE_ENV as Q8iDevAI runtime mode", () => {
    expect(resolveQ8iDevAINodeEnv({ NODE_ENV: "development" })).toBeUndefined();
    expect(resolveQ8iDevAINodeEnv({ NODE_ENV: "development", Q8IDEVAI_NODE_ENV: "production" })).toBe(
      "production",
    );
    expect(resolveQ8iDevAINodeEnv({ NODE_ENV: "test", Q8IDEVAI_NODE_ENV: "local" })).toBeUndefined();
  });
});
