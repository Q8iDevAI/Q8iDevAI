...
async function createQ8iDevAIHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-config-auth-"));
  roots.push(root);
...
describe("daemon auth config", () => {
  ...
  test("loads optional auth password hash from config.json", async () => {
    const q8idevaiHome = await createQ8iDevAIHome({
      version: 1,
      daemon: {
  ...
  test("lets Q8IDEVAI_PASSWORD override config.json auth password hash", async () => {
    const q8idevaiHome = await createQ8iDevAIHome({
      version: 1,
      daemon: {
        auth: { password: CONFIG_PASSWORD_HASH },
    ...
    const config = loadConfig(q8idevaiHome, {
      env: { Q8IDEVAI_PASSWORD: "from-env" },
    });
