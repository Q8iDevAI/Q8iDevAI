...
const originalMaxProcessesPerSecond = process.env.Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND;
...
afterEach(async () => {
  ...
  if (originalMaxProcessesPerSecond === undefined) {
    delete process.env.Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND;
  } else {
    process.env.Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND = originalMaxProcessesPerSecond;
  }
...
function seedFixture(siblingCount = SIBLING_COUNT): {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), "q8idevai-workspace-create-fanout-"));
  cleanupPaths.push(fixtureRoot);
  const repoRoot = join(fixtureRoot, "repo");
  const worktreesRoot = join(fixtureRoot, "siblings");
  const q8idevaiHomeRoot = join(fixtureRoot, "home");
  const projectsDir = join(q8idevaiHomeRoot, ".q8idevai", "projects");
  ...
  return { repoRoot, q8idevaiHomeRoot, projectId, siblingWorktrees };
}
...
async function startObservedFixture(siblingCount: number): Promise<ReturnType<typeof seedFixture>> {
  ...
  daemon = await createTestQ8iDevAIDaemon({
    q8idevaiHomeRoot: fixture.q8idevaiHomeRoot,
    cleanup: false,
    mcpEnabled: false,
  });
...
test("workspace create is admitted while 100 sibling observations hydrate", async () => {
  process.env.Q8IDEVAI_GIT_MAX_PROCESSES_PER_SECOND = "64";
  configureGitProcessPolicy({ maxProcessConcurrency: 8, maxProcessesPerSecond: 64 });
