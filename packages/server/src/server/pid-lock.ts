...
function getPidFilePath(q8idevaiHome: string): string {
  return join(q8idevaiHome, "q8idevai.pid");
}
...
function createLockHeldError(lock: PidLockInfo): PidLockError {
  return new PidLockError(
    `Another Q8iDevAI daemon is already running (PID ${lock.pid}, started ${lock.startedAt})`,
    lock,
  );
}
  ...
  try {
    ...
    if (raceLock) {
      throw new PidLockError(
        `Another Q8iDevAI daemon is already running (PID ${raceLock.pid})`,
        raceLock,
      );
    }
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(q8idevaiHome);

  ensurePrivateDirectory(q8idevaiHome);
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    heartbeat: true,
    ...(process.env.Q8IDEVAI_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
...
export function startPidLockHeartbeat(
  ...
  const timer = setInterval(() => {
    ...
    refreshing = true;
    refreshPidLock(q8idevaiHome, { ownerPid: options?.ownerPid })
      .catch((error) => {
        if (options?.onError) {
...
export async function getPidLockInfo(q8idevaiHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(q8idevaiHome);
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(q8idevaiHome);
  if (!info) {
    return { locked: false };
  }
