...
export function buildWorkspaceServiceEnv(
  ...
  const env: Record<string, string> = {
    ...
    Q8IDEVAI_PORT: String(selfPeer.port),
  };
  ...
  if (selfProxyUrl) {
    env.Q8IDEVAI_URL = selfProxyUrl;
  }

  for (const peer of options.peers) {
    const envName = normalizeServiceEnvName(peer.scriptName);
    env[`Q8IDEVAI_SERVICE_${envName}_PORT`] = String(peer.port);
    ...
    if (peerProxyUrl) {
      env[`Q8IDEVAI_SERVICE_${envName}_URL`] = peerProxyUrl;
    }
  }
