...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createQ8iDevAIDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "q8idevai-bootstrap-provider-"));
    tempRoots.push(root);
    ...
    const agentStoragePath = path.join(q8idevaiHome, "agents");
