...
import { asUint8Array, decodeBinaryFrame } from "@q8idevai/protocol/binary-frames/index";
...
} from "@q8idevai/protocol/agent-attention-notification";
...
export class VoiceAssistantWebSocketServer {
  ...
  constructor(
    ...
    this.messageReceipts = new MessageReceipts(join(q8idevaiHome, "agent-requests"));
    this.creationService = new CreationService(
      join(q8idevaiHome, "creations"),
      this.logger.child({ module: "creation" }),
      (snapshot) => this.validateCompletedCreation(snapshot),
      join(q8idevaiHome, "agent-requests"),
    );
    ...
    this.pushNotifications = createPushNotifications({
      logger: pushLogger,
      filePath: join(q8idevaiHome, "push-tokens.json"),
    });
