...
import { CheckoutPrStatusResponseSchema } from "@q8idevai/protocol/messages";
    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/q8idevai.git",
            url: "https://github.com/therainisme/q8idevai",
          },
        },
      },
    },
...
describe("ForgeService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    await expect(
      ...
      headRepositorySshUrl: "git@github.com:therainisme/q8idevai.git",
      headRepositoryUrl: "https://github.com/therainisme/q8idevai",
      isCrossRepository: true,
    });
    ...
    expect(runner.calls[1]?.args).toContain("owner=getq8idevai");
    expect(runner.calls[1]?.args).toContain("name=q8idevai");
      ...
      pullRequestTimelineJson({
        ...
        comments: {
          nodes: [
            {
              id: "IC_badge",
              body: "![build](https://img.shields.io/github/actions/workflow/status/getq8idevai/q8idevai/ci.yml)",
              bodyHTML:
  ...
  it("loads GitHub merge, auto-merge, permission, policy, and queue facts for PR 993 shape", async () => {
    const runner = createScriptedRunner([
      currentPullRequestJson({
        number: 993,
        url: "https://github.com/Q8iDevAI/Q8iDevAI/pull/993",
        title: "Auto-merge UX",
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/Q8iDevAI/Q8iDevAI/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
      ...
      checks: [
        {
          name: "server tests",
          status: "pending",
          url: "https://github.com/Q8iDevAI/Q8iDevAI/actions/runs/993",
          workflow: "CI",
        },
      ],
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/Q8iDevAI/Q8iDevAI/pull/793",
      limit: 5,
    });
  ...
  it("does not treat an unrelated tracker URL as a GitHub issue/PR number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://gitlab.com/getq8idevai/q8idevai/issues/793",
      limit: 5,
    });
    ...
    expect(runner.calls.map((call) => call.args)).toEqual([
      [
        ...
        "list",
        "--search",
        "https://gitlab.com/getq8idevai/q8idevai/issues/793",
        "--json",
        "number,title,url,state,body,labels,updatedAt",
        "--limit",
        "5",
  ...
  it("treats a GitHub Enterprise issue/PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.acme.internal/getq8idevai/q8idevai/pull/793",
      limit: 5,
    });
