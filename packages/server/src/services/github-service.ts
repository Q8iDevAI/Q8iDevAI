...
} from "@q8idevai/protocol/git-remote";
...
fragment Q8iDevAIPollPullRequest on PullRequest {
  number
  url
  title
  state
...
function buildBatchPullRequestStatusQuery(
  ...
  const aliases = entries.map(
    ...
    pullRequests(headRefName: ${graphqlString(
      ...
      nodes {
        ...Q8iDevAIPollPullRequest
      }
    }
  }`,
  );
  return `query Q8iDevAIBatchPullRequestStatus {
  rateLimit {
    limit
    remaining
    resetAt
...
function buildBatchPullRequestChecksQuery(
  ...
  return `query Q8iDevAIBatchPullRequestChecks {
  rateLimit {
    limit
    remaining
    resetAt
