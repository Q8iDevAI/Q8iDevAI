...
import { parseGitRemoteLocation, type GitRemoteLocation } from "@q8idevai/protocol/git-remote";
...
export function createForgeResolver(options: CreateForgeResolverOptions = {}): ForgeResolver {
  ...
  // from outside Q8iDevAI — `git remote add`/`set-url` run in a terminal — is
  // still picked up within a cycle instead of staying pinned forever; a Q8iDevAI
