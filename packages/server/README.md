...
Q8IDEVAI_HOME=~/.q8idevai        # Runtime state directory (agents/, etc.)
Q8IDEVAI_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`Q8IDEVAI_HOME` defaults to `~/.q8idevai` and isolates runtime artifacts like `agents/`. `Q8IDEVAI_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
Q8IDEVAI_HOME=~/.q8idevai-blue Q8IDEVAI_LISTEN=127.0.0.1:7777 npm run dev
```
