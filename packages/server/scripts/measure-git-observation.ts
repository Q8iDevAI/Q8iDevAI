...
async function main(): Promise<void> {
  const tempDir = mkdtempSync(path.join(tmpdir(), "q8idevai-git-observation-measurement-"));
  ...
  runGit(repoDir, ["config", "user.email", "measurement@q8idevai.local"]);
  runGit(repoDir, ["config", "user.name", "Q8iDevAI Measurement"]);
  ...
  try {
    healthy = createMeasuredService({ repoDir, q8idevaiHome });
    ...
    degraded = createMeasuredService({ repoDir, q8idevaiHome, failWorktreeWatch: true });
    startGitCommandMetrics();
    ...
    const outputPath = process.env.Q8IDEVAI_GIT_OBSERVATION_REPORT?.trim();
    if (outputPath) {
