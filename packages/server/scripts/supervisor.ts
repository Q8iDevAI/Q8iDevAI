...
interface SupervisorHeartbeatMessage {
  type: "q8idevai:supervisor-heartbeat";
}

interface SupervisorGracefulShutdownMessage {
  type: "q8idevai:graceful-shutdown";
  reason: string;
}
...
export function runSupervisor(options: SupervisorOptions): SupervisorController {
  ...
  const spawnWorker = () => {
    ...
    const heartbeat = setInterval(() => {
      const message: SupervisorHeartbeatMessage = { type: "q8idevai:supervisor-heartbeat" };
    ...
    child.on("message", (msg: unknown) => {
      ...
      if (lifecycleMessage.type === "q8idevai:ready") {
        reachedReady = true;
      ...
      if (lifecycleMessage.type === "q8idevai:shutdown") {
