...
async function main(): Promise<void> {
  const daemonUrl = process.env.Q8IDEVAI_DAEMON_URL ?? "ws://localhost:6767".replace(/^ws/, "http");
  const timeoutMs = Number(process.env.Q8IDEVAI_VOICE_E2E_TIMEOUT_MS ?? "120000");
