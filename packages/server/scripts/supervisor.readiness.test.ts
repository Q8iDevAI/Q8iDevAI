...
async function runWorker(worker: string, publicationDelay = 0) {
  const home = await mkdtemp(path.join(tmpdir(), "q8idevai-readiness-"));
  ...
  const child = spawn(process.execPath, ["--import", "tsx", runnerPath], {
    ...
    env: { ...env, HOME: home, Q8IDEVAI_HOME: home },
...
test("a replacement worker must independently reach ready", async () => {
  const result = await runWorker(`
    ...
    process.send({ type: "q8idevai:ready", listen: "test-endpoint" });
    process.send({ type: "q8idevai:restart" });
  `);
  expect(result.code).toBe(1);
...
test("exit clears publication after an in-flight ready write", async () => {
  const result = await runWorker(
    `
    process.send({ type: "q8idevai:ready", listen: "test-endpoint" }, () => process.exit(0));
  `,
    100,
  );
...
test("requested shutdown before first readiness exits successfully", async () => {
  const result = await runWorker(`
    ...
    process.send({ type: "q8idevai:shutdown", reason: "cancelled_start" });
  `);
  expect(result.code).toBe(0);
