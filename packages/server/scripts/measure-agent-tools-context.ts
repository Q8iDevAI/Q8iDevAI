...
const MEASUREMENT_CWD = "/tmp/q8idevai-agent-tools-measurement";
...
Measures the MCP tools/list payload for Q8iDevAI agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "q8idevai-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Q8iDevAI Agent Tool Catalog Context");
  lines.push("");
