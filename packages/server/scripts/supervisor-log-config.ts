...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.Q8IDEVAI_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.Q8IDEVAI_LOG_ROTATE_COUNT);
  let logPath = path.join(q8idevaiHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(q8idevaiHome, configuredPath);
  }
