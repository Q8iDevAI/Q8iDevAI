...
import { createTestQ8iDevAIDaemon } from "../src/server/test-utils/q8idevai-daemon.js";
...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.Q8IDEVAI_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".q8idevai");
  let frozenHomeRoot = process.env.Q8IDEVAI_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.Q8IDEVAI_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot = requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "q8idevai-real-home-"));
  if (process.env.Q8IDEVAI_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".q8idevai");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestQ8iDevAIDaemon({ q8idevaiHomeRoot: frozenHomeRoot, cleanup: false });
