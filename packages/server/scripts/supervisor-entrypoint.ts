...
function resolveWorkerExecArgv(workerEntry: string, devMode: boolean): string[] {
  ...
  const devArgs = [
    ...
    "--report-directory=/tmp/q8idevai-reports",
  ];
  const inspectArg = process.env.Q8IDEVAI_NODE_INSPECT ?? "--inspect";
...
function resolvePackagedNodeEntrypointRunnerPath(currentScriptPath: string): string | null {
  const packageMarker = `${path.sep}node_modules${path.sep}@getq8idevai${path.sep}server${path.sep}`;
...
async function main(): Promise<void> {
  ...
  const q8idevaiHome = resolveQ8iDevAIHome(workerEnv);
  const persistedConfig = loadPersistedConfig(q8idevaiHome);
  const supervisorLogFile = resolveSupervisorLogFile(q8idevaiHome, persistedConfig, workerEnv);

  try {
    await acquirePidLock(q8idevaiHome, null, {
  ...
  const stopLockHeartbeat = startPidLockHeartbeat(q8idevaiHome, {
    ownerPid: process.pid,
    onError: (error) => {
  ...
  const releaseLock = async (): Promise<void> => {
    ...
    lockReleased = true;
    stopLockHeartbeat();
    await releasePidLock(q8idevaiHome, {
      ownerPid: process.pid,
    });
  };
  ...
  const supervisor = runSupervisor({
    ...
    onWorkerReady: async ({ listen }) => {
      await updatePidLock(q8idevaiHome, { listen }, { ownerPid: process.pid });
    },
    onWorkerExit: () => updatePidLock(q8idevaiHome, { listen: null }, { ownerPid: process.pid }),
