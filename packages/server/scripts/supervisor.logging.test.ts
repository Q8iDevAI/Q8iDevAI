...
async function runSupervisorFixture(options: {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "q8idevai-supervisor-log-"));
  ...
  await writeFile(
    workerPath,
    `process.send?.({ type: "q8idevai:ready", listen: "fixture" });\n${options.workerSource}`,
  );
  await writeFile(
    runnerPath,
    `
...
describe("supervisor durable logging", () => {
  test("resolves rotation defaults", () => {
    const q8idevaiHome = path.join(path.sep, "tmp", "q8idevai-home");
    const logFile = resolveSupervisorLogFile(q8idevaiHome, {}, {});

    expect(logFile).toEqual({
      path: path.join(q8idevaiHome, "daemon.log"),
  ...
  test("lets persisted rotation override env rotation defaults", () => {
    ...
    const logFile = resolveSupervisorLogFile(
      ...
      {
        Q8IDEVAI_LOG_ROTATE_SIZE: "200m",
        Q8IDEVAI_LOG_ROTATE_COUNT: "12",
      },
    );

    expect(logFile).toEqual({
      path: path.resolve(q8idevaiHome, "logs", "daemon.log"),
      rotate: { maxSize: "25m", maxFiles: 4 },
    });
  });
  ...
  test("uses env rotation when persisted rotation is absent", () => {
    ...
    const logFile = resolveSupervisorLogFile(
      ...
      {},
      {
        Q8IDEVAI_LOG_ROTATE_SIZE: "50m",
        Q8IDEVAI_LOG_ROTATE_COUNT: "8",
      },
    );
    ...
    const result = await runSupervisorFixture({
      workerSource: `
        process.on("message", (message) => {
          if (message?.type === "q8idevai:graceful-shutdown") process.exit(0);
        });
        process.send?.({ type: "q8idevai:shutdown", reason: "client_shutdown_rpc" });
        setInterval(() => {}, 1000);
      `,
    });
  ...
  test("lets the worker clean up its descendant before supervised shutdown", async () => {
    const result = await runSupervisorFixture({
      workerSource: `
        ...
        process.on("message", (message) => {
          if (message?.type !== "q8idevai:graceful-shutdown") return;
        ...
        process.send?.({ type: "q8idevai:shutdown", reason: "descendant_cleanup_probe" });
        setInterval(() => {}, 1000);
      `,
    });
    ...
    const result = await runSupervisorFixture({
      ...
      workerSource: `
        ...
        if (!existsSync(marker)) {
          ...
          setTimeout(() => {
            process.send?.({ type: "q8idevai:shutdown", reason: "silent_worker_test_complete" });
          }, 16_000);
          setInterval(() => {}, 1_000);
        } else {
          process.send?.({ type: "q8idevai:shutdown", reason: "unexpected_silent_worker_restart" });
          setInterval(() => {}, 1_000);
        }
      `,
    });
  ...
  test("forces shutdown when a worker ignores the graceful shutdown request", async () => {
    const result = await runSupervisorFixture({
      timeoutMs: 15_000,
      workerSource: `
          process.send?.({ type: "q8idevai:shutdown", reason: "stalled_worker_shutdown" });
          setInterval(() => {}, 1_000);
        `,
    });
      ...
      const result = await runSupervisorFixture({
        ...
        workerSource: `
          ...
          if (!existsSync(marker)) {
            ...
            descendant.unref();
            process.send?.({ type: "q8idevai:restart", reason: "stdio_descendant" });
            setInterval(() => {}, 1000);
          } else {
            process.send?.({ type: "q8idevai:shutdown", reason: "stdio_restart_complete" });
            setInterval(() => {}, 1000);
          }
        `,
      });
