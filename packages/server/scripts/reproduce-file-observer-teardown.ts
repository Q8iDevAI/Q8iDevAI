...
const ITERATIONS = readPositiveInteger("Q8IDEVAI_WATCH_REPRO_ITERATIONS", 200);
const CHILD_TIMEOUT_MS = readPositiveInteger(
  "Q8IDEVAI_WATCH_REPRO_TIMEOUT_MS",
...
async function runChild(): Promise<ChildResult> {
  const base = await mkdtemp(join(tmpdir(), "q8idevai-watch-repro-"));
