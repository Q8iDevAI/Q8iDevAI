...
async function verifyInjectedMcpForProvider(
  ...
  try {
    const prompt = [
      ...
      "If you have a tool called list_agents or create_agent from a q8idevai MCP server, call list_agents once.",
      "After checking, reply with exactly Q8IDEVAI_MCP_FOUND.",
      "If you do not have those tools, reply with exactly Q8IDEVAI_MCP_NOT_FOUND.",
    ...
    if (!assistantText.includes("Q8IDEVAI_MCP_FOUND")) {
      throw new Error(
        `Expected assistant to confirm Q8iDevAI MCP availability. Assistant text:\n${assistantText}`,
      );
    }
    ...
    const listAgentsCalls = toolCalls.filter(
      (call) =>
        ...
        call.name === "q8idevai.list_agents" ||
...
async function main(): Promise<void> {
  ...
  const rootCwd = await mkdtemp(path.join(os.tmpdir(), "q8idevai-mcp-inject-real-"));
