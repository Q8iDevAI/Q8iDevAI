...
import type { AgentProviderNotice } from "@q8idevai/protocol/agent-types";
...
export async function runUpdateCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: q8idevai agent update <id> [--name <name>] [--label <key=value>]",
    };
    throw error;
  }
  ...
  try {
    ...
    if (!fetchResult) {
      const error: CommandError = {
        ...
        details: 'Use "q8idevai ls" to list available agents',
      };
      throw error;
    }
