...
import { getStructuredAgentResponse, StructuredAgentResponseError } from "@q8idevai/server";
import type { AgentSnapshotPayload } from "@q8idevai/protocol/messages";
...
function validateRunWorkspaceOptions(options: AgentRunOptions): void {
  ...
  if (hasWorktreeCreationOptions && newWorkspace !== "worktree") {
    throw {
      ...
      details: "Usage: q8idevai run --new-workspace worktree [worktree options] <prompt>",
    } satisfies CommandError;
  }
...
function validateRunOptions(prompt: string, options: AgentRunOptions, outputSchema: unknown): void {
  if (!prompt || prompt.trim().length === 0) {
    throw {
      ...
      details: "Usage: q8idevai agent run [options] <prompt>",
    } satisfies CommandError;
  }
...
// Workspace policy for `q8idevai run`. Precedence:
//   1. --workspace <id>            -> run in that existing workspace
//   2. $Q8IDEVAI_AGENT_ID             -> daemon resolves the caller's workspace
//   3. $Q8IDEVAI_WORKSPACE_ID         -> exported by workspace terminals
...
async function resolveRunWorkspace(
  ...
  const ambientWorkspaceId = newWorkspace ? undefined : process.env.Q8IDEVAI_WORKSPACE_ID?.trim();
  ...
  console.error(
    "Tip: pass --workspace <id> (or set Q8IDEVAI_WORKSPACE_ID) to run in an existing workspace.",
  );
  ...
  try {
    ...
    if (options.thinking !== undefined && !thinkingOptionId) {
      const error: CommandError = {
        ...
        details:
          'Provide a thinking option ID. Use "q8idevai provider models <provider> --thinking" to list valid IDs.',
      };
      throw error;
    }
...
export function resolveRunCallerAgentId(
  env: { Q8IDEVAI_AGENT_ID?: string } = process.env,
): string | undefined {
  return env.Q8IDEVAI_AGENT_ID?.trim() || undefined;
}
