...
describe("managed agent caller context", () => {
  it("propagates a trimmed Q8IDEVAI_AGENT_ID", () => {
    expect(resolveRunCallerAgentId({ Q8IDEVAI_AGENT_ID: "  parent-agent  " })).toBe("parent-agent");
  });

  it("omits blank caller ids", () => {
    expect(resolveRunCallerAgentId({ Q8IDEVAI_AGENT_ID: "   " })).toBeUndefined();
  });
});
...
describe("runRunCommand option validation", () => {
  const originalWorkspaceId = process.env.Q8IDEVAI_WORKSPACE_ID;

  beforeEach(() => {
    delete process.env.Q8IDEVAI_WORKSPACE_ID;
  });
  ...
  afterEach(() => {
    if (originalWorkspaceId === undefined) {
      ...
      process.env.Q8IDEVAI_WORKSPACE_ID = originalWorkspaceId;
    }
  });
