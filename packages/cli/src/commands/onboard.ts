...
function printNextSteps(pairingUrl: string | null, q8idevaiHome: string, richUi: boolean): void {
  const daemonLogPath = path.join(q8idevaiHome, "daemon.log");
  const nextStepsLines = [
    pairingUrl
      ? "1. Open Q8iDevAI and scan the QR code above, or paste the pairing link."
      : "1. Open Q8iDevAI and connect to your daemon.",
    "2. Web app: https://app.q8idevai.sh",
    "3. Desktop app: https://github.com/Q8iDevAI/Q8iDevAI/releases/latest",
    "4. Docs: https://q8idevai.sh/docs",
    `5. Example: q8idevai run --home ${JSON.stringify(q8idevaiHome)} --output-schema schema.json "extract fields"`,
  ];
  const quickReferenceLines = [
    "1. q8idevai --help",
    `2. q8idevai ls --home ${JSON.stringify(q8idevaiHome)}`,
    `3. q8idevai run --home ${JSON.stringify(q8idevaiHome)} "your prompt"`,
    `4. q8idevai status --home ${JSON.stringify(q8idevaiHome)}`,
...
async function resolveAndPersistVoice(
  ...
  let persisted = loadPersistedConfig(q8idevaiHome) as OnboardPersistedConfig;
  ...
  savePersistedConfig(q8idevaiHome, persisted);
  return voiceEnabled;
}

function persistSetupChoices(q8idevaiHome: string, options: OnboardOptions): void {
  const persisted = loadPersistedConfig(q8idevaiHome, { defaultsIfMissing: true });
...
export async function runOnboard(options: OnboardOptions): Promise<void> {
  ...
  const q8idevaiHome = options.daemonTarget.home;
  const alreadyRunning = await readDaemonInstance(q8idevaiHome);
  persistSetupChoices(q8idevaiHome, options);
  if (richUi) {
    renderNote(q8idevaiHome, "Q8iDevAI home");
  }

  const voiceEnabled = await resolveAndPersistVoice(q8idevaiHome, options);
  log.message(
    voiceEnabled
  ...
  if (alreadyRunning) {
    ...
    await launchLocalDaemon({ home: q8idevaiHome, timeoutMs });
  }
  const ready = await waitForDaemonReady(q8idevaiHome, { timeoutMs });
  log.message(`Daemon ready on ${ready.listen}`);

  if (options.relay === false) {
    log.message("Relay pairing skipped because --no-relay was provided.");
    printNextSteps(null, q8idevaiHome, richUi);
    if (richUi) outro("Q8iDevAI daemon is running.");
    return;
  }
  ...
  if (!pairing.relayEnabled) {
    ...
    pairing = await resolveLocalPairingOffer({ q8idevaiHome, enableRelay: true });
  ...
  if (!pairing.url) {
    ...
    printNextSteps(null, q8idevaiHome, richUi);
    if (richUi) {
      outro("Q8iDevAI daemon is running.");
    }
    return;
  }
  ...
  printNextSteps(pairing.url, q8idevaiHome, richUi);
  if (richUi) {
    outro("Q8iDevAI is ready!");
  }
}
