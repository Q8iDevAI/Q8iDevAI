...
import { resolveHookActivity, type AgentHookActivityState } from "@q8idevai/server/agent-hooks";

interface HookEnvironment {
  Q8IDEVAI_TERMINAL_ID?: string;
  Q8IDEVAI_ACTIVITY_TOKEN?: string;
  Q8IDEVAI_TERMINAL_ACTIVITY_URL?: string;
}
...
function resolveTarget(env: HookEnvironment) {
  const terminalId = env.Q8IDEVAI_TERMINAL_ID;
  const token = env.Q8IDEVAI_ACTIVITY_TOKEN;
  const url = env.Q8IDEVAI_TERMINAL_ACTIVITY_URL;
