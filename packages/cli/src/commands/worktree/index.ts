...
export function createWorktreeCommand(): Command {
  const worktree = new Command("worktree").description("Manage Q8iDevAI-managed git worktrees");

  addJsonAndDaemonHostOptions(
    worktree.command("ls").description("List Q8iDevAI-managed git worktrees"),
  ).action(withOutput(runLsCommand));

  addJsonAndDaemonHostOptions(
    worktree
      .command("create")
      .description("Create a Q8iDevAI-managed git worktree")
