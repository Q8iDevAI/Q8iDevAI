...
export async function runArchiveCommandWithDeps(
  ...
  if (!nameArg || nameArg.trim().length === 0) {
    const error: CommandError = {
      ...
      details: "Usage: q8idevai worktree archive <name>",
    };
    throw error;
  }
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details: "Start the daemon with: q8idevai daemon start",
    };
    throw error;
  }
  ...
  try {
    ...
    const listResponse = await client.getQ8iDevAIWorktreeList({});
    ...
    if (!worktree) {
      const error: CommandError = {
        ...
        details: 'Use "q8idevai worktree ls" to list available worktrees',
      };
      throw error;
    }

    // Archive the worktree. scope:"worktree" archives every active workspace on
    // the directory and then removes the directory (Q8iDevAI-owned gated).
    const response = await client.archiveQ8iDevAIWorktree({
