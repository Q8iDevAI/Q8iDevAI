...
import { PluginIdSchema } from "@q8idevai/protocol/messages";
...
const CLIENT_ENTRY = `import type { PluginClientContext } from "@q8idevai/plugin/client";
...
const SERVER_ENTRY = `import type { PluginServerContext } from "@q8idevai/plugin/server";
...
const SHARED_GREETING = `import { defineRpc } from "@q8idevai/plugin";
import { z } from "zod";
...
const SERVER_GREETING = `import type { RpcInput } from "@q8idevai/plugin";
import { greetingRpc } from "../shared/greeting";
...
const CLIENT_GREETING = `import type { PluginSurfaceProps } from "@q8idevai/plugin/client";
  ...
  return (
    <View style={styles.screen}>
      ...
      <Pressable
        ...
        style={styles.button}
        onPress={() => greeting.mutate({ name: "Q8iDevAI" })}
      >
        <Text style={styles.buttonText}>Create greeting</Text>
      </Pressable>
      <Pressable
        accessibilityRole="link"
        accessibilityLabel="Open the Q8iDevAI website"
        style={styles.button}
        onPress={() => openExternal("https://q8idevai.sh")}
      >
        <Text style={styles.buttonText}>Open q8idevai.sh</Text>
      </Pressable>
    </View>
  );
}
...
export async function scaffoldPluginDirectory(
  ...
  const files = new Map<string, string>([
    [
      ...
      `${JSON.stringify({ id, requirements: { q8idevai: `>=${version}` } }, null, 2)}\n`,
    ],
