...
describe("plugin scaffold", () => {
  it(
    ...
    async () => {
      ...
      expect(JSON.parse(await readFile(path.join(directory, "q8idevai-plugin.json"), "utf8"))).toEqual(
        {
          id: "hello-plugin",
          requirements: { q8idevai: `>=${cliPackageJson.version}` },
        },
      );
      expect(JSON.parse(await readFile(path.join(directory, "package.json"), "utf8"))).toEqual({
        ...
        devDependencies: {
          "@q8idevai/plugin": cliPackageJson.version,
      ...
      expect(await readdir(directory)).not.toContain("q8idevai-plugin.d.ts");
      ...
      await expect(
        readFile(path.join(directory, "client/greeting.tsx"), "utf8"),
      ).resolves.toContain(`openExternal("https://q8idevai.sh")`);
  ...
  it("typechecks client and server Q8iDevAI API access", async () => {
    const parent = await mkdtemp(path.join(process.cwd(), ".plugin-scaffold-"));
    directories.push(parent);
    const directory = path.join(parent, "q8idevai-api-plugin");
    await scaffoldPluginDirectory(directory);
    await Promise.all([
      writeFile(
...
export async function inspectConfig(
  ...
  { q8idevai }: PluginHandlerContext,
) {
  return { configured: Boolean((await q8idevai.config.get()).config) };
}
`,
      ),
      writeFile(
...
import { type PluginAgentPanelProps, type PluginClientContext, type PluginSurfaceProps, useAgent, useQ8iDevAI, useWorkspace } from "@q8idevai/plugin/client";
import { inspect } from "../shared/inspect";

export function Surface({ navigation }: PluginSurfaceProps) {
  const q8idevai = useQ8iDevAI();
  const toast = useToast();
  const createWorkspace = () => q8idevai.workspaces.create({
    source: { kind: "directory", path: "/repo" },
  });
  navigation?.openAgent({ agentId: "agent-1" });
  navigation?.openWorkspace({ workspaceId: "workspace-1" });
  void createWorkspace;
  return <><Icon name="Settings" size={18} color="#123456" /><Text onPress={() => toast.show("Ready")}>Q8iDevAI API</Text><Modal title="Example" icon={<Icon name="Settings" />} open={false} onOpenChange={() => {}}><Modal.Content><Text>Modal</Text></Modal.Content></Modal></>;
}
...
export default function contribute(client: PluginClientContext) {
  ...
  client.addCommandCenterItem({
    ...
    icon: "Scan",
    context: "agent",
    async onSelect({ q8idevai, rpc, workspace, openPanel }) {
      await q8idevai.workspaces.ref(workspace.id).setTitle("Review");
      await rpc(inspect, {});
      openPanel("review");
    },
  });
...
`,
  ...
  it("reports TypeScript errors for the old composer pill shape and cleanup", async () => {
    ...
    await writeFile(
      path.join(directory, "index.client.tsx"),
      `
import type { PluginClientContext, PluginComposerPillProps } from "@q8idevai/plugin/client";
...
`,
  ...
  it("refuses to write into a non-empty directory", async () => {
    const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-scaffold-"));
