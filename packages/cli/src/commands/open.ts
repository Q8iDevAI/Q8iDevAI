...
import { spawnProcess } from "@q8idevai/server";
import { buildAgentDeepLink, type AgentDeepLinkTarget } from "@q8idevai/protocol/agent-deep-link";

function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Q8iDevAI.app",
      path.join(homedir(), "Applications", "Q8iDevAI.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Q8iDevAI",
      "/opt/Q8iDevAI/Q8iDevAI",
      path.join(homedir(), "Applications", "Q8iDevAI.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Q8iDevAI", "Q8iDevAI.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.Q8IDEVAI_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Q8iDevAI Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Q8iDevAI desktop app not found. Install it from https://github.com/Q8iDevAI/Q8iDevAI/releases",
    );
  }
