...
export async function setDaemonPasswordInConfig(
  ...
  const q8idevaiHome = resolveQ8iDevAIHome({ Q8IDEVAI_HOME: options.home });
  const configPath = path.join(q8idevaiHome, CONFIG_FILENAME);
  const persisted = readPersistedConfig(q8idevaiHome);
  ...
  savePersistedConfig(q8idevaiHome, nextConfig);

  return {
    action: "password_set",
    configPath,
    restartCommand: `q8idevai daemon restart --home ${JSON.stringify(q8idevaiHome)}`,
    message: `Password written to ${configPath}\nRestart the daemon for the change to take effect.\nRun: q8idevai daemon restart --home ${JSON.stringify(q8idevaiHome)}`,
  };
}
