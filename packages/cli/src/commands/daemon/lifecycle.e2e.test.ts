...
import { startDaemonInstance, readDaemonInstance } from "@q8idevai/server";
...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), "q8idevai lifecycle "));
  const env = {
  ...
  async function run(args: string[], overrides: NodeJS.ProcessEnv = {}) {
    ...
    for (const home of homes) {
      try {
        const lock = JSON.parse(await readFile(path.join(home, "q8idevai.pid"), "utf8"));
        owned.set(home, lock);
      } catch {
        /* No acquired launch. */
      }
...
test("managed two-home restart retains its supervisor and never routes ordinary commands or stop to the other home", async () => {
  ...
  try {
    ...
    const poisoned = {
      Q8IDEVAI_HOME: a,
      Q8IDEVAI_HOST: `127.0.0.1:${portA}`,
      Q8IDEVAI_LISTEN: `127.0.0.1:${portA}`,
      PORT: String(portA),
      Q8IDEVAI_WEB_UI_ENABLED: "true",
      Q8IDEVAI_RELAY_ENABLED: "true",
    };
    ...
    if (process.platform !== "win32") {
      for (const home of [a, b, path.join(f.root, ".q8idevai")])
...
test("worker restart preserves an already-running legacy supervisor's launch flags", async () => {
  ...
  try {
    ...
    const launch = await startDaemonInstance({
      ...
      // The legacy CLI translated --port into Q8IDEVAI_LISTEN before spawning.
      env: { ...f.env, Q8IDEVAI_LISTEN: `127.0.0.1:${launchPort}` },
      mode: "deployment",
      timeoutMs: 30_000,
    });
    ...
    const restarted = await f.ok(["restart", "--home", home, "--timeout", "30"], {
      Q8IDEVAI_LISTEN: `127.0.0.1:${filePort}`,
    });
...
test.skipIf(process.platform === "win32")(
  ...
  async () => {
    ...
    try {
      ...
      await f.ok(["start", "--home", a, "--timeout", "30"]);
      await f.ok(["start", "--home", b, "--timeout", "30"], { Q8IDEVAI_SERVER_ID: "live-b" });
      const beforeA = await f.liveStatus(a);
      expect((await f.liveStatus(b)).serverId).toBe("live-b");
      await f.ok(["restart", "--home", b, "--timeout", "30"]);
      const bLock = JSON.parse(await readFile(path.join(b, "q8idevai.pid"), "utf8"));
      await writeFile(
        path.join(b, "q8idevai.pid"),
...
test.skipIf(process.platform === "win32").each([["start"], ["daemon", "run"]])(
  ...
  async (...launchArgs) => {
    ...
    try {
      ...
      await expect
        .poll(async () => existsSync(path.join(home, "q8idevai.pid")), { timeout: 10_000 })
        .toBe(true);
      const lock = JSON.parse(await readFile(path.join(home, "q8idevai.pid"), "utf8"));
      await f.ok(["status", "--home", home]);
      child.kill("SIGINT");
...
test("empty explicit selectors never select the ambient daemon or create local state", async () => {
  ...
  try {
    ...
    for (const args of [
      ...
      const refused = await f.run(["--json", ...args], { Q8IDEVAI_HOME: home });
      expect(refused.code).toBe(1);
      expect(refused.stderr).toContain("TARGET_INVALID");
      expect((await f.liveStatus(home)).workerPid).toBe(before.workerPid);
      expect(existsSync(path.join(f.root, "q8idevai.pid"))).toBe(false);
...
test("raw log following subscribes to a stored agent that exists only in B", async () => {
  ...
  try {
    ...
    follower = spawn(
      ...
      {
        cwd: f.root,
        env: { ...f.env, Q8IDEVAI_HOME: a, Q8IDEVAI_HOST: launchA.listen },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
    let output = "",
...
test("foreground deployment retains environment until its owner ends the launch; endpoint stop reports acknowledgment", async () => {
  ...
  try {
    ...
    deployment = spawn(process.execPath, [cli, "daemon", "run", "--home", home], {
      cwd: f.root,
      env: { ...f.env, Q8IDEVAI_LISTEN: deploymentEndpoint, Q8IDEVAI_RELAY_ENABLED: "false" },
      stdio: "ignore",
    });
    await expect
    ...
    const restarted = await f.ok(["restart", "--home", home], { Q8IDEVAI_LISTEN: fileEndpoint });
