...
export async function runDaemonReloadCommand(
  ...
  try {
    ...
    return {
      type: "single",
      data: {
        restartCommand: `q8idevai daemon restart ${options.daemonTarget.kind === "instance" ? `--home ${JSON.stringify(options.daemonTarget.home)}` : `--host ${JSON.stringify(describeDaemonTarget(options.daemonTarget))}`}`,
