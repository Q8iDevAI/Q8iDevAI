...
import { startDaemonInstance, resolveQ8iDevAIHome } from "@q8idevai/server";
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@q8idevai/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@q8idevai/server");
  ...
  throw new Error("Unable to resolve @q8idevai/server package root for daemon runner");
}
...
export async function launchLocalDaemon(options: {
  ...
  try {
    ...
    return await startDaemonInstance({
      home: resolveQ8iDevAIHome({ Q8IDEVAI_HOME: options.home }),
  ...
  command.hook("preAction", () => {
    for (const [flag, configPath] of Object.entries(REMOVED_LAUNCH_FLAGS)) {
      ...
      throw {
        ...
        message: `${flag.split(" ")[0]} was removed. ${configPath ? `Use q8idevai daemon config set ${configPath} <value> --home <path>, then start or restart.` : "Use q8idevai daemon run --home <path> for foreground deployment."} Deployment environment overrides belong to q8idevai daemon run.`,
      };
    }
  });
  return command;
