...
const RELAY_DOCS_URL = "https://q8idevai.sh/docs/security";
...
export async function resolveLocalPairingOffer(options: {
  ...
  const instance = await readDaemonInstance(options.q8idevaiHome);
  if (instance)
    return resolveDaemonPairingOffer(
      { kind: "instance", home: options.q8idevaiHome },
      options.enableRelay,
    );
  if (options.enableRelay)
    editPersistedConfig(options.q8idevaiHome, "daemon.relay.enabled", { value: true });
  const config = resolveConfigFromPersisted(
    options.q8idevaiHome,
    readPersistedConfig(options.q8idevaiHome, { defaultsIfMissing: true }),
    { env: {} },
  );
...
async function resolveDaemonPairingOffer(
  ...
  try {
    ...
    if (serverInfo?.features?.daemonStatusRpc !== true) {
      throw new Error("Update the Q8iDevAI daemon before pairing from this command.");
    }
    ...
    if (!offer.relayEnabled && enableRelay) {
      if (serverInfo.features.relayConfig !== true) {
        throw new Error("Update the Q8iDevAI daemon before enabling relay from this command.");
      }
...
export async function confirmRelayPairing(): Promise<boolean> {
  log.message("Your connection is end-to-end encrypted. Q8iDevAI cannot read your code or messages.");
...
export async function runPairCommand(options: PairOptions): Promise<void> {
  ...
  const resolveOffer = (enableRelay: boolean) =>
    target.kind === "instance"
      ? resolveLocalPairingOffer({ q8idevaiHome: target.home, enableRelay })
  ...
  if (offline)
    output.writeStderr(
      `Offline pairing offer. Start with: q8idevai daemon start --home ${JSON.stringify(target.kind === "instance" ? target.home : "")}\n`,
    );
    ...
    if (options.json) {
      output.writeStderr(
        `${JSON.stringify({
          ...
          action: "Run q8idevai daemon pair --relay --json to enable it explicitly.",
        })}\n`,
      );
    } else {
      output.writeStderr(`${chalk.red("Relay pairing is disabled for this daemon.")}\n`);
      output.writeStderr(`${chalk.yellow("Run q8idevai daemon pair --relay to enable it.")}\n`);
    }
