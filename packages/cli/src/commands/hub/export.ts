...
export async function runHubExport(
  ...
  const directory = path.resolve(dependencies.cwd(), directoryInput ?? ".q8idevai/triggers");
  ...
  addJsonOption(
    addHubResolutionHelp(
      parent
        ...
        .argument("[directory]", "Destination directory", ".q8idevai/triggers")
        .option("--hub <origin>", "Q8iDevAI Hub origin")
