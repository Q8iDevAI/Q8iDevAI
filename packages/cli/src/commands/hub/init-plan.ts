...
export function hubLoginResumeCommand(step: "connect" | "init", origin: string): string {
  return step === "connect" ? `q8idevai hub connect ${origin}` : "q8idevai hub init";
}
...
export function createHubInitScaffold(input: HubInitScaffoldInput): HubInitScaffold {
  ...
  return {
    triggerPath: `.q8idevai/triggers/${input.provider}-help.yml`,
...
function providerScaffold(
  ...
  if (provider === "github") {
    ...
    return {
      ...
      connection,
      filters: { repo, contains: "@q8idevai", from_users: [user] },
      testAction: `Comment \`@q8idevai have a look\` on ${repo}.`,
    };
  }
  ...
  if (provider === "slack") {
    return {
      ...
      reply: "slack.reply",
      testAction: "Mention `@Q8iDevAI have a look` in Slack.",
    };
  }
  ...
  return {
    ...
    filters: { from_users: [user] },
    reply: "discord.reply",
    testAction: "Mention `@Q8iDevAI have a look` in Discord.",
  };
}
