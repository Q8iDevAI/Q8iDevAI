...
export class PrivateHubCredentialStore implements HubCredentialStore {
  ...
  constructor(env: Readonly<Record<string, string | undefined>> = process.env) {
    this.filePath = path.join(resolveQ8iDevAIHome(env), "hub-credentials.json");
  }
...
function resolveQ8iDevAIHome(env: Readonly<Record<string, string | undefined>>): string {
  const configured = env.Q8IDEVAI_HOME ?? "~/.q8idevai";
...
function invalidCredentialFile(): HubCommandError {
  return new HubCommandError(
    ...
    "Stored Hub login is invalid. Run `q8idevai hub login <origin>` to replace it.",
  );
}
...
function credentialStorageError(): HubCommandError {
  return new HubCommandError(
    ...
    "Could not access the private Hub credential store under Q8IDEVAI_HOME.",
  );
}
