...
export async function runHubGuidedSetup(
  ...
  if (deploy) {
    ...
    reportMessage(environment, "Skipped deployment. Run `q8idevai hub deploy` when ready.");
  }
...
async function ensureLogin(
  ...
  const endpoint = await requiredSelect(environment, {
    ...
    options: [
      { value: "hosted", label: "hub.q8idevai.sh" },
      ...
      : await requiredText(environment, {
          ...
          initialValue:
            activeOrigin === undefined || activeOrigin === DEFAULT_HUB_ORIGIN
              ? environment.env.Q8IDEVAI_HUB_URL
              : activeOrigin,
          validate(value) {
            try {
  ...
  if (connection.kind === "connected") {
    if (permissions.includes("hub.execute") && !status.permissions.includes("hub.execute")) {
      throw new HubCommandError(
        ...
        "This daemon is connected to Hub but cannot run Hub automations. Run `q8idevai hub permissions grant hub.execute`, then run Hub init again.",
      );
    }
      ...
      while (true) {
        ...
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            ...
            "The daemon did not connect within 60 seconds. Check `q8idevai hub status`, then run Hub init again.",
          );
        }
...
async function resolveStarterTriggerConnections(
  ...
  if (connections.length === 0) {
    throw new HubInitCancelledError(
      "No Hub app connection is ready for this trigger.\nConnect GitHub, Slack, or Discord in Hub → Apps, then run `q8idevai hub init` again.",
    );
  }
  return connections;
}
...
function reportStarterTriggerConnections(
  ...
  const details = `${connections.map(({ label }) => label).join("\n")}\n\nOnly configured connections are shown. To add another, open Hub → Apps, then run \`q8idevai hub init\` again.`;
...
async function chooseStarterTriggerConnection(
  ...
  if (connection === undefined) {
    throw new HubCommandError(
      ...
      "The selected Hub app connection is no longer available. Run q8idevai hub init again.",
    );
  }
  return connection;
}
...
async function chooseStarterAgentRuntime(
  ...
  if (runtime?.mode === undefined) {
    throw new HubCommandError(
      ...
      "The selected starter agent runtime is no longer available. Run q8idevai hub init again.",
    );
  }
      ...
      while (true) {
        ...
        if (state.kind === "ready") {
          ...
          throw new HubCommandError(
            ...
            "No agent runtime with an execution mode is available from this daemon. Configure one, then run q8idevai hub init again.",
          );
        }
        if (state.kind === "unavailable") {
          throw new HubCommandError(
            "HUB_AGENT_RUNTIME_REQUIRED",
            "No usable agent runtime is available from this daemon. Configure an enabled provider with a selectable model, then run q8idevai hub init again.",
          );
        }
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            "HUB_AGENT_RUNTIME_TIMEOUT",
            "Agent runtime discovery did not finish within 60 seconds. Check the daemon's provider configuration, then run q8idevai hub init again.",
          );
        }
...
async function prepareScaffoldDestination(cwd: string, triggerPath: string): Promise<boolean> {
  ...
  await requireSafeScaffoldDirectory(path.join(root, ".q8idevai"), ".q8idevai");
  await requireSafeScaffoldDirectory(path.join(root, ".q8idevai", "triggers"), ".q8idevai/triggers");
...
function requireInteractiveTerminal(
  ...
  if (!(environment.isInteractive?.() ?? (process.stdin.isTTY && process.stdout.isTTY))) {
    throw new HubCommandError("HUB_INIT_INTERACTIVE_REQUIRED", "q8idevai hub init requires a TTY.");
  }
}
