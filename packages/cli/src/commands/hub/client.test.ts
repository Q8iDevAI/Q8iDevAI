...
describe("Hub HTTP client", () => {
    ...
    const origin = await startServer((url) => {
      ...
      return {
        ...
        body: {
          status: "authorized",
          interval: 5,
          credential: "q8idevai_cli_prefix_durable-secret-value",
  ...
  it("validates projects and enrollment responses once at the HTTP boundary", async () => {
    ...
    assert.equal(projects[0]?.slug, "q8idevai");
  ...
  it("reads configuration resources in the Hub's slug vocabulary", async () => {
    ...
    const origin = await startServer(
      () => ({
        ...
        body: {
          ...
          discord: [{ slug: "q8idevai", guildName: "Q8iDevAI" }],
          slack: [{ slug: "q8idevai", teamName: "Q8iDevAI" }],
          linear: [{ slug: "q8idevai-linear", organizationName: "Q8iDevAI" }],
        },
      }),
      requests,
    );
    ...
    assert.equal(resources.discord[0]?.slug, "q8idevai");
    assert.equal(resources.linear[0]?.slug, "q8idevai-linear");
  ...
  it("renders file-aware Hub validation issues without exposing credentials or response bodies", async () => {
    ...
    const origin = await startServer(
      () => ({
        ...
        body: {
          ...
          issues: [
            {
              path: [".q8idevai/workflows/answer.yml", "steps", "work", "agent"],
    ...
    await assert.rejects(
      hub.validateConfiguration({
        ...
        projectSlug: "studio",
        files: [{ path: ".q8idevai/hub.yml", content: "sensitive bundle content" }],
      }),
      (error: unknown) => {
        ...
        assert.equal(
          error.details,
          ".q8idevai/workflows/answer.yml: steps.work.agent: unknown named agent [redacted]",
        );
        return true;
      },
    );
