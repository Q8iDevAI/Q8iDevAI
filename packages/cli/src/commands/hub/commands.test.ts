...
describe("Hub commands", () => {
  it("exposes the hard-cut Hub command surface without connect --token", () => {
    ...
    connect?.outputHelp();
    assert.match(help, /active stored login.*https:\/\/hub\.q8idevai\.sh/u);
  });
  ...
  it("login stores the durable credential and marks its normalized origin active", async () => {
    ...
    assert.deepEqual(credentials.active(), {
      ...
      credential: "q8idevai_cli_prefix_durable-secret",
    });
  ...
  it("login without an origin uses the hosted default and reports it before authorization", async () => {
      ...
      {},
      {
        ...
        flow: {
          authorize: async (origin) => {
            ...
            return "q8idevai_cli_prefix_durable-secret";
          },
        },
    ...
    assert.deepEqual(events, [
      "progress:Logging in to https://hub.q8idevai.sh",
      "authorize:https://hub.q8idevai.sh",
      "progress:Logged in",
    ]);
    assert.equal(result.data.origin, "https://hub.q8idevai.sh");
  });
  ...
  it("connect without authority reports the hosted destination and contacts nothing", async () => {
    const progress: string[] = [];
    const credentials = new MemoryCredentials();
    const daemon = new FakeDaemonConnection(new FakeDaemon("https://hub.q8idevai.sh"));
    let hubRequests = 0;
    ...
    await assert.rejects(
      ...
      ),
      {
        ...
        message:
          "No stored Hub login matches https://hub.q8idevai.sh. Run `q8idevai hub login https://hub.q8idevai.sh`, pass --api-key <secret>, or set Q8IDEVAI_HUB_API_KEY.",
      },
    );

    assert.deepEqual(progress, ["Connecting this daemon to https://hub.q8idevai.sh"]);
  ...
  it("projects uses explicit API-key authority and renders auditable JSON output", async () => {
    ...
    const result = await runHubProjects(
      ...
      {
        env: { Q8IDEVAI_HUB_URL: "https://env.test", Q8IDEVAI_HUB_API_KEY: "env-secret" },
        credentials,
        hub: {
