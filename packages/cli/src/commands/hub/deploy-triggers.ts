...
const TRIGGER_DIRECTORY = ".q8idevai/triggers";
...
export async function discoverHubTriggers(cwd: string): Promise<HubDeployTrigger[]> {
  ...
  const q8idevaiDirectory = await readStats(path.join(root, ".q8idevai"), {
    code: "HUB_TRIGGER_DIRECTORY_MISSING",
    message: `${TRIGGER_DIRECTORY} does not exist. Run this command from the project root.`,
  });
  if (q8idevaiDirectory.isSymbolicLink()) throw unsafeTriggerPath(TRIGGER_DIRECTORY);
...
async function legacyBundleExists(root: string): Promise<boolean> {
  try {
    await lstat(path.join(root, ".q8idevai/hub.yml"));
    return true;
  } catch (error) {
    if (errorCode(error) === "ENOENT") return false;
    if (!isExpectedReadError(error)) throw error;
    throw unreadableTriggerPath(".q8idevai/hub.yml");
  }
}
