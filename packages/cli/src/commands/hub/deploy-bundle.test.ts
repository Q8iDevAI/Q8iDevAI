...
describe("Hub deployment bundle discovery", () => {
  it("discovers the canonical bundle in deterministic path order", async () => {
    ...
    await writeFile(
      path.join(cwd, ".q8idevai", "workflows", "z-last.yml"),
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).resolves.toEqual({
      ...
      workflowCount: 2,
      files: [
        { path: ".q8idevai/hub.yml", content: hubResource },
        {
          path: ".q8idevai/workflows/answer.yml",
          content: workflow("answer", "${{ q8idevai.inputs.agent }}", true),
        },
        {
          path: ".q8idevai/workflows/partials/safety.md",
          content: "Keep the request in q8idevai.prompt and evidence in q8idevai.context.\n",
        },
        { path: ".q8idevai/workflows/z-last.yml", content: workflow("z-last", "codex-safe") },
      ],
    });
  });
  ...
  it.each([
    {
      ...
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".q8idevai", "workflows"), { recursive: true });
      },
      code: "HUB_RESOURCE_MISSING",
      message: ".q8idevai/hub.yml does not exist",
    },
    {
      name: "missing workflow directory",
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".q8idevai"), { recursive: true });
        await writeFile(path.join(cwd, ".q8idevai", "hub.yml"), hubResource);
      },
      code: "HUB_WORKFLOW_DIRECTORY_MISSING",
      message: ".q8idevai/workflows does not exist",
    },
    {
      ...
      message: ".q8idevai/workflows must contain at least one direct-child .yml workflow",
    },
    {
      ...
      arrange: async (cwd: string) => {
        await mkdir(path.join(cwd, ".q8idevai", "workflows"), { recursive: true });
        await writeFile(path.join(cwd, ".q8idevai", "hub.yml"), hubResource);
        await writeFile(path.join(cwd, ".q8idevai", "workflows", "answer.yaml"), "name: answer\n");
      },
      code: "HUB_WORKFLOW_EXTENSION_UNSUPPORTED",
      message: ".q8idevai/workflows/answer.yaml must use the .yml extension",
    },
  ...
  it("rejects prompt partial traversal with a path-specific diagnostic", async () => {
    const cwd = await canonicalProject();
    await writeFile(
      path.join(cwd, ".q8idevai", "workflows", "answer.yml"),
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      ...
      message: expect.stringContaining(".q8idevai/workflows/answer.yml"),
    });
  });
  ...
  it("reports a referenced partial that is missing from the bundle", async () => {
    ...
    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      ...
      message: expect.stringContaining(".q8idevai/workflows/partials/missing.md"),
    });
  });

  it("rejects nested workflow files instead of discovering a second layout", async () => {
    const cwd = await canonicalProject();
    await mkdir(path.join(cwd, ".q8idevai", "workflows", "nested"));
    await writeFile(
      path.join(cwd, ".q8idevai", "workflows", "nested", "other.yml"),
      workflow("other", "codex-safe"),
    );

    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      code: "HUB_WORKFLOW_PATH_UNSUPPORTED",
      message: expect.stringContaining(".q8idevai/workflows/nested"),
    });
  });
  ...
  it("rejects symlinked workflow and partial paths without reading their targets", async () => {
    ...
    await symlink(outside, path.join(cwd, ".q8idevai", "workflows", "linked.yml"));

    await expect(discoverHubBundle({ cwd, project: "studio-api" })).rejects.toMatchObject({
      code: "HUB_BUNDLE_UNSAFE_PATH",
      message: expect.stringContaining(".q8idevai/workflows/linked.yml"),
    });
  });
...
function workflow(name: string, agent: string, include = false): string {
  return [
    ...
    "on: manual.run",
    "max_runtime: 1h",
    ...(agent.includes("q8idevai.inputs")
...
async function canonicalProject(): Promise<string> {
  const cwd = await temporaryDirectory();
  const workflows = path.join(cwd, ".q8idevai", "workflows");
  await mkdir(path.join(workflows, "partials"), { recursive: true });
  await writeFile(path.join(cwd, ".q8idevai", "hub.yml"), hubResource);
  await writeFile(
    path.join(workflows, "answer.yml"),
    workflow("answer", "${{ q8idevai.inputs.agent }}", true),
  );
  await writeFile(
    path.join(workflows, "partials", "safety.md"),
    "Keep the request in q8idevai.prompt and evidence in q8idevai.context.\n",
  );
  return cwd;
}

async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-hub-bundle-"));
