...
import type { ProviderSnapshotEntry } from "@q8idevai/protocol/agent-types";
...
describe("Hub guided setup continuation", () => {
  it("offers daemon connection after login, then points to Hub without scaffolding files", async () => {
    ...
    await runHubLogin(
      ...
      {},
      {
        env: {},
        credentials,
        flow: { authorize: async () => "q8idevai_cli_prefix_durable-secret" },
        isInteractive: () => true,
    ...
    await assert.rejects(readFile(path.join(cwd, ".q8idevai", "hub.yml")), { code: "ENOENT" });
  });
  ...
  it("prints exact actionable resume commands for login continuation declines", async () => {
    ...
    assert.deepEqual(connectDeclined.messages, [
      "Skipped daemon connection. Connect later with: q8idevai hub connect https://hub.test",
      "Configure triggers in Hub: https://hub.test/triggers\nOr scaffold triggers as code: q8idevai hub init",
    ]);
  });
  ...
  it("preserves an existing legacy bundle while adding the organization trigger", async () => {
    const cwd = await temporaryDirectory();
    await mkdir(path.join(cwd, ".q8idevai"));
    await writeFile(path.join(cwd, ".q8idevai", "hub.yml"), "legacy: bundle\n");
    const credentials = new MemoryCredentials();
    credentials.save({ origin: "https://hub.test", credential: "secret" });
    ...
    assert.equal(await readFile(path.join(cwd, ".q8idevai", "hub.yml"), "utf8"), "legacy: bundle\n");
    assert.match(
      await readFile(path.join(cwd, ".q8idevai", "triggers", "slack-help.yml"), "utf8"),
      /name: slack-help/u,
    );
    assert.deepEqual(prompts.confirmations, []);
  });
  ...
  it("asks before replacing only the selected trigger file", async () => {
    const cwd = await temporaryDirectory();
    const triggerPath = path.join(cwd, ".q8idevai", "triggers", "slack-help.yml");
    ...
    await assert.rejects(
      ...
      /.q8idevai\/triggers\/slack-help.yml left unchanged/u,
    );

    assert.equal(await readFile(triggerPath, "utf8"), "name: keep-me\n");
    assert.deepEqual(prompts.confirmations, [
      "Replace the existing .q8idevai/triggers/slack-help.yml?",
    ]);
    assert.deepEqual(
  ...
  it("rejects a symlinked trigger directory without writing outside the project", async () => {
    ...
    await mkdir(path.join(cwd, ".q8idevai"));
    await symlink(outside, path.join(cwd, ".q8idevai", "triggers"));
    const credentials = new MemoryCredentials();
    credentials.save({ origin: "https://hub.test", credential: "secret" });
    const prompts = new PromptAnswers([], ["codex", "gpt-5", "full-access"], ["U123"]);
  ...
  it("writes the explicitly selected Claude mode when the daemon has no default", async () => {
    ...
    const trigger = await readFile(path.join(cwd, ".q8idevai", "triggers", "slack-help.yml"), "utf8");
  ...
  it("stops before runtime discovery and file writes when no Hub app connection is usable", async () => {
    ...
    await assert.rejects(readFile(path.join(cwd, ".q8idevai", "triggers", "slack-help.yml")), {
      code: "ENOENT",
    });
  });
...
function setupEnvironment(
  ...
  return {
    ...
    credentials,
    hub,
    login: { authorize: async () => "q8idevai_cli_prefix_durable-secret" },
...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-hub-init-flow-"));
