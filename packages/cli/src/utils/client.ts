import { waitForDaemonReady, resolveQ8iDevAIHome, type DaemonInstance } from "@q8idevai/server";
...
} from "@q8idevai/protocol/daemon-endpoints";
import {
  parseConnectionOfferFromUrl,
  type ConnectionOffer,
} from "@q8idevai/protocol/connection-offer";
import { parseSshTransportUri } from "@q8idevai/protocol/ssh-transport";
  ...
  return {
    ...
    details:
      options.target.kind === "instance"
        ? `Start with: q8idevai daemon start --home ${JSON.stringify(options.target.home)}`
...
export function resolveDaemonPassword(host: string): string | undefined {
  ...
  const fromEnv = process.env.Q8IDEVAI_PASSWORD;
...
async function connectSelectedDaemon(options: ConnectOptions): Promise<DaemonClient> {
  ...
  const clientId = await getOrCreateCliClientId(resolveQ8iDevAIHome({}));
