...
test("explicit selectors win over both environment selectors", () => {
  const env = { Q8IDEVAI_HOME: "/tmp/a", Q8IDEVAI_HOST: "unused:12345" };
...
test("local operations ignore routing environment but reject an explicit endpoint", () => {
  expect(
    selectDaemonTarget({}, { Q8IDEVAI_HOME: "/tmp/b", Q8IDEVAI_HOST: "unused:12345" }, true),
...
test("endpoint descriptions redact pairing material and credentials", () => {
  ...
  expect(
    describeDaemonTarget({ kind: "endpoint", host: "https://app.q8idevai.sh/#offer=private" }),
