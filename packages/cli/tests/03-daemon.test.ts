 ...
 *
 * Tests daemon commands with an isolated Q8IDEVAI_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalQ8iDevAI(["daemon", ...args], { Q8IDEVAI_HOME: q8idevaiHome });
}
...
function resolveDaemonWorkerEntry(): string {
  let currentDir = dirname(require.resolve("@q8idevai/server"));

  while (true) {
    const packageJsonPath = join(currentDir, "package.json");
    if (existsSync(packageJsonPath)) {
      const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf-8"));
      if (packageJson.name === "@q8idevai/server") {
        const candidates = [
  ...
  throw new Error("Unable to resolve @q8idevai/server package root");
}

async function tailDaemonLog(): Promise<string> {
  try {
    const log = await readFile(join(q8idevaiHome, "daemon.log"), "utf-8");
    return log.split("\n").slice(-30).join("\n");
  } catch {
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalQ8iDevAI(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, q8idevaiHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
  ...
  {
    const result = await runLocalQ8iDevAI(["--host", "127.0.0.1:1", "daemon", "status", "--json"], {
  ...
  // Test 8: status uses the running daemon as relay authority over local IPC
  {
    ...
    const listen =
      process.platform === "win32"
        ? `\\\\.\\pipe\\q8idevai-status-${process.pid}-${Date.now()}`
        : join(q8idevaiHome, "status.sock");
    const configPath = join(q8idevaiHome, "config.json");
    ...
    // A supervised daemon owns and heartbeats q8idevai.pid. Launch the worker
    ...
    try {
      ...
      assert.deepStrictEqual(JSON.parse(nestedReload.stdout), {
        restartCommand: `q8idevai daemon restart --host ${JSON.stringify(listen)}`,
      ...
      await writeFile(configPath, `${JSON.stringify(reloadConfig, null, 2)}\n`, "utf-8");
      const aliasReload = await runLocalQ8iDevAI(["reload", "--host", listen, "--json"], {
        Q8IDEVAI_HOME: q8idevaiHome,
      });
      ...
      const foreignHome = await mkdtemp(join(tmpdir(), "q8idevai-test-foreign-home-"));
      try {
        await writeFile(
        ...
        const foreignPairing = await runLocalQ8iDevAI(
          ["daemon", "pair", "--home", foreignHome, "--json"],
          { Q8IDEVAI_HOME: foreignHome },
        );
