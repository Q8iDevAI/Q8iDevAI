...
const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-stop-tree-kill-"));
const childPidPath = join(q8idevaiHome, "descendant.pid");
...
try {
  ...
  const stopResult =
    await $`Q8IDEVAI_HOME=${q8idevaiHome} Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD} Q8IDEVAI_DICTATION_ENABLED=${testEnv.Q8IDEVAI_DICTATION_ENABLED} Q8IDEVAI_VOICE_MODE_ENABLED=${testEnv.Q8IDEVAI_VOICE_MODE_ENABLED} npx q8idevai daemon stop --home ${q8idevaiHome} --json --timeout 1 --force --kill-timeout 2`.nothrow();
