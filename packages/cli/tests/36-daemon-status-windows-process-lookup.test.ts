...
const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-windows-status-home-"));
...
try {
  for (const [field, value] of [
    ...
    const saved = await runLocalQ8iDevAI(["daemon", "config", "set", field!, value!], env);
    assert.equal(saved.exitCode, 0, saved.stderr);
  }
  const start = await runLocalQ8iDevAI(["daemon", "start"], env);
  assert.strictEqual(
    start.exitCode,
    0,
  ...
  const statusResult = await runLocalQ8iDevAI(
    ["daemon", "status", "--home", q8idevaiHome, "--json"],
    env,
  );
  assert.strictEqual(
  ...
  await runLocalQ8iDevAI(["daemon", "stop", "--home", q8idevaiHome, "--force"], env);
