...
const sdk = createQ8iDevAIClient({ url: `${ctx.wsUrl}/ws`, reconnect: { enabled: false } });

async function cli(args: string[]) {
  const result = await ctx.q8idevai([...args, "--json"]);
...
try {
  ...
  const invalid = await ctx.q8idevai([
    "terminal",
    "create",
    "--workspace",
    "wks_missing",
  ...
  const conflict = await ctx.q8idevai(["terminal", "ls", "--all", "--workspace", second.id]);
  ...
  await waitForTerminalOutput(ctx.q8idevai, terminal.id, "READY");
  assert.deepEqual(await cli(["terminal", "send-keys", terminal.id, "-l", "Enter"]), {
    terminalId: terminal.id,
    keysSent: 5,
  });
  await waitForTerminalOutput(ctx.q8idevai, terminal.id, "HEX:456e746572");
  assert.deepEqual(await cli(["terminal", "send-keys", terminal.id, "Enter"]), {
    terminalId: terminal.id,
    keysSent: 1,
  });
  await waitForTerminalOutput(ctx.q8idevai, terminal.id, "HEX:456e7465720d");
