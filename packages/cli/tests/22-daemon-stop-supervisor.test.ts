...
/**
 * Regression: `q8idevai daemon stop` must stop supervised dev daemons
...
async function readPidLockState(q8idevaiHome: string): Promise<PidLockState> {
...
async function readDaemonStatus(q8idevaiHome: string): Promise<DaemonStatus> {
  const result =
    await $`Q8IDEVAI_HOME=${q8idevaiHome} Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD} Q8IDEVAI_DICTATION_ENABLED=${testEnv.Q8IDEVAI_DICTATION_ENABLED} Q8IDEVAI_VOICE_MODE_ENABLED=${testEnv.Q8IDEVAI_VOICE_MODE_ENABLED} npx q8idevai daemon status --home ${q8idevaiHome} --json`.nothrow();
...
async function readCapturedSupervisorLogs(q8idevaiHome: string, recentLogs: string): Promise<string> {
  const durableLogs = await readFile(join(q8idevaiHome, "daemon.log"), "utf8").catch(() => "");
  return `${recentLogs}\n${durableLogs}`;
}
...
const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated Q8IDEVAI_HOME");
  ...
  await waitFor(
    async () => {
      const status = await readDaemonStatus(q8idevaiHome);
      return (
  ...
  const statusBeforeStop = await readDaemonStatus(q8idevaiHome);
  ...
  const pidLockBeforeStop = await readPidLockState(q8idevaiHome);
  ...
  console.log("Test 2: `q8idevai daemon stop` should stop without respawn");
  const stopResult =
    await $`Q8IDEVAI_HOME=${q8idevaiHome} Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD} Q8IDEVAI_DICTATION_ENABLED=${testEnv.Q8IDEVAI_DICTATION_ENABLED} Q8IDEVAI_VOICE_MODE_ENABLED=${testEnv.Q8IDEVAI_VOICE_MODE_ENABLED} npx q8idevai daemon stop --home ${q8idevaiHome} --json`.nothrow();
  ...
  const pidAfterStop = await readPidLockState(q8idevaiHome);
  ...
  const statusAfterStop = await readDaemonStatus(q8idevaiHome);
  assert.strictEqual(
    statusAfterStop.localDaemon,
    "stopped",
    "daemon should remain stopped after stop command",
  );
  const capturedSupervisorLogs = await readCapturedSupervisorLogs(q8idevaiHome, recentSupervisorLogs);
  assert(
  ...
  await $`Q8IDEVAI_HOME=${q8idevaiHome} Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD} Q8IDEVAI_DICTATION_ENABLED=${testEnv.Q8IDEVAI_DICTATION_ENABLED} Q8IDEVAI_VOICE_MODE_ENABLED=${testEnv.Q8IDEVAI_VOICE_MODE_ENABLED} npx q8idevai daemon stop --home ${q8idevaiHome} --force`.nothrow();
