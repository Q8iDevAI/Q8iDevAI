...
try {
  ...
  const renamed = await ctx.q8idevai(["workspace", "rename", workspaceId, "Auth rework", "--json"]);
  ...
  const renamedList = await ctx.q8idevai(["workspace", "ls", "--json"]);
  ...
  const reset = await ctx.q8idevai(["workspace", "rename", workspaceId, "--reset", "--json"]);
  assert.strictEqual(reset.exitCode, 0, reset.stderr);
  assert.deepStrictEqual(JSON.parse(reset.stdout), { workspaceId, title: null });

  const resetList = await ctx.q8idevai(["workspace", "ls", "--json"]);
