 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real Q8iDevAI daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own Q8IDEVAI_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.Q8IDEVAI_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ q8idevaiHome: string; workDir: string }> {
  const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "q8idevai-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(q8idevaiHome, "agents");
...
/**
 * Wait for daemon to be ready by running `q8idevai agent ls`
...
async function probeDaemonReady(
  ...
  try {
    const { exitCode } = await runQ8iDevAICli(
      {
        port,
...
async function waitForDaemonReady(
  ...
  async function poll(): Promise<void> {
    if (await probeDaemonReady(port, q8idevaiHome, env)) return;
 ...
 * with isolated Q8IDEVAI_HOME and Q8IDEVAI_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { q8idevaiHome, workDir } =
    options?.q8idevaiHome && options?.workDir
      ? { q8idevaiHome: options.q8idevaiHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn(process.execPath, [TSX_ENTRY, cliSrcPath, "daemon", "run"], {
    env: {
      ...
      Q8IDEVAI_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
  ...
  try {
    await waitForDaemonReady(port, q8idevaiHome, timeout, options?.env);
    ctx.isReady = true;
  } catch (err) {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(process.execPath, [TSX_ENTRY, cliSrcPath, ...args], {
      env: {
        ...
        ...TEST_DAEMON_ENV_DEFAULTS,
        Q8IDEVAI_HOME: ctx.q8idevaiHome,
        ...options?.env,
        HOME: ctx.q8idevaiHome,
        USERPROFILE: ctx.q8idevaiHome,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
      detached: process.platform !== "win32",
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error(`CLI command timed out after ${timeout}ms: q8idevai ${args.join(" ")}`));
    }, timeout);
...
export async function createE2ETestContext(options?: {
  ...
  TestDaemonContext & {
    /** Run a q8idevai CLI command against this daemon */
    q8idevai: (
      args: string[],
