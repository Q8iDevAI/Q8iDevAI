 ...
 *
 * Tests the worktree commands for managing Q8iDevAI-managed git worktrees.
...
try {
  // Test 1: worktree --help shows subcommands
  {
    ...
    const result = await $`npx q8idevai worktree --help`.nothrow();
  ...
  // Test 2: worktree ls --help shows options
  {
    ...
    const result = await $`npx q8idevai worktree ls --help`.nothrow();
  ...
  // Test 3: worktree ls handles daemon not running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree ls`.nothrow();
  ...
  // Test 4: worktree ls with --host flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree ls --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: worktree archive --help shows options
  {
    ...
    const result = await $`npx q8idevai worktree archive --help`.nothrow();
  ...
  // Test 6: worktree archive requires name argument
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree archive`.nothrow();
  ...
  // Test 7: worktree archive handles daemon not running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree archive test-worktree`.nothrow();
    // Should fail because daemon not running
    assert.notStrictEqual(result.exitCode, 0, "should fail when daemon not running");
    const output = result.stdout + result.stderr;
    const hasError =
  ...
  // Test 8: worktree archive with name and --host flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree archive test-worktree --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
    assert(!output.includes("unknown option"), "should accept --host flag");
    assert(!output.includes("error: option"), "should not have option parsing error");
  ...
  // Test 9: -q (quiet) flag is accepted with worktree ls
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} -q worktree ls`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 10: --json flag is accepted with worktree ls
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} worktree ls --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 11: q8idevai --help keeps the compatibility command hidden
  {
    console.log("Test 11: q8idevai --help hides worktree compatibility command");
    ...
    console.log("✓ q8idevai --help hides worktree compatibility command\n");
  }
} finally {
