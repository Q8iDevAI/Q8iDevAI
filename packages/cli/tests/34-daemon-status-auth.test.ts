...
const daemon = await startTestDaemon({
  env: { Q8IDEVAI_PASSWORD: "shared-secret" },
});

try {
  {
    console.log("Test 1: status reports password requirement without marking daemon unreachable");
    const result = await runLocalQ8iDevAI(["daemon", "status", "--json"], {
      Q8IDEVAI_HOME: daemon.q8idevaiHome,
      Q8IDEVAI_HOST: "",
      Q8IDEVAI_PASSWORD: "",
    });
...

  {
    ...
    const result = await runLocalQ8iDevAI(["daemon", "status", "--json"], {
      Q8IDEVAI_HOME: daemon.q8idevaiHome,
      Q8IDEVAI_HOST: "",
      Q8IDEVAI_PASSWORD: "wrong-secret",
    });
...

  {
    ...
    const result = await runLocalQ8iDevAI(["daemon", "status", "--json"], {
      Q8IDEVAI_HOME: daemon.q8idevaiHome,
      Q8IDEVAI_HOST: "",
      Q8IDEVAI_PASSWORD: "shared-secret",
    });
...
if (process.platform !== "win32") {
  const root = await mkdtemp(join(tmpdir(), "q8idevai status slow provider "));
...
`,
    ...
    const local = await runLocalQ8iDevAI(["daemon", "status", "--home", home, "--json"], {
      Q8IDEVAI_PASSWORD: "shared-secret",
    });
    ...
    const remote = await runLocalQ8iDevAI(
