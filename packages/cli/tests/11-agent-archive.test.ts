...
try {
  // Test 1: agent archive --help shows options
  {
    ...
    const result = await $`npx q8idevai agent archive --help`.nothrow();
  ...
  // Test 2: agent archive requires ID argument
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent archive`.nothrow();
  ...
  // Test 3: agent archive handles daemon not running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent archive abc123`.nothrow();
  ...
  // Test 4: agent archive --force flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent archive abc123 --force`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: agent archive with ID and --host flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent archive abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with agent archive
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} -q agent archive abc123`.nothrow();
    const output = result.stdout + result.stderr;
