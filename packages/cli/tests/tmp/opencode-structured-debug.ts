...
async function main() {
  ...
  try {
    ...
    const ls = await ctx.q8idevai(["ls", "--json"]);
    console.log("LS\n" + ls.stdout);
    const agents = JSON.parse(ls.stdout);
    const agentId = agents[0]?.id;
    if (agentId) {
      const inspect = await ctx.q8idevai(["inspect", agentId]);
      console.log("INSPECT\n" + inspect.stdout);
      const logs = await ctx.q8idevai(["logs", "--tail", "100", agentId], { timeout: 30000 });
      console.log("LOGS\n" + logs.stdout);
    }
  } finally {
    await ctx.stop();
