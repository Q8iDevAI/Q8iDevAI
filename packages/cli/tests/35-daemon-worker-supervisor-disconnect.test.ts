...
const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-worker-supervisor-disconnect-"));
...
try {
  console.log("Test 1: start supervised daemon with isolated Q8IDEVAI_HOME");
  ...
  const statusBeforeKill = await readDaemonStatus(q8idevaiHome);
