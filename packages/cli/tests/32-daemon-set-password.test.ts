...
import { isBearerTokenValid } from "@q8idevai/server";
import {
...
const root = await mkdtemp(join(tmpdir(), "q8idevai-set-password-"));
const q8idevaiHome = join(root, ".q8idevai");
...
try {
  {
    ...
    const result = await setDaemonPasswordInConfig("shared-secret", { home: q8idevaiHome });
    const config = JSON.parse(await readFile(join(q8idevaiHome, "config.json"), "utf-8"));

    assert.strictEqual(result.configPath, join(q8idevaiHome, "config.json"));
    assert.strictEqual(
      result.restartCommand,
      `q8idevai daemon restart --home ${JSON.stringify(q8idevaiHome)}`,
    );
...

  {
    ...
    const result = await runSetPasswordCommand(
      {
        home: q8idevaiHome,
        daemonTarget: { kind: "instance", home: q8idevaiHome },
