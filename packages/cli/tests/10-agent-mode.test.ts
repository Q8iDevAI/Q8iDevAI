...
try {
  // Test 1: agent mode --help shows options
  {
    ...
    const result = await $`npx q8idevai agent mode --help`.nothrow();
  ...
  // Test 2: agent mode requires id argument
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent mode`.nothrow();
  ...
  // Test 3: agent mode handles daemon not running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent mode abc123 bypass`.nothrow();
  ...
  // Test 4: agent mode --list flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent mode --list abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: agent mode with ID and --host flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent mode abc123 plan --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: -q (quiet) flag is accepted with agent mode
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} -q agent mode abc123 bypass`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: agent mode requires mode argument when not using --list
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} agent mode abc123`.nothrow();
