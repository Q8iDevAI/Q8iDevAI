...
try {
  // Test 1: stop --help shows options
  {
    ...
    const result = await $`npx q8idevai stop --help`.nothrow();
  ...
  // Test 2: stop requires ID, --all, or --cwd
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} stop`.nothrow();
  ...
  // Test 3: stop handles daemon not running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} stop abc123`.nothrow();
  ...
  // Test 4: stop --all flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} stop --all`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: stop --cwd flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} stop --cwd /tmp`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: stop with ID and --host flag is accepted
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} stop abc123 --host localhost:${port}`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: q8idevai --help shows stop command
  {
    console.log("Test 7: q8idevai --help shows stop command");
    ...
    console.log("✓ q8idevai --help shows stop command\n");
  }
  ...
  // Test 8: -q (quiet) flag is accepted with stop
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} -q stop abc123`.nothrow();
    const output = result.stdout + result.stderr;
