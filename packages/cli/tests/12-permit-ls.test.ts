...
try {
  // Test 1: permit --help shows subcommands
  {
    ...
    const result = await $`npx q8idevai permit --help`.nothrow();
  ...
  // Test 2: permit ls --help shows options
  {
    ...
    const result = await $`npx q8idevai permit ls --help`.nothrow();
  ...
  // Test 3: permit ls returns error when no daemon running
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} permit ls`.nothrow();
  ...
  // Test 4: permit ls --json handles errors
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} permit ls --json`.nothrow();
  ...
  // Test 5: -q (quiet) flag is accepted globally
  {
    ...
    const result =
      await $`Q8IDEVAI_HOME=${q8idevaiHome} npx q8idevai --host localhost:${port} -q permit ls`.nothrow();
    const output = result.stdout + result.stderr;
