...
import { runLocalQ8iDevAI } from "./helpers/local-cli.js";

const cwd = await mkdtemp(path.join(tmpdir(), "q8idevai-hub-installed-"));
...
try {
  ...
  const validate = await runLocalQ8iDevAI(
    [
      "hub",
      "deploy",
      "-p",
  ...
  const install = await runLocalQ8iDevAI(
