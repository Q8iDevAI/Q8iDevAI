...
try {
  {
    ...
    const created = await ctx.q8idevai(
      [
        "schedule",
        "create",
        "Review new PRs",
    ...
    const listed = await ctx.q8idevai(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.q8idevai(["schedule", "inspect", createdJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.q8idevaiHome, { recursive: true, force: true });
