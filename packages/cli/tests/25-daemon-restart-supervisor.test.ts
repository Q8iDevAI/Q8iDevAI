...
import { readDaemonInstance, isSameDaemonInstance } from "@q8idevai/server";
...
const q8idevaiHome = await mkdtemp(join(tmpdir(), "q8idevai-restart-supervisor-"));
...
const availabilityLog = join(q8idevaiHome, "availability.log");

try {
  if (process.platform !== "win32") {
    const provider = join(q8idevaiHome, "slow-provider");
    await writeFile(
      provider,
      `#!${process.execPath}
...
`,
  ...
  let supervisor = await readDaemonInstance(q8idevaiHome);
  await waitFor(
    async () => {
      supervisor = await readDaemonInstance(q8idevaiHome);
  ...
  const current = await readDaemonInstance(q8idevaiHome);
  ...
  await runLocalQ8iDevAI(["daemon", "stop", "--home", q8idevaiHome, "--force"]);
