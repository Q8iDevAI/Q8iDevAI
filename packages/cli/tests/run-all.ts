...
/**
 * Test runner for Q8iDevAI CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp Q8IDEVAI_HOME), so parallelism is safe.
 */
...
// packages live in the root. Prepend it so `npx q8idevai` resolves locally.
...
const concurrencyEnv = process.env.Q8IDEVAI_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.Q8IDEVAI_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.Q8IDEVAI_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `Q8IDEVAI_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@q8idevai/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Q8iDevAI CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "q8idevai-cli-test-npm-cache-"));
  const osHome = await mkdtemp(join(tmpdir(), "q8idevai-cli-test-os-home-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD,
          Q8IDEVAI_DICTATION_ENABLED: testEnvDefaults.Q8IDEVAI_DICTATION_ENABLED,
          Q8IDEVAI_VOICE_MODE_ENABLED: testEnvDefaults.Q8IDEVAI_VOICE_MODE_ENABLED,
        },
