...
import { buildRelayWebSocketUrl } from "@q8idevai/protocol/daemon-endpoints";
import { parseConnectionOfferFromUrl } from "@q8idevai/protocol/connection-offer";
import { generateLocalPairingOffer } from "@q8idevai/server";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      ...
      env: {
        ...
        Q8IDEVAI_RELAY_ENDPOINT: relayEndpoint,
        Q8IDEVAI_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      q8idevaiHome: ctx.q8idevaiHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `q8idevai --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.q8idevai(["ls", "--json"]);
    ...
    const relay = await ctx.q8idevai(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { Q8IDEVAI_HOST: offerUrl },
    });
