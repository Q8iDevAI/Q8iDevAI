 ...
 * 2. Create agent with `q8idevai run "say hello" --provider claude`
 * 3. List agents with `q8idevai ls`
 * 4. Wait for agent with `q8idevai wait <id>`
 * 5. Inspect agent with `q8idevai inspect <id>`
 * 6. Stop agent with `q8idevai stop <id>` and verify it remains inspectable
 * 7. Delete agent with `q8idevai delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`Q8IDEVAI_HOME: ${ctx.q8idevaiHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.q8idevai(["ls", "--json"]);
...
async function test_agent_run_detached(): Promise<string> {
  ...
  const result = await ctx.q8idevai(
    [
      "-q",
      "run",
      "-d",
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.q8idevai(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.q8idevai(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.q8idevai(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.q8idevai(["stop", agentId]);
  ...
  const inspectResult = await ctx.q8idevai(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.q8idevai(["delete", agentId]);
