...
async function main(): Promise<void> {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-cli-e2e-"));
  const gitDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-git-cli-e2e-"));
  const context = await createE2ETestContext({ timeout: 45_000 });
  try {
    const scaffold = path.join(context.workDir, "authored-plugin");
    const init = await context.q8idevai(["plugin", "init", scaffold, "--json"]);
    assert.equal(init.exitCode, 0, init.stderr);
    const manifestPath = path.join(scaffold, "q8idevai-plugin.json");
    const manifest = await readPluginManifest(scaffold);
    assert.deepEqual(manifest.requirements, { q8idevai: `>=${resolveCliVersion()}` });
    await writeFile(
      path.join(directory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "cli-e2e", requirements: { q8idevai: `>=${resolveCliVersion()}` } }),
    );
    await writeFile(path.join(directory, "index.server.ts"), pluginSource);

    const install = await context.q8idevai(["plugin", "install", directory, "--json"]);
    ...
    await writeFile(
      manifestPath,
      JSON.stringify({ ...manifest, requirements: { q8idevai: ">=999.0.0" } }),
    );
    const incompatibleInstall = await context.q8idevai(["plugin", "install", scaffold, "--json"]);
    assert.equal(incompatibleInstall.exitCode, 1);
    assert.match(incompatibleInstall.stderr, /requires Q8iDevAI >=999.0.0/);
    const afterRejection = await context.q8idevai(["plugin", "ls", "--json"]);
    ...
    const scaffoldInstall = await context.q8idevai(["plugin", "install", scaffold, "--json"]);
    ...
    await git(gitDirectory, ["config", "user.name", "Q8iDevAI Tests"]);
    await git(gitDirectory, ["config", "user.email", "q8idevai@example.test"]);
    await writeFile(
      path.join(gitDirectory, "q8idevai-plugin.json"),
      JSON.stringify({ id: "git-cli-e2e", requirements: { q8idevai: `>=${resolveCliVersion()}` } }),
    );
    ...
    const gitInstall = await context.q8idevai([
      "plugin",
      "add",
    ...
    const status = await context.q8idevai(["plugin", "status", "git-cli-e2e", "--json"]);
    ...
    const update = await context.q8idevai(["plugin", "update", "git-cli-e2e", "--json"]);
    ...
    await git(gitDirectory, ["add", "-A"]);
    await git(gitDirectory, ["commit", "-m", "requires a future Q8iDevAI"]);
    const incompatibleUpdate = await context.q8idevai(["plugin", "update", "git-cli-e2e", "--json"]);
    assert.equal(incompatibleUpdate.exitCode, 1);
    assert.match(incompatibleUpdate.stderr, /requires Q8iDevAI >=999.0.0/);
    await assert.rejects(readFile(buildMarker), { code: "ENOENT" });
    const retained = await context.q8idevai(["plugin", "ls", "git-cli-e2e", "--json"]);
    ...
    const incompatibleAdd = await context.q8idevai([
      "plugin",
      "add",
      pathToFileURL(gitDirectory).href,
      "--id",
    ...
    assert.match(incompatibleAdd.stderr, /requires Q8iDevAI >=999.0.0/);
    await assert.rejects(readFile(buildMarker), { code: "ENOENT" });

    const reload = await context.q8idevai(["plugin", "reload", "cli-e2e", "--json"]);
    assert.equal(reload.exitCode, 0, reload.stderr);
    assert.equal(JSON.parse(reload.stdout).status, "running");

    const disable = await context.q8idevai(["plugin", "disable", "cli-e2e", "--json"]);
    assert.equal(disable.exitCode, 0, disable.stderr);
    assert.equal(JSON.parse(disable.stdout).status, "disabled");

    const enable = await context.q8idevai(["plugin", "enable", "cli-e2e", "--json"]);
    assert.equal(enable.exitCode, 0, enable.stderr);
    assert.equal(JSON.parse(enable.stdout).status, "running");

    const remove = await context.q8idevai(["plugin", "remove", "cli-e2e", "--json"]);
    assert.equal(remove.exitCode, 0, remove.stderr);
    const removeGit = await context.q8idevai(["plugin", "remove", "git-cli-e2e", "--json"]);
    assert.equal(removeGit.exitCode, 0, removeGit.stderr);
    const removeScaffold = await context.q8idevai(["plugin", "remove", "authored-plugin", "--json"]);
    assert.equal(removeScaffold.exitCode, 0, removeScaffold.stderr);
    const list = await context.q8idevai(["plugin", "ls", "--json"]);
