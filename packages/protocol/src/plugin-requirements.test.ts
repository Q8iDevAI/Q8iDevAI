...
describe.each(["daemon", "app"] as const)("plugin requirements on %s", (runtime) => {
  it("rejects legacy manifests on the first breaking release with migration instructions", () => {
    expect(() => assertPluginCompatibility({ id: "legacy", version: "0.8.0", runtime })).toThrow(
      /legacy.*<0\.8\.0.*0\.8\.0.*https:\/\/q8idevai.sh\/docs\/plugins\/v0.8\/migration/,
    );
  });
  ...
  ])("accepts %s on %s", (q8idevai, version) => {
    expect(() =>
      assertPluginCompatibility({ id: "test", requirements: { q8idevai }, version, runtime }),
    ).not.toThrow();
  });
  ...
  it.each([
    ...
    ["^0.8.0", "0.9.0"],
    ["<0.8.0", "0.8.0-beta.1"],
  ])("rejects %s on %s", (q8idevai, version) => {
    expect(() =>
      assertPluginCompatibility({ id: "test", requirements: { q8idevai }, version, runtime }),
  ...
  it.each(["", "   ", "latest", ">=potato", "0.8.0 nonsense"])(
    ...
    (q8idevai) => {
      expect(() => validatePluginRequirements({ q8idevai })).toThrow("Invalid requirements.q8idevai");
    },
  );
  ...
  it.each([null, "unknown"])("fails closed when the runtime version is %s", (version) => {
    expect(() =>
      assertPluginCompatibility({
        id: "test",
        requirements: { q8idevai: "*" },
        version,
        runtime,
      }),
    ).toThrow(`Q8iDevAI ${runtime} version is unknown`);
  });
});
