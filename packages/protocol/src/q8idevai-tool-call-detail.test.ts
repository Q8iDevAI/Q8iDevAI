...
import { buildQ8iDevAIToolDetailSections } from "./q8idevai-tool-call-detail.js";

describe("Q8iDevAI tool-call detail presentation", () => {
  it.each(["mcp__q8idevai__create_agent", "q8idevai.create_agent", "q8idevai_remote.create_agent"])(
  ...
  it("formats schedule cadence and nested settings without JSON syntax", () => {
    const sections = buildQ8iDevAIToolDetailSections(
      "mcp__q8idevai__create_schedule",
      {
  ...
  it("unwraps MCP result envelopes instead of exposing JSON-encoded text", () => {
    expect(
      buildQ8iDevAIToolDetailSections(
        "mcp__q8idevai__send_agent_prompt",
        { prompt: "Say hello back." },
        {
          meta: null,
          content: [
  ...
  it("uses readable fallback fields for newly added Q8iDevAI tools", () => {
    expect(
      buildQ8iDevAIToolDetailSections(
        "mcp__q8idevai__future_tool",
  ...
  it("leaves non-Q8iDevAI tools alone", () => {
    expect(buildQ8iDevAIToolDetailSections("mcp__github__create_issue", {}, {})).toBeNull();
  });
});
