...
describe("relay websocket URLs", () => {
  ...
  test("uses wss for non-443 ports when TLS is enabled", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.q8idevai.sh:6767",
        useTls: true,
        serverId: "srv_test",
        role: "client",
      }),
...
describe("shouldUseTlsForDefaultHostedRelay", () => {
  test("returns true for the hosted Q8iDevAI relay on port 443", () => {
    expect(shouldUseTlsForDefaultHostedRelay("relay.q8idevai.sh:443")).toBe(true);
  });
