import { getQ8iDevAIToolLeafName } from "./tool-name-normalization.js";

export interface Q8iDevAIToolDetailField {
  label: string;
  value: string;
}

export type Q8iDevAIToolDetailSection =
  | {
      kind: "prose";
      title: string;
      text: string;
  ...
  | {
      kind: "fields";
      title: string;
      fields: Q8iDevAIToolDetailField[];
    };
...
export function buildQ8iDevAIToolDetailSections(
  toolName: string,
  input: unknown,
  output: unknown,
): Q8iDevAIToolDetailSection[] | null {
  const leafName = getQ8iDevAIToolLeafName(toolName);
  if (!leafName) return null;

  const spec = TOOL_SPECS[leafName] ?? {};
  const sections: Q8iDevAIToolDetailSection[] = [];
