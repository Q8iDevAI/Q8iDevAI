...
export function validatePluginRequirements(requirements: PluginRequirements | undefined): void {
  const range = requirements?.q8idevai;
  if (range !== undefined && (!range.trim() || validRange(range) === null)) {
    throw new Error(
      `Invalid requirements.q8idevai: ${JSON.stringify(range)}. Use an npm semver range such as ">=0.8.0".`,
    );
  }
}
...
export function assertPluginCompatibility(input: PluginCompatibilityInput): void {
  ...
  const range = input.requirements?.q8idevai ?? "<0.8.0";
  const version = input.version ? parse(input.version) : null;
  if (!version) {
    throw new Error(
      `Cannot check plugin "${input.id}" requirements: Q8iDevAI ${input.runtime} version is unknown. Update the ${input.runtime}.`,
    );
  }
  const stableCore = `${version.major}.${version.minor}.${version.patch}`;
  if (satisfies(version, range) || satisfies(stableCore, range)) return;
  const action =
    input.requirements?.q8idevai === undefined
      ? "This plugin has no requirements.q8idevai and targets Q8iDevAI before 0.8. Ask its author to migrate it: https://q8idevai.sh/docs/plugins/v0.8/migration"
      : `Use a compatible plugin version or update the ${input.runtime}.`;
  throw new Error(
    `Plugin "${input.id}" requires Q8iDevAI ${range}. Your ${input.runtime} is ${input.version}. ${action}`,
  );
}
