...
const APP_SETTINGS_KEY = "@q8idevai:app-settings";
...
declare global {
  interface Window {
    __Q8IDEVAI_SIDE_PANE_MEASUREMENT__?: BrowserMeasurementState;
  }
}
...
export function resolveProfileTarget(): ProfileTarget {
  ...
  const serverId = (
    ...
    readFileSync(resolve(repoRoot, ".dev/q8idevai-home/server-id"), "utf8")
  ).trim();
  const requestedName = process.env.Q8IDEVAI_PROFILE_WORKSPACE_NAME?.trim() || "Q8iDevAI";
  const requestedWorkspaceId = process.env.Q8IDEVAI_PROFILE_WORKSPACE_ID?.trim();
  ...
  if (!workspace) {
    throw new Error(`No Q8iDevAI workspace named ${requestedName} points at ${repoRoot}`);
  }
...
async function beginMeasurement(page: Page, selectors: string[]): Promise<void> {
  ...
  await page.evaluate((trackedSelectors) => {
    ...
    window.__Q8IDEVAI_SIDE_PANE_MEASUREMENT__ = state;
  }, selectors);
}
...
async function finishMeasurement(page: Page, name: string): Promise<InteractionMeasurement> {
  ...
  const raw = await page.evaluate((measurementName) => {
    const state = window.__Q8IDEVAI_SIDE_PANE_MEASUREMENT__;
    if (!state) throw new Error("Side-pane performance measurement was not started");
    state.observer?.disconnect();
    const samples = globalThis.__Q8IDEVAI_RENDER_PROFILE__ ?? [];
    return {
