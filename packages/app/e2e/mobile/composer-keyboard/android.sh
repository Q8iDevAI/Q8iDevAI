...
STATE_DIR="${Q8IDEVAI_COMPOSER_KEYBOARD_STATE_DIR:-${REPO_ROOT}/.dev/agent-device-composer-keyboard}"
ARTIFACTS_DIR="${REPO_ROOT}/.dev/agent-device-artifacts/composer-keyboard-android"
SESSION="${Q8IDEVAI_COMPOSER_KEYBOARD_SESSION:-composer-keyboard-android}"
APP_ID="${Q8IDEVAI_COMPOSER_KEYBOARD_APP_ID:-sh.q8idevai.debug}"
DEVICE="${Q8IDEVAI_COMPOSER_KEYBOARD_DEVICE:-q8idevai-api35}"
...
METRO_PORT="${Q8IDEVAI_MOBILE_E2E_METRO_PORT:-8082}"
DAEMON_HOST="${Q8IDEVAI_COMPOSER_KEYBOARD_DAEMON_HOST:-127.0.0.1:6770}"
DAEMON_HOME="${Q8IDEVAI_COMPOSER_KEYBOARD_DAEMON_HOME:-${REPO_ROOT}/.dev/composer-e2e-home}"
SERVER_ID="${Q8IDEVAI_COMPOSER_KEYBOARD_SERVER_ID:-}"
...
if [[ -z "${SERVER_ID}" ]]; then
  if [[ ! -f "${DAEMON_HOME}/server-id" ]]; then
    echo "Missing ${DAEMON_HOME}/server-id; set Q8IDEVAI_COMPOSER_KEYBOARD_SERVER_ID" >&2
    exit 1
  fi
...
capture_ui_xml() {
  local output_path="$1"
  local device_path="/sdcard/q8idevai-composer-keyboard-window.xml"
...
workspaces_json="$(env -u Q8IDEVAI_CALLER_AGENT_ID -u Q8IDEVAI_AGENT_ID -u Q8IDEVAI_WORKSPACE_ID \
...
run_json="$(env -u Q8IDEVAI_CALLER_AGENT_ID -u Q8IDEVAI_AGENT_ID -u Q8IDEVAI_WORKSPACE_ID \
...
adb shell am start \
  -a android.intent.action.VIEW \
  -d "q8idevai://h/${SERVER_ID}/agent/${agent_id}" \
  "${APP_ID}" >/dev/null
sleep 5
