...
import { Icon } from "@q8idevai/plugin/client/react-native";
import { useAgent, useWorkspace } from "@q8idevai/plugin/client";
...
function contributeClient(client) {
  ...
  void client.q8idevai.agents.list({ subscribe: {}, signal: lifetime.signal }).then(({ subscription }) => {
...
const pluginSharedSource = `import { defineRpc } from "@q8idevai/plugin";
import { z } from "zod";
...
export default function contribute(server) {
  server.handle(recordComposerOpen, async ({ workspaceId }, { q8idevai }) => {
    await q8idevai.workspaces.ref(workspaceId).setTitle("Opened from composer pill");
    return { opened: true };
  });
  return () => {};
}`;
...
test.describe("plugin workspace panels and Command Center", () => {
  ...
  test("follows workspace, agent, host, compact, and unavailable state", async ({
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-workspace-panel-e2e-"));
