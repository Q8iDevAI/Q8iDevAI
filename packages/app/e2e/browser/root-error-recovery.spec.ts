...
async function openBrokenStartup(page: Page) {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "q8idevai:last-workspace-route-selection",
      JSON.stringify({ serverId: "fixture-host", workspaceId: "broken" }),
    );
  });
  await page.goto(appUrl);
  await expect(page.getByText("Q8iDevAI ran into a problem.", { exact: true })).toBeVisible();
}
async function readSavedWorkspaceState(page: Page) {
  return page.evaluate(() => ({
    layout: localStorage.getItem("workspace-layout-state"),
    selection: localStorage.getItem("q8idevai:last-workspace-route-selection"),
  }));
}
async function reloadToPicker(page: Page) {
  ...
  await expect(page.getByText("Q8iDevAI ran into a problem.", { exact: true })).toHaveCount(0);
}
