...
const SHARED_PROJECT_NAME = "Q8iDevAI";
const SHARED_REMOTE_URL = "https://github.com/Q8iDevAI/Q8iDevAI.git";
...
const test = base.extend<{ crossHostProject: CrossHostProjectScenario }>({
  crossHostProject: async ({ page: _page }, provide) => {
    ...
    const primarySharedRepo = await createTempGitRepo("new-workspace-q8idevai-primary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const primaryDuplicateRepo = await createTempGitRepo("new-workspace-q8idevai-duplicate-", {
      originUrl: SHARED_REMOTE_URL,
    });
    const secondaryFallbackRepo = await createTempGitRepo("new-workspace-fallback-");
    const secondarySharedRepo = await createTempGitRepo("new-workspace-q8idevai-secondary-", {
      originUrl: SHARED_REMOTE_URL,
    });
    ...
    try {
      ...
      const primarySharedProject = await createProject(primaryClient, {
        ...
        workspaceName: "Primary Q8iDevAI workspace",
      });
      ...
      const primaryDuplicateProject = await createProject(primaryClient, {
        ...
        projectName: "Q8iDevAI duplicate",
        workspaceName: "Duplicate Q8iDevAI workspace",
      });
      ...
      const secondarySharedProject = await createProject(secondaryClient, {
        ...
        projectName: SHARED_PROJECT_NAME,
        workspaceName: "Secondary Q8iDevAI workspace",
      });
      ...
      if (
        ...
        throw new Error("Expected the Q8iDevAI clones to have the same equivalence key");
      }
