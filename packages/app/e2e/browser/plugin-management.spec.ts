...
export default function contribute(plugin) {
  function Surface() {
    ...
    const cleanups = Number(globalThis.sessionStorage?.getItem("q8idevai-plugin-cleanups") || "0");
  ...
  return () => {
    ...
    if (storage) {
      const cleanups = Number(storage.getItem("q8idevai-plugin-cleanups") || "0");
      storage.setItem("q8idevai-plugin-cleanups", String(cleanups + 1));
    }
  };
}`;
}
...
test("installs, reloads, recovers, disables, and removes a trusted local plugin", async ({
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-e2e-"));
