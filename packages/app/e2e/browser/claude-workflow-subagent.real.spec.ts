...
// opts in explicitly so it tests Q8iDevAI's integration rather than the account's rollout cohort.
...
test.describe("real Claude workflow subagent row", () => {
  ...
  test("shows a real Claude workflow running and completed through the generic subagent UI", async ({
    ...
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "q8idevai-claude-workflow-row-")));
