...
test.use({ e2eInjectQ8iDevAITools: true });

function verificationPrompt(workspacePath: string): string {
  return [
    "Use Q8iDevAI tools to perform this exact UI verification.",
    ...
    "Use each named Q8iDevAI tool exactly once except for the matching schedule and heartbeat deletion tools.",
  ].join(" ");
}
...
test.describe("real Codex Q8iDevAI tool-call presentation", () => {
  test.setTimeout(600_000);

  test("renders owned tool inputs and results as readable text", async ({ page }, testInfo) => {
    const cwd = realpathSync(mkdtempSync(path.join(tmpdir(), "q8idevai-tool-call-presentation-")));
