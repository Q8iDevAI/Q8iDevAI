...
const ASSISTANT_MARKDOWN = [
  ...
  "",
  "[workspace file](file:///tmp/q8idevai%20notes.md#L4)",
  "",
...
async function selectAssistantListItemFromMarker(
  ...
  const item = assistantMessage
    .locator(`[data-q8idevai-markdown-tag="${listTag}"]`)
    .filter({ hasText: itemText })
    .locator(':scope > [data-q8idevai-markdown-tag="li"]')
    .filter({ hasText: itemText });

  await item.evaluate((element, selectedEndText) => {
    const marker = element.querySelector(':scope > [data-q8idevai-markdown-list-marker="true"]');
...
async function doubleClickAssistantMarkdownText(
  ...
  await assistantMessage
    .locator(`[data-q8idevai-markdown-tag="${tag}"]`)
    .filter({ hasText: text })
...
test("copying an assistant selection preserves Markdown structure and links", async ({
  ...
  await page.addInitScript(() => {
    localStorage.setItem("@q8idevai:app-settings", JSON.stringify({ uiFontFamily: "serif" }));
  });
  ...
  try {
    ...
    const inlineCode = assistantMessage
      .locator('[data-q8idevai-markdown-tag="code"]')
    ...
    expect(clipboard.html).toContain(
      '<a href="file:///tmp/q8idevai%20notes.md#L4">workspace file</a>',
    );
    ...
    await selectAssistantElement(
      page,
      '[data-q8idevai-markdown-language="typescript"] [data-q8idevai-markdown-tag="code"]',
    );
    await copySelection(page);
    ...
    const typescriptFence = assistantMessage.locator('[data-q8idevai-markdown-language="typescript"]');
    // The button is opacity 0 / pointerEvents none until the fence is hovered.
    await typescriptFence.hover();
    await typescriptFence.locator("[data-q8idevai-markdown-ignore]").click();
    ...
    const bashFence = assistantMessage.locator('[data-q8idevai-markdown-language="bash"]');
    await bashFence.hover();
    await bashFence.locator("[data-q8idevai-markdown-ignore]").click();
