...
async function seedRestartHome(): Promise<SeededRestartHome> {
  const q8idevaiHome = mkdtempSync(path.join(tmpdir(), "q8idevai-playwright-restart-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "q8idevai-playwright-restart-cwd-"));
  const projectsDir = path.join(q8idevaiHome, "projects");
  const agentDir = path.join(q8idevaiHome, "agents", projectDirNameFromCwd(cwd));
...
async function startRestartDaemon(input: {
  ...
  const child = spawnTsx("scripts/supervisor-entrypoint.ts", ["--dev"], {
    ...
    env: withDisabledE2ESpeechEnv({
      ...process.env,
      Q8IDEVAI_HOME: input.q8idevaiHome,
      Q8IDEVAI_SERVER_ID: SERVER_ID,
      Q8IDEVAI_LISTEN: `127.0.0.1:${port}`,
      Q8IDEVAI_CORS_ORIGINS: input.origin,
      Q8IDEVAI_RELAY_ENABLED: "0",
      Q8IDEVAI_NODE_ENV: "development",
...
async function seedBrowserForDaemon(page: Page, input: { serverId: string; port: number }) {
  ...
  await page.evaluate(
    ({ daemon, preferences }) => {
      localStorage.setItem("@q8idevai:e2e", "1");
      localStorage.setItem("@q8idevai:daemon-registry", JSON.stringify([daemon]));
      localStorage.removeItem("@q8idevai:settings");
      localStorage.setItem("@q8idevai:create-agent-preferences", JSON.stringify(preferences));
    },
    {
      daemon: host,
      preferences: {
...
test.describe("Workspace model restart regressions", () => {
  test("browser-created same-cwd workspace preserves restarted agent status and migrated tab ownership", async ({
    ...
    const daemon = await startRestartDaemon({ q8idevaiHome: seeded.q8idevaiHome, origin });
