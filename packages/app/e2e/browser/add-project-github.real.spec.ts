...
test.describe("Add Project GitHub flow", () => {
  ...
  test("searches the host's repositories and clones into a clearly shown final path", async ({
    ...
    const parentDirectory = await mkdtemp(path.join(tmpdir(), "q8idevai-e2e-github-clone-"));
    ...
    try {
      ...
      await addProjectFlowInput(page).fill("getq8idevai/q8idevai");
      await expect(addProjectFlow(page).getByText("getq8idevai/q8idevai", { exact: true })).toBeVisible({
        timeout: 30_000,
      });
