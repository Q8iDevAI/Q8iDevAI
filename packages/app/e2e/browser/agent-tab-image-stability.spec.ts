...
test("reloading a timeline anchors near-tail assistant image growth", async ({
    ...
    ({ accessibleName }) => {
      ...
      (
        window as unknown as {
          __q8idevaiReloadImageGeometry?: typeof state;
        }
      ).__q8idevaiReloadImageGeometry = state;
    },
  ...
  const events = await page.evaluate(() => {
    const state = (
      window as unknown as {
        __q8idevaiReloadImageGeometry?: { events: Array<unknown> };
      }
    ).__q8idevaiReloadImageGeometry;
