...
interface Q8iDevAIOwnedWorktree {
  projectName: string;
  workspaceId: string;
  worktreeSlug: string;
}

async function withQ8iDevAIOwnedWorktree(
  run: (workspace: Q8iDevAIOwnedWorktree) => Promise<void>,
): Promise<void> {
  ...
  try {
    ...
    if (!created.workspace) {
      throw new Error(created.error ?? "Failed to create Q8iDevAI-owned worktree");
    }
...
test.describe("Sidebar workspace list", () => {
  ...
  test("Q8iDevAI-owned worktree hover card shows the worktree directory name", async ({ page }) => {
    await withQ8iDevAIOwnedWorktree(async ({ projectName, workspaceId, worktreeSlug }) => {
      await gotoAppShell(page);
      await waitForSidebarProject(page, projectName);
