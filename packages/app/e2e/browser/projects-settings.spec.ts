...
import {
  ...
  openProjects,
  removeProjectScript,
  restoreQ8iDevAIConfig,
  returnToProjectsList,
  saveProjectEdits,
  unblockQ8iDevAIConfigWrites,
...
const test = base.extend<ProjectsSettingsFixtures>({
  editableProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: { q8idevaiConfig: initialQ8iDevAIConfig },
    });
  ...
  gitlabRemoteProject: async ({ page: _page }, provide) => {
    const workspace = await seedWorkspace({
      ...
      repo: {
        q8idevaiConfig: initialQ8iDevAIConfig,
...
async function expectProjectConfigSaved(project: ProjectsSettingsProject): Promise<void> {
  await expect
    ...
    .toMatchObject({
      worktree: {
        setup: updatedSetup,
        teardown: initialQ8iDevAIConfig.worktree.teardown,
        customWorktreeField: initialQ8iDevAIConfig.worktree.customWorktreeField,
      },
      scripts: {
        dev: {
          command: initialQ8iDevAIConfig.scripts.dev.command,
          type: initialQ8iDevAIConfig.scripts.dev.type,
          port: initialQ8iDevAIConfig.scripts.dev.port,
          customScriptField: initialQ8iDevAIConfig.scripts.dev.customScriptField,
        },
      },
      customTopLevelField: initialQ8iDevAIConfig.customTopLevelField,
    });
...
async function readProjectConfigFile(project: ProjectsSettingsProject): Promise<string> {
  return readFile(path.join(project.path, "q8idevai.json"), "utf8");
}
...
test.describe("Projects settings", () => {
  ...
  test("user edits worktree setup from the projects page", async ({ page, editableProject }) => {
    ...
    commitQ8iDevAIConfig(editableProject.path);
...
test.describe("Projects settings — error UX", () => {
  test("stale-write callout appears on save, disables save, and reload clears it", async ({
    ...
    await bumpQ8iDevAIConfigOnDisk(editableProject.path);
  ...
  test("invalid q8idevai.json shows read-error callout, reload after fix shows form", async ({
    page,
    editableProject,
  }) => {
    await corruptQ8iDevAIConfig(editableProject.path);
    ...
    await restoreQ8iDevAIConfig(editableProject.path, initialQ8iDevAIConfig);
  ...
  test("write_failed callout appears on save with blocked directory, retry re-attempts, reload clears it", async ({
    ...
    await blockQ8iDevAIConfigWrites(editableProject.path);
    ...
    await unblockQ8iDevAIConfigWrites(editableProject.path);
    await clickReloadProjectSettings(page);
