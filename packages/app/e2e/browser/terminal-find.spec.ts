...
async function viewport(page: Page) {
  return page.evaluate(
    () =>
      (window as Window & { __q8idevaiTerminal?: { buffer: { active: { viewportY: number } } } })
        .__q8idevaiTerminal?.buffer.active.viewportY,
  );
}
...
async function readVimScreen(page: Page) {
  const rows = await page.evaluate(
    () => (window as Window & { __q8idevaiTerminal: { rows: number } }).__q8idevaiTerminal.rows,
  );
