...
const SCREENSHOT_DIRECTORY = path.join(
  ...
  ".q8idevai/plans/import-session-ux",
);
const claudeConfigDirectory = mkdtempSync(path.join(tmpdir(), "q8idevai-import-flow-claude-"));
...
test.beforeAll(async () => {
  ...
  const repo = await createTempGitRepo("isf-", {
    originUrl: "https://github.com/q8idevai-e2e/import-session-fixture.git",
  });
