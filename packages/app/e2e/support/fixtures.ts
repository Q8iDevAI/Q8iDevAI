...
const EXTRA_HOSTS_KEY = "@q8idevai:e2e-extra-hosts";
  ...
  e2eWorker: [
    async (
      ...
      const worker = await startE2EWorker(workerInfo.workerIndex, {
        ...
        injectQ8iDevAITools: e2eInjectQ8iDevAITools,
      });
      try {
        await provide();
      } finally {
    ...
    async ({ page }, provide, testInfo) => {
      ...
      await page.addInitScript(
        ({ daemon, preferences, seedNonce: nonce, extraHostsKey }) => {
          ...
          const disableOnceKey = "@q8idevai:e2e-disable-default-seed-once";
          ...
          localStorage.setItem("@q8idevai:e2e-seed-nonce", nonce);
          ...
          localStorage.setItem("@q8idevai:daemon-registry", JSON.stringify([daemon, ...extraHosts]));
