...
function startMetro(port: number, buffer: ReturnType<typeof createLineBuffer>): ChildProcess {
  ...
  const child = spawn(process.execPath, [expoCli, "start", "--web", "--port", String(port)], {
    ...
    env: {
      ...process.env,
      BROWSER: "none",
      ...(process.env.E2E_DESKTOP_RUNTIME === "1" ? { Q8IDEVAI_WEB_PLATFORM: "electron" } : {}),
    },
...
export default async function globalSetup() {
  if (process.env.Q8IDEVAI_REPLICA_CACHE_MEASUREMENT === "1") {
    if (!process.env.Q8IDEVAI_REPLICA_CACHE_MEASUREMENT_URL) {
      throw new Error("Q8IDEVAI_REPLICA_CACHE_MEASUREMENT_URL must be set for live measurement");
    }
    return;
  }
