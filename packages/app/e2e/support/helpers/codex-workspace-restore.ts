...
async function callQ8iDevAITool(
  mcp: Client,
  name: string,
...
async function defaultCodexModel(mcp: Client): Promise<string> {
  const result = z
    ...
    .parse(await callQ8iDevAITool(mcp, "list_models", { provider: "codex" }));
  ...
  return {
    async createWorktreeWithCodex() {
      ...
      workspace = workspaceSchema.parse(
        await callQ8iDevAITool(mcp, "create_workspace", {
          isolation: "worktree",
          path: repo.path,
          baseBranch: "main",
      ...
      const agent = z.object({ agentId: z.string() }).parse(
        await callQ8iDevAITool(mcp, "create_agent", {
    ...
    async dispose() {
      ...
      try {
        ...
        await info.attach("daemon-log", {
          body: await readFile(path.join(process.env.E2E_Q8IDEVAI_HOME!, "daemon.log")),
          contentType: "text/plain",
        });
      } catch (error) {
        errors.push(error);
      ...
      if (!testFailed || process.env.E2E_KEEP_Q8IDEVAI_HOME !== "1") {
