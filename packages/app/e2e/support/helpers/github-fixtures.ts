...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@q8idevai.test"], localPath);
  git(["config", "user.name", "Q8iDevAI E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "q8idevaitmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@q8idevai.test"], basePath);
  git(["config", "user.name", "Q8iDevAI E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@q8idevai.test"], clonePath);
  git(["config", "user.name", "Q8iDevAI E2E"], clonePath);
...
export async function createLocalGithubPrFixture(): Promise<LocalGhPrFixture> {
  const fixtureRoot = await mkdtemp(path.join("/tmp", "q8idevai-e2e-local-github-pr-"));
  ...
  const githubUrl = "https://github.com/q8idevai-e2e/local-fixture.git";
  ...
  git(["config", "user.email", "e2e@q8idevai.test"], checkoutPath);
  git(["config", "user.name", "Q8iDevAI E2E"], checkoutPath);
  ...
  return {
    pr: {
      ...
      url: "https://github.com/q8idevai-e2e/local-fixture/pull/1",
      branch: "pr-branch-1",
      localPath: basePath,
    },
    mainCheckout: {
