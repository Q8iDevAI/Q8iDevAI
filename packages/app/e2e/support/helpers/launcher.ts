...
export async function sampleTabsDuringTransition(
  ...
  await page.evaluate((duration) => {
    const scope = globalThis as typeof globalThis & {
      __q8idevaiTabTrackFrames?: Array<Array<{ id: string; width: number }>>;
    };
    scope.__q8idevaiTabTrackFrames = [];
    ...
    function sample() {
      ...
      scope.__q8idevaiTabTrackFrames?.push(
        tabs.map((element) => ({
  ...
  return page.evaluate(() => {
    ...
    return scope.__q8idevaiTabTrackFrames ?? [];
  });
}
