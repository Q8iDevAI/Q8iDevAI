...
export async function startPackagedWebDaemon(input: {
  ...
  const home = await mkdtemp(path.join(tmpdir(), "q8idevai-relay-deployment-e2e-"));
  const serverId = `relay-deployment-${Date.now().toString(36)}`;
  const q8idevai = path.resolve(__dirname, "../../../../../node_modules/.bin/q8idevai");
  ...
  try {
    ...
    await execFileAsync(q8idevai, ["daemon", "start", "--home", home], { env });
    ...
    return {
      ...
      close: async () => {
        await execFileAsync(q8idevai, ["daemon", "stop", "--home", home], { env }).catch(
          () => undefined,
        );
    ...
    await execFileAsync(q8idevai, ["daemon", "stop", "--home", home], { env }).catch(() => undefined);
    await rm(home, { recursive: true, force: true });
    throw error;
  }
}
