...
export async function startIsolatedHostDaemon(
  ...
  const q8idevaiHome =
    options.q8idevaiHome ?? (await mkdtemp(path.join(tmpdir(), "q8idevai-e2e-secondary-host-")));
  let publishedPackageRoot: string | null = null;
  if (options.publishedVersion) {
    publishedPackageRoot = await mkdtemp(path.join(tmpdir(), "q8idevai-e2e-published-server-"));
    await writeFile(
    ...
    try {
      ...
      execFileSync(
        ...
        [
          ...
          "--no-fund",
          "--no-package-lock",
          `@q8idevai/server@${options.publishedVersion}`,
        ],
  ...
  const serverDir = publishedPackageRoot
    ? path.join(publishedPackageRoot, "node_modules", "@getq8idevai", "server")
  ...
  const spawnDaemon = async (): Promise<ChildProcess> => {
    const spawnOptions: SpawnOptions = {
      ...
      env: withDisabledE2ESpeechEnv({
        ...
        Q8IDEVAI_SERVER_ID: serverId,
        Q8IDEVAI_LISTEN: `127.0.0.1:${port}`,
        Q8IDEVAI_CORS_ORIGINS: `http://localhost:${metroPort}`,
        Q8IDEVAI_RELAY_ENABLED: options.mutableRelay ? undefined : "0",
