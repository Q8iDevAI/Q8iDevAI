...
export interface Q8iDevAIHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveQ8iDevAIHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkQ8iDevAIHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<Q8iDevAIHomeMetadataForkResult> {
  const sourceHome = resolveQ8iDevAIHomePath(input.sourceHome);
  const targetHome = resolveQ8iDevAIHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Q8iDevAI metadata onto the same Q8IDEVAI_HOME.");
  }
