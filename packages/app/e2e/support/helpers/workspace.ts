...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@q8idevai.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Q8iDevAI E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.q8idevaiConfig) {
    await writeFile(
      path.join(repoPath, "q8idevai.json"),
      JSON.stringify(options.q8idevaiConfig, null, 2),
    );
  }
  ...
  if (options?.q8idevaiConfig) {
    execSync("git add q8idevai.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.q8idevai-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "q8idevai-e2e-dir-"): Promise<TempDirectory> {
...
export async function trackForkUpstream(repoPath: string): Promise<string> {
  ...
  const upstreamClone = await mkdtemp(path.join(await resolveTempRoot(), "q8idevai-e2e-upstream-"));
  ...
  execSync('git config user.email "e2e@q8idevai.test"', { cwd: upstreamClone, stdio: "ignore" });
  execSync('git config user.name "Q8iDevAI E2E"', { cwd: upstreamClone, stdio: "ignore" });
...
export async function readWorktreeBaseMetadata(
  ...
  const metadata = JSON.parse(
    await readFile(path.join(gitDir, "q8idevai", "worktree.json"), "utf8"),
