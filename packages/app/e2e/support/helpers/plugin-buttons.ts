...
import { useWorkspace, useRpc, useQ8iDevAI } from "@q8idevai/plugin/client";
...
function Details({ workspaceId, theme, layout, close }) {
  ...
  React.useEffect(() => {
    const owner = q8idevai.observeEvents(["project.update"]);
...
export default function contribute(client) {
  ...
  void client.q8idevai.workspaces.list({ subscribe: {}, signal: lifetime.signal }).then(({ subscription }) => {
...
async function installShowcase(workspaceId: string, agentId: string) {
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-buttons-"));
  ...
  await writeFile(
    ...
    `import { defineRpc } from "@q8idevai/plugin"; import { z } from "zod"; export const summary = defineRpc({ name: "summary", input: z.object({}), output: z.object({ message: z.string() }) });`,
  );
  await writeFile(
