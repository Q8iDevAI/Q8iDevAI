...
import type { CreateAgentRequestMessage, SessionInboundMessage } from "@q8idevai/protocol/messages";
import type { DaemonClient as InternalDaemonClient } from "@q8idevai/client/internal/daemon-client";
...
export async function archiveWorkspaceFromDaemon(
  ...
  const payload = await client.archiveQ8iDevAIWorktree({
...
export async function createWorktreeViaDaemon(
  ...
  const payload = await client.createQ8iDevAIWorktree({
    cwd: input.cwd,
