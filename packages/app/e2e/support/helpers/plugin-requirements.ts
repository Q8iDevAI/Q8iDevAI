...
export const test = base.extend<{
  ...
  requirementHost: async ({ e2eWorker }, provide) => {
    void e2eWorker;
    const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-requirement-ui-"));
...
async function writeRequirements(directory: string, q8idevai: string) {
  await writeFile(
    path.join(directory, "q8idevai-plugin.json"),
    JSON.stringify({ id: "requirements-example", requirements: { q8idevai } }),
  );
}
...
export async function expectAppMismatch(page: Page) {
  ...
  await expect(
    page.getByText(/Plugin "requirements-example" requires Q8iDevAI \^99.0.0. Your app is/),
  ).toBeVisible();
}

export async function correctRequirementAndReload(page: Page, directory: string) {
  await writeRequirements(directory, pluginRequirements.q8idevai);
