...
export async function primeAdditionalPage(page: Page): Promise<void> {
  ...
  await page.addInitScript(
    ({ daemon: seededDaemon, preferences: seededPreferences, seedNonce: nonce }) => {
      ...
      localStorage.setItem("@q8idevai:daemon-registry", JSON.stringify([seededDaemon]));
      localStorage.removeItem("@q8idevai:settings");
      localStorage.setItem("@q8idevai:create-agent-preferences", JSON.stringify(seededPreferences));
    },
