...
export async function createProjectPickerFixture(): Promise<ProjectPickerFixtureResource> {
  const root = await mkdtemp(path.join(homedir(), "q8idevai-e2e-project-picker-"));
  const nonce = randomUUID().replaceAll("-", "").slice(0, 8);
  const projectPath = path.join(root, "client", "team", `q8idevai-desktop-fuzzy-target-${nonce}`);
