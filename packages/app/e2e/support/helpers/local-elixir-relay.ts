...
export async function startLocalElixirRelay(): Promise<LocalElixirRelay> {
  ...
  const relayRoot =
    process.env.Q8IDEVAI_RELAY_CHECKOUT ?? path.resolve(__dirname, "../../../../../..", "q8idevai-relay");
  if (!existsSync(path.join(relayRoot, "mix.exs"))) {
    throw new Error(
      `Expected the Elixir relay checkout at ${relayRoot}. Set Q8IDEVAI_RELAY_CHECKOUT to override it.`,
    );
  }
  ...
  const start = async () => {
    ...
    child = spawn("asdf", ["exec", "mix", "run", "--no-halt"], {
      ...
      env: {
        ...process.env,
        MIX_ENV: "prod",
        Q8IDEVAI_RELAY_HOST: "127.0.0.1",
        Q8IDEVAI_RELAY_PORT: String(port),
        Q8IDEVAI_RELAY_MIN_CLUSTER_SIZE: "1",
      },
