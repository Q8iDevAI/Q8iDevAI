...
export async function startWithoutRememberedModel(page: Page) {
  await page.addInitScript(() => localStorage.removeItem("@q8idevai:create-agent-preferences"));
}
...
export async function expectSavedSelection(page: Page, provider: string, model: string) {
  await expect
    .poll(() =>
      page.evaluate(() =>
        JSON.parse(localStorage.getItem("@q8idevai:create-agent-preferences") ?? "null"),
      ),
    )
