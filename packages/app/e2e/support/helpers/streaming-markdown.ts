  ...
  const agent = await seedMockAgentWorkspace({
    ...
    featureValues: {
      mockStreamingAssistantResponse:
        "**Bold text stays bold** and [Q8iDevAI docs](https://example.com/documentation). Done.",
...
export async function expectUnfinishedBold(page: Page): Promise<void> {
  ...
  const bold = message.locator('[data-q8idevai-markdown-tag="strong"]');
...
export async function expectUnfinishedLink(
  ...
  const message = page.getByTestId("assistant-message").last();
  await agent.stream.showThrough("**Bold text stays bold** and [Q8iDevAI docs");
  ...
  await expect(message).toContainText("Q8iDevAI");
...
async function expectCompletedMarkdown(page: Page): Promise<void> {
  const message = page.getByTestId("assistant-message").last();
  await expect(message).toHaveText("Bold text stays bold and Q8iDevAI docs. Done.");
  await expect(
    message.getByRole("link", { name: "Q8iDevAI docs" }).and(message.locator("a")),
  ).toHaveAttribute("href", "https://example.com/documentation");
  await expect(message.locator('[data-q8idevai-markdown-tag="strong"]')).toHaveCSS(
    "font-weight",
    "500",
  );
}
