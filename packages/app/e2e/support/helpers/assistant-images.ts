    ...
    ({ accessibleName, errorText, tabTestId }) => {
      ...
      (
        window as unknown as {
          __q8idevaiAssistantImageObservation?: typeof record;
        }
      ).__q8idevaiAssistantImageObservation = record;
    },
    {
      accessibleName: alt,
...
async function expectNoVisibleImageInstability(page: Page): Promise<void> {
  const observation = await page.evaluate(() => {
    const owner = window as unknown as {
      __q8idevaiAssistantImageObservation?: {
        errorSeen: boolean;
        missingSeen: boolean;
        inspect(): void;
    ...
    const record = owner.__q8idevaiAssistantImageObservation;
    if (!record) throw new Error("Assistant image stability observation was not started");
    record.inspect();
    record.observer?.disconnect();
    delete owner.__q8idevaiAssistantImageObservation;
