...
export const pluginRequirements = { q8idevai: `>=${appPackage.version}` };

export async function copyPluginExample(name: string) {
  const directory = await mkdtemp(path.join(tmpdir(), "q8idevai-plugin-example-"));
  try {
    await cp(path.resolve(__dirname, "../../../../../plugin-examples", name), directory, {
      recursive: true,
    });
    const manifestPath = path.join(directory, "q8idevai-plugin.json");
    const manifest = z
