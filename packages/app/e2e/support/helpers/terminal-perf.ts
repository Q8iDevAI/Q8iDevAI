...
export async function measureKeystrokeLatency(page: Page, char: string): Promise<number> {
  await page.evaluate(() => {
    const win = window as Window & {
      __q8idevaiTerminal?: { onWriteParsed: (cb: () => void) => { dispose: () => void } };
      __perfKeystroke?: { promise: Promise<number> | null };
    };
    if (!win.__q8idevaiTerminal) {
      throw new Error("__q8idevaiTerminal not available");
    }
    const term = win.__q8idevaiTerminal;
