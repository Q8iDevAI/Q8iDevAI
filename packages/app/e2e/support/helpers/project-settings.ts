...
export async function expectUncommittedSetupWarning(page: Page): Promise<void> {
  const warning = page.getByRole("alert").filter({ hasText: "Commit q8idevai.json changes" });
  await expect(warning).toContainText("Commit q8idevai.json changes");
...
export async function corruptQ8iDevAIConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "q8idevai.json"), "{not valid json}");
}

export async function bumpQ8iDevAIConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "q8idevai.json");
...
export async function restoreQ8iDevAIConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "q8idevai.json"), JSON.stringify(config, null, 2) + "\n");
}

export function commitQ8iDevAIConfig(repoPath: string): void {
  execFileSync("git", ["add", "q8idevai.json"], { cwd: repoPath });
...
export async function blockQ8iDevAIConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockQ8iDevAIConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o755);
}

// --- WebSocket helpers ---

// Proxies all daemon WS traffic transparently, but rejects q8idevai.json reads
