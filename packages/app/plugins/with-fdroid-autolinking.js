...
const FDROID_ABI_VERSION_CODE_BLOCK = `// Q8iDevAI F-Droid single-ABI version codes
def q8idevaiAbiVersionCodes = [
${FDROID_ABI_VERSION_CODE_ENTRIES}
]
def q8idevaiArchitectures = (findProperty("reactNativeArchitectures") ?: "")
    .toString()
    .split(",")
    .collect { it.trim() }
    .findAll { !it.isEmpty() }

if (q8idevaiArchitectures.size() == 1) {
    def q8idevaiAbi = q8idevaiArchitectures[0]
    def q8idevaiAbiVersionCode = q8idevaiAbiVersionCodes[q8idevaiAbi]
    if (q8idevaiAbiVersionCode == null) {
        throw new GradleException("Unsupported Q8iDevAI Android ABI: " + q8idevaiAbi)
    }
    android.defaultConfig.versionCode = android.defaultConfig.versionCode * 10 + q8idevaiAbiVersionCode
}
`;
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// Q8iDevAI F-Droid single-ABI version codes")) {
