...
OUT_DIR="/tmp/q8idevai-sidebar-drag-cancellation-$(date +%s)"
CLIENT_EXPORTS="$REPO_ROOT/packages/client/dist/daemon-client.js"
RELAY_EXPORTS="$REPO_ROOT/node_modules/@q8idevai/relay/dist/e2ee.js"
FIXTURE_ROOT=""
PROJECT_IDS_FILE="$OUT_DIR/project-ids.json"

export Q8IDEVAI_MAESTRO_APP_ID="${Q8IDEVAI_MAESTRO_APP_ID:-sh.q8idevai.debug}"
export Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT="${Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT:-127.0.0.1:6767}"
export Q8IDEVAI_MAESTRO_DAEMON_WS_URL="${Q8IDEVAI_MAESTRO_DAEMON_WS_URL:-ws://127.0.0.1:6767/ws}"
export Q8IDEVAI_MAESTRO_DAEMON_HEALTH_URL="${Q8IDEVAI_MAESTRO_DAEMON_HEALTH_URL:-http://127.0.0.1:6767/api/health}"
...
if ! curl --fail --silent --show-error --max-time 3 "$Q8IDEVAI_MAESTRO_DAEMON_HEALTH_URL" >/dev/null; then
  echo "Q8iDevAI daemon is unavailable at $Q8IDEVAI_MAESTRO_DAEMON_HEALTH_URL" >&2
  exit 1
fi

FIXTURE_ROOT="$(mktemp -d /tmp/q8idevai-sidebar-drag-fixture-XXXXXX)"
export Q8IDEVAI_MAESTRO_DRAG_A_NAME="000-q8idevai-drag-a-$(basename "$FIXTURE_ROOT")"
export Q8IDEVAI_MAESTRO_DRAG_B_NAME="001-q8idevai-drag-b-$(basename "$FIXTURE_ROOT")"
export Q8IDEVAI_MAESTRO_DRAG_Z_NAME="zzz-q8idevai-drag-z-$(basename "$FIXTURE_ROOT")"

mkdir -p "$OUT_DIR/flows"

if [ -z "${Q8IDEVAI_MAESTRO_IOS_UDID:-}" ]; then
  export Q8IDEVAI_MAESTRO_IOS_UDID
  Q8IDEVAI_MAESTRO_IOS_UDID="$({ xcrun simctl list devices booted -j || true; } | node -e '
    let input = "";
...
render_flow() {
  ...
  local target="$2"
  perl -0pe '
    s/^appId: sh\.q8idevai$/appId: $ENV{Q8IDEVAI_MAESTRO_APP_ID}/m;
    s/\$\{Q8IDEVAI_MAESTRO_APP_ID\}/$ENV{Q8IDEVAI_MAESTRO_APP_ID}/g;
    s/\$\{Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT\}/$ENV{Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT}/g;
    s/\$\{Q8IDEVAI_MAESTRO_DRAG_A_NAME\}/$ENV{Q8IDEVAI_MAESTRO_DRAG_A_NAME}/g;
    s/\$\{Q8IDEVAI_MAESTRO_DRAG_B_NAME\}/$ENV{Q8IDEVAI_MAESTRO_DRAG_B_NAME}/g;
    s/\$\{Q8IDEVAI_MAESTRO_DRAG_Z_NAME\}/$ENV{Q8IDEVAI_MAESTRO_DRAG_Z_NAME}/g;
  ' "$source" > "$target"
}

for project_name in \
  "$Q8IDEVAI_MAESTRO_DRAG_A_NAME" \
  "$Q8IDEVAI_MAESTRO_DRAG_B_NAME" \
  "$Q8IDEVAI_MAESTRO_DRAG_Z_NAME"; do
  ...
  git -C "$project_path" config user.name "Q8iDevAI Maestro"
  git -C "$project_path" config user.email "maestro@getq8idevai.local"
...
const client = new DaemonClient({
  url: process.env.Q8IDEVAI_MAESTRO_DAEMON_WS_URL,
...
const { DaemonClient } = await import(moduleUrl);
const projectNames = [
  process.env.Q8IDEVAI_MAESTRO_DRAG_A_NAME,
  process.env.Q8IDEVAI_MAESTRO_DRAG_B_NAME,
  process.env.Q8IDEVAI_MAESTRO_DRAG_Z_NAME,
];
const client = new DaemonClient({
  url: process.env.Q8IDEVAI_MAESTRO_DAEMON_WS_URL,
...
echo "Running sidebar drag cancellation regression on $Q8IDEVAI_MAESTRO_IOS_UDID"
echo "Artifacts: $OUT_DIR"
(cd "$OUT_DIR" && maestro test "$FLOW" --udid "$Q8IDEVAI_MAESTRO_IOS_UDID")
