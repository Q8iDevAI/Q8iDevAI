...
#   Q8IDEVAI_MAESTRO_APP_ID=sh.q8idevai.debug
#   Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:6767
#   Q8IDEVAI_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:6767/ws
#   Q8IDEVAI_MAESTRO_PROJECT_PATH=/path/to/git/repo
set -euo pipefail
...
OUT_DIR="/tmp/q8idevai-workspace-create-android-$(date +%s)"
...
render_flow() {
  ...
  perl -0pe '
    ...
    s/\$\{Q8IDEVAI_MAESTRO_PROJECT_NAME\}/$ENV{Q8IDEVAI_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ -z "${Q8IDEVAI_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/q8idevai-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export Q8IDEVAI_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$Q8IDEVAI_MAESTRO_PROJECT_PATH"
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" config user.name "Q8iDevAI Maestro"
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" config user.email "maestro@getq8idevai.local"
  printf "# Workspace create Android repro\n" > "$Q8IDEVAI_MAESTRO_PROJECT_PATH/README.md"
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" add README.md
  git -C "$Q8IDEVAI_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export Q8IDEVAI_MAESTRO_PROJECT_NAME="${Q8IDEVAI_MAESTRO_PROJECT_NAME:-$(basename "$Q8IDEVAI_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $Q8IDEVAI_MAESTRO_APP_ID"
echo "Android direct endpoint: $Q8IDEVAI_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $Q8IDEVAI_MAESTRO_DAEMON_WS_URL"
echo "Project: $Q8IDEVAI_MAESTRO_PROJECT_PATH"
echo "Project name: $Q8IDEVAI_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.Q8IDEVAI_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.Q8IDEVAI_MAESTRO_DAEMON_WS_URL;
