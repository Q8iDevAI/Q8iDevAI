...
const {
  ...
  const routePreferenceByServerId: Record<string, "public" | "q8idevai" | "direct"> = {};
  const routePreferenceListeners = new Set<() => void>();
  const setPreferredRoute = vi.fn((serverId: string, kind: "public" | "q8idevai" | "direct") => {
  ...
  it("uses service icon color for service health and running unknown status only", () => {
    current = renderScripts([
      ...
      script({
        scriptName: "worker",
        type: "service",
        hostname: "worker.q8idevai.localhost",
        lifecycle: "running",
        health: null,
        port: 5000,
      }),
      script({
        ...
        type: "service",
        hostname: "old-service.q8idevai.localhost",
        lifecycle: "stopped",
        exitCode: 1,
      }),
    ]);
