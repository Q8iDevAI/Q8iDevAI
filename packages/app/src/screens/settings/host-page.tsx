...
} from "@q8idevai/protocol/terminal-profiles";
    ...
    <View>
      {isConnected ? (
        <SettingsSection title={t("settings.hostSections.agents")}>
          <InjectQ8iDevAIToolsCard serverId={serverId} />
...
function InjectQ8iDevAIToolsCard({ serverId }: { serverId: string }) {
  const { t } = useTranslation();
  const isConnected = useHostRuntimeIsConnected(serverId);
    ...
    <View style={settingsStyles.card} testID="host-page-auto-archive-merged-workspaces-card">
      <View style={settingsStyles.row}>
        <View style={settingsStyles.rowContent}>
          ...
          <Text style={settingsStyles.rowHint}>
            Automatically archive clean Q8iDevAI workspaces after their pull request is merged
          </Text>
        </View>
        <Switch
