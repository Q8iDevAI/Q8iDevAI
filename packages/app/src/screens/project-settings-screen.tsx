...
const WORKTREE_DOCS_URL = "https://q8idevai.sh/docs/worktrees";
...
function ProjectSettingsBody({
  ...
  const loadedConfig: Q8iDevAIConfigRaw | null = data?.ok ? (data.config ?? {}) : null;
  const loadedRevision: Q8iDevAIConfigRevision | null = data?.ok ? data.revision : null;
...
interface RenderContentInput {
  ...
  loadedConfig: Q8iDevAIConfigRaw | null;
  loadedRevision: Q8iDevAIConfigRevision | null;
...
function revisionToKey(revision: Q8iDevAIConfigRevision | null): string {
...
interface ProjectConfigFormProps {
  baseConfig: Q8iDevAIConfigRaw;
  revision: Q8iDevAIConfigRevision | null;
  hasUncommittedWorktreeSetupChanges: boolean;
  repoRoot: string;
  queryKey: readonly [string, string, string];
  client: DaemonClient;
