...
} from "@q8idevai/protocol/provider-snapshot-codec";
import { CompactProviderSnapshotSchema } from "@q8idevai/protocol/messages";
import { z } from "zod";
import type { GetProvidersSnapshotResponseMessage } from "@q8idevai/protocol/messages";
type SnapshotPayload = GetProvidersSnapshotResponseMessage["payload"];

const CACHE_VERSION = 2;
const CACHE_KEY_PREFIX = "@q8idevai/provider-snapshot/v2";
const CACHE_INDEX_KEY = "@q8idevai/provider-snapshot-index/v2";
...
export function createProviderSnapshotCache(
  ...
  async function reconcileCache(): Promise<void> {
    ...
    const legacyKeys = allKeys.filter(
      (key) =>
        key.startsWith("@q8idevai/provider-snapshot/v1:") ||
        key === "@q8idevai/provider-snapshot-index/v1" ||
        key === CACHE_INDEX_KEY,
    );
