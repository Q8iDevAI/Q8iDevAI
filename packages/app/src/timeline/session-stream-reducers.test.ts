...
import type { AgentTimelineItem, ToolCallDetail } from "@q8idevai/protocol/agent-types";
import {
  createUserMessage,
  hydrateStreamState,
...
describe("processAgentStreamEvents", () => {
  ...
  it("keeps Claude image tool-result output before following assistant blocks while text streams", () => {
    const imageMarkdown =
      "![Image](/tmp/q8idevai-attachments/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png)";
    const result = processAgentStreamEvents({
      events: [
        makeStreamReducerEvent(
          {
