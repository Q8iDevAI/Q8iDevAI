...
function registry(version: string) {
  ...
  const result = new PluginRegistry({
    ...
    createRuntime() {
      starts++;
      return {
        q8idevai: createQ8iDevAIApi(client),
        rpc: async () => {
...
it("checks the app version before creating a runtime or evaluating plugin code", () => {
  ...
  expect(result.getEvaluationError("host", "example")).toBe(
    'Plugin "example" requires Q8iDevAI >=0.9.0. Your app is 0.8.0. Use a compatible plugin version or update the app.',
  );
});
...
it("unloads on a requirement-only edit and recovers after correction", () => {
  ...
  const install = (q8idevai: string) =>
    result.installCatalog("host", [{ id: "example", clientBundle, requirements: { q8idevai } }], {
      client,
    });
  install("^0.8.0");
  ...
  expect(starts()).toBe(1);
  expect(result.getSnapshot()).toEqual([]);
  expect(result.getEvaluationError("host", "example")).toContain("requires Q8iDevAI >=0.9.0");
  install(">=0.8.0");
