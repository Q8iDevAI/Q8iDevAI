  ...
  installCatalog(
    ...
    const preserved = catalog.flatMap((entry) => {
      const existing = previous.find(
        (plugin) =>
          ...
          plugin.requirements?.q8idevai === entry.requirements?.q8idevai,
      );
    ...
    const installed = catalog.flatMap((entry) => {
      ...
      try {
        ...
        const q8idevai = runtime.q8idevai;
        installation.cleanup = async () => {
          const results = await Promise.allSettled([q8idevai.dispose(), evaluated.cleanup()]);
