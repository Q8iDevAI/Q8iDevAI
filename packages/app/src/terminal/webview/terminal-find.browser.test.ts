...
interface TerminalFrameWindow extends Window {
  __Q8IDEVAI_TERMINAL_WEBVIEW_RECEIVE__(message: unknown): void;
  __q8idevaiTerminal?: Terminal;
}
...
test("the generated WebView searches its mounted stream and keeps Find commands off the input bridge", async () => {
  ...
  const send = (message: object) =>
    win.__Q8IDEVAI_TERMINAL_WEBVIEW_RECEIVE__({ streamKey: "owned", ...message });
  send({
    type: "mount",
  ...
  await expect
    .poll(() => {
      const b = win.__q8idevaiTerminal?.buffer.active;
      return b
  ...
  expect(win.__q8idevaiTerminal!.getSelection()).toBe("a.b");
  send({ type: "clearFind" });
  await expect
    .poll(() => messages.findLast((m) => m.type === "findResult")?.result)
