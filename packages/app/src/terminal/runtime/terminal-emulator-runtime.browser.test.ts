...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__q8idevaiTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__q8idevaiTerminal !== undefined,
    });

    expect(window.__q8idevaiTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__q8idevaiTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__q8idevaiTerminal).toBe(terminal);
    expect(window.__q8idevaiTerminal?.options.scrollback).toBe(42_000);
  });
