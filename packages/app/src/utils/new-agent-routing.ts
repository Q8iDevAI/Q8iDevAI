...
function inferMainRepoRootFromQ8iDevAIWorktreePath(cwd: string): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.q8idevai/worktrees";
...
export function resolveNewAgentWorkingDir(
  ...
  const explicitMainRepoRoot = checkout?.isQ8iDevAIOwnedWorktree
  ...
  return inferMainRepoRootFromQ8iDevAIWorktreePath(cwd) ?? cwd;
}
