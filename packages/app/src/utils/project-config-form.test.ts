...
import { Q8iDevAIConfigRawSchema } from "@q8idevai/protocol/q8idevai-config-schema";
import type { Q8iDevAIConfigRaw } from "@q8idevai/protocol/messages";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: Q8iDevAIConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = Q8iDevAIConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
