...
describe("clientIdResolver", () => {
  it("returns the stored client id when present and does not regenerate", async () => {
    const storage = inMemoryStorage({ "@q8idevai:client-id-v1": "cid_existing" });
  ...
  it("creates and persists a new client id when storage is empty", async () => {
    ...
    expect(storage.items.get("@q8idevai:client-id-v1")).toBe("cid_123456781234123412341234567890ab");
  });
  ...
  it("ignores stored blank strings and treats them as missing", async () => {
    const storage = inMemoryStorage({ "@q8idevai:client-id-v1": "   " });
    const resolver = createClientIdResolver({
      storage,
    ...
    expect(storage.items.get("@q8idevai:client-id-v1")).toBe("cid_newuuid");
  });
});
