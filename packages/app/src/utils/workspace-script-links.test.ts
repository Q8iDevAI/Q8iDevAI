...
import type { WorkspaceScriptPayload } from "@q8idevai/protocol/messages";
...
const runningService: WorkspaceScriptPayload = {
  scriptName: "web",
  type: "service",
  hostname: "web--feature--q8idevai.localhost",
  port: 3000,
  localProxyUrl: "http://web--feature--q8idevai.localhost:6767",
  publicProxyUrl: null,
  proxyUrl: "http://web--feature--q8idevai.localhost:6767",
  lifecycle: "running",
  health: "healthy",
  exitCode: null,
  terminalId: null,
...
describe("resolveWorkspaceScriptLink", () => {
  it("defaults to the memorable Q8iDevAI URL locally and keeps direct as a fallback", () => {
    expect(
      resolveLink({ type: "directTcp", endpoint: "localhost:6767", display: "localhost:6767" }),
    ).toEqual({
      primary: {
        kind: "q8idevai",
        label: "web--feature--q8idevai.localhost:6767",
        url: "http://web--feature--q8idevai.localhost:6767",
      },
      targets: [
        {
          kind: "q8idevai",
  ...
  it("defaults to an explicitly configured reverse proxy", () => {
    const publicUrl = "https://web--feature--q8idevai.services.example.com";
    expect(
      resolveLink(
        { type: "directSocket", endpoint: "/tmp/q8idevai.sock", display: "socket" },
        { ...runningService, publicProxyUrl: publicUrl, proxyUrl: publicUrl },
      ),
    ).toEqual({
      primary: {
        kind: "public",
        label: "web--feature--q8idevai.services.example.com",
        url: publicUrl,
      },
      targets: [
        {
  ...
  it("offers the reverse proxy and direct route over a direct network connection", () => {
    ...
    expect(
      ...
      { kind: "public", label: "web--feature--q8idevai.services.example.com", url: publicUrl },
      {
        kind: "q8idevai",
        label: "web--feature--q8idevai.localhost:6767",
        url: "http://web--feature--q8idevai.localhost:6767",
  ...
  it("classifies proxyUrl from older daemons", () => {
    ...
    expect(
      resolveLink(
        { type: "relay", endpoint: "relay.q8idevai.sh:443", display: "relay" },
