...
const Q8IDEVAI_WORKTREE_PATH_PATTERN = /(^|\/)\.q8idevai\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isQ8iDevAIOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isQ8iDevAIOwnedWorktreePath(cwd: string): boolean {
  return Q8IDEVAI_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
