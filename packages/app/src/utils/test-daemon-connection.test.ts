...
import type { DaemonClientConfig } from "@q8idevai/client/internal/daemon-client";
  ...
  readonly deps: DaemonConnectionDependencies<FakeDaemonClient> = {
    ...
    buildDesktopTransportUrl: (target) => {
      if (target.transportType === "ssh") {
        return `q8idevai+desktop://ssh?host=${encodeURIComponent(target.host)}`;
      }
      return `q8idevai+desktop://${target.transportType}?path=${encodeURIComponent(target.transportPath)}`;
    },
    createClient: (config) => {
...
describe("test-daemon-connection connectToDaemon", () => {
  ...
  it("encodes the local socket target into the client config", async () => {
    ...
    const result = await connectToDaemon(
      {
        id: "socket:/tmp/q8idevai.sock",
        type: "directSocket",
        path: "/tmp/q8idevai.sock",
      },
      undefined,
      probe.deps,
    );
    ...
    expect(probe.createdConfigs()[0]?.url).toBe("q8idevai+desktop://socket?path=%2Ftmp%2Fq8idevai.sock");
  });
  ...
  it("uses the desktop transport for Remote SSH connections", async () => {
    ...
    const result = await connectToDaemon(
      {
        id: "ssh:deploy%40example.com:2222:%2Fkeys%2Fq8idevai",
        type: "remoteSsh",
    ...
    expect(probe.createdConfigs()[0]).toMatchObject({
      url: "q8idevai+desktop://ssh?host=deploy%40example.com",
      transportFactory,
    });
  });
  ...
  it("uses relay TLS from the stored connection", async () => {
    ...
    expect(probe.createdConfigs()[1]?.url).toMatch(/^ws:\/\/relay\.q8idevai\.sh:443\/ws\?/);
  });
