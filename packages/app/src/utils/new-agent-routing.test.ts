...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.q8idevai/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style q8idevai worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.q8idevai\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for q8idevai-owned worktrees", () => {
    const checkout = {
      isQ8iDevAIOwnedWorktree: true,
      worktreeRoot: "/repo/.q8idevai/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.q8idevai/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
