...
describe("resolveAssistantImageSource", () => {
  ...
  it("falls back to filesystem root for absolute paths outside the workspace", () => {
    expect(
      resolveAssistantImageSource({
        source: "/tmp/q8idevai-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "/",
      path: "/tmp/q8idevai-codex-screenshot.png",
    });
  });
  ...
  it("uses the same home-root target as file previews for tilde paths", () => {
    expect(
      resolveAssistantImageSource({
        source: "~/.q8idevai/screenshots/output.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "~",
      path: "~/.q8idevai/screenshots/output.png",
    });
  });
  ...
  it("normalizes file URIs into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "file:///tmp/q8idevai-codex-screenshot.png",
        workspaceRoot: "/Users/test/project",
      }),
    ).toEqual({
      kind: "file_rpc",
  ...
  it("normalizes markdown-encoded Windows paths into file RPC requests", () => {
    expect(
      resolveAssistantImageSource({
        source: "C:%5CUsers%5Chanse%5CAppData%5CLocal%5CTemp%5Cq8idevai-attachments%5Cimage.png",
        workspaceRoot: "C:/Users/hanse/eatingkat",
      }),
    ).toEqual({
      kind: "file_rpc",
      cwd: "C:/",
      path: "C:/Users/hanse/AppData/Local/Temp/q8idevai-attachments/image.png",
    });
  });
