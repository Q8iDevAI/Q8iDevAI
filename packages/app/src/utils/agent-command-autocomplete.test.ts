...
describe("filterAndRankCommandAutocompleteEntries", () => {
  const entries = [
    { source: "provider" as const, command: { name: "q8idevai-committee" } },
    { source: "provider" as const, command: { name: "commit" } },
    { source: "provider" as const, command: { name: "q8idevai-advisor" } },
  ];
  ...
  it("ranks command-name prefixes above later word-boundary partial matches", () => {
    ...
    expect(result.map((entry) => entry.command.name)).toEqual(["commit", "q8idevai-committee"]);
  });
