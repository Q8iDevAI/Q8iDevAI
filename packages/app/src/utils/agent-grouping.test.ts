...
describe("deriveProjectDisplayName", () => {
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/Q8iDevAI/Q8iDevAI",
        projectName: "q8idevai",
      }),
  ...
  it("falls back to projectName for local keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "/Users/me/dev/q8idevai",
        projectName: "q8idevai",
      }),
    ).toBe("q8idevai");
  });
...
describe("groupAgents", () => {
  ...
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/q8idevai" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/q8idevai-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:getq8idevai/q8idevai.git",
    });
