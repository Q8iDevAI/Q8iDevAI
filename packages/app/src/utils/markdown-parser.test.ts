...
describe("createMarkdownParser", () => {
  ...
  it("linkifies bare URLs only when asked", () => {
    expect(createMarkdownParser({ linkify: true }).render("see https://q8idevai.sh now")).toContain(
      'href="https://q8idevai.sh"',
    );
    expect(
      createMarkdownParser({ linkify: false }).render("see https://q8idevai.sh now"),
    ).not.toContain("href");
  });
});
