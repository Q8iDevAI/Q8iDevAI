...
import type { CheckoutPrMergeMethod, PullRequestMergeable } from "@q8idevai/protocol/messages";
...
function getPrimaryActionId(input: BuildGitActionsInput): GitActionId | null {
  ...
  // Only Q8iDevAI-owned worktrees get Archive as a fallback primary action.
  // Regular Git checkouts should not show the destructive archive CTA by default.
  if (input.isQ8iDevAIOwnedWorktree) {
    return "archive-workspace";
  }
...
function hasPushableCommits(input: BuildGitActionsInput): boolean {
  ...
  // No-upstream Q8iDevAI worktrees are first-pushable: the daemon push sets upstream with `git push -u`.
  // Do not fold this into aheadOfOrigin; null also covers deleted/pruned upstream branches.
  return input.isQ8iDevAIOwnedWorktree && input.aheadOfOrigin === null && input.aheadCount > 0;
}
