...
import { CheckoutPrStatusSchema } from "@q8idevai/protocol/messages";
...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream Q8iDevAI worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("offers archive workspace for Git checkouts and worktrees", () => {
    ...
    const worktree = buildGitActions(
      createInput({ hasUncommittedChanges: true, isQ8iDevAIOwnedWorktree: true }),
    );
  ...
  it("still promotes archive as primary for an idle Q8iDevAI-owned worktree", () => {
    const actions = buildGitActions(createInput({ isQ8iDevAIOwnedWorktree: true }));
  ...
  it("uses the active language for policy-owned action labels and unavailable messages", async () => {
    ...
    try {
      ...
      expect(actions.secondary.find((entry) => entry.id === "pr")).toMatchObject({
        label: "إنشاء PR",
        unavailableMessage: "إنشاء PR غير متاح لأن هذا الفرع ليس لديه أي التزامات جديدة حتى الآن",
      });
    } finally {
      await i18n.changeLanguage("en");
    }
