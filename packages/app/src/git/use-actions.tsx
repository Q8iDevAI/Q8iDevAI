...
import type { CheckoutPrMergeMethod } from "@q8idevai/protocol/messages";
...
function deriveGitActionsState(args: DeriveGitActionsStateArgs): DerivedGitActionsState {
  ...
  const isQ8iDevAIOwnedWorktree = gitStatus?.isQ8iDevAIOwnedWorktree ?? false;
...
export function useGitActions({ serverId, cwd, icons }: UseGitActionsInput): UseGitActionsResult {
  ...
  const shipDefaultStorageKey = useMemo(() => {
    ...
    return `@q8idevai:changes-ship-default:${gitStatus.repoRoot}`;
  }, [gitStatus?.repoRoot]);
...
function translateGitActionUnavailableMessage(
  ...
  const keyByMessage: Record<string, string> = {
    ...
    "Archive isn't available here because this workspace was not created as a Q8iDevAI worktree":
