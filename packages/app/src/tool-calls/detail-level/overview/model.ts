import { isQ8iDevAIToolName } from "@q8idevai/protocol/tool-name-normalization";
import { describeToolCall, type ToolCallRun } from "../grouping";

const DIRECT_Q8IDEVAI_TOOL_PREFIX = "q8idevai_";
...
function isQ8iDevAICall(name: string, normalizedName: string): boolean {
  return isQ8iDevAIToolName(name) || normalizedName.startsWith(DIRECT_Q8IDEVAI_TOOL_PREFIX);
}
...
export function buildOverviewGroup(run: ToolCallRun): OverviewToolCallGroup {
  ...
  for (const call of run.calls) {
    ...
    if (isQ8iDevAICall(descriptor.name, normalizedName)) {
      q8idevaiCallCount += 1;
