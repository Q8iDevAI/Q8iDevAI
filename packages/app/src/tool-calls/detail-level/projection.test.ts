...
describe("tool call detail-level projection", () => {
  ...
  it("distinguishes reads, searches, and other tools in overview", () => {
    const calls = [
      ...
      toolCall(
        "4",
        { type: "search", query: "q8idevai", toolName: "web_search" },
        { status: "failed" },
      ),
  ...
  it("counts Q8iDevAI calls separately from other tools", () => {
    const calls = [
      toolCall("1", { type: "unknown", input: null, output: null }, { name: "q8idevai.list_agents" }),
      toolCall(
        "2",
        { type: "unknown", input: null, output: null },
        { name: "mcp__q8idevai__list_worktrees" },
      ),
      toolCall("3", { type: "fetch", url: "https://q8idevai.sh" }),
      toolCall("4", { type: "fetch", url: "https://github.com/Q8iDevAI" }),
    ];

    const result = project({ level: "overview", head: calls });

    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { otherToolCount: 2, q8idevaiCallCount: 2 },
    });
  });

  it("classifies direct Brave search and Q8iDevAI runtime tool names", () => {
    ...
    const calls = [
      ...
      toolCall("3", unknownDetail, { name: "q8idevai_list_providers" }),
      toolCall("4", unknownDetail, { name: "q8idevai_list_worktrees" }),
      toolCall("5", unknownDetail, { name: "q8idevai_list_worktrees" }),
    ...
    expect(result.groupsByHostId.get("1")).toMatchObject({
      summary: { searchCount: 3, otherToolCount: 0, q8idevaiCallCount: 3 },
    });
  });
