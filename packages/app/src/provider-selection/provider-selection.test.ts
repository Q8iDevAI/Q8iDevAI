...
import type { AgentModelDefinition, ProviderSnapshotEntry } from "@q8idevai/protocol/agent-types";
import type { AgentProviderDefinition } from "@q8idevai/protocol/provider-manifest";
...
describe("combined model selector data", () => {
  ...
  it("uses the active app language for utility labels", async () => {
    ...
    try {
      ...
      expect(getAllModelLabels(providers)).toContain("تقصير");
      ...
      ).toEqual({ ok: false, reason: "مطلوب موجه الأولي" });
    } finally {
