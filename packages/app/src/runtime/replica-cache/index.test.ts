...
function workspacePayload(): WorkspaceDescriptorPayload {
  return {
    ...
    workspaceDirectory: "/repo/q8idevai",
    projectKind: "git",
...
describe("ReplicaCache", () => {
  ...
  it("round-trips explicit directory and timeline commits", async () => {
    ...
    expect(restoredDirectory.projects.get("project-1")?.projectDisplayName).toBe("Q8iDevAI");
  ...
  it("treats a corrupt row as a scoped miss", async () => {
    ...
    expect(restored.projects.get("project-1")?.projectDisplayName).toBe("Q8iDevAI");
