...
import type {
  ...
  FetchAgentsEntry,
  FetchAgentsOptions,
} from "@q8idevai/client/internal/daemon-client";
import type { ConnectionOffer } from "@q8idevai/protocol/connection-offer";
import type { SessionOutboundMessage } from "@q8idevai/protocol/messages";
import type { AgentPermissionRequest } from "@q8idevai/protocol/agent-types";
...
afterEach(() => {
  vi.useRealTimers();
  delete (globalThis as Record<string, unknown>).__Q8IDEVAI_INITIAL_DAEMON_CONNECTION__;
...
function makeHost(input?: Partial<HostProfile>): HostProfile {
  ...
  const relay: HostConnection = {
    id: "relay:relay.q8idevai.sh:443",
    type: "relay",
    relayEndpoint: "relay.q8idevai.sh:443",
...
function makeOffer(input?: Partial<ConnectionOffer>): ConnectionOffer {
  return {
    ...
    relay: {
      endpoint: input?.relay?.endpoint ?? "relay.q8idevai.sh:443",
...
function encodeOfferUrl(payload: unknown): string {
  const encoded = Buffer.from(JSON.stringify(payload), "utf8")
    ...
    .replace(/\//g, "_")
    .replace(/=+$/g, "");
  return `https://app.q8idevai.sh/#offer=${encoded}`;
}
...
describe("HostRuntimeController", () => {
  ...
  it("replaces the active relay client when re-pairing changes the daemon public key", async () => {
    const oldRelay: HostConnection = {
      id: "relay:wss:relay.q8idevai.sh:443",
      type: "relay",
      relayEndpoint: "relay.q8idevai.sh:443",
      useTls: true,
  ...
  it("adopts the first successful probe on startup", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 82,
      "relay:relay.q8idevai.sh:443": 18,
    };
    const controller = new HostRuntimeController({
      host,
    ...
    const controller = new HostRuntimeController({
      ...
      deps: {
        ...
        connectToDaemon: async ({ host: hostProfile, connection }) => {
          ...
          if (connection.id === "relay:relay.q8idevai.sh:443") {
  ...
  it("ranks the live connection by its heartbeat RTT without pinging it again", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 12,
      "relay:relay.q8idevai.sh:443": 65,
    };
    const controller = new HostRuntimeController({
      host,
      deps: {
  ...
  it("keeps the live connection when one probe cycle looks slow", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.q8idevai.sh:443": 55,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.q8idevai.sh:443"] = 42;
  ...
  it("backs off inactive connection probes while a host is online", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 10,
      "relay:relay.q8idevai.sh:443": 50,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    const initialRelayProbe = controller
      .getSnapshot()
      .probeByConnectionId.get("relay:relay.q8idevai.sh:443");

    latencies["direct:lan:6767"] = 12;
    latencies["relay:relay.q8idevai.sh:443"] = 25;
    ...
    expect(snapshot.probeByConnectionId.get("relay:relay.q8idevai.sh:443")).toEqual(initialRelayProbe);
  });
  ...
  it("switches only after the faster alternative wins consecutive probes", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.q8idevai.sh:443": 60,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.q8idevai.sh:443"] = 30;
    ...
    let switched = controller.getSnapshot().activeConnectionId === "relay:relay.q8idevai.sh:443";
    for (let index = 0; index < 6 && !switched; index += 1) {
      await vi.advanceTimersByTimeAsync(120_000);
      await controller.runProbeCycleNow();
      switched = controller.getSnapshot().activeConnectionId === "relay:relay.q8idevai.sh:443";
    }
  ...
  it("does not switch on a transient latency spike", async () => {
    ...
    const latencies: Record<string, number | Error> = {
      "direct:lan:6767": 15,
      "relay:relay.q8idevai.sh:443": 80,
    };
    const controller = new HostRuntimeController({
      host,
      deps: makeDeps(latencies, clients),
    ...
    latencies["relay:relay.q8idevai.sh:443"] = 20;
    ...
    latencies["relay:relay.q8idevai.sh:443"] = 90;
  ...
  it("ignores stale switch failures after a newer connection is already online", async () => {
    ...
    const switchRelay = controller.activateConnection({
      connectionId: "relay:relay.q8idevai.sh:443",
    });
    await waitUntil(() => {
      const snapshot = controller.getSnapshot();
      return (
        snapshot.activeConnectionId === "relay:relay.q8idevai.sh:443" &&
        snapshot.connectionStatus === "online"
      );
    });
    ...
    const snapshot = controller.getSnapshot();
    expect(snapshot.activeConnectionId).toBe("relay:relay.q8idevai.sh:443");
    expect(snapshot.connectionStatus).toBe("online");
...
describe("HostRuntimeStore", () => {
  it.each(["active", "inactive", "background"] as const)(
    ...
    async (currentState) => {
      const relay = (suffix: string): HostConnection => ({
        id: `relay:relay-${suffix}.q8idevai.sh:443`,
        type: "relay",
        relayEndpoint: `relay-${suffix}.q8idevai.sh:443`,
  ...
  it("revokes push notifications before removing a host", async () => {
    ...
    const storage = createMemoryHostRuntimeStorage({
      "@q8idevai:daemon-registry": JSON.stringify([host]),
      "@q8idevai:e2e": "1",
    });
    const store = new HostRuntimeStore({
      storage,
  ...
  it("loads the host registry and restores its cached directory", async () => {
    ...
    await storage.setItem("@q8idevai:daemon-registry", JSON.stringify([host]));
    await storage.setItem("@q8idevai:e2e", "1");
  ...
  it("records a chosen host color and writes it through to storage", async () => {
    ...
    const persisted = await storage.getItem("@q8idevai:daemon-registry");
  ...
  it("serializes overlapping host appearance writes", async () => {
    ...
    const persistedHosts = JSON.parse((await storage.getItem("@q8idevai:daemon-registry")) ?? "[]");
      ...
      makeFetchAgentsPayload({
        entries: [
          makeFetchAgentsEntry({
            ...
            cwd: "/workspaces/q8idevai-pr67-review",
  ...
  it("upsertDirectConnection stores SSL and password settings", async () => {
    ...
    await store.upsertDirectConnection({
      ...
      endpoint: "example.q8idevai.test:7443",
      useTls: true,
    ...
    expect(host?.connections).toEqual([
      {
        id: "direct:example.q8idevai.test:7443",
        type: "directTcp",
        endpoint: "example.q8idevai.test:7443",
        useTls: true,
        password: "shared-secret",
  ...
  it("uses TLS for old pairing URLs that omit relay TLS on port 443", async () => {
    ...
    const oldPairingUrl = encodeOfferUrl({
      ...
      serverId: "srv_offer",
      daemonPublicKeyB64: "pk_test_offer",
      relay: { endpoint: "relay.q8idevai.sh:443" },
    });
...
describe("readInitialDaemonConnectionHint", () => {
  ...
  it("parses a valid listen-only hint", () => {
    (globalThis as Record<string, unknown>).__Q8IDEVAI_INITIAL_DAEMON_CONNECTION__ = {
  ...
  it("preserves useTls when explicitly true", () => {
    (globalThis as Record<string, unknown>).__Q8IDEVAI_INITIAL_DAEMON_CONNECTION__ = {
      listen: "q8idevai.example.com:443",
      useTls: true,
    };
    expect(readInitialDaemonConnectionHint({ isWebRuntime: true })).toEqual({
      listen: "q8idevai.example.com:443",
  ...
  it("ignores invalid shapes", () => {
    (globalThis as Record<string, unknown>).__Q8IDEVAI_INITIAL_DAEMON_CONNECTION__ = "localhost:6767";
    expect(readInitialDaemonConnectionHint({ isWebRuntime: true })).toBeNull();
