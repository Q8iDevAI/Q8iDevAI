...
import type { Q8iDevAISubagentRow, ProviderSubagentRow } from "./select";

function q8idevai(id: string, status: Q8iDevAISubagentRow["status"] = "idle"): Q8iDevAISubagentRow {
  return {
    kind: "q8idevai",
    id,
    provider: "codex",
...
describe("createArchiveFinishedSubagents", () => {
  it("dismisses provider rows before sequential managed archives and reports combined progress", async () => {
    ...
    const archive = createArchiveFinishedSubagents(
      [provider("native"), q8idevai("first"), q8idevai("second", "error"), q8idevai("resumed")],
      {
        parentAgentId: "parent",
    ...
    await expect(operation).resolves.toEqual({
      archivedQ8iDevAIIds: ["first", "second"],
      dismissedProviderIds: ["native"],
      skippedQ8iDevAIIds: ["resumed"],
      failures: [],
    });
  ...
  it("retains failure through optimistic rollback, retries current rows, and resets for a later eligible-set change", async () => {
    ...
    const calls: string[] = [];
    const archive = createArchiveFinishedSubagents([q8idevai("first"), q8idevai("second")], {
      parentAgentId: "parent",
      getManagedSubagent: (id) => current.get(id),
      archiveManagedSubagent: (id) => {
        calls.push(id);
    ...
    archive.setRows([q8idevai("second")]);
    ...
    archive.setRows([q8idevai("first")]);
    ...
    archive.setRows([q8idevai("first", "error")]);
    expect(archive.getState()).toEqual({
      eligibleCount: 1,
      status: { kind: "failed", failedCount: 1, totalCount: 1 },
    });
    ...
    archive.setRows([q8idevai("first"), q8idevai("later")]);
  ...
  it("keeps retained failure for eligible status changes and row reorder, then resets on membership change", async () => {
    ...
    archive.setRows([]);
    await operation;
    archive.setRows([q8idevai("first"), q8idevai("second")]);

    archive.setRows([q8idevai("second", "error"), q8idevai("first", "error")]);
    expect(archive.getState()).toEqual({
      eligibleCount: 2,
      status: { kind: "failed", failedCount: 2, totalCount: 2 },
    });

    archive.setRows([q8idevai("second", "error")]);
  ...
  it("clears retry state when the authoritative failed child is running", async () => {
    ...
    const archive = createArchiveFinishedSubagents([q8idevai("failed")], {
      parentAgentId: "parent",
      getManagedSubagent: (id) => current.get(id),
    ...
    archive.setRows([]);
    await operation;
    archive.setRows([q8idevai("failed", "running")]);
  ...
  it("clears retry state when the authoritative failed child is initializing", async () => {
    ...
    archive.setRows([]);
    await operation;
    archive.setRows([q8idevai("failed", "initializing")]);
  ...
  it("retains only eligible rolled-back failures and retries only those children", async () => {
    ...
    archive.setRows([q8idevai("first"), q8idevai("second", "running")]);
  ...
  it("rechecks each managed child and skips resumed, missing, archived, reparented, running, and initializing rows", async () => {
    ...
    await expect(archive.archiveFinished()).resolves.toEqual({
      archivedQ8iDevAIIds: ["first"],
      dismissedProviderIds: [],
      skippedQ8iDevAIIds: ["resumed", "missing", "archived", "reparented", "running", "initializing"],
      failures: [],
    });
