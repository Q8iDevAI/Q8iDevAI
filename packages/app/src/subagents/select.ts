...
import type { ProviderSubagentDescriptorPayload } from "@q8idevai/protocol/messages";

export interface Q8iDevAISubagentRow {
  kind: "q8idevai";
  id: Agent["id"];
...
export type SubagentRow = Q8iDevAISubagentRow | ProviderSubagentRow;
...
export function useSubagentsForParent(params: SelectSubagentsParams): SubagentRow[] {
  ...
  const q8idevaiRows = useStoreWithEqualityFn(
    useSessionStore,
  ...
  return useMemo(() => {
    ...
    if (providerRows.length === 0) return q8idevaiRows;
    const rows = [...q8idevaiRows, ...providerRows];
    rows.sort((left, right) => left.createdAt.getTime() - right.createdAt.getTime());
    return rows;
  }, [params.providerParentSubagentId, q8idevaiRows, providerRows]);
}
