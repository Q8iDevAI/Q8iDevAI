...
export interface ArchiveFinishedOutcome {
  archivedQ8iDevAIIds: string[];
  dismissedProviderIds: string[];
  skippedQ8iDevAIIds: string[];
...
export function isFinishedSubagent(row: SubagentRow): boolean {
  if (row.kind === "q8idevai") return row.status === "idle" || row.status === "error";
...
function rowIdentity(row: SubagentRow): string {
  return row.kind === "q8idevai" ? managedRowIdentity(row.id) : `provider:${row.id}`;
}
...
async function runArchiveFinished(
  ...
  const q8idevaiIds = rows.filter((row) => row.kind === "q8idevai").map((row) => row.id);
  ...
  for (const id of q8idevaiIds) {
    if (canArchiveManagedSubagent(deps.getManagedSubagent(id), deps.parentAgentId)) {
      ...
      outcome.skippedQ8iDevAIIds.push(id);
    }
    completedCount += 1;
    reportProgress(completedCount);
  }
