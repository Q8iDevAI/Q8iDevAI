...
function makeSources(): DesktopDiagnosticSources {
  return {
    ...
    getDaemonLogs: async () => ({
      logPath: "/q8idevai/home/daemon.log",
      contents: "daemon line one\ndaemon line two",
    }),
    getAppLogs: async () => ({
      logPath: "/logs/Q8iDevAI/main.log",
    ...
    getUpdaterDiagnostics: async () => ({
      ...
      shipItDirectory: "/cache/sh.q8idevai.desktop.ShipIt",
      state: {
        path: "/cache/sh.q8idevai.desktop.ShipIt/ShipItState.plist",
        exists: true,
      ...
      stdout: {
        path: "/cache/sh.q8idevai.desktop.ShipIt/ShipIt_stdout.log",
        exists: false,
        modifiedAt: null,
        contents: "",
        error: null,
      ...
      stderr: {
        path: "/cache/sh.q8idevai.desktop.ShipIt/ShipIt_stderr.log",
        exists: true,
...
describe("desktop diagnostic report", () => {
  ...
  test("includes the Electron main-process log after the daemon log", async () => {
    ...
    expect(report).toContain("  Log path: /q8idevai/home/daemon.log");
    expect(report).toContain("  App log path: /logs/Q8iDevAI/main.log");
