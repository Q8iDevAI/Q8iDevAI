...
function makeHost(): HostProfile {
  return {
    ...
    connections: [
      ...
      {
        id: "socket:/tmp/q8idevai-secret.sock",
        type: "directSocket",
        path: "/tmp/q8idevai-secret.sock",
      },
      {
        id: "pipe:\\\\.\\pipe\\q8idevai-secret",
        type: "directPipe",
        path: "\\\\.\\pipe\\q8idevai-secret",
      },
    ],
  };
}
...
describe("app diagnostics report", () => {
  test("reports whether the connected daemon is managed by Q8iDevAI Desktop", () => {
  ...
  test("formats connection rows without raw connection details", () => {
    ...
    expect(report).not.toContain("/tmp/q8idevai-secret.sock");
  ...
  test("redacts saved connection secrets from collected daemon and desktop text", () => {
    ...
    const redacted = redactAppDiagnosticReport(
      [
        ...
        "/tmp/q8idevai-secret.sock",
        "\\\\.\\pipe\\q8idevai-secret",
        "password=tcp-password",
        "q8idevai://pairing-secret",
      ].join("\n"),
      [host],
    );
    ...
    expect(redacted).not.toContain("/tmp/q8idevai-secret.sock");
    expect(redacted).not.toContain("\\\\.\\pipe\\q8idevai-secret");
