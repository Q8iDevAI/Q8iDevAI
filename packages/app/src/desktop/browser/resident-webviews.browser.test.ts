...
const RESIDENT_HOST_ID = "q8idevai-browser-resident-webviews";
...
const profileHost: BrowserWebviewProfileHost = {
  profilePartition: "persist:q8idevai-browser",
...
describe("resident browser webviews", () => {
  ...
  it("creates a resident webview for an agent-created unfocused tab", () => {
    ...
    expect(webview?.isConnected).toBe(true);
    expect(webview?.getAttribute("data-q8idevai-browser-id")).toBe("browser-agent");
    expect(webview?.getAttribute("partition")).toBe("persist:q8idevai-browser");
  ...
  it("shares one profile and registers attached guests with explicit identity", () => {
    ...
    expect(firstWebview.getAttribute("partition")).toBe("persist:q8idevai-browser");
    expect(secondWebview.getAttribute("partition")).toBe("persist:q8idevai-browser");
