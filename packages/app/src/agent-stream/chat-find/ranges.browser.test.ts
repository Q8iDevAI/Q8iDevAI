...
it("finds each actual occurrence across inline formatting with original Unicode offsets", () => {
  const row = content(
    '<div data-q8idevai-markdown-tag="p">İ😀hello <strong>world</strong> then <em>hello</em> world</div>',
  );
...
it("ignores controls, keeps block boundaries, and preserves literal code punctuation", () => {
  const row = content(
    '<div data-q8idevai-markdown-tag="p">first</div><div data-q8idevai-markdown-tag="p">second</div><div data-q8idevai-markdown-tag="pre"><span>const a</span><span>.b</span><button>copy a.b</button><span data-q8idevai-markdown-ignore="true">a.b</span></div>',
  );
...
it("matches whitespace across text nodes without inspecting hidden link destinations", () => {
  const row = content(
    '<div data-q8idevai-markdown-tag="p"><a href="https://example.com/hidden">hello &amp; world</a>\nline</div>',
  );
