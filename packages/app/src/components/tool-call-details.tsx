...
import {
  buildQ8iDevAIToolDetailSections,
  type Q8iDevAIToolDetailSection,
} from "@q8idevai/protocol/q8idevai-tool-call-detail";
...
function Q8iDevAIDetailSection({ section }: { section: Q8iDevAIToolDetailSection }) {
  return (
    <View style={styles.q8idevaiSection}>
      <Text style={styles.q8idevaiSectionTitle}>{section.title}</Text>
      {section.kind === "prose" ? (
        <Text selectable style={styles.q8idevaiProse}>
          {section.text}
        </Text>
      ) : (
        <View style={styles.q8idevaiFields}>
          {section.fields.map((field) => (
            <View key={field.label} style={styles.q8idevaiFieldRow}>
              <Text style={styles.q8idevaiFieldLabel}>{field.label}</Text>
              <Text selectable style={styles.q8idevaiFieldValue}>
                {field.value}
              </Text>
            </View>
          ))}
...
function buildQ8iDevAIUnknownSections(
  toolName: string | undefined,
  detail: UnknownDetail,
): ReactNode[] | null {
  if (!toolName) return null;
  const sections = buildQ8iDevAIToolDetailSections(toolName, detail.input, detail.output);
  if (!sections) return null;
  return sections.map((section) => <Q8iDevAIDetailSection key={section.title} section={section} />);
}
...
function buildDetailSections(
  ...
  if (detail.type === "unknown") {
    return buildQ8iDevAIUnknownSections(toolName, detail) ?? buildUnknownSections(detail, ds, t);
  }
  return [];
}
