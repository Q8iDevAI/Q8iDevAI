...
import type { FetchRecentProviderSessionEntry } from "@q8idevai/client/internal/daemon-client";
import {
...
describe("resolveDirectoryLabel", () => {
  const projects = [
    { rootPath: "/home/me/q8idevai", name: "q8idevai" },
    { rootPath: "/home/me/q8idevai/packages/app", name: "q8idevai app" },
  ];

  it("names the project root after the project alone", () => {
    expect(resolveDirectoryLabel("/home/me/q8idevai", projects)).toEqual({ name: "q8idevai" });
  });

  it("qualifies a worktree under the project with its path below the root", () => {
    expect(
      resolveDirectoryLabel("/home/me/q8idevai/.dev/worktrees/abc/zebra", [projects[0]!]),
    ).toEqual({ name: "q8idevai", detail: ".dev/worktrees/abc/zebra" });
  });

  it("tells two worktrees of the same project apart", () => {
    const zebra = resolveDirectoryLabel("/home/me/q8idevai/.dev/worktrees/abc/zebra", [projects[0]!]);
    const otter = resolveDirectoryLabel("/home/me/q8idevai/.dev/worktrees/def/otter", [projects[0]!]);
  ...
  it("picks the most specific project root containing the directory", () => {
    expect(resolveDirectoryLabel("/home/me/q8idevai/packages/app/src", projects)).toEqual({
      name: "q8idevai app",
      detail: "src",
    });
  });
  ...
  it("ignores trailing slashes on both sides", () => {
    expect(
      resolveDirectoryLabel("/home/me/q8idevai/", [{ rootPath: "/home/me/q8idevai/", name: "p" }]),
    ).toEqual({ name: "p" });
  });

  it("does not match a project root that is only a string prefix", () => {
    expect(resolveDirectoryLabel("/home/me/q8idevai-fork", projects)).toEqual({
      name: "/home/me/q8idevai-fork",
    });
  });
...
describe("formatDirectoryLabel", () => {
  it("shows the project name alone at its root", () => {
    expect(formatDirectoryLabel({ name: "q8idevai" })).toBe("q8idevai");
  });

  it("appends the path under the root so worktrees of one project read apart", () => {
    expect(formatDirectoryLabel({ name: "q8idevai", detail: ".dev/worktrees/zebra" })).toBe(
      "q8idevai · .dev/worktrees/zebra",
    );
  });
});
...
describe("resolveImportTarget", () => {
  it("trusts a scoped listing, whose rows the daemon already matched realpath-aware", () => {
    expect(
      resolveImportTarget({
        entryCwd: "/private/repo/q8idevai",
        workspaceCwd: "/repo/q8idevai",
        workspaceId: "ws-1",
  ...
  it("treats a sheet with no workspace as cross-workspace", () => {
    expect(resolveImportTarget({ entryCwd: "/repo/q8idevai", isScopedListing: false })).toEqual({
      crossWorkspace: true,
    });
  });
});
