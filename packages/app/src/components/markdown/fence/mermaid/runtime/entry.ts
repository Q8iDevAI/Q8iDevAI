...
declare global {
  interface Window {
    ...
    __Q8IDEVAI_MERMAID_RUNTIME_RECEIVE__?: (message: unknown) => void;
  }
}
...
async function render(message: MermaidRuntimeRenderMessage): Promise<void> {
  try {
    ...
    const { svg } = await mermaid.render(`q8idevai-mermaid-${message.revision}`, message.source);
...
window.__Q8IDEVAI_MERMAID_RUNTIME_RECEIVE__ = receiveRender;
