...
export function WelcomeScreen({ onHostAdded }: WelcomeScreenProps) {
  ...
  const handleOpenQ8iDevAISite = useCallback(() => {
    void openExternalUrl("https://q8idevai.sh");
  }, []);
    ...
    <View style={styles.root}>
      ...
      >
        <View style={styles.content}>
          <Q8iDevAILogo size={96} />
          <View style={styles.copyBlock}>
            <Text style={styles.title}>{t("onboarding.title")}</Text>
            <Text style={styles.subtitle}>{t("onboarding.subtitle")}</Text>
            {isNative ? (
              <Pressable style={styles.setupLink} onPress={handleOpenQ8iDevAISite}>
                <Text style={styles.setupLinkText}>q8idevai.sh</Text>
