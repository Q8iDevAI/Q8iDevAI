...
describe("createSidebarWorkspaceEntry workspace directory label", () => {
  it("uses the daemon-provided slug for a Q8iDevAI-owned worktree", () => {
    ...
    const projects = buildSidebarProjectsFromStructure({
      projects: [
        project({
          ...
          hosts: [
            {
              serverId: "relay:q8idevai-host",
              iconWorkingDir: "/repo/project-1",
              worktreeSupport: "supported" as const,
            },
          ],
          workspaceKeys: ["relay:q8idevai-host:ws-main"],
        }),
      ],
    });

    expect(projects[0]?.workspaces[0]).toMatchObject({
      workspaceKey: "relay:q8idevai-host:ws-main",
      serverId: "relay:q8idevai-host",
      workspaceId: "ws-main",
    });
  });
...
describe("shared sidebar workspace model", () => {
  it("feeds project placement and status grouping from the same cross-host workspace identities", () => {
    const model = buildSidebarWorkspacePlacementModel({
      projects: [
        project({
          projectKey: "getq8idevai/q8idevai",
          projectName: "getq8idevai/q8idevai",
          iconWorkingDir: "/repo/getq8idevai/q8idevai",
          hosts: [
            {
              serverId: "host-a",
              iconWorkingDir: "/repo/getq8idevai/q8idevai",
          ...
          workspaces: new Map([
            [
              ...
              workspace({
                id: "main",
                name: "main",
                projectId: "getq8idevai/q8idevai",
                projectDisplayName: "getq8idevai/q8idevai",
                status: "done",
              }),
            ],
          ]),
    ...
    expect(model.projectNamesByViewKey).toEqual(new Map([["getq8idevai/q8idevai", "getq8idevai/q8idevai"]]));
  });
