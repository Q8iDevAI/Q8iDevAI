...
#define METHOD(name) Java_sh_q8idevai_diffprototype_Q8iDevAIDiffPrototypeModule_##name
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(prepareDocument)(
  ...
  try {
    ...
    q8idevai::diff::prepare(*document, SkFontMgr_New_Android(nullptr), fontFamily.c_str(), size, job->cancelled);
...
extern "C" JNIEXPORT jdoubleArray JNICALL METHOD(readDocument)(
  ...
  try {
    ...
    return numbers(env, q8idevai::diff::read(*document, start, count));
  } catch (const std::exception& error) {
    fail(env, error);
    return nullptr;
  }
