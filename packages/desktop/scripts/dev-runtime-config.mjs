...
export function resolveDevRuntime(env = process.env) {
  const remoteDebuggingPort = env.Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT
    ? parsePort(env.Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT, "Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT")
    : 0;
  return {
    electronFlags: buildElectronFlags(env.Q8IDEVAI_ELECTRON_FLAGS, remoteDebuggingPort),
    userDataDir: resolveDevUserDataDir({
      devRoot: env.Q8IDEVAI_DEV_ROOT,
      inheritedUserDataDir: env.Q8IDEVAI_ELECTRON_USER_DATA_DIR,
      fallbackRoot: env.Q8IDEVAI_DEV_RUNTIME_FALLBACK_ROOT,
    }),
  };
}
