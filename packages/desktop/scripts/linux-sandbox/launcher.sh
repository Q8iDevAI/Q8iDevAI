...
for arg in "$@"; do
  if [ "$arg" = '--no-sandbox' ]; then
    export Q8IDEVAI_DESKTOP_SANDBOX_REASON='requested by --no-sandbox'
    printf '[linux-sandbox] disabled: %s\n' "$Q8IDEVAI_DESKTOP_SANDBOX_REASON" >&2 || true
    exec "$executable" "$@"
  fi
done
...
set -f
for flag in ${Q8IDEVAI_ELECTRON_FLAGS:-}; do
  if [ "$flag" = '--no-sandbox' ]; then
    export Q8IDEVAI_DESKTOP_SANDBOX_REASON='requested by Q8IDEVAI_ELECTRON_FLAGS'
    printf '[linux-sandbox] disabled: %s\n' "$Q8IDEVAI_DESKTOP_SANDBOX_REASON" >&2 || true
...
if namespace_error=$(unshare --user --map-root-user --net true 2>&1); then
  export Q8IDEVAI_DESKTOP_SANDBOX_REASON='user namespaces available'
...
if [ -x "$helper" ] && [ "$(stat -Lc '%u:%a' -- "$helper")" = '0:4755' ] &&
  ...
  case ",$mount_options," in
    *,nosuid,*|,,) ;;
    *)
      export Q8IDEVAI_DESKTOP_SANDBOX_REASON='root-owned SUID helper available'
      printf '[linux-sandbox] enabled: %s\n' "$Q8IDEVAI_DESKTOP_SANDBOX_REASON" >&2 || true
      exec "$executable" "$@"
      ;;
  esac
...
export Q8IDEVAI_DESKTOP_SANDBOX_REASON="user namespaces unavailable${namespace_error:+ ($namespace_error)}; no usable SUID helper"
printf '[linux-sandbox] disabled: %s\n' "$Q8IDEVAI_DESKTOP_SANDBOX_REASON" >&2 || true
exec "$executable" --no-sandbox "$@"
