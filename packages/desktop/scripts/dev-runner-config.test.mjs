...
describe("desktop dev process ownership", () => {
  test("makes the signal-handling runner the workspace terminal process", () => {
    const q8idevaiConfig = JSON.parse(
      readFileSync(new URL("../../../q8idevai.json", import.meta.url), "utf8"),
    );
    const devScript = readFileSync(new URL("./dev.sh", import.meta.url), "utf8");

    expect(q8idevaiConfig.scripts.desktop.command).toContain("exec ./packages/desktop/scripts/dev.sh");
