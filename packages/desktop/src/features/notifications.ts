...
export function registerNotificationHandlers(): void {
  ipcMain.handle("q8idevai:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("q8idevai:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("q8idevai:event:notification-click", payload);
      }
