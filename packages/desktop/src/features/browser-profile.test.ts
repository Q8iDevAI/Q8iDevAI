...
describe("listQ8iDevAIBrowserProfileGuests", () => {
  test("returns every live webview and popup in the shared profile", () => {
    ...
    const guests = listQ8iDevAIBrowserProfileGuests({
      profileSession,
      webContents: [
        firstWindowGuest,
        secondWindowGuest,
...
describe("legacy browser profiles", () => {
  test("accepts only unique saved browser ids and resolves their old partitions", () => {
    ...
    const browserIds = readLegacyQ8iDevAIBrowserIds([uuid, fallbackId, uuid, "not-a-browser-id", 123]);
    const partitions: string[] = [];
    const sessions = getQ8iDevAIBrowserProfileSessions(
      {
    ...
    expect(partitions).toEqual([
      "persist:q8idevai-browser",
      `persist:q8idevai-browser-${uuid}`,
      `persist:q8idevai-browser-${fallbackId}`,
    ]);
  ...
  test("resolves one valid legacy profile for tab-close cleanup", () => {
    ...
    expect(getLegacyQ8iDevAIBrowserProfileSession(sessions, "1700000000000-abcd")).not.toBeNull();
    expect(getLegacyQ8iDevAIBrowserProfileSession(sessions, "invalid")).toBeNull();
    expect(partitions).toEqual(["persist:q8idevai-browser-1700000000000-abcd"]);
  });
});

describe("clearQ8iDevAIBrowserProfile", () => {
  test("clears site data, HTTP cache, and auth before reloading live guests", async () => {
    ...
    const clearing = clearQ8iDevAIBrowserProfile({
