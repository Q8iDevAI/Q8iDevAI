...
import { BrowserAutomationExecuteRequestSchema } from "@q8idevai/protocol/browser-automation/rpc-schemas";
import type {
...
import {
  ...
  listRegisteredQ8iDevAIBrowserIdsForWorkspace,
  getQ8iDevAIBrowserWebContentsForHostWindow,
  getWorkspaceActiveQ8iDevAIBrowserIdForHostWindow,
...
function createRegistry(hostWebContentsId: number): BrowserRegistry {
  return {
    listRegisteredBrowserIds: listRegisteredQ8iDevAIBrowserIds,
    listRegisteredBrowserIdsForWorkspace: listRegisteredQ8iDevAIBrowserIdsForWorkspace,
    getTabContents(browserId: string): TabContents | null {
      const contents = getQ8iDevAIBrowserWebContentsForHostWindow(browserId, hostWebContentsId);
      return contents ? adaptWebContents(contents) : null;
    },
    getBrowserWorkspaceId: getQ8iDevAIBrowserWorkspaceId,
    getWorkspaceActiveBrowserId(workspaceId: string): string | null {
      return getWorkspaceActiveQ8iDevAIBrowserIdForHostWindow(workspaceId, hostWebContentsId);
    },
  };
}
...
export function registerBrowserAutomationIpc(options?: { ipc?: IpcHandlerRegistry }): void {
  ...
  ipc.handle("q8idevai:browser:execute-automation-command", async (event, rawRequest: unknown) => {
