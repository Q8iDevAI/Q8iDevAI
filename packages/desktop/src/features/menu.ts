...
import { getActiveQ8iDevAIBrowserWebContentsForHostWindow } from "./browser-webviews/index.js";
        ...
        { type: "separator" },
        {
          ...
          click: withBrowserWindow((win) => {
            reloadActiveBrowserOrWindow({
              win,
              getActiveBrowserContentsForHostWindow: getActiveQ8iDevAIBrowserWebContentsForHostWindow,
            });
          }),
        },
        {
...
export function setupApplicationMenu(options: ApplicationMenuOptions): void {
  ...
  ipcMain.handle("q8idevai:menu:showContextMenu", (event, input?: ShowContextMenuInput) => {
  ...
  ipcMain.handle("q8idevai:menu:set-capturing-shortcut", (_event, capturing?: boolean) => {
