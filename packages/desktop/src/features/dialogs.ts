...
export function registerDialogHandlers(): void {
  ipcMain.handle("q8idevai:dialog:ask", async (event, message: string, options?: AskOptions) => {
  ...
  ipcMain.handle(
    "q8idevai:dialog:askWithCheckbox",
  ...
  ipcMain.handle("q8idevai:dialog:open", async (event, options?: OpenOptions) => {
    const win = BrowserWindow.fromWebContents(event.sender);
