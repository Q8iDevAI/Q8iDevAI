export const Q8IDEVAI_BROWSER_PROFILE_PARTITION = "persist:q8idevai-browser";
...
const Q8IDEVAI_BROWSER_STORAGE_TYPES = [
  "cookies",
  "filesystem",
  "indexdb",
  "localstorage",
...
interface BrowserProfileSession {
  clearStorageData(options: {
    storages: Array<(typeof Q8IDEVAI_BROWSER_STORAGE_TYPES)[number]>;
  }): Promise<void>;
...
export function getQ8iDevAIBrowserProfileSession(sessions: ElectronSessions): BrowserProfileSession {
  return sessions.fromPartition(Q8IDEVAI_BROWSER_PROFILE_PARTITION);
}

export function readLegacyQ8iDevAIBrowserIds(input: unknown): string[] {
...
export function getQ8iDevAIBrowserProfileSessions(
  sessions: ElectronSessions,
  legacyBrowserIds: string[],
): [BrowserProfileSession, ...BrowserProfileSession[]] {
  return [
    getQ8iDevAIBrowserProfileSession(sessions),
    // COMPAT(browserProfile): added in v0.1.108; remove after 2027-01-15.
    ...legacyBrowserIds.map((browserId) =>
      sessions.fromPartition(`${Q8IDEVAI_BROWSER_PROFILE_PARTITION}-${browserId}`),
    ),
  ];
}

export function getLegacyQ8iDevAIBrowserProfileSession(
  sessions: ElectronSessions,
  browserId: string,
): BrowserProfileSession | null {
  const [legacyBrowserId] = readLegacyQ8iDevAIBrowserIds([browserId]);
  return legacyBrowserId
    ? sessions.fromPartition(`${Q8IDEVAI_BROWSER_PROFILE_PARTITION}-${legacyBrowserId}`)
    : null;
}

export function listQ8iDevAIBrowserProfileGuests(
...
export async function clearQ8iDevAIBrowserProfile(input: ClearBrowserProfileInput): Promise<void> {
  await Promise.all(
    input.profileSessions.flatMap((profileSession) => [
      profileSession.clearStorageData({ storages: [...Q8IDEVAI_BROWSER_STORAGE_TYPES] }),
