...
describe("open-project-routing", () => {
  it("returns a bare absolute path argument", () => {
    const projectPath = mkdtempSync(path.join(tmpdir(), "q8idevai-open-project-"));

    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
  ...
  it("does not treat flags as project paths", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", "--version", flagLikeDirectory],
        isDefaultApp: false,
      }),
  ...
  it("returns the path from an explicit --open-project flag for backward compatibility", () => {
    ...
    expect(
      parseOpenProjectPathFromArgv({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", "--open-project", projectPath],
        isDefaultApp: false,
      }),
    ).toBe(projectPath);
  });
