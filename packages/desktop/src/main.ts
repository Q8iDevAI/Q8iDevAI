...
import {
  ...
  getQ8iDevAIBrowserWebContentsForHostWindow,
  getQ8iDevAIBrowserWebviewRegistry,
  listRegisteredQ8iDevAIBrowserIds,
  ...
  setWorkspaceActiveQ8iDevAIBrowserId,
  unregisterQ8iDevAIBrowserHost,
} from "./features/browser-webviews/index.js";
import {
  clearQ8iDevAIBrowserProfile,
  getLegacyQ8iDevAIBrowserProfileSession,
  Q8IDEVAI_BROWSER_PROFILE_PARTITION,
  getQ8iDevAIBrowserProfileSession,
  getQ8iDevAIBrowserProfileSessions,
  listQ8iDevAIBrowserProfileGuests,
  readLegacyQ8iDevAIBrowserIds,
...
} from "@q8idevai/protocol/agent-deep-link";
import { AgentNavigationInbox, parseAgentDeepLinkFromArgv } from "./agent-navigation.js";

const DEV_SERVER_URL = process.env.EXPO_DEV_URL ?? "http://localhost:8081";
const APP_SCHEME = "q8idevai";
const Q8IDEVAI_DEBUG = process.env.Q8IDEVAI_DEBUG === "1";
const DISABLE_SINGLE_INSTANCE_LOCK = process.env.Q8IDEVAI_DISABLE_SINGLE_INSTANCE_LOCK === "1";
const APP_NAME = process.env.Q8IDEVAI_TEST_APP_NAME?.trim() || "Q8iDevAI";
const DESKTOP_WINDOW_CHROME_MODE = resolveDesktopWindowChromeMode({
  platform: process.platform,
  override: process.env.Q8IDEVAI_DESKTOP_WINDOW_CONTROLS,
...
const browserKeyboard = new BrowserKeyboard(getQ8iDevAIBrowserWebviewRegistry());
          ...
          { type: "separator" as const },
          {
            ...
            click: () => {
              log.info("[browser-devtools] inspect-element.request", {
                ...
                browserId: getQ8iDevAIBrowserIdForWebContents(contents),
                x: params.x,
                y: params.y,
...
function installBrowserWindowOpenHandler(input: {
  ...
  contents.setWindowOpenHandler(({ url, disposition, frameName, features, postBody }) => {
    ...
    const sourceBrowserId = getQ8iDevAIBrowserIdForWebContents(sourceContents);
    if (sourceBrowserId) {
...
const forcedUserDataDir = process.env.Q8IDEVAI_ELECTRON_USER_DATA_DIR?.trim();
if (forcedUserDataDir) {
  ...
  try {
    ...
    // Main checkout (e.g. "q8idevai") gets default userData — only worktrees diverge.
    ...
    if (isWorktree) {
      app.setPath("userData", path.join(app.getPath("appData"), `Q8iDevAI-${devWorktreeName}`));
...
// Allow users to pass Chromium flags via Q8IDEVAI_ELECTRON_FLAGS for debugging
// rendering issues (e.g. "--disable-gpu --ozone-platform=x11").
// Must run before app.whenReady().
const electronFlags = process.env.Q8IDEVAI_ELECTRON_FLAGS?.trim();
if (electronFlags) {
...
if (process.platform === "linux") {
  ...
  app.setDesktopName("Q8iDevAI.desktop");
  if (!app.commandLine.hasSwitch("class")) app.commandLine.appendSwitch("class", "Q8iDevAI");
  log.info("[linux-sandbox]", {
    enabled: !app.commandLine.hasSwitch("no-sandbox"),
    reason: process.env.Q8IDEVAI_DESKTOP_SANDBOX_REASON ?? "Chromium default",
  });
}
...
ipcMain.handle("q8idevai:get-pending-open-project", (event) => {
...
ipcMain.handle("q8idevai:agent-navigation:ready", (event) => {
  return agentNavigationInbox.windowReady(event.sender.id);
});

ipcMain.handle("q8idevai:browser:register-attached", (event, rawInput: unknown) => {
  ...
  const registered = registerAttachedQ8iDevAIBrowser({
    ...input,
    sender: event.sender,
    profileSession: getQ8iDevAIBrowserProfileSession(session),
  ...
  log.info("[browser-webview] registered", {
    ...
    registeredBrowserIds: listRegisteredQ8iDevAIBrowserIds(),
  });
...
ipcMain.handle("q8idevai:browser:unregister-workspace-browser", async (event, browserId: unknown) => {
  if (typeof browserId === "string" && browserId.trim().length > 0) {
    const normalizedBrowserId = browserId.trim();
    const hasOtherHost = getQ8iDevAIBrowserWebviewRegistry().hasBrowserInOtherHostWindow(
      event.sender.id,
      normalizedBrowserId,
    );
    unregisterQ8iDevAIBrowserFromHost(event.sender.id, normalizedBrowserId);
    // COMPAT(browserProfile): added in v0.1.108; remove after 2027-01-15.
    const legacyProfile = hasOtherHost
      ? null
      : getLegacyQ8iDevAIBrowserProfileSession(session, normalizedBrowserId);
    if (legacyProfile) {
      try {
        await clearQ8iDevAIBrowserProfile({
...
ipcMain.handle("q8idevai:browser:set-workspace-active-browser", (event, rawInput: unknown) => {
  const input = readActiveBrowserInput(rawInput);
  if (input) {
    setWorkspaceActiveQ8iDevAIBrowserId({ ...input, hostWebContentsId: event.sender.id });
  }
});

ipcMain.handle("q8idevai:browser:focus", (event, browserId: unknown): boolean => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    return false;
  }
  const contents = getQ8iDevAIBrowserWebContentsForHostWindow(browserId, event.sender.id);
  if (!contents) {
    return false;
  }
  contents.focus();
...
ipcMain.handle("q8idevai:browser:open-devtools", (event, browserId: unknown) => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    const result = {
      ok: false,
...
ipcMain.handle("q8idevai:browser:clear-profile", async (_event, rawLegacyBrowserIds: unknown) => {
  const profileSessions = getQ8iDevAIBrowserProfileSessions(
    session,
    readLegacyQ8iDevAIBrowserIds(rawLegacyBrowserIds),
  );
...
const browserCapture = createBrowserCaptureService<Electron.NativeImage>({
  findGuest: getQ8iDevAIBrowserWebContentsForHostWindow,
...
ipcMain.handle("q8idevai:browser:capture-element", (event, browserId: unknown, rect: unknown) =>
  browserCapture.capture({ browserId, hostWebContentsId: event.sender.id, rect }),
);

ipcMain.handle("q8idevai:browser:copy-element", (_event, payload: unknown) =>
...
function getDevBuildLabel(): string | null {
  ...
  return process.env.EXPO_PUBLIC_Q8IDEVAI_DEV_BUILD_LABEL?.trim() || null;
}
...
async function createWindow(
  ...
  mainWindow.on("closed", () => {
    ...
    unregisterQ8iDevAIBrowserHost(webContentsId);
  ...
  mainWindow.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    if (!isQ8iDevAIBrowserWebviewAttach(params)) {
  ...
  mainWindow.webContents.on("did-attach-webview", (_event, contents) => {
    prepareQ8iDevAIBrowserWebContents(contents);
...
function ownedDesktopWindow(win: BrowserWindow): OwnedDesktopWindow<AgentDeepLinkTarget> {
  return {
    ...
    show: () => win.show(),
    focus: () => win.focus(),
    sendAgent: (target) => win.webContents.send("q8idevai:event:open-agent", target),
  };
}
...
function setupSingleInstanceLock(): boolean {
  if (DISABLE_SINGLE_INSTANCE_LOCK) {
    log.info("[single-instance] disabled by Q8IDEVAI_DISABLE_SINGLE_INSTANCE_LOCK");
    return true;
  }
  ...
  app.on("second-instance", (_event, commandLine) => {
    ...
    // Relaunching the app (CLI `q8idevai [path]`, double-click, etc.) opens a new
...
async function bootstrap(): Promise<void> {
  ...
  ipcMain.handle("q8idevai:opener:openUrl", (_event, value: unknown) => openExternalUrl(value));
  ...
  ipcMain.handle("q8idevai:window:openNew", async (_event, options?: unknown) => {
    const pendingPath =
...
function showDaemonShutdownDialog(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("q8idevai:event:quitting", {});
  }
}
