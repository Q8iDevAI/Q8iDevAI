...
describe("desktop updater diagnostics", () => {
  it("collects the staged version and existing ShipIt evidence", () => {
    testDirectory = mkdtempSync(path.join(tmpdir(), "q8idevai-updater-diagnostics-"));
    const shipItDirectory = path.join(testDirectory, "sh.q8idevai.desktop.ShipIt");
    const updateBundlePath = path.join(shipItDirectory, "update.test", "Q8iDevAI.app");
  ...
  it("reports bundle version lookup failures", () => {
    ...
    writeFileSync(
      path.join(shipItDirectory, "ShipItState.plist"),
      JSON.stringify({ updateBundleURL: pathToFileURL(path.join(shipItDirectory, "Q8iDevAI.app")) }),
    );
