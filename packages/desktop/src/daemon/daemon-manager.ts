...
function getQ8iDevAIHome(): string {
  return resolveQ8iDevAIHome(process.env);
}

function logFilePath(): string {
  return path.join(getQ8iDevAIHome(), DAEMON_LOG_FILENAME);
}

export function isDesktopManagedDaemonRunningSync(): boolean {
  if (!ownedLaunch) return false;
  try {
    const lock = JSON.parse(readFileSync(path.join(ownedLaunch.home, "q8idevai.pid"), "utf8"));
...
async function startDaemon(): Promise<DesktopDaemonStatus> {
  ...
  try {
    await startDaemonInstance({
      ...
      timeoutMs: 30_000,
      ...invocation,
      env: { ...invocation.env, Q8IDEVAI_CLI: getBundledCliShimPath() },
      mode: "managed",
      desktopManaged: true,
      onAcquired: (instance) => {
...
async function restartDaemon(): Promise<DesktopDaemonStatus> {
  await runExternalCliJsonCommand(["daemon", "restart", "--home", getQ8iDevAIHome(), "--json"]);
  return resolveDesktopDaemonStatus();
}
...
async function getCliDaemonStatus(): Promise<string> {
  return await runExternalCliTextCommand(["daemon", "status", "--home", getQ8iDevAIHome()]);
}
  ...
  return {
    ...
    desktop_sandbox_diagnostics: () =>
      describeSandbox({
        ...
        launcherReason: process.env.Q8IDEVAI_DESKTOP_SANDBOX_REASON,
      }),
