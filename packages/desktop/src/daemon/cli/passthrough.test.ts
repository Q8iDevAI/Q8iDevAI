...
const originalDesktopCli = process.env.Q8IDEVAI_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.Q8IDEVAI_DESKTOP_CLI;
    } else {
      process.env.Q8IDEVAI_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Q8iDevAI", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("returns null when only --no-sandbox is present", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Q8iDevAI", "--no-sandbox"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
    ...
    expect(
      parsePassthroughCliArgs({
        argv: [
          ...
          "/nix/store/q8idevai-desktop/share/q8idevai-desktop/electron-app",
          "--no-sandbox",
          "--class=q8idevai-desktop",
          "daemon",
          "status",
        ],
        isDefaultApp: true,
  ...
  it("ignores Electron remote debugging switches", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Q8iDevAI", "--remote-debugging-port=9233"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("preserves CLI flags for direct app invocations", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", "--version"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["--version"]);
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
  ...
  it("parses terminal args for direct app CLI passthrough", () => {
    ...
    expect(
      parsePassthroughCliArgsFromArgv([
        "/Applications/Q8iDevAI.app/Contents/MacOS/Q8iDevAI",
        "daemon",
        "set-password",
      ]),
