...
vi.mock("electron", () => ({
  app: {
    getPath: vi.fn(() => mocks.q8idevaiHome),
...
vi.mock("@q8idevai/server", () => ({
  resolveQ8iDevAIHome: vi.fn(() => mocks.q8idevaiHome),
...
vi.mock("./runtime-paths.js", () => ({
  ...
  resolveDaemonRunnerEntrypoint: vi.fn(() => ({
    entryPath: path.join(mocks.q8idevaiHome, "daemon.js"),
    execArgv: [],
  })),
}));
...
describe("daemon-manager commands", () => {
  ...
  beforeEach(() => {
    fixtureRoot = mkdtempSync(path.join(tmpdir(), "q8idevai daemon manager "));
    mocks.q8idevaiHome = path.join(fixtureRoot, "home");
