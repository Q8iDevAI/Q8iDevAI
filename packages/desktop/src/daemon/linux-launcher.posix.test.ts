...
async function launch(
  ...
  const root = mkdtempSync(join(tmpdir(), "q8idevai-launcher-"));
  try {
    ...
    chmodSync(join(app, "Q8iDevAI"), 0o755);
    ...
    const executablePath = options.symlink ? join(root, "q8idevai") : join(app, "Q8iDevAI");
    if (options.symlink) symlinkSync(join(app, "Q8iDevAI"), executablePath);
    ...
    const result = spawnSync(executablePath, args, {
      ...
      env: {
        ...
        APPIMAGE: "/tmp/Q8iDevAI.AppImage",
        Q8IDEVAI_DESKTOP_SMOKE: "0",
        ...options.env,
      },
    });
...
it("applies a debugging environment sandbox override before Chromium starts", async () => {
  const result = await launch({
    namespaces: true,
    env: { Q8IDEVAI_ELECTRON_FLAGS: "--disable-gpu\t--no-sandbox" },
  });
  expect(result.args).toEqual(["--no-sandbox", ...result.input]);
  expect(result.stderr).toContain("requested by Q8IDEVAI_ELECTRON_FLAGS");
});
