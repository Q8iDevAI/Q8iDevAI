...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const root = mkdtempSync(join(tmpdir(), "q8idevai-cli-shim-test-"));
  const appPath = join(root, "Q8iDevAI.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "q8idevai");
  const mainPath = join(contentsPath, "MacOS", "Q8iDevAI");
  ...
  copyFileSync(join(packageRoot, "bin", "q8idevai"), shimPath);
  ...
  if (options.includeHelper) {
    ...
    writeExecutable(
      ...
      [
        "#!/bin/sh",
        'printf "helper env=%s/%s cli=%s\\n" "$ELECTRON_RUN_AS_NODE" "$Q8IDEVAI_NODE_ENV" "$Q8IDEVAI_CLI"',
        'printf "args=%s\\n" "$*"',
        "",
      ].join("\n"),
    );
...
describe("desktop packaging", () => {
  ...
  it("unpacks server zsh shell integration files for external shells", () => {
    ...
    expect(config).toContain(
      "node_modules/@q8idevai/server/dist/server/terminal/shell-integration/**/*",
    );
    expect(config).not.toContain(
      "node_modules/@q8idevai/server/dist/src/terminal/shell-integration/**/*",
    );
  });
  ...
  it("excludes package debug/source files from the packaged app", () => {
    ...
    expect(config).toContain("!node_modules/@q8idevai/*/src/**");
    expect(config).toContain("!node_modules/@q8idevai/**/*.test.*");
    expect(config).toContain("!node_modules/@q8idevai/**/*.spec.*");
  });

  it("excludes the bundled daemon web UI from the packaged app", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("!node_modules/@q8idevai/server/dist/server/web-ui/**");
  });
  ...
  it("registers Q8iDevAI agent links with the operating system", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("name: Q8iDevAI agent link");
    expect(config).toContain("- q8idevai");
  });

  // electron-builder packs production dependencies declared in package.json into
  // app.asar. Runtime code in runtime-paths.ts and bin/q8idevai dynamically resolves
  ...
  it("declares all workspace packages required at runtime", () => {
    ...
    for (const required of ["@q8idevai/cli", "@q8idevai/server"]) {
  ...
  it("launches the packaged macOS CLI through Helper instead of the main app executable", () => {
    ...
    try {
      ...
      expect(result.stdout).toContain("@q8idevai/cli/dist/index.js");
  ...
  it("fails packaged macOS CLI startup when Helper is missing", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Bundled Q8iDevAI Helper executable not found");
      expect(result.stdout).not.toContain("main-executable");
    } finally {
      rmSync(bundle.root, { recursive: true, force: true });
    }
