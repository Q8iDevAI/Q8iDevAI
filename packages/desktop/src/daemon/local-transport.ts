...
import {
  ...
  validatePort,
  validateSshHost,
} from "@q8idevai/protocol/ssh-transport";
...
function emitTransportEvent(payload: TransportEventPayload): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("q8idevai:event:local-daemon-transport-event", payload);
  }
}
 ...
 *   ws+unix://./pipe/q8idevai:/ws        (Windows named pipe)
 *
