...
export function registerWindowManager(input: { mode: DesktopWindowChromeMode }): void {
  ipcMain.handle("q8idevai:window:minimize", (event) => {
    BrowserWindow.fromWebContents(event.sender)?.minimize();
  });

  ipcMain.handle("q8idevai:window:close", (event) => {
    BrowserWindow.fromWebContents(event.sender)?.close();
  });

  ipcMain.handle("q8idevai:window:toggleMaximize", (event) => {
  ...
  ipcMain.handle("q8idevai:window:isFullscreen", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return win?.isFullScreen() ?? false;
  });

  ipcMain.handle("q8idevai:window:isMaximized", (event) => {
    return BrowserWindow.fromWebContents(event.sender)?.isMaximized() ?? false;
  });

  ipcMain.handle("q8idevai:window:setFullscreen", (event, fullscreen: unknown) => {
    if (typeof fullscreen !== "boolean") return;
    BrowserWindow.fromWebContents(event.sender)?.setFullScreen(fullscreen);
  });

  ipcMain.handle("q8idevai:window:setBadgeCount", (_event, count?: unknown) => {
  ...
  ipcMain.handle("q8idevai:window:updateChrome", (event, update?: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) {
      return;
    }
...
export function setupWindowResizeEvents(win: BrowserWindow): void {
  ...
  const notifyResized = () => {
    ...
    win.webContents.send("q8idevai:window:resized", {});
  };
