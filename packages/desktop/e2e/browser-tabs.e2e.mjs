...
function seedQ8iDevAIHome(q8idevaiHome, listen, workspaceRoot) {
  ...
  writeJson(path.join(q8idevaiHome, "config.json"), {
    version: 1,
    daemon: {
      listen,
      relay: { enabled: false },
  ...
  writeJson(path.join(q8idevaiHome, "projects", "projects.json"), projects);
  writeJson(path.join(q8idevaiHome, "projects", "workspaces.json"), workspaces);
}
  ...
  while (Date.now() < deadline) {
    try {
      const status = await page.evaluate(async () => {
        if (typeof window.q8idevaiDesktop?.invoke !== "function") return null;
        return await window.q8idevaiDesktop.invoke("desktop_daemon_status");
      });
...
async function waitForGuestSelector(client, browserId) {
  ...
  while (Date.now() < deadline) {
    const evaluated = await callBrowserTool(client, "browser_evaluate", {
      browserId,
      function: "() => Boolean(globalThis.__q8idevaiSelector)",
    });
...
async function readGuest(page, browserId) {
  return await page.evaluate((id) => {
    const webview = document.querySelector(`[data-q8idevai-browser-id="${id}"]`);
...
async function readPresentation(page, browserId) {
  return await page.evaluate((id) => {
    const surface = document.querySelector(`[data-q8idevai-browser-surface="${id}"]`);
    ...
    const webview = surface.querySelector(`[data-q8idevai-browser-id="${id}"]`);
...
async function clickGuestElement(page, client, browserId, selector) {
  ...
  await page.locator(`[data-q8idevai-browser-id="${browserId}"]`).click({
    position: {
...
async function runRegression({
  ...
  const focusedGuest = await page.evaluate(
    (id) => window.q8idevaiDesktop?.browser?.focus?.(id),
    browserId,
  );
  ...
  await page.waitForFunction(
    ({ id, webContentsId }) => {
      const webview = document.querySelector(`[data-q8idevai-browser-id="${id}"]`);
      return (
        webview?.parentElement?.getAttribute("data-q8idevai-browser-surface") === id &&
  ...
  await callBrowserTool(client, "browser_evaluate", {
    browserId,
    function: `() => {
      globalThis.__q8idevaiFocusContinuity = ${JSON.stringify(focusContinuitySentinel)};
      globalThis.__q8idevaiViewportTransitions = [{ width: innerWidth, height: innerHeight }];
      addEventListener('resize', () => {
        globalThis.__q8idevaiViewportTransitions.push({ width: innerWidth, height: innerHeight });
      });
      return globalThis.__q8idevaiFocusContinuity;
    }`,
  });
  await page.evaluate((id) => {
    ...
    const events = [];
    globalThis.__q8idevaiBrowserReactivationEvents = events;
    for (const name of [
      "did-start-loading",
  ...
  const reactivationEvents = await page.evaluate(
    () => globalThis.__q8idevaiBrowserReactivationEvents ?? [],
  );
  ...
  const viewportTransitionsResult = await callBrowserTool(client, "browser_evaluate", {
    browserId,
    function: "() => globalThis.__q8idevaiViewportTransitions ?? []",
  });
  ...
  const continuityResult = await callBrowserTool(client, "browser_evaluate", {
    browserId,
    function: "() => globalThis.__q8idevaiFocusContinuity ?? null",
  });
  ...
  await page.evaluate((id) => {
    ...
    globalThis.__q8idevaiOriginalIsLoadingDescriptor = Object.getOwnPropertyDescriptor(
      webview,
      "isLoading",
    );
  ...
  await page.evaluate((id) => {
    const webview = document.querySelector(`[data-q8idevai-browser-id="${id}"]`);
    if (!(webview instanceof HTMLElement)) throw new Error(`Browser webview ${id} was unavailable`);
    const descriptor = globalThis.__q8idevaiOriginalIsLoadingDescriptor;
    if (descriptor) Object.defineProperty(webview, "isLoading", descriptor);
    else delete webview.isLoading;
    delete globalThis.__q8idevaiOriginalIsLoadingDescriptor;
  }, browserId);
...
async function main() {
  const artifactDir =
    process.env.Q8IDEVAI_DESKTOP_BROWSER_E2E_ARTIFACT_DIR ??
    fs.mkdtempSync(path.join(os.tmpdir(), "q8idevai-desktop-browser-e2e-artifacts-"));
  const runtimeDir = fs.mkdtempSync(path.join(os.tmpdir(), "q8idevai-desktop-browser-e2e-"));
  fs.mkdirSync(artifactDir, { recursive: true });
  const q8idevaiHome = path.join(runtimeDir, "q8idevai-home");
  const userData = path.join(runtimeDir, "electron-user-data");
  const workspaceRoot = path.join(runtimeDir, "workspaces");
  fs.mkdirSync(q8idevaiHome, { recursive: true });
  ...
  seedQ8iDevAIHome(q8idevaiHome, listen, workspaceRoot);
  ...
  try {
    const commonEnv = {
      ...
      Q8IDEVAI_LISTEN: listen,
      Q8IDEVAI_DAEMON_ENDPOINT: `localhost:${daemonPort}`,
      Q8IDEVAI_CORS_ORIGINS: "*",
      Q8IDEVAI_LOCAL_SPEECH_AUTO_DOWNLOAD: "0",
    ...
    const daemon = spawnLogged(
      ...
      { cwd: rootDir, env: { ...commonEnv, Q8IDEVAI_NODE_ENV: "development" } },
      artifactDir,
    );
    ...
    const desktop = spawnLogged(
      ...
      desktopCommandArgs,
      {
        ...
        env: {
          ...
          Q8IDEVAI_ELECTRON_REMOTE_DEBUGGING_PORT: String(cdpPort),
          Q8IDEVAI_ELECTRON_USER_DATA_DIR: userData,
          Q8IDEVAI_ELECTRON_FLAGS: `--remote-debugging-address=127.0.0.1 --remote-debugging-port=${cdpPort}`,
        },
      },
      artifactDir,
    );
    ...
    if (process.env.Q8IDEVAI_DESKTOP_SETTINGS_MEMORY_ONLY === "1") {
