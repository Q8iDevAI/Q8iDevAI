...
const root = await mkdtemp(path.join(tmpdir(), "q8idevai desktop lifecycle "));
const home = path.join(root, "daemon");
const env = Object.fromEntries(
  Object.entries(process.env).filter(
    ([key]) => !key.startsWith("Q8IDEVAI_") && key !== "ELECTRON_RUN_AS_NODE",
  ),
);
env.HOME = root;
env.USERPROFILE = root;
env.Q8IDEVAI_HOME = home;
...
async function openDesktop() {
  desktop = await electron.launch({
    ...
    env: {
      ...env,
      Q8IDEVAI_LISTEN: "127.0.0.1:1",
      Q8IDEVAI_WEB_UI_ENABLED: "false",
      Q8IDEVAI_HOST: "unused:1",
    },
  });
  await expect
...
try {
  ...
  const artifacts =
    process.env.Q8IDEVAI_DESKTOP_LIFECYCLE_ARTIFACT_DIR ??
    (await mkdtemp(path.join(tmpdir(), "q8idevai-desktop-lifecycle-artifacts-")));
