...
async function main() {
  ...
  const artifactRoot = process.env.Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR;
  const installedOnly = process.argv.includes("--installed-only");
  const extracted = fs.mkdtempSync(path.join(os.tmpdir(), "q8idevai-linux-artifacts-"));
  ...
  try {
    process.env.Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "installed");
    const helper = fs.statSync("/opt/Q8iDevAI/chrome-sandbox");
    ...
    await smokePackagedDesktopApp({ appPath: "/opt/Q8iDevAI", expectedSandbox: true });
    ...
    const desktopEntry = fs.readFileSync(path.join(appDir, "Q8iDevAI.desktop"), "utf8");
    if (/^Exec=.*--no-sandbox/m.test(desktopEntry)) {
      throw new Error("AppImage desktop entry bypasses runtime sandbox policy");
    }
    process.env.Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "appimage");
    ...
    const appPath = fs.existsSync(path.join(tarDir, "Q8iDevAI"))
      ? tarDir
      : path.join(tarDir, fs.readdirSync(tarDir)[0]);
    process.env.Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR = path.join(artifactRoot, "tar");
