...
declare global {
  interface Window {
    __q8idevaiStartupPresentationTrace?: StartupPresentation[];
  }
}
...
async function observeStartupPresentation(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    window.__q8idevaiStartupPresentationTrace = trace;
...
async function getStartupPresentation(page: Page): Promise<StartupPresentation[]> {
  return page.evaluate(() => window.__q8idevaiStartupPresentationTrace?.slice() ?? []);
}
