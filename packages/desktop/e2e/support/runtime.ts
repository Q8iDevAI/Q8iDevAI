 ...
 * status endpoint, PID from the q8idevai.pid lock file, log path from the
 * E2E_Q8IDEVAI_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  const port = getE2EDaemonPort();
  const q8idevaiHome = process.env.E2E_Q8IDEVAI_HOME;
  if (!q8idevaiHome) throw new Error("E2E_Q8IDEVAI_HOME not set — the worker fixture must run first");
  ...
  try {
    const raw = readFileSync(`${q8idevaiHome}/q8idevai.pid`, "utf8");
    ...
    console.warn("[desktop-updates] q8idevai.pid not found:", err);
  }

  return { version: data.version, pid, logPath: `${q8idevaiHome}/daemon.log` };
}
...
/**
 * Injects window.q8idevaiDesktop before app load so all Electron-gated code
...
export async function installDesktopRuntime(
  ...
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { q8idevaiDesktop: unknown }).q8idevaiDesktop = desktopBridge;
  }, config);
}
