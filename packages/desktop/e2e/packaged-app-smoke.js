...
const EXECUTABLE_NAME = "Q8iDevAI";
...
function getCliShimPath(appPath) {
  if (process.platform === "darwin") {
    return path.join(appPath, "Contents", "Resources", "bin", "q8idevai");
  }

  if (process.platform === "win32") {
    return path.join(appPath, "resources", "bin", "q8idevai.cmd");
  }

  return path.join(appPath, "resources", "bin", "q8idevai");
}
...
function getTerminalHookSmokeCommand(marker) {
  if (process.platform === "win32") {
    const script = [
      "& $env:Q8IDEVAI_HOOK_CLI hooks codex Stop",
  ...
  return `"$Q8IDEVAI_HOOK_CLI" hooks codex Stop && echo ${marker}`;
}
...
async function writeSmokeArtifacts({ page, stdout, stderr, userData, daemonHome, error }) {
  const artifactDir = process.env.Q8IDEVAI_DESKTOP_SMOKE_ARTIFACT_DIR?.trim();
  if (!artifactDir) {
    return;
  }
  ...
  if (page) {
    const renderer = await page
      .evaluate(() => ({
        ...
        bridgeKeys:
          typeof window.q8idevaiDesktop === "object" && window.q8idevaiDesktop !== null
            ? Object.keys(window.q8idevaiDesktop)
            : [],
      }))
...
async function waitForPackagedAppPage(browser, deadline) {
  while (Date.now() < deadline) {
    const page = browser
      ...
      .find((candidate) => candidate.url().startsWith("q8idevai://app/"));
    if (page) {
      return page;
    }
    await delay(250);
  }
  throw new Error("Timed out waiting for the packaged q8idevai://app/ renderer");
}
...
async function waitForRendererStartedDaemon({
  ...
  while (Date.now() < deadline) {
    try {
      lastStatus = await page.evaluate(() => window.q8idevaiDesktop.invoke("desktop_daemon_status"));
      if (
...
async function smokeColdCliDaemonStart({ appPath }) {
  const home = createTempDir("q8idevai-smoke-cli-daemon-home-");
  const pidPath = path.join(home, "q8idevai.pid");
...
async function smokeCliTerminal({ appPath, env }) {
  const cwd = createTempDir("q8idevai-smoke-terminal-cwd-");
  const marker = `q8idevai-packaged-terminal-smoke-${Date.now()}`;
...
async function assertSandboxState({ browser, page, expectedSandbox, stdout, stderr }) {
  const diagnostics = await page.evaluate(() =>
    window.q8idevaiDesktop.invoke("desktop_sandbox_diagnostics"),
  );
...
async function smokePackagedDesktopApp({
  ...
  const userData = createTempDir("q8idevai-smoke-user-data-");
  const daemonHome = createTempDir("q8idevai-smoke-daemon-home-");
...
if (require.main === module) {
  ...
  if (!appPath) {
    process.stderr.write("Usage: node smoke-packaged-desktop-app.js --app <Q8iDevAI.app>\n");
    process.exit(2);
  }
