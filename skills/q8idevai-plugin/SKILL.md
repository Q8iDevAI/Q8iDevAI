---
name: q8idevai-plugin
description: Build and manage trusted local Q8iDevAI plugins. Use when the user asks to create, edit, install, reload, enable, disable, remove, or troubleshoot a Q8iDevAI plugin; add lifecycle hooks; transform agent configuration, environment, MCP servers, or workspace creation; automate permissions or turn follow-ups; add a native surface, sidebar item, or workspace panel; add Command Center items or slash commands; add composer pills or attachment sources; transform, render, or append agent timeline items; contribute a theme; use Q8iDevAI from plugin code; or add plugin RPCs.
---
...
Fetch [https://Q8iDev.CoM/Q8iDevAI/llms.txt](https://Q8iDev.CoM/Q8iDevAI/llms.txt) first. Select and fetch the current plugin Markdown pages from that index before changing a plugin:

- [Plugin quickstart](https://Q8iDev.CoM/Q8iDevAI/docs/plugins.md) ([browser page](https://Q8iDev.CoM/Q8iDevAI/docs/plugins))
- [Plugin reference](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference.md) ([browser page](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference))
...
In the Q8iDevAI repository, use `public-docs/plugins/v0.8/reference.md` for the checkout's API, including
...
| Timeline row              | `q8idevai.agents.ref(id).timeline.append(...)`      | Push a plugin-owned row into an agent timeline from a server handler and update it later                      | reference.md → Append a timeline row from the daemon                                               |
...
| Plugin RPC                | `defineRpc` + `server.handle` + `useRpc`         | Daemon-side work that is not a normal Q8iDevAI operation: vendor APIs, credentials, local files                  | reference.md → Add plugin-specific backend behavior                                                |
| Lifecycle events          | `server.on`                                      | Observe agent/workspace lifecycle, inspect ended turns, and answer permission requests                        | [Lifecycle hooks](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference.md#lifecycle-hooks)                 |
| Creation and launch hooks | `server.before`                                  | Change agent config, provider options, MCP servers, environment, or workspace isolation before the operation  | [Before hooks](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference.md#before-hooks)                       |
| Q8iDevAI SDK                 | `useQ8iDevAI()` / handler `{ q8idevai }`               | Normal Q8iDevAI operations: workspaces, agents, providers, config                                                | reference.md → Use the Q8iDevAI SDK                                                                   |

| Lifecycle task                                                      | Example                                                                                                |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Log all eleven hooks                                                | [lifecycle-logger](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/lifecycle-logger)       |
| Follow-ups, permissions, environment, provider switching, worktrees | [lifecycle-actions](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/lifecycle-actions)     |
| Inject MCP servers and change Codex sandbox/approval options        | [agent-configuration](https://github.com/Q8iDevAI/Q8iDevAI/tree/main/plugin-examples/agent-configuration) |
...
```bash
q8idevai plugin init /absolute/path/to/my-plugin
...
The manifest supplies the default install ID and supported Q8iDevAI versions:

```json
{ "id": "my-plugin", "requirements": { "q8idevai": ">=0.8.0" } }
```

Keep `requirements.q8idevai` correct whenever creating or editing a plugin. `init` uses `>=` followed
by the CLI version. Raise the minimum when adopting newer APIs; add an upper bound when a later
Q8iDevAI release is incompatible. Use npm semver ranges and explicitly include beta versions when
...
See [requirements](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference#requirements).
...
created by plugin code. Every client `add*` method returns an idempotent remover. Q8iDevAI calls the
...
mobile, and Q8iDevAI has multiple themes. Every `Text` must take its color from `theme.colors`.
...
the active workspace or agent. Command callbacks receive the selected host's `q8idevai`, typed
...
```tsx
import type { PluginClientContext, PluginSurfaceProps } from "@q8idevai/plugin/client";
...
Icons are Lucide icon names. `theme` is a typed `PluginTheme` on every surface and panel. Primary text uses `theme.colors.foreground`; labels use `theme.colors.foregroundMuted`; the root view uses `theme.colors.surface0`. `layout.compact` is true on mobile and narrow windows. Q8iDevAI owns the route, header, host picker, close action, error boundary, and per-installation query client.

Before writing imports, classify each module as shared, client, or server. Follow the
[SDK import boundaries](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/reference.md#runtime-modules), including
transitive and type dependencies. The root is shared-only; hooks and client contexts belong to
`@q8idevai/plugin/client`, server contexts to `/server`, and host UI to `/client/react-native` or `/client/ui`.
Install dependencies locally for typechecking; Q8iDevAI supplies host runtime modules. JSX uses the
...
Use the existing Q8iDevAI SDK for normal Q8iDevAI operations. Use plugin RPC only for plugin-specific backend behavior.

### Call Q8iDevAI from a surface

`useQ8iDevAI()` borrows the selected host's current connection. Never create another client inside a surface.
...
The API covers workspaces, agents, providers, and daemon config. It omits connection lifecycle because Q8iDevAI owns the connection. Consult the current [SDK reference](https://Q8iDev.CoM/Q8iDevAI/docs/sdk/reference.md) for method details.
...
`console.error()` writes to stderr. Q8iDevAI captures both streams without interfering with plugin IPC.
...
Return complete text snapshots. Q8iDevAI owns the composer menu, picker, pills, drafts, and submission. Credentials and vendor calls stay in the daemon handler.
...
Call `contributeClient(client)` from `index.client.tsx`, or move its body into that entry. The component owns its icon and text; Q8iDevAI owns the pressable, chrome, pending state, error reporting, and placement. Removal functions are idempotent, and Q8iDevAI removes every pill when the plugin, client entrypoint, or host connection is torn down.
...
```ts
server.handle(publishReview, async ({ agentId, verdict }, { q8idevai }) => {
...
`addTheme` takes a small light or dark palette; Q8iDevAI expands it into the full token set. Every color is a hex string.
...
Plugins are installed per daemon and are trusted, unsandboxed code. Backend code can access files, processes, credentials, and network services on the daemon machine. Client contributions run inside the Q8iDevAI app. Do not install a plugin the user has not authorized or source code you have not inspected.

### Check the global switch before installing

Identify the target daemon and inspect its root `pluginsEnabled` value in `config.json`. For the local daemon, `q8idevai daemon status --json` reports its `home`; the file is `<home>/config.json`. Treat a missing field as `false`. Do not infer the global value from a plugin's `disabled` status, because an individual plugin can also be disabled.
...
> Plugins are trusted, unsandboxed code. Backend plugin code can access your daemon machine, including files, processes, credentials, and network services. Client plugin code runs inside the Q8iDevAI app. May I enable plugins on this daemon?
...
2. Run `q8idevai reload --json` against that daemon.
...
4. Run `q8idevai plugin ls` and verify the intended plugin reaches `running` after installation.

If the user asks to disable the global switch, set `pluginsEnabled` to `false`, run `q8idevai reload --json`, and verify configured plugins report `disabled`.

Do not edit a local config when the target is a remote daemon. Perform the edit on the daemon machine, or ask the user to use **Settings → Plugins → Enable plugins**. `q8idevai reload --host <url>` reloads the remote daemon's own file but does not edit it.

When the same sidebar contribution exists on several connected hosts, Q8iDevAI shows it once with a host picker. The selected host owns the bundle, SDK calls, RPCs, and query cache. An offline selected host does not fall through to another host. Attachment sources stay scoped to the composer's host.
...
q8idevai plugin add owner/repository              # Git source; append :path for a monorepo subdirectory
q8idevai plugin add owner/repository --ref main   # branches track, tags and commits pin
q8idevai plugin status
...
Use `--host <url>` when managing a daemon other than the CLI default. A Git source that must install or generate something declares `build` in `q8idevai-plugin.json` as a list of argv arrays; Q8iDevAI runs them without a shell on install and update and keeps the old version if one fails. Plugin source edits require `q8idevai plugin reload`; config changes to the global switch require `q8idevai reload`. A failed plugin reload stays failed; inspect `q8idevai plugin ls` for the load error and `q8idevai plugin logs <id>` for subprocess output, fix the source, typecheck, and reload again. `remove` deletes configuration, never the source directory.

Do not restart the daemon to load source changes. Restarting it can kill the agent performing the work.

For an old mixed entry, follow the standalone [v0.8 runtime-entry migration guide](https://Q8iDev.CoM/Q8iDevAI/docs/plugins/v0.8/migration) mechanically.
...
3. Run `q8idevai plugin ls` and require `running` with no error.
...
- RPC rejection: input or output failed its Zod schema, or the handler threw. Inspect `q8idevai plugin logs <id>` for handler output.
- Plugin exits or reload fails: inspect `q8idevai plugin ls` for status and `q8idevai plugin logs <id>` for initialization, cleanup, or crash output.
- Stale UI: source was edited without `q8idevai plugin reload <id>`.
