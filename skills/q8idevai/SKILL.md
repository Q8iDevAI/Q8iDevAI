---
name: q8idevai
description: Q8iDevAI reference for managing projects, workspaces, workspace scripts, agents, schedules, and heartbeats.
---

Q8iDevAI is a remote daemon that manages coding agents, terminals. Control it through MCP tools or the CLI.
...
```bash
q8idevai project create [path]
q8idevai project ls
q8idevai project rename <project-id> <name>
q8idevai project rename <project-id> --reset
q8idevai project delete <project-id>
```

For a local daemon, `project create` defaults to the current directory and resolves relative paths on the CLI machine. With `--host` or `Q8IDEVAI_HOST`, always provide a path; the target daemon interprets it on its own machine. Deleting a project archives its active workspaces and removes the project from Q8iDevAI without deleting the project directory.
...
Choose `baseBranch` explicitly: `origin/main` selects the remote-tracking branch; `refs/heads/main` selects local main. Bare `main` prefers local main when it exists, otherwise origin/main. Q8iDevAI retains the resolved ref for workspace comparisons, even after rebasing the branch or changing its PR target.

**`list_workspaces`** — list active workspaces.

**`archive_workspace`** — `{ workspaceId }`. Archives the workspace, its agents, and its terminals. Local directories remain; Q8iDevAI removes an owned worktree only after its final active workspace reference is archived.

**`rename_workspace`** — `{ workspaceId, name }`. Rename workspace.

## Workspace scripts

Configured `q8idevai.json` scripts use the same supervised lifecycle from tools and the CLI.
...
**`start_workspace_script`** — `{ workspaceId, scriptName }`. Starts one configured script through Q8iDevAI's managed workspace-script launcher and returns its status metadata.
...
```bash
q8idevai workspace create --isolation worktree --mode branch-off --new-branch fix-x --base origin/main
q8idevai workspace create --isolation worktree --mode checkout-branch --branch existing-work
q8idevai workspace create --isolation worktree --mode checkout-pr --pr-number 42
q8idevai run --provider codex/gpt-5.4 --mode full-access --workspace <workspace-id> "<prompt>"
q8idevai run --provider codex/gpt-5.4 --mode full-access --new-workspace worktree --worktree-mode branch-off --new-branch fix-x --base origin/main "<prompt>"
q8idevai send <agent-id> "<follow-up>"
q8idevai ls
q8idevai schedule create --cron "*/15 * * * *" "ping main build"
q8idevai heartbeat create --cron "*/15 * * * *" "check the build"
```

Discover with `q8idevai --help` and `q8idevai <cmd> --help`.

For product questions, setup, logs, version problems, or troubleshooting, use the **q8idevai-help** skill.
