...
| ------------------------------------- | ------------------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **Button examples: action**           | Refresh icon only                     | Refresh icon and label          | Refresh reads the workspace from the daemon and updates the label and tooltip. Q8iDevAI owns pending and error feedback. |
...
From the repository root, `npm run typecheck --workspace=@q8idevai/plugin` checks this example
against the SDK. Existing plugin projects must update `@q8idevai/plugin` before running their own
...
```sh
npm run test:e2e --workspace=@q8idevai/app -- e2e/browser/plugin-button-example.spec.ts
```
