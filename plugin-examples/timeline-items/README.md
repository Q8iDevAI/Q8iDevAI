...
calls return `undefined`, leaving Q8iDevAI's original timeline entry unchanged.

The transformer is a pure client contribution. It receives each source item and its streaming phase
while Q8iDevAI builds the render model, then returns plain plugin item objects. The renderer validates
`data` before Q8iDevAI mounts the component.
