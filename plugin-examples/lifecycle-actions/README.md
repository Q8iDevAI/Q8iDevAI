...
- Inject `Q8IDEVAI_HOOK_CREATE_EXAMPLE=created` on creation and `Q8IDEVAI_HOOK_OPEN_EXAMPLE=opened` on each session opening.
...
```bash
q8idevai plugin install /absolute/path/to/plugin-examples/lifecycle-actions
q8idevai plugin install /absolute/path/to/plugin-examples/lifecycle-logger
q8idevai plugin logs lifecycle-logger
```

Outside the Q8iDevAI repository, `server/inspect.ts` needs `@q8idevai/protocol` installed for its
type imports. Add it to your plugin development dependencies at the same version as
`@q8idevai/plugin` before installing this example.
